...
def _check_vector_segment_fof_coverage_schema46(
    ...
    """Check segment FOF selection, cross closure, and terminal accounting."""
    ...
    crosses = _csv_counts(result, "actual_vector_segment_fof_cross", 112)
    ...
    if target_fof != 0:
        _require(
            ...
            and target_addressing[0] != 0
            and target_lmul[3] != 0
            and target_emul[3] != 0
            and any(
                target_eew[eew] != 0 and target_sew[eew] != 0
                for eew in range(4)
            )
            and any(weight != 0 for weight in target_nf),
            "enabled segment FOF lacks a legal stage-1 unit-stride m1 shape",
        )
        ...
        for mode in range(2):
            for eew in range(4):
                for nf in range(7):
                    ...
                    _require(
                        (count > 0) == enabled,
                        "actual_vector_segment_fof_cross does not match "
                        "enabled classes: "
                        f"fault={fault} mode={mode} eew={eew} nf={nf}",
                    )
                    cross_total += count
                    crossed_faults[fault] += count
                    crossed_modes[mode] += count
...
def _check_constraint_coverage(result: dict[str, Any]) -> None:
    ...
    if schema >= 8:
        ...
        if target_operations[4] != 0:
            ...
            _require(
                sum(actual_segment_direction) + segment_fof_actions
                == actual_operations[4],
    ...
    if schema >= 9:
        ...
        for dimension, fields in segment_dimensions:
            ...
            if target_operations[4] != 0:
                ...
                _require(
                    sum(actual)
                    == actual_operations[4] - segment_fof_actions,
