...
normal FLW/FLD NaN-boxing, or vector writeback behavior.

## Regression Contract

`fp-loads` covers normal cacheable FLW/FLD plus PBMT=IO FLW/FLD. The MMIO
portion crosses normal, denied, and corrupt responses and requires exact FP
destination data for successful operations. Every exceptional operation must
retain its exact exception and MMIO metadata while deasserting both `rfWen`
and `fpWen`; every MMIO operation must bypass DCache.
