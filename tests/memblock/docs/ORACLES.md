...
| Translation fences | Independent page-table update model for `SFENCE.VMA`, `HFENCE.VVMA`, and `HFENCE.GVMA`, including selective/global scope and responses already accepted by the PTW manager | Updates become visible only after the architecturally required fence and only to matching context; global/selective leaf updates, targeted host-ASID/VS-ASID/VMID root reuse, and one global stage-1 stale-response race are implemented; outstanding VS/G-stage fences remain |
