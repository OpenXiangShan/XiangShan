...
| Translation fences | Independent page-table update model for `SFENCE.VMA`, `HFENCE.VVMA`, and `HFENCE.GVMA`, including selective/global scope and responses already accepted by the PTW manager | Updates become visible only after the architecturally required fence and only to matching context; global/selective leaf updates, targeted host-ASID/VS-ASID/VMID root reuse, and five global plus five selective stale-response races for stage-1, isolated VS/G, and fully nested Sv39/Sv39x4 VS/G are implemented; Sv48-family outstanding fences remain |
