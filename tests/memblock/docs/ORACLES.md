...
| Dirty eviction | Reference memory tracks committed bytes; an independent immutable pre-eviction line image is captured before replacement | Correct Release/ReleaseData/ReleaseAck sequence and exact bytes for the captured pressure lines | Partial; the dedicated pressure phase is byte-checked. Schema 21 adds weighted 9/10-line same-set store pressure across SB/SH/SW/SD, four set-index quarters, both store-half issue orders, Bare/stage-1/nested translation, and common manager latency. Each fresh target line must issue once; at least `depth - 8` target lines must return byte-exact ReleaseData and update manager memory, while all background dirty releases remain independently verified. Clean-release and wider concurrent replacement crosses remain |
...
| HLV/HLVX/HSV | Privilege/virtualization/PMP/PMA reference model including SPVP and execute permission | Effective mode, permission cause, final PA, RF/store side effect, and exception metadata. All three operation families execute under every Sv39/Sv48 and Sv39x4/Sv48x4 pair; a five-case PBMT basis covers final PMA/NC/IO and VS-over-G priority, cacheable misaligned HLV.D/HLVX.WU/HSV.D use exact split-data/readback oracles, M-mode execution crosses SPVP=U/S with physical PMP R/X/RW/RX plus locked R/RWX permissions, and a fixed PMA device target checks HLV/HSV Uncache plus HLVX execute denial. The same three families execute immediately below and at the exact `0x80000000` device-to-DDR boundary, requiring the translated PA's expected Uncache/fault versus cacheable-success behavior. Schema 23 additionally requires every enabled family x SPVP x aligned/misaligned random cross. Schema 24 requires every enabled family x SPVP x five-case PBMT-pair cross, independently calculates final PA/PBMT, checks exact translated load or committed store bytes, forbids Uncache for PMA, and requires exactly one Uncache request with no new DCache request for NC/IO. Schema 25 adds a weighted DDR/fixed-device address class and closes every enabled family x SPVP x class bin. Device HLV/HSV require exact Uncache data/store effects; HLVX requires LoadAccessFault, no data-manager request, and one-for-one wakeup cancellation. Non-PMA random pairs and fixed-device actions are naturally aligned; other fixed-PMA boundaries and broader hypervisor PMP region-edge crosses remain |
...
| Remaining PMP/PMA matrix | Extend the current hand-calculated TOR/NAPOT cases with other fixed-PMA class/permission boundaries and hypervisor PMP region edges | Access allowed/denied, cause, constrained manager requests, and exact region boundary behavior. Data-side TOR/NAPOT, R/W/AMO denial, lock/priority, 4-KiB-grain NA4 WARL behavior, M-mode HLV/HLVX/HSV using SPVP-selected unlocked and locked PMP R/X/W permissions, the schema-25 weighted fixed-device hypervisor class, all three hypervisor families at the exact `0x80000000` device-to-DDR edge, bare edge loads, device atomic denial, and schema-24 translated PBMT PMA/NC/IO routing are executable |
