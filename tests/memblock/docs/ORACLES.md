...
| Translation fences | Independent page-table update model for `SFENCE.VMA`, `HFENCE.VVMA`, and `HFENCE.GVMA`, including selective/global scope and responses already accepted by the PTW manager | Updates become visible only after the architecturally required fence and only to matching context; global/selective leaf updates, targeted host-ASID/VS-ASID/VMID root reuse, both stage-1 and isolated VS/G modes, and all four fully nested mode pairs are implemented for global/selective stale-response races; concurrent multi-walk fences remain |
