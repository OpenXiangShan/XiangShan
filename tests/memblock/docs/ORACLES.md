...
| Top-level control and metadata bridges | Independently generated hart/reset/power/halt/error, MSI acknowledgement, frontend-reset bypass, valid-gated MSI-info/CLINT, I-cache BEU metadata, hardware-counter events, and L2 prefetch control | Combinational paths match current inputs; registered metadata matches the exact one- or two-cycle input history on every monitored cycle, with payload checked under its valid contract | Implemented continuously; `top-control-contracts` covers all eight power/halt/error combinations, eight hart/reset-vector patterns, all 32 five-bit L2-prefetch enable combinations, all four MSI/CLINT valid combinations, zero/maximum delay, all 67 shared perf-event lanes, and all 64 event values. The standalone elaboration ties output event lane 0 to zero and prunes input 0/output 68, so only the 67 shared lanes carry an end-to-end claim |
...
| --- | --- | --- |
| MMIO and device ordering | A side-effecting device model with read values, write log, byte enables, access width, and ordering points | Partial: `mmio-contracts` executes two ordered reads of a read-clear register, an offset `SW`, and a final readback; it checks old/cleared/written values, sequence, PMA MMIO class, address, size, mask, replicated bus data, response flags, and zero DCache requests. Concurrent requests and response-error side-effect policy remain |
