...
| Memory trigger metadata | Program every `mem_trigger.tUpdate` slot, enable/load/store/select controls, EQ/GE/LT comparison, breakpoint gate, debug state, and chain relation independently | A qualifying scalar load/store reports the exact breakpoint action/exception, suppresses the load RF write and all external DCache requests, and leaves a triggered store image unchanged; a disabled, mismatched, or debug-suppressed trigger permits a normal cold request and completion | Partial: `trigger-contracts` executes 15 scalar cases across all four slots, all three supported match types, five control suppressions, a chain hit/miss pair, and load/store breakpoint actions. Vector, misaligned-store, and DebugMode-action regeneration remain |
