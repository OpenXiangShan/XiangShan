...
across stage-1/G-stage, Sv39/Sv48, leaf/top-level non-leaf, and load/store. A
leaf fault requires a complete cold load walk followed by the store path's
single AF-leaf reread through the shared PageTableCache; a top-level non-leaf
fault stops each walk after its root PTE. G-stage PPN overflow sets `gaf` and
maps to the original load/store access fault, not a guest-page fault. Every
case forbids DCache/Uncache requests.
