...
| Vector load data | Decode unit/strided/indexed addresses independently, including ordinary multi-uop unit-stride and strided `vuopIdx` base advancement, per-uop indexed vectors, and whole-register 16-byte uop windows; apply independent SEW/EEW, global-element `vl`/`vstart`/mask slicing, tail/merge policy, and old destination. Whole-register EVL is derived independently from NF and EEW | Exact 128-bit writeback, active-element behavior, independent `vsew/veew/vlmul` metadata, operation encoding, and identity | Implemented for modeled vector memory operations, including all 78 legal ELEN=64 EEW/SEW/LMUL/EMUL configurations for both unit-stride and alternating positive/negative strided load/store/readback, ordered/unordered indexed loads at all four equal-SEW/EEW values with repeated/non-monotonic indices across cache-line and Bare 4-KiB boundaries, a representative LMUL=2 ordered-indexed load, and all 16 NF 1/2/4/8 x EEW 8/16/32/64 whole-register combinations with an intentionally irrelevant input VL |
| Vector store data | Generate each active element's byte addresses and bytes from source data; apply mask, split, indexed permutation, and whole-register EVL/address rules | Active-byte readback, completion, replay progress, queue drain, RF write enables, flush, debug sidebands, indexed readback, and whole-register load readback are checked for modeled stores | Partial; ordered/unordered indexed stores and readbacks cover all four EEWs with non-monotonic unique indices, and all 16 whole-register NF/EEW store combinations are byte-exact, while generic mixed stores remain limited by the output-only store boundary |
...
| VSegment/VFOF | Segment-element address/data model across multiple uops, fault-only-first truncation, per-segment mask, whole-register EVL/addressing, and ROB redirect cancellation | Partial: multi-uop identity, takeover/ready behavior, unit-stride/strided/indexed-unordered/indexed-ordered field offsets, store readback, later-element FOF truncation, first-element architectural fault/fault-VA preservation, exact fix-VL, load/store redirect, survivor reuse, FOF data/fix-VL cancellation, all 16 legal whole-register NF/EEW load/store/readback combinations, and all 78 legal ordinary unit-stride plus strided EEW/SEW/LMUL/EMUL configurations are executable; broader indexed and segmented LMUL/EMUL crosses remain |
