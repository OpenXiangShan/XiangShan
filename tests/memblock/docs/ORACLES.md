...
| Top-level control bypasses | Independently generated 6-bit hart ID, 48-bit reset vector, power-down enable, CPU-halted status, and CPU-critical-error status | Hart ID and power-down match the current-cycle inputs; reset vector, halted status, and critical-error status match the preceding-cycle inputs on every monitored cycle | Implemented continuously; `top-control-contracts` additionally covers all eight boolean combinations, eight hart/reset-vector patterns, and every exercised rising/falling edge |
