...
| Exceptions | Reference ISA cause plus circular ROB age, independent of LQ/SQ allocation order | Cause bits, fault VA/GPA, marker, single terminal exception writeback, and oldest-fault address when several faults coexist | Partial: exact scalar/vector causes and metadata plus concurrent scalar load/store exception-address selection across ROB wrap with deliberately disagreeing LQ/SQ order are implemented; same-ROB `uopIdx`, cross-class priority, PMP/PMA, and physical ECC crosses remain |
