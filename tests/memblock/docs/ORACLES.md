...
| Remaining PMP/PMA matrix | Extend the current hand-calculated TOR/NAPOT cases with broader fixed-PMA cacheability/atomic classification and hypervisor region edges | Access allowed/denied, cause, constrained manager requests, and exact region boundary behavior. Data-side TOR/NAPOT, R/W/AMO denial, lock/priority, 4-KiB-grain NA4 WARL behavior, M-mode HLV/HLVX/HSV using SPVP-selected unlocked and locked PMP R/X/W permissions, and one fixed device PMA target are executable |
