...
| Stage-1 translation | Walk independently populated Bare/Sv39/Sv48 PTEs, checking canonical VA, valid/leaf, all supported leaf levels, superpage alignment, permissions, SUM/MXR, A/D, and PBMT policy | Returned data comes from calculated PA, or exact access/page fault and fault VA | Partial: generic Sv39/Sv48 walker, Bare bypass, 4-KiB builders, all supported superpage leaf execution, invalid root and malformed 2 MiB alignment faults, read-only load, read-only scalar-store rejection, and execute-only load cases are implemented; full permission matrix remains |
| G-stage translation | Walk independently populated Sv39x4/Sv48x4 PTEs with a 16-KiB root, widened root index, 41/50-bit GPA checks, G-stage U-mode permission rule, and guest-page-fault semantics | Exact host PA or guest fault cause, fault GPA, and stage metadata | Partial: generic 41/50-bit walker, Bare bypass, 4-KiB x4 builders, all supported superpage leaf execution, Sv39x4/Sv48x4 high-GPA fault execution, and execute-only data-read rejection are implemented; full permission matrix remains |
| Nested translation | Compose independent VS-stage and G-stage walks for `Sv39->Sv39x4`, `Sv39->Sv48x4`, `Sv48->Sv39x4`, and `Sv48->Sv48x4`; model Bare degenerations separately | No stage skipped; exact host PA, stage-specific fault, VA/GPA metadata, and implicit VS-page-table accesses | Partial: all four 4-KiB pairs and VS-only/G-only/fully-Bare degenerations are executable; context isolation and broader stage-only accesses remain |
...
| Translation context/fences | Include `satp/vsatp/hgatp` MODE/root changes, ASID/VMID tags, `V` transitions, `SFENCE.VMA`, `HFENCE.VVMA`, and `HFENCE.GVMA` scope | Updated translations observe only architecturally ordered page-table writes; stale contexts cannot alias | Partial: direct Sv39-to-Sv48 root/MODE switch plus global/selective `SFENCE.VMA`, selective `HFENCE.VVMA`, and global `HFENCE.GVMA` leaf-update visibility are implemented; VS/G-stage isolation, `V` transitions, and outstanding-walk ordering remain |
...
| Atomics/LR-SC/AMO | Reservation and atomic-memory model with success/failure, alignment, ordering, and optional bus error injection | Atomic read-modify-write result, reservation invalidation, failure code, one memory update, error lifetime, and no register write-enable on exceptions | Partial: all exposed W/D-width AMO ALU variants, AMOCAS.W/D compare success/failure, LR/SC old-value/writeback, success/failure, cache visibility, and representative misaligned D/W exception metadata are executable in `atomic-contracts`; reservation interference, cache-error injection, and full alignment/error matrices remain |
