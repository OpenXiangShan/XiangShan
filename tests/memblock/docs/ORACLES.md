...
7. Bank conflicts, arbitration choices, replay/cancel pulse counts, cache
   residency, and hardware-prefetch trigger/source/count are microarchitectural
   observations. They may bias stimulus, classify coverage, or aid debug, but
   must not decide architectural PASS/FAIL. Legal background prefetch traffic
   is allowed unless an interface control explicitly disables that output.
8. Initialize manager memory before the DUT can legally fetch it. After traffic
   starts, change that image only through modeled stores or externally visible
...
| Scalar load early feedback | Do not predict replay/arbitration from cache residency or bank mapping. Independently generated destination metadata can be used to sanity-check an observed wakeup only when a unique live owner exists | `wakeup`/`ld2Cancel` pins are sampled for protocol debug and coverage. Their presence, exact cycle, and cardinality do not close `random-mixed`; correctness comes from exact terminal data/exception/identity, legal cancellation, and LQ conservation | Diagnostic/implementation-contract coverage in `load-feedback` and error scenarios. It is intentionally not a general architectural oracle, because split loads, replay policy, and hardware prefetch may change the pulse stream without changing legal terminal behavior |
...
| DCache refill, partial progress, same-line merge, and ordinary MLP | Choose cold-load virtual addresses with bit 5 clear/set and build each 64-byte line independently in sparse memory. For merging, independently choose 2/3-way depth, exact-address/same-beat/cross-beat placement, critical beat, translation context, load width/sign, and manager latency. For MLP, choose 2..17 distinct lines, initial scalar issue width 1..3, and scalar-only or scalar+vector-load composition. Select denied over the whole transaction or independent corrupt on exactly the first/last beat without consulting DUT data | AcquireBlock echo must report both critical-beat orders; each sink receives one GrantAck. Every clean merged member returns exact data; lines already observed at the manager are skipped, while one or more target requests and paired global refills/GrantAcks are accepted so duplicate/background traffic cannot decide correctness. An ordinary miss burst uses the same external freshness rule and address-qualified holds. Depths through 16 must reach their selected external outstanding depth; depth 17 reaches the generated capacity of 16, schedules one fair response release, then requires a request for every target, with all three external stages conserved once per depth-17 action. Those depth observations are configuration-qualified stimulus coverage, not an architectural expected value. Each scalar load requires one exact writeback and LQ dequeue; the legal two-flow vector member requires its exact vector writeback and two LQ dequeues. Under error, the critical word follows only its own beat's corrupt value, the opposite half observes the accumulated poisoned-line cause, and an unrelated second MSHR stays exact. The response queue must remain nonempty after critical-beat completion and become empty only after every D beat | Implemented in `single-load` and the six-case `dcache-errors` matrix, including 128/256-cycle interbeat gaps, maximum outstanding A depth two, and healthy/poisoned resident followups. Schema-17 `random-mixed` adds common response-wide clean/corrupt/denied outcomes on unique cold lines in Bare and translated contexts, exact exception/redirect/LQ cleanup, two error beats, and sink-attributed errored GrantAck/refill conservation under concurrent clean prefetch. Schema 20 adds nonrepeating 64-MiB clean merge pressure with all 12 depth/pattern/critical-beat bins and Bare/stage-1/nested coverage. Schemas 43/45 close distinct-line cold-burst depth x legal scalar issue-width x scalar/vector composition x translation, including capacity-plus-one recovery, using only external TileLink, architectural, and queue conservation. Internal reject/replay events remain diagnostic. Schema 40 adds resident two/three-way same-address-bank waves over all eight address classes and enabled translation regimes; bank/replay classification is stimulus coverage only, while the oracle remains exact per-identity load data, one terminal writeback, and one LQ dequeue |
...
| Hardware data prefetch | Treat training, confidence, source selection, depth, width, and replacement as implementation choices. The generated loads and sparse-memory model remain authoritative for architectural behavior | Any emitted L2/L3 prefetch must obey the exposed output protocol; disabling an output gate suppresses that output. Prefetch requests may otherwise appear or not appear without changing load/store correctness | The focused scenario records current stride/stream/SMS behavior as diagnostics and checks control suppression. `random-mixed` biases stride-shaped traffic and reports source counts but has no positive-output correctness gate. Prefetch usefulness and algorithmic accuracy are performance/integration concerns |
...
| Set replacement | Reference memory tracks committed bytes; dirty actions capture an immutable pre-eviction line image, while clean actions retain exact initial and revisit load expectations | Correct Release/ReleaseData/ReleaseAck behavior and exact data for captured pressure lines | Partial. Schema 21 adds weighted 9/10-line same-set dirty pressure across SB/SH/SW/SD, four set-index quarters, both store-half issue orders, Bare/stage-1/nested translation, and manager latency. Schema 27 crosses the same dimensions with clean and dirty line state. Each fresh line issues once. Dirty actions require at least `depth - 8` byte-exact target ReleaseData and manager-memory updates. Clean actions revisit in reverse, require at least `depth - 8` misses and address-attributed target Releases, reject target ReleaseData, and conserve exact load writebacks/LQ dequeues. Schema 28 crosses both states and every existing dimension with an independent held refill. The address-attributed target release minimum must be reached while that load identity remains pending, after which its held response is released and its exact request/writeback/LQ disposition is checked. Schema 29 adds address-qualified C backpressure: selected actions hold the first target Release or ReleaseData for exactly 16 valid cycles and compare every C payload field for stability; disabled endpoint actions require zero target stalls. ProbeAck and background Release traffic cannot satisfy the target oracle, while every data-bearing C transaction remains byte-verified. Schema 38 crosses one through eight replacement windows and closes all 1536 state x overlap x backpressure x window-count x depth x width x translation bins. A multi-window overlap holds one distinct D response per set, requires every independent target set to emit at least `depth - 8` attributed transactions while all associated loads remain pending, then releases every response by address and conserves each request, writeback, and dequeue. Dirty baselines are committed before overflow issue so every required ReleaseData comes from a dirty target set. Target allocation excludes every line covered by an earlier DCache request before the reference and bus memories are initialized, so the oracle never rewrites memory behind a potentially resident line. The eight-window ceiling is below the independently checked 16-entry miss capacity. Composition with Probe/CMO/atomic traffic remains |
...
| Ready/valid | Protocol definition: payload remains stable while `valid && !ready`; accepted items need identity-preserving disposition | Generated SVA checks producer stability; channel monitors validate response identity, legality, D-channel error flags, and source lifetime | Partial; DCache, PTW, and Uncache denied/corrupt injection are executable. DCache and PTW preserve fixed multibeat `denied`, assert `corrupt` on denied data, and vary independent corruption over first/last beats. Schema 29 independently samples every DCache C payload field and requires 16 stable comparisons while an address-attributed Release is deliberately backpressured; non-target C transactions are bypassed and cannot satisfy the oracle. Schema 38 holds and releases up to eight complete D transactions by cache-line address and treats an in-progress multibeat ReleaseData as non-idle. Schemas 43/45 hold two through sixteen scalar/vector-composed target responses to the selected external outstanding depth, then use a seventeenth target to check forward progress after one fair response is released. Schema 36 holds C unready until one through eight B requests have all been accepted while CBOAck remains pending, then matches every completion by source/address. Schema 37 deliberately holds an atomic target D response, permits legal B backpressure during that interval, releases D, then holds C until all selected B requests are accepted and source/address-matched. The DCache agent rejects A-source reuse before the final D beat and separately observes response-queue drain; malformed/duplicate/early/late PTW and Uncache responses remain planned |
...
For a fault-only-first segment load, the independent page-table image proves
the selected boundary before the instruction is issued. The common schema-47
tail enumerates all 338 decoder-legal unit-stride EEW/SEW/LMUL/derived-EMUL/NF
shapes and crosses each with stage-1 Sv39/Sv48 and later/first fault position.
This is the complete Segment FOF addressing class: the decoder exposes FOF only
for `VLE{8,16,32,64}FF`, and `NF>0` selects VSegment. A later
fault requires exact bytes and mask bits for the mapped element prefix, trims
`vl`, and suppresses the page-fault writeback. A first-element fault preserves
the original `vl` and requires at least one identified page-fault writeback.
Which segment field reports that exception, and destination data/mask bits for
the faulting element or elements beyond the trim point, are architecturally
unspecified by the vector specification; the scoreboard therefore allows only
those fields and still requires exactly-once identity, fix-`vl`, and manager
conservation. The driver's lack of `enqLsq` for segment traffic is checked as
a top-level stimulus contract, not as a DUT result. This external architectural
oracle does not inspect the FOF buffer, replay count, bank, or cycle timing.
...
| --- | --- | --- |
| Atomics/LR-SC/AMO | Reservation and atomic-memory model with success/failure, alignment, ordering, and optional bus error injection | Partial: all exposed W/D-width AMO ALU variants, AMOCAS.W/D compare success/failure, LR/SC old-value/writeback, success/failure, cache visibility, the complete 24-opcode x every-illegal-byte-offset alignment matrix, and fixed-PMA atomic denial are executable in `atomic-contracts`. The 120 alignment cases distinguish LR load-misaligned from SC/AMO/AMOCAS store-misaligned exceptions, suppress RF write, forbid new DCache traffic, and cross the ROB pointer wrap. The PMA case requires `StoreAccessFault`, suppressed `rfWen`, no manager request, and unchanged backing memory. `atomic-dchannel-errors` covers the complete refill-capable W/D opcode x denied/corrupt matrix, persistent poisoned-line exceptions, exact request counts, and clean error-lifetime recovery. Schema-18 `random-mixed` composes atomic error presence/kind with family, width, translation, latency, and surrounding traffic; all 18 enabled family x width x outcome bins and the exact error-response/beat/GrantAck/refill tuple are independently verified. Schema 37 adds independent zero-through-eight Probe-burst selection to every successful family/width pair. For nonzero depth it chooses a fresh cold atomic target and distinct clean resident Probe lines, checks exactly one target refill, exact old-value/result and final memory, forbids early atomic writeback while D is held, then matches every no-data ProbeAck by source/address after legal B backpressure clears. All 54 family x width x depth bins and the depth-weighted manager count are conserved online and offline. Backing memory is checked unchanged at exception time, while a separate cache-image oracle predicts any later dirty ReleaseData from the intentional poisoned-line policy. Exceptional data and internal reservation state are not used as ISA oracles; cross-hart reservation interference and replacement/concurrent atomic ordering remain |
| CBO/CMO/fence | Separate architectural and bus line images, explicit permission transitions, and ordering/flush points | Correct invalidate/clean/zero effect, exact dirty writeback, required completion or exception, and ordering relative to loads/stores | Partial; cacheable CBO.ZERO's StoreQueue/SBuffer line-zero and readback are executable (`cbo-zero-contracts`). `cmo-contracts` observes CMO through the external DCache A/B/C/D boundary despite the internal LSQ response: CLEAN/FLUSH/INVAL require exact opcode/source/size/alignment, all six operation x clean/dirty line-state combinations, automatic drain of an already committed dirty store without a direct testbench flush, a 1024-cycle delayed CBOAck, exact TtoB/BtoB/TtoN/BtoN Probe reports and dirty data, retained-hit versus cold-refill line state, `flushPipe=1`, and all six operation x denied/corrupt exception cases. A younger cold load is accepted into another MSHR while CBOAck is pending, then must be canceled by the CMO flush with no delayed-response writeback. Schema-15 `random-mixed` uses the same success oracle in the active Bare/stage-1/nested context and adds independent `cmo-error`/`cmo-error-denied` controls. The opcode-qualified error agent requires exact StoreAccessFault/HardwareError, no Probe, unchanged bus memory, and self/younger redirect cleanup; per-seed gates cover every enabled operation x error kind and conserve errors separately from successful CMO-derived Probes. Schema 36 crosses CLEAN/FLUSH/INVAL x clean/dirty x one-through-eight accepted Probe depth while CBOAck is pending. Zero through seven fresh clean auxiliary lines are independently warmed and invalidated; the target response retains its operation-specific permission/data contract. All B sources and C addresses are matched, target dirty data is byte-exact, and the depth-weighted success total plus zero-Probe errors reconstructs manager traffic. `fence.i`, multiple simultaneous CMO sources, replacement/atomic composition, and wider multi-class ordering remain |
| VSegment/VFOF | Segment-element address/data model across multiple uops, fault-only-first truncation, per-segment mask, and whole-register EVL/addressing | Partial: multi-uop identity, takeover/ready behavior, store readback, later-element FOF truncation, first-element architectural fault/fault-VA preservation, exact fix-VL, and all 16 legal whole-register NF/EEW combinations are executable. Schema 44 adds common ordinary VFOF across later/first fault x Sv39/Sv48 x EEW8/16/32/64. Schema 47 extends common unit-stride segment FOF across all 338 decoder-legal EEW/SEW/LMUL/derived-EMUL/NF shapes and all 1,352 later/first fault x Sv39/Sv48 x shape bins, with independent page-walk, exact mapped-prefix data/mask, identified exception or suppression, final-VL, fix-VL, identity, and terminal-writeback oracles. The decoder has no indexed or strided Segment FOF encoding. Segment-field exception choice and faulting/remainder destination elements are allowed only where the vector specification leaves them unspecified. Exact first-fault VA is checked in isolated ordinary and segmented FOF environments because the exported address payload has no valid bit or transaction identity. Trigger metadata on an exceptional terminal response is left unconstrained where the interface does not define it. Every ordinary segment addressing mode exhausts 338 legal NF 2..8 x EEW/SEW/LMUL/EMUL configurations per mode with exact load/store/readback. Indexed modes additionally model the complete index register group and the index-only uops required when EMUL exceeds the segmented data group. The same legal shape model drives schema-9-and-later constrained random with separately weighted and observed addressing/EEW/SEW/LMUL/EMUL/NF dimensions. Internal VFOF-buffer state, replay count, and timing are never oracle inputs. An active-segment redirect is excluded from this boundary: full-core decode/dispatch/ROB enforce `waitForward`/`blockBackward` serialization and inhibit interrupts while the segment is resident. Overlapping indexed stores and observable ordered access to side-effecting targets remain |
...
| Probes/coherence errors | TileLink Probe/ProbeAck(Data) state model with separate bus/architectural line images, B-source allocation, response-address matching, manager outstanding depth, and source-lifecycle accounting | Partial: clean and dirty toB/toN permission reports, requested and mandatory data, exact dirty bytes, retained-line cleanup, and E-channel GrantAck are executable. Schema 33 `random-mixed` holds an unrelated cold refill, queues one through eight Probes on distinct active B sources, keeps C unready until every selected B request is accepted, requires that exact accepted-but-unanswered depth, matches every C response by source/address, and forbids delayed-load writeback until all finish. Its independent gate closes all 32 depth x cap x need-data bins and checks depth/overlap/marginal/manager conservation. Schema 34 derives the six-bit B-source namespace from the generated top-level port, rejects reuse while an earlier same-source response is active, walks all 64 IDs across sequences, and checks exact accepted-ID uniqueness, completed-ID reuse, and 63-to-0 wrap counts online and offline. Schema 36 repeats every accepted depth while each CMO operation is outstanding and closes all 48 operation x clean/dirty x depth bins. Schema 37 closes all 54 atomic family x width x zero-through-eight-depth bins. Nonzero depth queues unrelated clean Probes while a cold atomic D response is held; because the DCache may backpressure B until that response is released, accepted-depth pressure is created afterward by holding C. Malformed B/D responses, replacement cross-operation bursts, and random denied/corrupt composition remain |
