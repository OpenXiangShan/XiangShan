...
| Probes/coherence errors | TileLink Probe/ProbeAck(Data) state model with separate bus/architectural line images, B-source allocation, response-address matching, and manager outstanding depth | Partial: clean and dirty toB/toN permission reports, requested and mandatory data, exact dirty bytes, retained-line cleanup, and E-channel GrantAck are executable. Schema 32 `random-mixed` optionally holds an unrelated refill for 2048..4096 cycles, queues one or two clean auxiliary Probes plus the dirty primary Probe on distinct active B sources, requires the selected two- or three-request accepted-but-unanswered depth, matches every C response by source/address, and forbids delayed-load writeback until all finish. Four-or-more simultaneous Probes, malformed B/D responses, and random denied/corrupt composition remain |
