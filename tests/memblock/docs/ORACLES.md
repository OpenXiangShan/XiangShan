...
| DCache refill and same-line merge | Choose cold-load virtual addresses with bit 5 clear/set and build each 64-byte line independently in sparse memory. For merging, co-issue two offsets in one cold line before a deliberately delayed response | AcquireBlock echo must report one non-keyword and one keyword request; the manager returns the corresponding ordinary/critical-beat-first GrantData order, both exact load values complete, and each sink receives one GrantAck. The merged pair must return two exact values from exactly one external refill | Implemented in `single-load` |
...
| HLV/HLVX/HSV | Privilege/virtualization/PMP/PMA reference model including SPVP and execute permission | Effective mode, permission cause, final PA, RF/store side effect, and exception metadata. All three operation families execute under every Sv39/Sv48 and Sv39x4/Sv48x4 pair; a five-case PBMT basis covers final PMA/NC/IO and VS-over-G priority, cacheable misaligned HLV.D/HLVX.WU/HSV.D use exact split-data/readback oracles, M-mode execution crosses SPVP=U/S with physical PMP R/X/W permissions, and a fixed PMA device target checks HLV/HSV Uncache plus HLVX execute denial. Broader PMA and locked/edge hypervisor PMP crosses remain |
...
| Remaining PMP/PMA matrix | Extend the current hand-calculated TOR/NAPOT cases with broader fixed-PMA cacheability/atomic classification and locked/edge hypervisor regions | Access allowed/denied, cause, constrained manager requests, and exact region boundary behavior. Data-side TOR/NAPOT, R/W/AMO denial, lock/priority, 4-KiB-grain NA4 WARL behavior, M-mode HLV/HLVX/HSV using SPVP-selected PMP R/X/W permissions, and one fixed device PMA target are executable |
