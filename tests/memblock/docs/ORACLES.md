...
| Vector load data | Decode unit/strided/indexed addresses independently, including ordinary multi-uop unit-stride and strided `vuopIdx` base advancement, per-uop indexed vectors, and whole-register 16-byte uop windows; apply independent SEW/EEW, global-element `vl`/`vstart`/mask slicing, tail/merge policy, and old destination. Whole-register EVL is derived independently from NF and EEW | Exact 128-bit writeback, active-element behavior, independent `vsew/veew/vlmul` metadata, operation encoding, and identity | Implemented for modeled vector memory operations, including all 78 legal ELEN=64 unit-stride EEW/SEW/LMUL/EMUL configurations with 404 load/readback uops, ordered/unordered indexed loads at all four EEWs with repeated/non-monotonic indices across cache-line and Bare 4-KiB boundaries, representative LMUL=2 negative-stride/ordered-indexed loads, and all 16 NF 1/2/4/8 x EEW 8/16/32/64 whole-register combinations with an intentionally irrelevant input VL |
...
| VSegment/VFOF | Segment-element address/data model across multiple uops, fault-only-first truncation, per-segment mask, whole-register EVL/addressing, and ROB redirect cancellation | Partial: multi-uop identity, takeover/ready behavior, unit-stride/strided/indexed-unordered/indexed-ordered field offsets, store readback, later-element FOF truncation, first-element architectural fault/fault-VA preservation, exact fix-VL, load/store redirect, survivor reuse, FOF data/fix-VL cancellation, all 16 legal whole-register NF/EEW load/store/readback combinations, and all 78 legal ordinary unit-stride EEW/SEW/LMUL/EMUL configurations are executable; broader non-unit-stride and segmented LMUL/EMUL crosses remain |
