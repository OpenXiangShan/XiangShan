...
| RAW memory violation | Allocate an older unresolved-address store and a younger load with independently chosen ROB/FTQ/RVC metadata; let the load complete before resolving the store to the same physical bytes | Exactly one `memoryViolation` pulse identifies the younger load and requests self-flush | Partial: exact scalar RAW conflict and all eight exposed output fields are checked; non-overlap, oldest-of-multiple, wraparound, vector, RAR, and concurrent redirect arbitration crosses remain |
