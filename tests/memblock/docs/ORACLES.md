...
| FP and NaN-boxed MMIO loads | IEEE bit-pattern/reference load formatter plus NaN-box rule for narrow FP values | Partial: cacheable FLW/FLD exact FP destination bits, integer-RF suppression, and FLW NaN-boxing are executable in `fp-loads`; MMIO FP and broader exception behavior remain |
