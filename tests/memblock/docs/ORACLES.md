...
| Vector load data | Decode unit/strided/indexed addresses independently, including ordinary multi-uop unit-stride and strided `vuopIdx` base advancement, per-uop indexed vectors, and whole-register 16-byte uop windows; apply EEW plus global-element `vl`/`vstart`/mask slicing, tail/merge policy, and old destination. Whole-register EVL is derived independently from NF and EEW | Exact 128-bit writeback, active-element behavior, metadata, operation encoding, and identity | Implemented for modeled vector memory operations, including ordered/unordered indexed loads at all four EEWs with repeated/non-monotonic indices across cache-line and Bare 4-KiB boundaries, representative LMUL=2 two-uop unit/negative-stride/ordered-indexed loads, and all 16 NF 1/2/4/8 x EEW 8/16/32/64 whole-register combinations with an intentionally irrelevant input VL |
