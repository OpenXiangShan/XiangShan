...
| Scalar load early wakeup | Use the accepted issue lane and its independently generated `rfWen`, `fpWen`, and `pdest`; classify cache residency only by absence of a new external TileLink request | Every normal load that reaches one terminal writeback has one more matching wakeup than lane-local `ld2Cancel` pulses. Replay count and exact cycle are deliberately unconstrained | Partial: `load-feedback` covers all three lanes, cold misses, resident hits, both destination classes, and arbitrary legal replay counts; `random-mixed` requires canceled and uncanceled wakeups on every lane under mixed pressure. With hardware prefetch enabled, this gate uses a pre-training snapshot because prefetch cancel pulses intentionally have no backend wakeup; raw full-run counters are also recorded. Exception/MMIO/NC/forwarding/bank-conflict crosses are not yet independently classified |
