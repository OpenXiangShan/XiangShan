...
| Memory ordering violation | For RAW, allocate an older unresolved-address store and let a younger same-byte load complete before resolving the store. For RAR, let a younger load complete before an older load, force the shared dirty line through DCache writeback/release, then issue the older load. ROB/FTQ/RVC metadata is independently chosen | Exactly one `memoryViolation` pulse identifies the younger load with self-flush for RAW, or the older load with `flushAfter` for RAR | Partial: exact scalar RAW and RAR conflicts plus all eight exposed output fields are checked; non-overlap, oldest-of-multiple, wraparound, vector, and concurrent redirect arbitration crosses remain |
...
| Hardware data prefetch | For an isolated fixed-PC stride stream, independently advance the two-bit confidence state and calculate the L2 target from the detected byte stride and configured depth ratio | Exact L2 address and `Prefetch2L2Stride` source beginning at confidence saturation; disabling the prefetch CSR suppresses later outputs | Partial: exact positive stride and disabled negative controls are implemented; `random-mixed` composes weighted fixed-PC stride streams with ordinary and corner traffic and gates source-12 observation. SMS/stream causality, arbitration, exact random-stream address attribution, and L3-enabled configurations remain |
