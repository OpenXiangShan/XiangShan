...
| VSegment/VFOF | Segment-element address/data model across multiple uops, fault-only-first truncation, per-segment mask, whole-register EVL/addressing, and ROB redirect cancellation | Partial: multi-uop identity, takeover/ready behavior, unit-stride/strided/indexed-unordered/indexed-ordered field offsets, store readback, later-element FOF truncation, first-element architectural fault/fault-VA preservation, exact fix-VL, load/store redirect, survivor reuse, FOF data/fix-VL cancellation, and all 16 legal whole-register NF/EEW combinations are executable. Ordinary vector memory exhausts all 78 legal configurations per addressing mode; segment unit-stride and strided each exhaust 338 legal NF 2..8 x EEW/SEW/LMUL/EMUL configurations with exact load/store/readback. Indexed-segment LMUL/EMUL crosses remain |
