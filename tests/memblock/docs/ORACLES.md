...
| DFT bridge transport | Enumerate the ten standalone SRAM-broadcast/DFT-reset input bits while external functional reset is asserted | Every elaborated frontend/backend bridge output equals its corresponding current input for all 1,024 combinations; idle controls are restored before a fresh functional reset | Implemented in `dft-bridge-contracts` for ten frontend and four backend outputs. This proves MemBlock combinational routing only, not MBIST, scan, SRAM, or reset-tree function |
...
| --- | --- | --- |
| MMIO and device ordering | A side-effecting device model with read values, write log, byte enables, access width, response policy, and ordering points | Partial: `mmio-contracts` executes denied/corrupt/normal reads of a read-clear register, denied/corrupt/normal offset `SW` operations, and a final readback. The eight-entry log checks old/cleared/written values, zero error-response side effects, exact sequence, PMA MMIO class, address, size, mask, replicated bus data, response flags, no duplicates, and zero DCache requests. A second phase queues three same-address loads before the first request, delays two responses, and requires exact old/zero/zero results with maximum external outstanding depth one even when Uncache outstanding mode is enabled. Mixed queued load/store ordering remains |
| Atomics/LR-SC/AMO | Reservation and atomic-memory model with success/failure, alignment, ordering, and optional bus error injection | Partial: all exposed W/D-width AMO ALU variants, AMOCAS.W/D compare success/failure, LR/SC old-value/writeback, success/failure, cache visibility, representative misaligned D/W exception metadata, and fixed-PMA atomic denial are executable in `atomic-contracts`. The PMA case requires `StoreAccessFault`, suppressed `rfWen`, no manager request, and unchanged backing memory. `atomic-dchannel-errors` covers the complete refill-capable W/D opcode x denied/corrupt matrix, persistent poisoned-line exceptions, exact request counts, and clean error-lifetime recovery. Exceptional data and internal reservation state are not used as oracles; cross-hart reservation interference, full alignment crosses, and concurrent ordering remain |
