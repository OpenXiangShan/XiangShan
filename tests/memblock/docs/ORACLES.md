...
| Exceptions | Reference ISA cause plus circular ROB age, independent of LQ/SQ allocation order | Cause bits, fault VA/GPA, marker, single terminal exception writeback, and oldest-fault address when several faults coexist | Partial: exact scalar/vector causes and metadata, concurrent scalar load/store exception-address selection across ROB wrap with deliberately disagreeing LQ/SQ order, load/store buffer selection while both retain faults, and data-side PMP load/store/AMO access faults are implemented; same-ROB `uopIdx`, cross-cause priority, instruction/hypervisor PMP, and physical ECC crosses remain |
...
| FP and NaN-boxed MMIO loads | IEEE bit-pattern/reference load formatter plus NaN-box rule for narrow FP values | Partial: cacheable and PBMT=IO FLH/FLW/FLD exact FP destination bits, integer-RF suppression, FLH/FLW NaN-boxing, and FP-write suppression for page-fault/denied/corrupt are executable in `fp-loads`; PMP, permission, guest-fault, and misalignment crosses remain |
