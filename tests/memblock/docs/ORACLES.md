...
| DCache refill beat order | Choose cold-load virtual addresses with bit 5 clear/set and build each 64-byte line independently in sparse memory | AcquireBlock echo must report one non-keyword and one keyword request; the manager returns the corresponding ordinary/critical-beat-first GrantData order, both exact load values complete, and each sink receives one GrantAck | Implemented in `single-load` |
