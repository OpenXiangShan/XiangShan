...
| L2-to-L1 DTLB boundary | Drive the top-level `io_l2_tlb_req_req_*` request fields and observe the TLB response followed one cycle later by the registered PMP response | An ordinary cold lookup returns miss while starting an internal PTW refill; a same-VA retry returns the independently mapped PA with cacheable PBMT and clear fault fields, no additional external PTW TileLink A request, and correct PMP/PMA classification. Fields other than `miss` are not constrained on a miss because the real consumer gates them with `!miss`. Killed requests return nothing; `no_translate` follows its pruned-zero-`pmp_addr` boundary contract | Partial: `l2-tlb-contracts` covers ordinary miss/refill/hit, prefetch miss, kill, `no_translate`, cacheable PMP allow, locked 4-KiB PMP deny, and the address-zero MMIO classification. Positive PBMT NC/IO and PF/GPF/AF hit payloads remain; an external-L2 retry policy is integration-owned, but the internal shared-PFTLB refill is executable at MemBlock |
...
| Uncache/PBMT-NC | Ordered byte memory model with request size, mask, beat address, and source identity | Correct AccessAck/Data, legal size/address/mask, ordered store visibility, exact load data at every byte lane, denied/independent-corrupt load propagation, denied-store propagation, aligned external store-error address, scalar PBMT=IO store request/retirement, side-effecting device log, and CBO.ZERO line image | Partial; width/lane, both legal load-error forms, and denied-store response propagation are executable (`uncache-widths`, `uncache-errors`), including one external Uncache store-error pulse and no DCache error pulse. Denied load data also asserts corrupt, while corrupt on data-less store `AccessAck` is rejected as illegal. PBMT=IO direct three-cycle load metadata plus scalar store bypass/retirement, a physical non-DebugModule `c=0` PMA load/store pair, guarded DebugModule access-fault case, fixed device-to-DDR boundary loads, a seven-access error/side-effect sequence, queued same-class and mixed load/store device serialization, and cacheable CBO.ZERO line-zero/readback are executable (`mmio-contracts`, `cbo-zero-contracts`); malformed/duplicate/late response handling remains |
