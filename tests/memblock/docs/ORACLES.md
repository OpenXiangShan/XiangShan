...
| Vector load data | Decode unit/strided/indexed addresses independently, including ordinary multi-uop unit-stride and strided `vuopIdx` base advancement, per-uop indexed vectors, and whole-register 16-byte uop windows; apply EEW plus global-element `vl`/`vstart`/mask slicing, tail/merge policy, and old destination. Whole-register EVL is derived independently from NF and EEW | Exact 128-bit writeback, active-element behavior, metadata, operation encoding, and identity | Implemented for modeled vector memory operations, including representative LMUL=2 two-uop unit/negative-stride/ordered-indexed loads and all 16 NF 1/2/4/8 x EEW 8/16/32/64 whole-register combinations with an intentionally irrelevant input VL |
| Vector store data | Generate each active element's byte addresses and bytes from source data; apply mask, split, and whole-register EVL/address rules | Active-byte readback, completion, replay progress, queue drain, RF write enables, flush, debug sidebands, and whole-register load readback are checked for modeled stores | Partial; all 16 whole-register NF/EEW store combinations are byte-exact, while generic mixed stores remain limited by the output-only store boundary |
...
| VSegment/VFOF | Segment-element address/data model across multiple uops, fault-only-first truncation, per-segment mask, whole-register EVL/addressing, and ROB redirect cancellation | Partial: multi-uop identity, takeover/ready behavior, unit-stride/strided/indexed-unordered/indexed-ordered field offsets, store readback, later-element FOF truncation, first-element architectural fault/fault-VA preservation, exact fix-VL, load/store redirect, survivor reuse, FOF data/fix-VL cancellation, and all 16 legal whole-register NF/EEW load/store/readback combinations are executable; the broader ordinary LMUL/EMUL cross matrix remains |
