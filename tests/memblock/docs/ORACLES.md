...
| DCache refill, partial progress, same-line merge, and ordinary MLP | Choose cold-load virtual addresses with bit 5 clear/set and build each 64-byte line independently in sparse memory. For merging, independently choose 2/3-way depth, exact-address/same-beat/cross-beat placement, critical beat, translation context, load width/sign, and manager latency. For MLP, choose 2..16 distinct lines, initial scalar issue width 1..3, and scalar-only or scalar+vector-load composition. Select denied over the whole transaction or independent corrupt on exactly the first/last beat without consulting DUT data | AcquireBlock echo must report both critical-beat orders; each sink receives one GrantAck. Every clean merged member returns exact data; lines already observed at the manager are skipped, while one or more target requests and paired global refills/GrantAcks are accepted so duplicate/background traffic cannot decide correctness. An ordinary miss burst uses the same external freshness rule, arms address-qualified holds before advancing the DUT, and must externally reach its selected outstanding depth. Each scalar load requires one exact writeback and LQ dequeue; the legal two-flow vector member requires its exact vector writeback and two LQ dequeues. Under error, the critical word follows only its own beat's corrupt value, the opposite half observes the accumulated poisoned-line cause, and an unrelated second MSHR stays exact. The response queue must remain nonempty after critical-beat completion and become empty only after every D beat | Implemented in `single-load` and the six-case `dcache-errors` matrix, including 128/256-cycle interbeat gaps, maximum outstanding A depth two, and healthy/poisoned resident followups. Schema-17 `random-mixed` adds common response-wide clean/corrupt/denied outcomes on unique cold lines in Bare and translated contexts, exact exception/redirect/LQ cleanup, two error beats, and sink-attributed errored GrantAck/refill conservation under concurrent clean prefetch. Schema 20 adds nonrepeating 64-MiB clean merge pressure with all 12 depth/pattern/critical-beat bins and Bare/stage-1/nested coverage. Schema 43 closes distinct-line cold-burst depth x legal scalar issue-width x scalar/vector composition x translation using only external TileLink, architectural, and queue conservation. Schema 40 adds resident two/three-way same-address-bank waves over all eight address classes and enabled translation regimes; bank/replay classification is stimulus coverage only, while the oracle remains exact per-identity load data, one terminal writeback, and one LQ dequeue |
