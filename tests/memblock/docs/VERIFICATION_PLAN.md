...
| DCache manager | Coherent 64-byte lines on a 256-bit bus; finite randomized delay | Agent drives and observes A/B/C/D/E traffic, distinguishes Get/AcquireBlock/AcquirePerm/CLEAN/FLUSH/INVAL, checks ProbeAck(Data), ReleaseData, GrantAck, and CBOAck, and applies A/C/D/E timing pressure. CMO must use size 64, source 17, and an aligned line; directed manager behavior derives the operation-specific Probe before Ack and permits a distinct younger MSHR. Denied data responses also assert corrupt, while data-less Grant and CBOAck errors follow their operation-specific contracts. Common random Probe injection is implemented while wider randomized CMO concurrency, concurrent/multiple-source, and malformed response modes remain planned |
...
- wider multi-class CMO ordering and full-core `fence`/`fence.i` integration
  (CBO.ZERO and CLEAN/FLUSH/INVAL functional/error paths, a younger cold-load
  overlap canceled by CMO `flushPipe`, plus
  translation-fence ordering for global/selective leaf updates, both supported
  stage modes, and all four fully nested VS/G pairs are covered);
