...
| IFU-to-Mem PTW bridge | Typed `PtwReq` driver over `io_fetch_to_mem_itlb_*` with an independent page-table image | Exact Sv39/Sv48 and nested sector response metadata; request acceptance and response payload stability under backpressure | Partial: `ifetch-ptw-bridge` covers valid Sv39, Sv48, and all four Sv39/Sv48 x Sv39x4/Sv48x4 4-KiB walks with ASID/VMID, permission, PPN, index, and fault checks. Bare, faults, PBMT, duplicate requests, and concurrent DTLB activity remain planned |
...
| Backend `memoryViolation` (8), `ldCancel` (3), and wakeup (12) outputs | Partial, high priority | `load-feedback` independently samples all wakeup/cancel pins, matches lane and destination metadata, and requires one uncanceled wakeup per completed cold/warm load without constraining replay count. `random-mixed` gates canceled/uncanceled observations on every lane under mixed pressure. Add RAW/RAR `memoryViolation` and independently classified exception/MMIO/NC/forwarding/bank-conflict crosses |
...
The ordering for the next closure work is: first the remaining independently
observable backend correctness signal (`memoryViolation`) and broader
`ldCancel`/wakeup crosses, followed by the
remaining IFU PTW fault/concurrency cases; then concurrent Probe overlap and
...
separate top-level transaction; its valid-walk path is now executable while
fault and concurrency closure remains visible in this audit.
