...
oracle. Schema 46 adds the low-rate unit-stride M1 segment fault-only-first
subclass. It closes later/first fault x stage-1 mode x EEW x NF2..8, checks the
independent mapped/unmapped page layout, and accounts for one fix-VL writeback
per FOF action without depending on field selection, replay count, or timing.
Schema 40 adds an
...
| Vector segmented/whole-register | NF/segment count, multi-uop streams, load/store direction, EEW, mask/`vstart`, fault-only-first, and partial completion | Partial: `vector-segment` checks lane-0 takeover, interleaved load/store data, multi-uop identity, the driver-side no-`enqLsq` input contract, and all addressing modes. Every addressing mode exhausts 338 legal NF 2..8 x EEW/SEW/LMUL/EMUL configurations per mode with exact load/store/readback; strided cases cover positive/negative strides, while indexed cases cover the complete index group and 252 index-only uops required by 32 `EMUL>LMUL*NF` configurations across both modes. `vector-fof` and `vector-segment-fof` cover first/later faults, including exact first-fault VA in an isolated exception epoch; schema 44 brings ordinary VFOF into `random-mixed` and closes first/later fault x Sv39/Sv48 x EEW with identified exception writeback and exact fix-VL conservation. Schema 46 adds common unit-stride M1 segment FOF and closes first/later fault x Sv39/Sv48 x EEW x NF with independent mapped-prefix data, identified exception/suppression, final-VL, fix-VL, identity, and terminal-writeback accounting. Field-level exception choice and faulting/remainder destination elements remain unspecified where required by the vector specification. `trigger-contracts` covers field-1 indexed and strided-store breakpoints; `vector-addressing` exhausts all ordinary addressing matrices plus all 16 whole-register NF/EEW combinations. Schema-9-and-later `random-mixed` exposes addressing, EEW, SEW, LMUL, EMUL, NF, and direction weights through one interface and independently gates every enabled class. Full-core `waitForward`/`blockBackward` serialization makes an active segment plus redirect unreachable, so that combination is an upstream assumption rather than a MemBlock stimulus. Segment FOF beyond unit-stride M1, overlapping indexed stores, and ordered access to a side-effecting translated target remain |
...
Schema 46 now adds a common-generator segment FOF subclass for unit-stride M1
loads. Its independent page-table setup and identified writeback contract
cover later/first faults across stage-1 mode, EEW, and NF; arbitrary field
selection and faulting/remainder destination elements are intentionally not
treated as architectural mismatches. Indexed/strided segment FOF and active
segment redirect composition remain boundary gaps.
...
- wider simultaneous multi-class/multiple-CMO ordering and full-core
  ...
  pairs are covered);
- segment FOF beyond the schema-46 unit-stride M1 subset and redirect
  composition in the common constrained-random interface remain open; schema
  44 ordinary VFOF closes first/later fault x Sv39/Sv48 x EEW with exact
  fix-VL/LQ conservation, while schema 46 closes segment FOF first/later fault
  x Sv39/Sv48 x EEW x NF with exact fix-VL and terminal-writeback conservation.
  All 78 legal ordinary unit-
