...
| PMP/PMA | TOR/NA4/NAPOT, overlap priority, lock, M/R/W/X, cacheability, atomic/MMIO permissions, exact region edges | Partial; `pmp-contracts` programs the distributed PMP CSR boundary and checks TOR/NAPOT exact edges, R/W plus AMO denial, overlap priority, M-mode bypass, lock enforcement, and lock immutability. `PlatformGrain=12` makes NA4 unselectable and coerces `A=2` to a minimum 4-KiB NAPOT region. HLV/HLVX/HSV check SPVP-selected physical R/R+X/W permissions under M-mode for unlocked and locked regions plus a fixed SoC PMA device mapping; schema 25 promotes an interior device target into the common weighted class. DebugModule access, the fixed device-to-DDR boundary, and device-PMA AMO denial are also focused. Broader fixed-PMA permission/overlap/edge matrices and hypervisor PMP region edges remain |
...
- all 78 legal ordinary unit-stride, strided, indexed-unordered, and
  ...
  covered;
- broader fixed-PMA region/permission/edge crosses for HLV/HLVX/HSV,
