...
| TLB behavior | cold miss, hit, refill, duplicate miss, replay, invalidation, `sfence.vma`, concurrent page walks | Partial; `translation-fence` holds stale stage-1, VS-only, G-only, and fully nested VS/G root-leaf responses across their global fences, rejects canceled writeback, and requires new-data refills; duplicate/concurrent multi-walk coverage remains |
...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, selective/global scope and updates with outstanding traffic | Partial; global/selective leaf updates, targeted same-ID host-ASID/VS-ASID/VMID root reuse, and five global stale-response races spanning stage-1 and isolated/fully nested VS/G are implemented by `translation-fence`; selective and Sv48-family outstanding-traffic ordering remains |
...
- cross-hart LR/SC reservation interference, atomic ordering with concurrent
  ...
  cases are covered);
- CBO/CMO line operations, `fence`, `fence.i`, and selective or Sv48-family
  outstanding-traffic translation-fence ordering (global/selective leaf
  updates and five global stage-1/isolated-VS-G/fully-nested-VS-G
  stale-response races are covered by `translation-fence`);
...
- hardware A/D updates, selective/Sv48-family invalidation races, and
  ...
  `translation-permissions`; five global stage-1/isolated-VS-G/fully-nested-VS-G
  invalidation races are covered by `translation-fence`);
