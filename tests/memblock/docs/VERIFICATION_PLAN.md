...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, root/mode/ASID/VMID changes with outstanding requests | Global/selective leaf-update invalidation, same-ID host-ASID/VS-ASID/VMID root reuse with targeted fences, and delayed stale responses across both stage-1 modes, both isolated VS/G modes, and all four fully nested pairs are implemented at global/selective scope by `translation-fence-all`; concurrent multi-walk ordering remains |
