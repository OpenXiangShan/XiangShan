...
| Exception priority | multiple legal faults, differing ROB/uop vs LQ/SQ age, load/store/vector competition | Partial; wrapped ROB age deliberately disagrees with LQ/SQ order for simultaneous scalar load faults and simultaneous scalar store faults, the retained load/store selector is switched while both buffers hold exceptions, and a same-ROB LMUL=2 vector pair checks `uopIdx` ordering. Cross-cause ordering and scalar/vector competition remain planned |
...
- cross-cause and scalar/vector exception competition (concurrent wrapped-age
  scalar load/store selection and same-ROB vector `uopIdx` ordering are covered);
