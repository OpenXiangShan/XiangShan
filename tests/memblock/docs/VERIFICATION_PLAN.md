...
younger-load overlap/error presence/error kind, Probe/refill overlap, NC/MMIO load/store
...
pipeline while LR/SC/AMO is active. CMO errors are now in the common interface;
DCache refill/load, PTW, Uncache, and atomic errors remain interface work rather
than reasons for separately maintained random scenarios.
