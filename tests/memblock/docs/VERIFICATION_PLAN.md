...
| Refill/replay | delayed A/D responses, critical-beat-first ordering, partial refill, killed request, replay after miss | Partial; `single-load` forces both `isKeyword` polarities, returns the corresponding ordinary/reversed two-beat GrantData sequence, checks exact load data, and requires one GrantAck per refill; delayed and killed refills are exercised elsewhere, while explicit partial-refill progress remains |
