...
| T4: context and fences | ASID/VMID reuse, root changes, `V` transitions, `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, outstanding walks | Host/VS/G root and ID switches, host/nested `V` transitions, global/selective leaf-update fences, targeted same-ID host-ASID/VS-ASID/VMID root reuse, global/selective stale-response races, co-issued distinct-page walks, and redirected root/ASID/VMID/MODE changes with a delayed walk execute through both stage-1 modes and both G-stage modes; in-flight `V` transitions remain |
...
- complete PMP/PMA TOR/NA4/NAPOT, lock, priority, cacheability, and region-edge
  matrices;
- `V` transitions while page walks remain outstanding (host/VS/G
  root, ID, and MODE changes with delayed old responses are covered by
  `translation-inflight-context-all`; same-ID fenced root reuse and drained
