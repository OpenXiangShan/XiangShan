...
pipeline while LR/SC/AMO is active. CMO, ordinary scalar-load refill, and
Uncache errors are now in the common interface, including both legal Uncache
store-error contracts. PTW and atomic error controls remain interface work
rather than reasons for separately maintained random scenarios.
