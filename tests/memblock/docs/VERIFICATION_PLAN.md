...
| Data-side PMP | Hand-calculated TOR/NAPOT regions and architectural first-match, permission, lock, current/effective privilege, and platform-grain rules | Exact allow/fault result and constrained manager behavior on denial; ordinary load/store/AMO denials emit no DCache/Uncache request, while the current HLVX R-allowed/X-denied pipeline exposes exactly one killed early DCache lookup and no Uncache/RF effect | Partial: `pmp-contracts` covers TOR and NAPOT edges, R/W and AMO denial, overlap priority, M-mode unlocked bypass, locked enforcement/immutability, and 4-KiB-grain NA4-to-NAPOT WARL behavior. `hypervisor-contracts` crosses M-mode HLV/HLVX/HSV with SPVP=U/S and R-only/X-only/RW/RX PMP regions; locked/edge hypervisor regions and broader PMA crosses remain |
...
| Hypervisor memory ops | HLV/HLVX/HSV, effective privilege/SPVP, VSUM/VMXR, execute permission, guest/host faults | Partial: `hypervisor-contracts` executes all exposed HLV/HLVX/HSV encodings, SPVP user/supervisor cases, VSUM/VMXR permission changes, VS- and G-stage faults, HLVX execute-only access, representative operations under all four Sv39/Sv48 and Sv39x4/Sv48x4 pairs, a five-combination PBMT basis, and cacheable misaligned split paths. Twelve additional M-mode cases cross HLV/HLVX/HSV and SPVP=U/S with R-only/X-only/RW/RX physical PMP regions, requiring R, R+X, and W respectively. Schema-7+ `random-mixed` weights all three families. Physical PMA-device behavior and locked/edge PMP crosses remain |
...
- PMA-device crosses for HLV/HLVX/HSV plus locked/edge hypervisor PMP cases;
  ...
  permission, and the M-mode/SPVP physical R/R+X/W PMP matrix are covered;
