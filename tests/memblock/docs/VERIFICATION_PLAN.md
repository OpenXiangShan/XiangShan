...
| Vector segmented/whole-register | NF/segment count, multi-uop streams, load/store direction, EEW, mask/`vstart`, fault-only-first, redirect, and partial completion | Partial: `vector-segment` checks lane-0 takeover, interleaved load/store data, multi-uop identity, zero LSQ allocation, all addressing modes, delayed-refill redirect with survivor reuse, FOF data/fix-VL cancellation, and early store cancellation with original-data readback. Every addressing mode exhausts 338 legal NF 2..8 x EEW/SEW/LMUL/EMUL configurations per mode with exact load/store/readback; strided cases cover positive/negative strides, while indexed cases cover the complete index group and 252 index-only uops required by 32 `EMUL>LMUL*NF` configurations across both modes. `vector-segment-fof` covers first/later faults; `trigger-contracts` covers field-1 indexed and strided-store breakpoints; `vector-addressing` exhausts all ordinary addressing matrices plus all 16 whole-register NF/EEW combinations; schema-8 `random-mixed` independently gates load/store, all four EEWs, and NF 1..7. Overlapping indexed stores and ordered access to a side-effecting translated target remain |
