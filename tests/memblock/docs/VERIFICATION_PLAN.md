...
younger-load overlap/error presence/error kind, Probe/refill overlap, NC/MMIO
...
Uncache load errors and both legal store-error contracts are also in the common
interface. DCache non-CMO refill/load, PTW, and atomic error controls remain
interface work rather than reasons for separately maintained random scenarios.
