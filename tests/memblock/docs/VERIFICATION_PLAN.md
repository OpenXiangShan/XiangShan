...
| ECC/cache errors | correctable/uncorrectable data, error lifetime, retry or architectural exception | Partial; D-channel denied/corrupt metadata persistence and subsequent clean AtomicsUnit recovery are executable in `dcache-errors` and `atomic-dchannel-errors`. `dcache-errors` also uses `L1DCacheCtrl` one-shot bank-0 bit-0 injection on proven-resident lines: tag ECC must cancel, redirect, suppress terminal writeback, report the exact physical address to BEU, and recover on a clean survivor; data ECC in default `EnableAccurateLoadError=false` mode must report BEU, return the exact bit-flipped value without cancellation, and recover on a clean survivor. Other banks/masks, multi-bit correctable/uncorrectable classes, delayed/persistent injection, and concurrent traffic crosses remain |
...
- malformed TileLink coherence responses, broader Probe/source-reuse stress,
  multi-bank/multi-bit/delayed/persistent physical ECC injection, and
  ...
  and one-shot bank-0 bit-0 tag/data ECC are covered);
