...
| Address translation | Mode-parameterized software walk for Bare/Sv39/Sv48 and Sv39x4/Sv48x4; independent canonicality, PTE validity, PPN physical-width overflow, leaf level, alignment, permission, PBMT/N/reserved, and A/D checks | Accesses reach the independently calculated PA; invalid walks report the access-specific page/access/guest-page fault without a data-manager request | Partial: generic walker, Bare degenerations, all four 4-KiB nested paths, superpage leaves, Sv39/Sv48 high-half canonical and both noncanonical sign-extension directions, physical PPN bits 48/55 at leaf/top-level non-leaf for stage-1/G-stage load/store access faults, permission cases, the complete valid two-stage PBMT matrix, and 52 stage-1/G-stage invalid/reserved/PBMT/NAPOT encodings are implemented for both scalar loads and stores; remaining fault crosses remain |
...
| Translation faults | Invalid/non-leaf-at-level-0, W=1/R=0, invalid PTE, misaligned superpage, reserved/N/PBMT bits, leaf/non-leaf PTE.PPN beyond the 48-bit PA width, stage-1 vs G-stage access fault | First-fault level and cause are independent of DUT internals; no data or forbidden RF/store side effect; overflowing PPNs produce original load/store access faults with exact PTW traffic and no DCache/Uncache request, including G-stage `gaf`; scalar store against a read-only leaf must produce `StorePageFault` without a DCache/Uncache request |
