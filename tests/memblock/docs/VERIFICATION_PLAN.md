...
| Data-side PMP | Hand-calculated TOR/NAPOT regions and architectural first-match, permission, lock, privilege, and platform-grain rules | Exact allow/fault result and no DCache/Uncache request on denial | Partial: `pmp-contracts` covers TOR and NAPOT edges, R/W and AMO denial, overlap priority, M-mode unlocked bypass, locked enforcement/immutability, and 4-KiB-grain NA4-to-NAPOT WARL behavior; execute permission and hypervisor effective-mode crosses remain |
...
| Software prefetch | `prefetch.i/r/w`, mapped/unmapped, cacheable/NC, all lanes, duplicate and outstanding requests | Partial: `ifetch-prefetch` checks every class/lane, a same-cycle `i/r/w` batch, unmapped cold data-hint drop without PTW/data traffic, warmed Sv39 cacheable data-prefetch hits, and individual `r/w` DCache requests; PBMT-NC classification and broader duplicate/same-line contention remain |
...
| Hypervisor memory ops | HLV/HLVX/HSV, effective privilege/SPVP, VSUM/VMXR, execute permission, guest/host faults | Partial: `hypervisor-contracts` executes all exposed HLV/HLVX/HSV encodings, SPVP user/supervisor cases, VSUM/VMXR permission changes, VS- and G-stage faults, HLVX execute-only access, physical PMP execute denial, representative HLV/HLVX/HSV operations under all four Sv39/Sv48 and Sv39x4/Sv48x4 pairs, a five-combination PBMT basis that covers final PMA/NC/IO plus VS-over-G priority for every family, and cacheable misaligned HLV.D/HLVX.WU/HSV.D split paths with exact load/store readback. Schema-7+ `random-mixed` weights HLV/HLVX/HSV as one common-generator class. Physical PMA-device behavior and broader PMP crosses remain |
...
| Exception priority | multiple legal faults, differing ROB/uop vs LQ/SQ age, load/store/vector competition | Partial; wrapped ROB age deliberately disagrees with LQ/SQ order for simultaneous scalar load/store faults, the retained load/store selector is switched while both buffers hold exceptions, a same-ROB LMUL=2 vector pair checks `uopIdx`, page and PBMT-NC-misaligned faults are co-issued, and scalar/vector page faults replace each other in both arrival directions. Additional cause pairs, same-operation multi-cause priority, and store/vector competition remain planned |
...
- PMA-device crosses for HLV/HLVX/HSV plus the broader hypervisor PMP matrix;
  ...
  permission, and physical execute denial are covered;
...
- the broader cross-cause/source exception matrix beyond the co-issued scalar
  page-fault/PBMT-NC-misaligned pair and bidirectional scalar/vector page-fault
  replacement; wrapped-age load/store selection and same-ROB vector `uopIdx`
  ordering are also covered;
