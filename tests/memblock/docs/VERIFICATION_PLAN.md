...
| Performance, trace, interrupt, DFT, hart/reset, and bypass plumbing | Partial for simple MemBlock bridges; otherwise integration-owned | `top-control-contracts` checks current-cycle hart ID/power-down/MSI-ack/frontend-reset, one-cycle reset-vector/halt/critical-error/MSI-info/CLINT/BEU/perf-event transport, and two-cycle L2-prefetch control. It covers all 32 prefetch enable combinations and all 67 shared hardware-counter lanes; output lane 0 is tied zero while the standalone top prunes input 0/output 68. This proves transport, not performance-event meaning. Keep remaining pin/reset sanity in `pin-space`; test architectural interrupt/trace/DFT semantics at their owning integration boundary instead of claiming them from MemBlock data-path tests |
