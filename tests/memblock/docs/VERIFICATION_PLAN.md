...
| Vector segmented/whole-register | NF/segment count, multi-uop streams, load/store direction, EEW, mask/`vstart`, fault-only-first and partial completion | Partial: `vector-segment` checks lane-0 takeover, interleaved load/store data, multi-uop identity, zero LSQ allocation, and unit-stride/strided/indexed-unordered/indexed-ordered load/store/readback addressing; `vector-segment-fof` checks a later-element page fault is suppressed and the segment-owned fix-VL uop reports `VL=1`; schema-8 `random-mixed` independently gates load/store, all four EEWs, and NF 1..7 (2..8 fields). Whole-register transfers, LMUL/EMUL crosses, first-element faults, and redirect remain |
