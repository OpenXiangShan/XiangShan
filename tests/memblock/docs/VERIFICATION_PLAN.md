...
| Vector loads | EEW/SEW 8/16/32/64; fractional/integer LMUL and legal derived EMUL; both lanes; unit, strided, indexed unordered/ordered; mask, `vstart`, partial `vl`; split windows | Exact 128-bit result, active mask, metadata, replay, LQ drain; schema-10 `random-mixed` independently gates every enabled shape and expands complete 1..8-uop instructions |
| Vector stores | All legal EEW/SEW/LMUL/EMUL shapes and address modes; mask, `vstart`, partial `vl`; misaligned and cross-page split/replay | Exact active-byte readback, completion, commit, SQ drain; schema-10 shape counts conserve against instructions and queue-capacity windows preserve full uop identity |
...
EEW/SEW/LMUL/EMUL matrices. Schema-10 `random-mixed` now carries those ordinary
...
single-uop vector members. The next closure work is negative protocol
