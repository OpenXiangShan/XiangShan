...
| PTW manager | PTE memory and response source/size match the programmed roots | Reference page tables and PTW agent share sparse memory; selected whole-block responses may inject `denied`/`corrupt`, while malformed response validation remains planned |
