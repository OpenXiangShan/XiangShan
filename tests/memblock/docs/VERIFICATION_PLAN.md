...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, root/mode/ASID/VMID changes with outstanding requests | Global/selective leaf-update invalidation, same-ID host-ASID/VS-ASID/VMID root reuse with targeted fences, and one delayed stale stage-1 PTW response across global `SFENCE.VMA` are implemented by `translation-fence`; selective and VS/G-stage outstanding-walk ordering remains |
...
| TLB behavior | cold miss, hit, refill, duplicate miss, replay, invalidation, `sfence.vma`, concurrent page walks | Partial; `translation-fence` now holds a stale stage-1 root-leaf response across global `SFENCE.VMA`, rejects canceled writeback, and requires a new-data refill; duplicate/concurrent multi-walk coverage remains |
...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, selective/global scope and updates with outstanding traffic | Partial; global/selective leaf updates, targeted same-ID host-ASID/VS-ASID/VMID root reuse, and global stage-1 stale-response rejection are implemented by `translation-fence`; selective and VS/G-stage outstanding-traffic ordering remains |
...
- cross-hart LR/SC reservation interference, atomic ordering with concurrent
  ...
  cases are covered);
- CBO/CMO line operations, `fence`, `fence.i`, and selective or VS/G-stage
  outstanding-traffic translation-fence ordering (global/selective
  `sfence.vma`, selective `hfence.vvma`, global `hfence.gvma` leaf updates, and
  one global stage-1 stale-response race are covered by `translation-fence`);
...
- hardware A/D updates, nested/selective invalidation races, and concurrent page
  walks (missing-A load/store faults, missing-D store faults, and
  SUM/MXR/VSUM/VMXR permission outcomes are covered by
  `translation-permissions`; one global stage-1 invalidation race is covered by
  `translation-fence`);
