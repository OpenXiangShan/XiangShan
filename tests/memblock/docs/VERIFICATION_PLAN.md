...
followed by IFU PTW duplicate/concurrency cases; then concurrent Probe overlap and
hardware-prefetch causality;
...
transaction now covers the same mode depth plus PBMT and stage-1 faults;
duplicate and cross-source concurrency closure remains visible in this audit.
