...
| Performance-event meaning and DFT plumbing | Integration-owned | `top-control-contracts` proves interrupt and 67-lane performance-event transport but not event meaning. Keep DFT pin sanity in `pin-space`; test performance semantics and DFT behavior at their owning integration boundaries instead of claiming them from MemBlock data-path tests |
