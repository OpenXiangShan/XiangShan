...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, root/mode/ASID/VMID changes with outstanding requests | Global/selective leaf-update invalidation, same-ID host-ASID/VS-ASID/VMID root reuse with targeted fences, and delayed stale stage-1/isolated-VS/isolated-G responses across their global fences are implemented by `translation-fence`; selective and fully nested outstanding-walk ordering remains |
...
| TLB behavior | cold miss, hit, refill, duplicate miss, replay, invalidation, `sfence.vma`, concurrent page walks | Partial; `translation-fence` holds stale stage-1, VS-only, and G-only root-leaf responses across their global fences, rejects canceled writeback, and requires new-data refills; duplicate/concurrent multi-walk coverage remains |
...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, selective/global scope and updates with outstanding traffic | Partial; global/selective leaf updates, targeted same-ID host-ASID/VS-ASID/VMID root reuse, and global stale-response rejection for stage-1, VS-only, and G-only are implemented by `translation-fence`; selective and fully nested outstanding-traffic ordering remains |
...
- cross-hart LR/SC reservation interference, atomic ordering with concurrent
  ...
  cases are covered);
- CBO/CMO line operations, `fence`, `fence.i`, and selective or fully nested
  outstanding-traffic translation-fence ordering (global/selective leaf
  updates and global stage-1/VS-only/G-only stale-response races are covered by
  `translation-fence`);
...
- hardware A/D updates, fully nested/selective invalidation races, and
  ...
  `translation-permissions`; global stage-1/VS-only/G-only invalidation races
  are covered by `translation-fence`);
