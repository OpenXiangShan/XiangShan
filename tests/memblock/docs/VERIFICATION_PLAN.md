...
| Address translation | Mode-parameterized software walk for Bare/Sv39/Sv48 and Sv39x4/Sv48x4; independent canonicality, PTE validity, leaf level, alignment, permission, PBMT/N/reserved, and A/D checks | Accesses reach the independently calculated PA; invalid walks report the access-specific page/access/guest-page fault without a data-manager request | Partial: generic walker, Bare degenerations, all four 4-KiB nested paths, superpage leaves, Sv39/Sv48 permission cases, and 52 stage-1/G-stage invalid/reserved/PBMT/NAPOT encodings are implemented; store-side encoding and remaining fault crosses remain |
| Nested translation | Independent VS-stage walk followed by independent G-stage walk for all four `vsatp` x `hgatp` mode pairs, including implicit page-table accesses | Exact host PA or stage-specific fault; no stage may be skipped or silently treated as Bare | Partial: all four 4-KiB pairs, VS/G/Bare degenerations, VS/G context switches, host/nested `V` transitions, and same-ID root reuse with targeted fences are covered; outstanding-walk ordering remains |
...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, root/mode/ASID/VMID changes with outstanding requests | Global/selective leaf-update invalidation and same-ID host-ASID/VS-ASID/VMID root reuse with targeted fences are implemented by `translation-fence`; outstanding-walk ordering remains |
...
| Stage-1 Sv39 walk | Bare/Sv39 mode, L2/L1/L0 leaves (1-GiB/2-MiB/4-KiB), invalid/non-leaf, misaligned superpage, permission/PBMT/A-D combinations | Partial; all leaf levels, malformed superpage, U/S, SUM, MXR, missing-A/D, and 13 invalid/reserved/PBMT/NAPOT encoding cases execute; store-side encoding faults remain |
| Stage-1 Sv48 walk | Bare/Sv48 mode, L3/L2/L1/L0 leaves (512-GiB/1-GiB/2-MiB/4-KiB), L3 faults, canonicality, permission/PBMT/A-D combinations | Partial; all leaf levels, noncanonical VA, U/S, SUM, MXR, missing-A/D, and the same 13 encoding cases execute; store-side encoding faults remain |
| G-stage Sv39x4 walk | 16-KiB root, widened root index, 41-bit GPA, all leaf levels, high-GPA overflow, G-stage permissions | Partial; all leaf levels, high-GPA overflow, permission faults, and 13 invalid/reserved/PBMT/NAPOT encoding cases execute; store-D and valid two-stage PBMT composition remain |
| G-stage Sv48x4 walk | 16-KiB root, widened root index, 50-bit GPA, all leaf levels, high-GPA overflow, G-stage permissions | Partial; all leaf levels, high-GPA overflow, permission faults, and the same 13 encoding cases execute; store-D and valid two-stage PBMT composition remain |
...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, selective/global scope and updates with outstanding traffic | Partial; global/selective leaf updates and targeted same-ID host-ASID/VS-ASID/VMID root reuse are implemented by `translation-fence`; outstanding-traffic ordering remains |
...
- complete PMP/PMA TOR/NA4/NAPOT, lock, priority, cacheability, and region-edge
  matrices;
- context transitions while page walks remain outstanding (host/VS/G
  root-and-ID isolation, same-ID fenced root reuse, and
