...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, root/mode/ASID/VMID changes with outstanding requests | Global/selective leaf-update invalidation, same-ID host-ASID/VS-ASID/VMID root reuse with targeted fences, and five delayed stale responses across stage-1/VS/G fences are implemented for both global and selective scope by `translation-fence` and `translation-fence-selective`; Sv48-family outstanding-walk ordering remains |
...
| TLB behavior | cold miss, hit, refill, duplicate miss, replay, invalidation, `sfence.vma`, concurrent page walks | Partial; `translation-fence` and `translation-fence-selective` hold stale stage-1, VS-only, G-only, and fully nested VS/G root-leaf responses across global and selective fences, reject canceled writeback, and require new-data refills; duplicate/concurrent multi-walk coverage remains |
...
| Translation fences | `SFENCE.VMA`, `HFENCE.VVMA`, `HFENCE.GVMA`, selective/global scope and updates with outstanding traffic | Partial; global/selective leaf updates, targeted same-ID host-ASID/VS-ASID/VMID root reuse, and five global plus five selective stale-response races spanning stage-1 and isolated/fully nested VS/G are implemented by `translation-fence` and `translation-fence-selective`; Sv48-family outstanding-traffic ordering remains |
...
- cross-hart LR/SC reservation interference, atomic ordering with concurrent
  ...
  cases are covered);
- CBO/CMO line operations, `fence`, `fence.i`, and Sv48-family
  outstanding-traffic translation-fence ordering (global/selective leaf
  updates and five global plus five selective stage-1/isolated-VS-G/
  fully-nested-VS-G stale-response races are covered by the translation-fence
  targets);
...
- hardware A/D updates, Sv48-family invalidation races, and
  concurrent page walks (missing-A load/store faults, missing-D store faults, and
  SUM/MXR/VSUM/VMXR permission outcomes are covered by
  `translation-permissions`; five global plus five selective stage-1/
  isolated-VS-G/fully-nested-VS-G invalidation races are covered by the
  translation-fence targets);
