...
| Direct maintenance/configuration controls | Partial | Focused tests cover direct SBuffer flush, timeout-driven eviction, Uncache outstanding enable/disable behavior, MBMC BME/CMODE/BCLEAR/BMA bitmap policy, architectural prefetch-control defaults, guarded DebugModule PMA access, all outer-L2 flush enable/completion combinations, and the hart/reset-vector/power-down/halt/critical-error bridge. L2 enable, hart ID, and power-down are checked combinationally; L2 completion, reset vector, halt, and critical error are checked at exact one-cycle latency on every monitored functional-test cycle. Dynamic reprogramming during arbitrary overlapping traffic and the remaining CSR cross-product remain |
...
| Performance, trace, interrupt, DFT, hart/reset, and bypass plumbing | Partial for simple MemBlock bridges; otherwise integration-owned | `top-control-contracts` semantically checks current-cycle hart ID and power-down plus one-cycle reset-vector, CPU-halt, and CPU-critical-error propagation across all eight boolean combinations and eight data patterns; its monitor remains active in functional tests. Keep remaining pin/reset sanity in `pin-space`; test architectural interrupt/trace/DFT semantics at their owning integration boundary instead of claiming them from MemBlock data-path tests |
...
- concurrent MMIO/device ordering and error-response side-effect policy beyond
  the current ordered read-clear/partial-write log, PBMT=IO denied/corrupt
  metadata, and SQ-retirement contracts;
