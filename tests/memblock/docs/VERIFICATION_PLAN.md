...
hypervisor family, CMO operation/line state/younger overlap, NC/MMIO direction, legal
...
younger-load overlap, NC/MMIO load/store
...
| DCache manager | Coherent 64-byte lines on a 256-bit bus; finite randomized delay | Agent drives and observes A/B/C/D/E traffic, distinguishes Get/AcquireBlock/AcquirePerm/CLEAN/FLUSH/INVAL, checks ProbeAck(Data), ReleaseData, GrantAck, and CBOAck, and applies A/C/D/E timing pressure. CMO must use size 64, source 17, and an aligned line; operation-specific Probe-before-Ack behavior and a distinct younger MSHR are modeled in both directed and schema-13 random traffic. Random CMO varies operation, clean/dirty state, overlap, locality, active translation context, and response latency. Denied data responses also assert corrupt, while data-less Grant and CBOAck errors follow their operation-specific contracts. Concurrent multiple-CMO/source windows, common random CMO error injection, and malformed response modes remain planned |
