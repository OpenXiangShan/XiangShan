...
| Backend `memoryViolation` (8), `ldCancel` (3), and wakeup (12) outputs | Partial, high priority | `load-feedback` independently samples all wakeup/cancel pins, matches lane and destination metadata, requires one uncanceled wakeup per completed cold/warm load without constraining replay count, and checks three resident same-bank loads with no TileLink request, at least two cancellations, and cross-lane wakeup conservation under replay migration. It separately classifies an Sv39 load-page fault, PBMT=IO MMIO, and PBMT=NC: the fault cancels every wakeup without DCache traffic, while MMIO/NC each retain one uncanceled wakeup, use PTW plus Uncache, and avoid DCache; MMIO also exercises `pendingMMIOld`. `random-mixed` gates canceled/uncanceled observations on every lane under mixed pressure; with hardware prefetch enabled it freezes this architectural gate before training and records full-run raw counts separately because prefetch cancels have no backend wakeup. `memory-violation` checks all eight redirect fields, a same-line byte-disjoint negative control, three-candidate oldest selection with ROB wraparound, a vector load flow behind an unresolved store, and two store-address lanes accepted together with the lane-1 candidate deliberately older to verify cross-source ROB-age arbitration; `rar-violation` checks dirty-line release ordering, the architectural enable, and RAR's distinct `flushAfter` result. Add independently classified forwarding, access-fault, guest-page-fault, and bus-error cancellation crosses plus RAR-specific multi-candidate/wraparound cases |
...
The ordering for the next closure work is: first the remaining broader
`ldCancel`/wakeup cause crosses and RAR multi-candidate/wraparound cases,
followed by concurrent Probe overlap and hardware-prefetch causality;
