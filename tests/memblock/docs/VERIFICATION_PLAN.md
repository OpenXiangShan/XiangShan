...
| Trigger and DynInst sidebands | Independent CSR trigger update plus explicit LSQ enqueue exception/trigger/flush and scalar issue RVC/FTQ/store-set/load-wait fields | Exact action/cause policy, manager suppression, store-image preservation, exception-vector mapping, and no dropped issue sideband | Partial: `trigger-contracts` covers all four memory-trigger slots, EQ/GE/LT hit/miss boundaries, both legal actions, enable/load/store/select/breakpoint/debug-state gates, a two-entry chain hit/miss pair, aligned/misaligned scalar stores, and unit-stride vector loads/stores. Scalar issue RVC/FTQ/store-set/load-wait paths and generated enqueue exception-vector mapping are also executable; the top-level issueLda boundary does not expose that vector. Segment/wider vector addressing-action crosses and broad sideband randomization remain |
