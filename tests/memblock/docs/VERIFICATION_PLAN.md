...
| Direct maintenance/configuration controls | Partial | Focused tests cover direct SBuffer flush, timeout-driven eviction, Uncache outstanding enable/disable behavior, MBMC BME/CMODE/BCLEAR/BMA bitmap policy, architectural prefetch-control defaults, guarded DebugModule PMA access, and all outer-L2 flush enable/completion combinations. The L2 enable is checked combinationally and completion is checked at exact one-cycle latency on every monitored functional-test cycle. Dynamic reprogramming during arbitrary overlapping traffic and the remaining CSR cross-product remain |
