...
| Performance, trace, interrupt, DFT, hart/reset, and bypass plumbing | Partial for simple MemBlock bridges; otherwise integration-owned | `top-control-contracts` checks current-cycle hart ID/power-down/MSI-ack/frontend-reset, one-cycle reset-vector/halt/critical-error/MSI-info/CLINT/BEU/perf-event transport, all 256 combinations of the eight interrupt sinks, and two-cycle L2-prefetch control. It checks all seven external-interrupt outputs, including the BEU-local/NMI0 OR mapping, all 32 prefetch enable combinations, and all 67 shared hardware-counter lanes; output lane 0 is tied zero while the standalone top prunes input 0/output 68. This proves interrupt and performance-event transport, not subsystem semantics. Keep remaining pin/reset sanity in `pin-space`; test architectural interrupt/trace/DFT semantics at their owning integration boundary instead of claiming them from MemBlock data-path tests |
