...
| CBO/CMO/fences | clean/invalidate/flush/zero, `fence`, `fence.i`, `sfence.vma`, ordering with outstanding traffic | Partial; cacheable `CBO.ZERO` StoreQueue/SBuffer line-zero and readback are executable (`cbo-zero-contracts`). `cmo-contracts` covers CLEAN/FLUSH/INVAL opcode/source/line alignment, automatic SBuffer drain of a previously committed dirty store without a direct flush, SBuffer-before-request ordering proven by the resulting exact dirty Probe data, clean no-data invalidation, retained-hit/cold-refill outcomes, delayed completion, `flushPipe`, and every operation crossed with denied/corrupt CBOAck. The internal `cmoOpResp` is inferred through external DCache A/B/C/D and final StoreQueue writeback. Translation fences are covered separately; `fence.i` and wider concurrent CMO ordering remain |
...
- full CMO ordering against arbitrary concurrent traffic, `fence`, and
  `fence.i` (CBO.ZERO and CLEAN/FLUSH/INVAL functional/error paths plus
