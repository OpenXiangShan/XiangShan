...
coverage. Atomic AMO/LRSC/AMOCAS family, W/D width, and D-channel error
presence/kind, CMO operation/line state/
...
| Hypervisor memory ops | HLV/HLVX/HSV, effective privilege/SPVP, VSUM/VMXR, execute permission, guest/host faults | Partial: `hypervisor-contracts` executes all exposed HLV/HLVX/HSV encodings, SPVP user/supervisor cases, VSUM/VMXR permission changes, VS- and G-stage faults, HLVX execute-only access, representative operations under all four Sv39/Sv48 and Sv39x4/Sv48x4 pairs, a five-combination PBMT basis, and cacheable misaligned split paths. Eighteen M-mode cases cross all three families and SPVP=U/S with R/X/RW/RX plus locked R/RWX physical PMP regions, requiring R, R+X, and W; nine fixed-PMA cases cover an interior device target plus every family immediately below and at the exact `0x80000000` device-to-DDR boundary, checking HLV/HSV manager routing and HLVX execute permission. Schema-22 `random-mixed` independently constrains SPVP and closes all enabled family x SPVP crosses through supervisor identity mappings or separate user-region physical aliases covering every 4-KiB/Svnapot VS/G leaf combination. Schema 23 closes every enabled family x SPVP x aligned/misaligned cross. Schema 24 closes every enabled family x SPVP x five-pair PBMT basis with independent PA/data/store and manager-route oracles. Schema 25 closes every enabled family x SPVP x DDR/fixed-device address-class bin; device HLV/HSV require exact Uncache effects and device HLVX requires precise execute denial with no manager request and conserved cancellation. Non-PMA random pairs and device actions are naturally aligned; other fixed-PMA boundaries and broader hypervisor PMP region-edge crosses remain |
...
| PMP/PMA | TOR/NA4/NAPOT, overlap priority, lock, M/R/W/X, cacheability, atomic/MMIO permissions, exact region edges | Partial; `pmp-contracts` programs the distributed PMP CSR boundary and checks TOR/NAPOT exact edges, R/W plus AMO denial, overlap priority, M-mode bypass, lock enforcement, and lock immutability. `PlatformGrain=12` makes NA4 unselectable and coerces `A=2` to a minimum 4-KiB NAPOT region. HLV/HLVX/HSV check SPVP-selected physical R/R+X/W permissions under M-mode for unlocked and locked regions, an interior fixed SoC PMA device mapping, and both sides of the exact `0x80000000` device-to-DDR boundary; schema 25 promotes the interior target into the common weighted class. DebugModule access and device-PMA AMO denial are also focused. Other fixed-PMA permission/overlap/edge matrices and hypervisor PMP region edges remain |
...
| DCache lookup | warm hit, cold miss, same-line merge, bank conflict, set pressure beyond associativity, synonym/alias | Partial; cold/warm and bank conflict are executable. Schema-20 `random-mixed` promotes same-line merging into the common constraint interface: fresh cold lines cross 2/3 same-cycle load lanes, exact-address/same-beat/cross-beat patterns, both critical beats, Bare/stage-1/nested translation, and the selected manager latency. Schema 21 similarly promotes dirty set pressure: each compound action selects one of four set-index quarters and drives 9/10 distinct cold tags through an eight-way set, with exact request, writeback, dequeue, and ReleaseData attribution. Broader physical synonym/alias crosses remain |
...
| Eviction | clean release, dirty ReleaseData, partial byte masks, replacement under pressure, release backpressure | Partial; immutable whole-line snapshots are checked by the dedicated dirty-pressure phase and schema-21 random pressure. The common interface crosses 9/10-line pressure, SB/SH/SW/SD masks, four set-index quarters, both store-half issue orders, all translation regimes, and manager latency. Every target request/writeback/dequeue is exact; target and background ReleaseData are byte-checked separately. Clean-release and wider concurrent replacement/backpressure crosses remain |
| TileLink coherence | Probe/B/C/E traffic, source reuse, denied/corrupt/error responses, manager ordering | Partial; `dcache-coherence` executes three manager Probes spanning clean no-data, clean requested-data, and dirty mandatory-data responses, checks byte-exact C beats, forces E backpressure, and matches every GrantAck sink. It separately accepts two distinct Probe sources before an unrelated cold miss completes and checks outstanding depth and response addresses. `cmo-contracts` checks custom A opcode 12/13/14, fixed source 17, CBOAck opcode 8, all clean/dirty permission reports, Probe-before-Ack ordering, exact dirty C beats, a concurrent younger MSHR plus cancellation, and denied/corrupt propagation. `random-mixed` adds constrained dirty toB/toN Probes with requested/mandatory data and checked toB cleanup, schema-15 translated CMO operation/state/younger-MSHR/error crosses, and schema-14 weighted overlap of a delayed refill with two distinct Probe B sources. CMO errors match address plus CBO opcode and are excluded exactly from Probe conservation. The overlap oracle requires both address-matched C responses, no early delayed-load writeback, and maximum Probe depth at least two. `dcache-errors` rejects A-source reuse until final D completion, reaches two simultaneous AcquireBlock lifetimes, and checks exact D-beat/GrantAck drain. Load and atomic denied/corrupt D responses are also injected and checked; malformed responses, more than two simultaneous Probes, and broader source-reuse stress remain planned |
...
| DCache A/B/C/D/E TileLink (53 ports) | Implemented | A/C/D refill, error, eviction, ReleaseData, and CMO paths plus deterministic B Probe and E GrantAck are checked. `dcache-coherence` accepts two distinct B source IDs before an unrelated cold miss writes back, requires accepted Probe depth >= 2, and address-matches both C responses; C-source remains an independent WritebackQueue ID. `cmo-contracts` observes CLEAN/FLUSH/INVAL and CBOAck at this boundary, including every clean/dirty permission transition, a simultaneous younger MSHR, flush cancellation, and all error cases. Schema-15 `random-mixed` constrains dirty toB/toN Probe sequences, requested/mandatory data, CMO operation/state/overlap, and CBOAck error presence/kind across address contexts and randomized response latency. Schema 17 adds ordinary scalar-load clean/corrupt/denied refill outcomes, exact two-beat error accounting, and sink-attributed errored GrantAck/refill conservation under concurrent clean prefetch traffic; schema 18 extends the same conservation to all enabled atomic family/width outcomes and checks any eventual poisoned dirty ReleaseData. Schema 21 drives 9/10 fresh dirty tags through each selected eight-way set and separately attributes target versus older-workload ReleaseData while checking every C beat against immutable or live reference bytes |
...
Schema-2 queue-conservation gate. Schema 21 now brings real same-set dirty
replacement into this interface. The next breadth closure is clean replacement
plus wider overlapping refill/release pressure. Malformed manager responses
...
| DCache manager | Coherent 64-byte lines on a 256-bit bus; finite randomized delay | Agent drives and observes A/B/C/D/E traffic, distinguishes Get/AcquireBlock/AcquirePerm/CLEAN/FLUSH/INVAL, checks ProbeAck(Data), ReleaseData, GrantAck, and CBOAck, and applies A/C/D/E timing pressure. CMO must use size 64, source 17, and an aligned line; operation-specific Probe-before-Ack behavior and a distinct younger MSHR are modeled in directed and common random traffic. Random CMO varies operation, clean/dirty state, overlap, locality, active translation context, response latency, and opcode-qualified denied/corrupt Ack. Schema-14 random Probe traffic can queue two distinct B sources while an unrelated refill is delayed and proves both C responses precede the load writeback; all outstanding D responses and GrantAcks are then drained by state rather than a fixed cycle delay. Schema 17 adds address-qualified ordinary load-refill errors and records errored refills and GrantAcks separately from concurrent clean prefetch traffic. Schema 18 adds address-qualified AMO/LR/AMOCAS errors, exact two-beat plus GrantAck/refill accounting, and a cache-versus-backing-memory model for later poisoned-line eviction. Denied data responses also assert corrupt, while data-less Grant and CBOAck errors follow their operation-specific contracts. More-than-two Probe depth, concurrent multiple-CMO/source windows, and malformed response modes remain planned |
...
- broader fixed-PMA region/permission/edge crosses for HLV/HLVX/HSV beyond the
  ...
  non-PMA PBMT/misalignment priority, plus hypervisor PMP region-edge cases;
