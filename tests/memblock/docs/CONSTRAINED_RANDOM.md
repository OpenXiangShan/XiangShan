...
`random-mixed` requires at least 864 actions so the mandatory architectural
...
| Set replacement concurrency | Schema 30 adds `set-pressure-dual-window`, crosses single/dual with every clean/dirty, refill-overlap, C-backpressure, depth, width, and translation class, and closes 384 bins. Dual actions allocate distinct sets from opposite quarters. Overlap actions keep two address-qualified D responses pending and require each set to reach its own replacement minimum before either response is released; all request/writeback/dequeue accounting is weighted by window count | Extend to three-or-more independent sets and compose replacement with Probe/CMO concurrency only after their legal scheduling and attribution contracts are explicit |
