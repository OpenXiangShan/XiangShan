...
class TransformForEncDec extends Module {
  ...
  val byte_0 = Wire(UInt(8.W))
  val byte_1 = Wire(UInt(8.W))
  val byte_2 = Wire(UInt(8.W))
  val byte_3 = Wire(UInt(8.W))
  val byte_0_replaced = Wire(UInt(8.W))
  val byte_1_replaced = Wire(UInt(8.W))
  val byte_2_replaced = Wire(UInt(8.W))
  val byte_3_replaced = Wire(UInt(8.W))
  val word_replaced = Wire(UInt(32.W))



  byte_0 := io.data_in(7, 0)
  byte_1 := io.data_in(15, 8)
  byte_2 := io.data_in(23, 16)
  byte_3 := io.data_in(31, 24)

  val sbox_replace_u_0 = Module(new SboxReplace)
  sbox_replace_u_0.io.dataIn := byte_0
  byte_0_replaced := sbox_replace_u_0.io.resultOut

  val sbox_replace_u_1 = Module(new SboxReplace)
  sbox_replace_u_1.io.dataIn := byte_1
  byte_1_replaced := sbox_replace_u_1.io.resultOut

  val sbox_replace_u_2 = Module(new SboxReplace)
  sbox_replace_u_2.io.dataIn := byte_2
  byte_2_replaced := sbox_replace_u_2.io.resultOut

  val sbox_replace_u_3 = Module(new SboxReplace)
  sbox_replace_u_3.io.dataIn := byte_3
  byte_3_replaced := sbox_replace_u_3.io.resultOut

  word_replaced := Cat(byte_3_replaced, byte_2_replaced, byte_1_replaced, byte_0_replaced)
...
class OneRoundForKeyExp extends Module {
  ...
  val word0 = io.dataIn(127, 96)
  val word1 = io.dataIn(95, 64)
  val word2 = io.dataIn(63, 32)
  val word3 = io.dataIn(31, 0)
  ...
  val tmp0 = Mux(io.countRoundIn === 0.U, k1 ^ k2, word1 ^ word2)
  val tmp1 = Mux(io.countRoundIn === 0.U, k3 ^ dataForXor, word3 ^ dataForXor)
  ...
  io.resultOut := Mux(io.countRoundIn === 0.U,
                      ...
                      Cat(word1, word2, word3, transformKey.io.data_after_linear_key_out ^ word0))
}
...
class OneRoundForEncDec  extends Module {
  ...
  val word0 = io.data_in(127, 96)
  val word1 = io.data_in(95, 64)
  val word2 = io.data_in(63, 32)
  val word3 = io.data_in(31, 0)


  val tmp0 = word1 ^ word2
  val tmp1 = word3 ^ io.round_key_in
  ...
  io.result_out := Cat(word1, word2, word3, transform_encdec.io.result_out ^ word0)
}
...
class GF128 extends Module{
  ...
  val tweal_1_isgf = io.tweak_in(127)
  val tweal_2_isgf = io.tweak_in(126)
  val tweal_3_isgf = io.tweak_in(125)
  ...
  tweak_1_out := Mux(tweal_1_isgf, tweak_1_shifted ^ gf_128_fdbk, tweak_1_shifted)
  tweak_2_out := Mux(tweal_2_isgf, tweak_2_shifted ^ gf_128_fdbk, tweak_2_shifted)
  tweak_3_out := Mux(tweal_3_isgf, tweak_3_shifted ^ gf_128_fdbk, tweak_3_shifted)
...
class WriteChanelRoute(implicit p:Parameters) extends MemEncryptModule
{
  val io = IO(new Bundle {
    ...
    // Unencrypt Chanel
    val out0 = new Bundle{
      val aw = DecoupledIO(new AXI4BundleAW(MemcedgeIn.bundle))
      val w = DecoupledIO(new AXI4BundleW(MemcedgeIn.bundle))
    }
    // Encrypt Chanel
    val out1 = new Bundle{
      val aw = DecoupledIO(new AXI4BundleAW(MemcedgeIn.bundle))
  ...
  val aw_queue = Module(new Queue(io.in.aw.bits.cloneType, 1, flow = true))
  ...
  val unencrypt_aw_queue = Module(new Queue(io.in.aw.bits.cloneType, MemencPipes+1, flow = true))
  val unencrypt_w_queue = Module(new Queue(io.in.w.bits.cloneType, (MemencPipes+1)*2, flow = true))
...
class WriteChanelArbiter(implicit p:Parameters) extends MemEncryptModule
{
  val io = IO(new Bundle {
    // Unencrypt Chanel
    val in0 = new Bundle{
      val aw = Flipped(DecoupledIO(new AXI4BundleAW(MemcedgeOut.bundle)))
      val w = Flipped(DecoupledIO(new AXI4BundleW(MemcedgeOut.bundle)))
    }
    // Encrypt Chanel
    val in1 = new Bundle{
      val aw = Flipped(DecoupledIO(new AXI4BundleAW(MemcedgeOut.bundle)))
  ...
  val reg_idle = RegInit(true.B)
  // Cut aw_queue.io.enq.ready from io.out*.awready
  val aw_queue = Module(new Queue(io.in0.aw.bits.cloneType, 1, flow = true))
...
class RdataChanelRoute(implicit p:Parameters) extends MemEncryptModule
{
  val io = IO(new Bundle {
    ...
    // Unencrypt Chanel
    val out_r0 = DecoupledIO(new AXI4BundleR(MemcedgeOut.bundle))
    // Encrypt Chanel
    val out_r1 = DecoupledIO(new AXI4BundleR(MemcedgeOut.bundle))
  ...
  Connectable.waiveUnmatched(io.out_r0.bits, io.in_r.bits) match {
    case (lhs, rhs) => lhs.squeezeAll :<>= rhs.squeezeAll
  }

  Connectable.waiveUnmatched(io.out_r1.bits, io.in_r.bits) match {
    case (lhs, rhs) => lhs.squeezeAll :<>= rhs.squeezeAll
  }
