...
{
  val io = IO(new Bundle {
    val in_w = Flipped(DecoupledIO(new AXI4BundleW(MemcedgeOut.bundle)))
    ...
    val out_ar = DecoupledIO(new AXI4BundleAR(MemcedgeOut.bundle))
    val in_r = Flipped(DecoupledIO(new AXI4BundleR(MemcedgeOut.bundle)))
...
class WdataEncrptyPipe(implicit p:Parameters) extends MemEncryptModule
{
  val io = IO(new Bundle {
    ...
    val out_w = DecoupledIO(new AXI4BundleW(MemcedgeIn.bundle))
...
class RdataDecrptyPipe(implicit p:Parameters) extends MemEncryptModule
{
  val io = IO(new Bundle {
    ...
    val out_r = DecoupledIO(new AXI4BundleR(MemcedgeOut.bundle))
...
class RdataRoute(implicit p:Parameters) extends MemEncryptModule
{
  val io = IO(new Bundle {
    ...
    val out_r1 = DecoupledIO(new AXI4BundleR(MemcedgeIn.bundle))
  })
...
class AXI4MemEncrypt(address: AddressSet)(implicit p: Parameters) extends LazyModule with Memconsts
{
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val waddr_q = Module(new Queue(in.aw.bits.cloneType, entries = MemencPipes+1))
      val wdata_q = Module(new Queue(in.w.bits.cloneType, entries = MemencPipes+1))
      ...
      val ar_arb = Module(new Arbiter(out.ar.bits.cloneType, 2))
      ...
      val r_arb = Module(new Arbiter(out.r.bits.cloneType, 2))
