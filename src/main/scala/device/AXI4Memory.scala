  ...
  with HasExtModuleInline
{
  ...
  val cppExtModule =
    """
      ...
      |) {
      |  if (reset) response = 0;
      |  else if (enable) response = memory_response(REQUEST_TYPE);
      |  else response = 0;
      |}
      |""".stripMargin
...
trait MemoryHelper { this: Module =>
  ...
  private def request(valid: Bool, addr: UInt, id: UInt, isWrite: Boolean): Bool = {
    ...
    RegNext(helper.io.response, false.B)
  }
  ...
  private def response(enable: Bool, isWrite: Boolean): (Bool, UInt) = {
    ...
    (
      RegNext(helper.response(32), false.B),
      RegNext(helper.response(31, 0), 0.U(32.W))
    )
  }
