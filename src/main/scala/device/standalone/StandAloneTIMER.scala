...
class StandAloneTIMER (
  ...
  private def clintParam = TIMERParams(baseAddress)
  def addressSet: AddressSet = clintParam.address

  private val clint = LazyModule(new TIMER(clintParam, dataWidth / 8))
  clint.node := xbar
  ...
  class StandAloneTIMERImp(outer: StandAloneTIMER)(implicit p: Parameters) extends StandAloneDeviceImp(outer) {
    ...
    outer.clint.module.io.time := io.time
  }

  override lazy val module = new StandAloneTIMERImp(this)
