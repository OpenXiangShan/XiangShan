...
class PMU extends Module {
  ...
  val PON_DLY_CLKEN_ISO = 70.U(8.W) // PowerOn delay for clockEn -> Isolate
  val PON_DLY_ISO_RST = 70.U(8.W)   // PowerOn delay for Isolate -> Reset 
  val PON_DLY_ISO_CLKEN = 70.U(8.W) // PowerOff delay for Isolate -> clockEn
  val PON_DLY_CLKEN_RST = 70.U(8.W) // PowerOff delay for clockEn -> Reset
  ...
  // Control signals
  val coreResetReg = RegInit(false.B)   //1: Reset          0: no Reset(reset)
  val coreIsolateReg = RegInit(false.B) //1: Isolate        0: no Isolate(reset)
  val coreClkenReg = RegInit(true.B)   // 1: Enable(reset)  0: clock not enable
  ...
  switch(state) {
    ...
    is(sPOFF_ISO) {
      when(delayCounter === 0.U) {
        // Step 2: Set coreIsolate =1 to Isolate output after PDN_DLY_CLKEN_ISO
    ...
    is(sPOFF_RST) {
      when(delayCounter === 0.U) {
        // Step 3: Set coreReset =1 to Reset after PDN_DLY_ISO_RST
        delayCounter := 0.U
        coreResetReg := true.B
        state := sPOFF
      }
    ...
    is(sPOFF) {
      ...
      when(io.coreWakeReq === true.B) {
        // Step 1: Set coreIsolate = 0 to Disable isolation
    ...
    is(sPON_RST) {
      when(delayCounter === 0.U) {
        // Step 3: Set coreReset =0 to exit Reset after PON_DLY_CLKEN_RST
        delayCounter := 0.U
  ...
  val nPOWERUP = power_off
  val nISOLATE = !coreIsolateReg
  dontTouch(nPOWERUP)
  dontTouch(nISOLATE)
