...
class imsic_axi_top(
  ...
  val io = IO(new Bundle {
    ...
    val m_s = Flipped(new AXI4LiteBundle(AXI_ADDR_WIDTH, 32, AXI_ID_WIDTH))
    // bus to access the s interrupt file
    val s_s = Flipped(new AXI4LiteBundle(AXI_ADDR_WIDTH, 32, AXI_ID_WIDTH))
    // imsic_csr_top
...
class imsic_bus_top(
  ...
  class imsic_bus_top_imp(wrapper: imsic_bus_top) extends LazyModuleImp(wrapper) {
    ...
    // axi4lite io
    val m_s = Option.when(!useTL)(IO(Flipped(new AXI4LiteBundle(AXI_ADDR_WIDTH, 32, AXI_ID_WIDTH))))
    val s_s = Option.when(!useTL)(IO(Flipped(new AXI4LiteBundle(AXI_ADDR_WIDTH, 32, AXI_ID_WIDTH))))
    ...
    wrapper.axi4.foreach { axi4 =>
      axi4.map(_.in.head._1) zip Seq(u_imsic_axi_top.io.m_s, u_imsic_axi_top.io.s_s) foreach {
        case (axi4, axi4lite) => axi4lite.connectFromAXI4(axi4)
      }
    }
  }
