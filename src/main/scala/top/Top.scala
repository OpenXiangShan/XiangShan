...
class XSTop()(implicit p: Parameters) extends BaseXSSoc() with HasSoCParameter
{
  ...
  class XSTopImp(wrapper: LazyModule) extends LazyRawModuleImp(wrapper) {
    ...
    val reset_sync = if (debugOpts.FPGAPlatform) withClockAndReset(io.clock.asClock, io.reset) { ResetGen() } else io.reset
    val jtag_reset_sync = if (debugOpts.FPGAPlatform) withClockAndReset(io.systemjtag.jtag.TCK, io.systemjtag.reset) { ResetGen() } else io.systemjtag.reset
