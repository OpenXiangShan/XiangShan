...
class L2Prefetcher()(implicit p: Parameters) extends LazyModule with HasPrefetchParameters {
  val clientParameters = TLMasterPortParameters.v1(
    Seq(TLMasterParameters.v1(
      name = "l2prefetcher",
      sourceId = IdRange(0, l2PrefetcherParameters.nEntries+1)
    ))
  )
...
class L2PrefetcherImp(outer: L2Prefetcher) extends LazyModule(outer) with HasPrefetchParameters with HasXSLog {  
  ...
  if (l2PrefetcherParameters.enable && l2PrefetcherParameters._type == "stream") {
    val p = l2PrefetcherParameters.streamParams
    val dPrefetch = Module(new StreamPrefetch(p))
