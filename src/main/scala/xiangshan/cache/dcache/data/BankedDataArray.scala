...
class BankedDataArray(implicit p: Parameters) extends AbstractBankedDataArray {
  ...
  // read result: expose banked read result
  private val mbist_r_way = OHToUInt(mbistSramPorts.map(_.flatMap(_.map(w => Cat(w.map(_.re)))).reduce(_ | _)).getOrElse(0.U(DCacheWays.W)))
  ...
  (0 until DCacheBanks).foreach(i => {
    ...
    io.readline_resp(i) := RegEnable(bank_result(readline_r_div_addr)(i)(readline_r_way_addr), io.readline_can_go | mbist_ack)
