  ...
  with HasPerfEvents
{
  ...
  for ((forward, i) <- io.forward.zipWithIndex) {
    val f0_fwdValid = forward.req.valid
    val f1_fwdValid = RegNext(f0_fwdValid)

    /* f0 */
    // vaddr match
    val f0_vtagMatches = sizeMap(w => addrMatch(entries(w).vaddr, forward.req.bits.vaddr))
    ...
    val f1_fwdPAddr = RegEnable(forward.req.bits.paddr, f0_fwdValid)
    // select
    ...
    XSDebug(
      ...
      RegEnable(forward.req.bits.vaddr, f0_fwdValid),
      RegEnable(forward.req.bits.paddr, f0_fwdValid)
    )
    // response
    forward.resp.addrInvalid := false.B // addr in ubuffer is always ready
    forward.resp.dataInvalid := false.B // data in ubuffer is always ready
    forward.resp.matchInvalid := f1_tagMismatchVec(i) // paddr / vaddr cam result does not match
    for (j <- 0 until VDataBytes) {
      forward.resp.forwardMaskFast(j) := f0_flyMaskFast(j) || f0_idleMaskFast(j)

      forward.resp.forwardData(j) := f1_fwdData(j)
  ...
  XSPerfAccumulate("forward_count", PopCount(io.forward.map(_.resp.forwardMask.asUInt.orR)))
  ...
  val perfEvents = Seq(
    ...
    ("forward_count", PopCount(io.forward.map(_.resp.forwardMask.asUInt.orR))),
