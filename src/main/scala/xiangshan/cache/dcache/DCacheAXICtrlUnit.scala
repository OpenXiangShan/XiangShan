...
// AXI4RegisterNode D$ CtrlUnit. Register map and pseudo-error injection match CtrlUnit
// (CtrlUnitCtrlBundle field comments live on the shared bundle in CtrlUnit.scala).
class DCacheAXICtrlUnit(params: L1CacheCtrlParams)(implicit p: Parameters) extends LazyModule
  with HasDCacheParameters
{
  ...
  lazy val module = new DCacheAXICtrlUnitImp

  class DCacheAXICtrlUnitImp extends LazyModuleImp(this) {
    ...
    for (i <- 0 until params.nSignalComps) {
      NewPipelineConnect(
        ...
        Option(s"DCacheAXICtrlUnitPseudoErrorPipelineConnect${i}")
      )
    }
