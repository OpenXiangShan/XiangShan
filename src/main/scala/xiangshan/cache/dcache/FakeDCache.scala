...
class FakeDCache()(implicit p: Parameters) extends XSModule with HasDCacheParameters {
  ...
  io.lsu.store.mainPipeHitResp := DontCare
  //io.lsu.store.refill_hit_resp := DontCare
  io.lsu.store.replayResp := DontCare
  io.lsu.store.mainPipeHitResp.valid := RegNext(io.lsu.store.req.valid)
  io.lsu.store.mainPipeHitResp.bits.id := RegEnable(io.lsu.store.req.bits.id, io.lsu.store.req.valid)
  // to atomics
  ...
  amoHelper.wdata := io.lsu.atomics.req.bits.amoData
  amoHelper.mask := io.lsu.atomics.req.bits.amoMask
