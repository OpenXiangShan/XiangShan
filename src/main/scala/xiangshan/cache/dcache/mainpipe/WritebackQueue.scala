...
class WritebackQueue(edge: TLEdgeOut)(implicit p: Parameters) extends DCacheModule with HasTLDump with HasPerfEvents
{
  ...
  val perfEvents = Seq(
    ("dcache_wbq_req"      , io.req.fire).withDescription("Requests accepted by the data-cache writeback queue."),
    ("dcache_wbq_1_4_valid", perfValidCount <= (cfg.nReleaseEntries.U / 4.U)).withDescription("Cycles with at most one quarter of data-cache writeback entries valid."),
    ("dcache_wbq_2_4_valid", perfValidCount > (cfg.nReleaseEntries.U / 4.U) && perfValidCount <= (cfg.nReleaseEntries.U / 2.U)).withDescription("Cycles with writeback-queue occupancy above one quarter and at most one half."),
    ("dcache_wbq_3_4_valid", perfValidCount > (cfg.nReleaseEntries.U / 2.U) && perfValidCount <= (cfg.nReleaseEntries.U * 3.U / 4.U)).withDescription("Cycles with writeback-queue occupancy above one half and at most three quarters."),
    ("dcache_wbq_4_4_valid", perfValidCount > (cfg.nReleaseEntries.U * 3.U / 4.U)).withDescription("Cycles with more than three quarters of data-cache writeback entries valid."),
  )
  generatePerfEvent()
