...
// for manually CSE
class MatchSignals extends Bundle {
...
class MissReqPipeRegBundle(edge: TLEdgeOut)(implicit p: Parameters) extends DCacheBundle
 with HasCircularQueuePtrHelper
 {
  ...
  def isKeyword(): Bool= {
    ...
    val merge_store = (req.isFromLoad || req.isFromPrefetch) && req.isFromStore
    val merge_req_rst = Mux(
                          alloc,
                          (merge_load || merge_store),
                          false.B
                        )
...
class MissEntry(edge: TLEdgeOut, reqNum: Int)(implicit p: Parameters) extends DCacheModule
  with HasCircularQueuePtrHelper
 {
  ...
  for (i <- 0 until reqNum) {
    ...
    val _signals = io.miss_req_pipe_reg.computeMatchSignals(req, io.queryME(i).req.bits)
  ...
  io.prefetch_info.late_prefetch := io.req.valid && !(io.req.bits.isFromPrefetch) && req_valid && signals.block_match && prefetch
...
class MissQueue(edge: TLEdgeOut, reqNum: Int)(implicit p: Parameters) extends DCacheModule
  ...
  val signals = miss_req_pipe_reg.computeMatchSignals(miss_req_pipe_reg.req, io.req.bits)
  ...
  for (i <- 0 until reqNum) {
    ...
    val _signals = miss_req_pipe_reg.computeMatchSignals(miss_req_pipe_reg.req, io.queryMQ(i).req.bits)
...
//  assert(RegNext(PopCount(Seq(merge, reject)) <= 1.U))
  ...
  // prefetch related
  io.prefetch_info.naive.late_miss_prefetch := io.req.valid && io.req.bits.isPrefetchRead && (miss_req_pipe_reg.matched(signals) || Cat(entries.map(_.io.matched)).orR)
  io.prefetch_info.naive.pf_source := io.req.bits.pf_source

  io.prefetch_info.fdp.late_miss_prefetch := (miss_req_pipe_reg.prefetch_late_en(signals, io.req.bits.toMissReqWoStoreData(), io.req.valid) || Cat(entries.map(_.io.prefetch_info.late_prefetch)).orR)
