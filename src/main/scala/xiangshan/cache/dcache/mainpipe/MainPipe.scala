...
class MainPipe(implicit p: Parameters) extends DCacheModule with HasPerfEvents with HasL1PrefetchSourceParameter {
  ...
  if (cacheCtrlParamsOpt.nonEmpty && EnableTagEcc) {
    encTag_resp.zip(io.tag_resp).map {
    ...
    encTag_resp := io.tag_resp
  }
  tag_resp := VecInit(encTag_resp.map(encTag => encTag(tagBits - 1, 0)))

  def wayMap[T <: Data](f: Int => T) = VecInit((0 until nWays).map(f))
  val s1_tag_errors = wayMap((w: Int) => Meta(meta_resp(w)).coh.isValid() && dcacheParameters.tagCode.decode(encTag_resp(w)).error).asUInt
  ...
  val s1_encTag = ParallelMux(s1_tag_match_way.asBools, (0 until nWays).map(w => encTag_resp(w)))
  ...
  io.pseudo_error.ready := s2_fire_to_s3 && (s2_tag_error || s2_hit && s2_may_report_data_error)
