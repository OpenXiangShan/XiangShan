...
class MainPipe(implicit p: Parameters) extends DCacheModule with HasPerfEvents with HasL1PrefetchSourceParameter {
  ...
  val s2_replace_block = io.replace_addr.valid && io.replace_block
  ...
  val s2_can_go_to_mq_replay = (s2_req_miss_without_data && RegEnable(s2_req_miss_without_data && !io.mainpipe_info.s2_replay_to_mq, false.B, s2_valid)) || s2_replace_block // miss_req in s2 but refill data is invalid, can block 1 cycle
