...
class MainPipe(implicit p: Parameters) extends DCacheModule with HasPerfEvents with HasL1PrefetchSourceParameter {
  ...
  val s2_need_replacement = RegEnable(s1_need_replacement, s1_fire) && !s2_refill_tag_eq_way
  val s2_need_eviction = RegEnable(s1_need_eviction, s1_fire) && !s2_refill_tag_eq_way
  ...
  if(EnableTagEcc) {
    val s2_probe_or_atomic = s2_req.probe || s2_req.isAMO
