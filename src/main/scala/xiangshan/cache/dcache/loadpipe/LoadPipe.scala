...
class LoadPipe(id: Int)(implicit p: Parameters) extends DCacheModule with HasPerfEvents with HasL1PrefetchSourceParameter {
  ...
  resp.bits.replay := (resp.bits.miss && s2_nack) || io.bank_conflict_slow || s2_wpu_pred_fail
  resp.bits.replayCarry.valid := (resp.bits.miss && s2_nack) || io.bank_conflict_slow || s2_wpu_pred_fail
  ...
  resp.bits.handled := s2_miss_req_fire && !io.wbq_block_miss_req && io.miss_resp.handled
  ...
  io.lsu.s2_mq_nack       := (resp.bits.miss && (s2_nack_no_mshr || io.wbq_block_miss_req))
