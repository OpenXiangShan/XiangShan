  ...
  with HasPerfEvents
{
  ...
  def TLBRead(i: Int) = {
    ...
    val mptPerm = WireInit(VecInit(Seq.fill(nRespDups)(0.U.asTypeOf(new MptPermBundle(isTlbPort = true))))) // HasMptCheck
    for (d <- 0 until nRespDups) {
  ...
  def perm_check(mptPerm: MptPermBundle, mptEn: Bool, perm: TlbPermBundle, cmd: UInt, idx: Int, nDups: Int,
    ...
    val mptWf = Option.when(HasMptCheck) (!(mptPerm.r && mptPerm.w) && (isSt || isShadowStack))
    ...
    val isMptAf = Option.when(HasMptCheck) ((mptInstf.get || mptWf.get || mptRf.get || mptAf.get) && mptValid.get)
    ...
    when (RegNext(prepf || pregpf || preaf)) {
      ...
      resp(idx).bits.excp(nDups).af.ld := (af && TlbCmd.isRead(cmd) && !isShadowStack && fault_valid) ||
        (if (HasMptCheck) (((mptRf.get || (mptAf.get && TlbCmd.isRead(cmd))) && !isShadowStack) && mptValid.get) else false.B)
      ...
      resp(idx).bits.excp(nDups).af.st := (af && (TlbCmd.isWrite(cmd) || isShadowStack) && fault_valid) ||
        ...
        (mptAf.get && TlbCmd.isWrite(cmd))) && mptValid.get) else false.B) ||
