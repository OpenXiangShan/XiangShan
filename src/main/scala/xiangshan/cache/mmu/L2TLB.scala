...
class L2TLBImp(outer: L2TLB)(implicit p: Parameters) extends PtwModule(outer) with HasCSRConst with HasPerfEvents {
  ...
  mq_arb.io.in(0).valid := cache.io.resp.valid && !cache.io.resp.bits.hit &&
    ...
    (cache.io.resp.bits.bypassed || (
      (((!cache.io.resp.bits.toFsm.l1Hit || toFsm_toPTW) || cache.io.resp.bits.toFsm.stage1Hit) && !cache.io.resp.bits.isHptwReq && (cache.io.resp.bits.isFirst || !ptw.io.req.ready)) // send to ptw, is first or ptw is busy;
  ...
  ptw.io.req.valid := cache.io.resp.valid && !cache.io.resp.bits.hit && (!cache.io.resp.bits.toFsm.l1Hit || toFsm_toPTW) &&
    !cache.io.resp.bits.bypassed &&
