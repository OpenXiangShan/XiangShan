...
class L2TLBImp(outer: L2TLB)(implicit p: Parameters) extends PtwModule(outer) with HasCSRConst with HasPerfEvents {
  ...
  if(HasCVMExtension){
    ...
    cache.io.wakeup.bits := DontCare
    cache.io.wakeup.valid := DontCare
  }
  ...
  llptw.io.in.valid := cache.io.resp.valid &&
    ...
    (cache.io.resp.bits.toFsm.toLLPTW || cache.io.resp.bits.toFsm.l1Hit) &&
    !cache.io.resp.bits.bypassed &&
  ...
  llptw.io.in.bits.jmp_bitmap_check := cache.io.resp.bits.toFsm.jmp_bitmap_check
  llptw.io.in.bits.ptes := cache.io.resp.bits.toFsm.ptes
  llptw.io.in.bits.cfs := cache.io.resp.bits.toFsm.cfs
  llptw.io.in.bits.hitway := cache.io.resp.bits.toFsm.hitway
  ...
  cache.io.resp.ready := MuxCase(mq_arb.io.in(0).ready || ptw.io.req.ready, Seq(
    ...
    ((cache.io.resp.bits.toFsm.toLLPTW || cache.io.resp.bits.toFsm.l1Hit) && !cache.io.resp.bits.bypassed && llptw.io.in.ready) -> llptw.io.in.ready,
  ...
  ptw.io.req.valid := cache.io.resp.valid && !cache.io.resp.bits.hit && !cache.io.resp.bits.toFsm.l1Hit &&
    ...
    !cache.io.resp.bits.isHptwReq  &&
    !cache.io.resp.bits.toFsm.toLLPTW
  ...
  ptw.io.req.bits.jmp_bitmap_check := cache.io.resp.bits.toFsm.jmp_bitmap_check
  ptw.io.req.bits.pte := cache.io.resp.bits.toFsm.pte
  ptw.io.req.bits.cfs := cache.io.resp.bits.toFsm.cfs
  ptw.io.req.bits.SPlevel := cache.io.resp.bits.toFsm.SPlevel
  ...
  hptw.io.req.bits.jmp_bitmap_check := cache.io.resp.bits.toHptw.jmp_bitmap_check
  hptw.io.req.bits.pte := cache.io.resp.bits.toHptw.pte
  hptw.io.req.bits.ptes := cache.io.resp.bits.toHptw.ptes
  hptw.io.req.bits.cfs := cache.io.resp.bits.toHptw.cfs
  hptw.io.req.bits.hitway := cache.io.resp.bits.toHptw.hitway
  hptw.io.req.bits.fromSP := cache.io.resp.bits.toHptw.fromSP
  hptw.io.req.bits.SPlevel := cache.io.resp.bits.toHptw.SPlevel
  ...
  if(HasCVMExtension){
    ...
    bitmap_arb.io.in(0) <> ptw.io.bitmap.req
    bitmap_arb.io.in(1) <> llptw.io.bitmap.req
    bitmap_arb.io.in(2) <> hptw.io.bitmap.req
    ...
    Bitmap.io.resp.ready := (llptw.io.bitmap.resp.ready && bitmapresp_to_llptw) || (hptw.io.bitmap.resp.ready && bitmapresp_to_hptw) || (ptw.io.bitmap.resp.ready && bitmapresp_to_ptw)

    //bitmap -> llptw ptw hptw
    llptw.io.bitmap.resp.valid := Bitmap.io.resp.valid && bitmapresp_to_llptw
    hptw.io.bitmap.resp.valid := Bitmap.io.resp.valid && bitmapresp_to_hptw
    ptw.io.bitmap.resp.valid := Bitmap.io.resp.valid && bitmapresp_to_ptw

    //add ptw、hptw、llptw with bitmap resp connect
    ptw.io.bitmap.resp.bits := Bitmap.io.resp.bits
    hptw.io.bitmap.resp.bits := Bitmap.io.resp.bits
    llptw.io.bitmap.resp.bits := Bitmap.io.resp.bits
    ...
    llptw.io.bitmap.resp.valid := DontCare
    hptw.io.bitmap.resp.valid := DontCare
    ptw.io.bitmap.resp.valid := DontCare
    ptw.io.bitmap.resp.bits := DontCare
    hptw.io.bitmap.resp.bits := DontCare
    llptw.io.bitmap.resp.bits := DontCare
    ptw.io.bitmap.req.ready := DontCare
    hptw.io.bitmap.req.ready := DontCare
    llptw.io.bitmap.req.ready := DontCare
    ptw.io.bitmap.req.bits := DontCare
    hptw.io.bitmap.req.bits := DontCare
    llptw.io.bitmap.req.bits := DontCare
  }
  //ptwcache -> hptw llptw
  hptw.io.l0_way_info := cache.io.l0_way_info
  llptw.io.l0_way_info := cache.io.l0_way_info
