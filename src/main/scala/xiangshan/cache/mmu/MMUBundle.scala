  ...
  def apply(item: PtwRespS2): TlbSectorEntry = {
    ...
    */
    ...
    val s1ppn = if(!HasMptCheck) (item.s1.entry.ppn(sectorppnLen - 1, 0))
        ...
        0.U -> Mux(item.s1.entry.n.getOrElse(0.U) === 0.U, item.s1.entry.ppn(sectorppnLen - 1, 0), Cat(item.s1.entry.ppn(sectorppnLen - 1, pteNapotBits- sectortlbwidth), item.s1.entry.tag(pteNapotBits - 1- sectortlbwidth, 0)))
        ))
      )
    val s1ppn_low =  if(!HasMptCheck) (item.s1.ppn_low) else (Mux((item.s1.entry.level.getOrElse(0.U) > 0.U)||item.s1.entry.n.getOrElse(0.U)>0.U,VecInit(Seq.fill(tlbcontiguous)(item.s1.addr_low)),item.s1.ppn_low))
...
class TlbStorageIO(nSets: Int, nWays: Int, ports: Int, nDups: Int = 1)(implicit p: Parameters) extends MMUIOBaseBundle {
  val r = new Bundle {
    ...
    val resp = Vec(ports, ValidIO(new Bundle{
      ...
      val mptperm = Option.when(HasMptCheck)(Vec(nDups, Output(new MptPermBundle(isTlbPort = true)))) //HasMptCheck,add mptperm
    }))
  }
...
class MptPermBundle(isTlbPort: Boolean = true)(implicit p: Parameters) extends TlbBundle {
  ...
  def resp_apply( mptresp:mptTlbRespBundle) = {
    ...
    if(isTlbPort) (this.af.get:=mptresp.accessFault)
...
class HptwResp(implicit p: Parameters) extends PtwBundle {
  ...
  def hit(gvpn: UInt, vmid: UInt, mergeMptLevel: Boolean = false, mptTlbResp:mptTlbRespBundle = 0.U.asTypeOf(new mptTlbRespBundle)): Bool = {
    ...
    val mpt_NAPOT= Option.when(mergeMptLevel) (mpt_merge_n(this.entry.level.get, this.entry.n.getOrElse(0.U), mptTlbResp.mptLevel, mptTlbResp.permIsNAPOT))
    val mpt_level = Option.when(mergeMptLevel) (mptTlbResp.mptLevel min this.entry.level.get)

    val tag_match = Wire(Vec(Level + 1, Bool()))
    tag_match(0) := Mux((if(mergeMptLevel) (mpt_NAPOT.get) else (entry.n.getOrElse(0.U)) )=== 0.U,
...
class PtwSectorResp(implicit p: Parameters) extends PtwBundle {
  ...
  def hit(vpn: UInt, asid: UInt, vmid: UInt, allType: Boolean = false, ignoreAsid: Boolean = false, mptIgnoreAsid: Bool = false.B, mergeMptLevel: Boolean = false, mptTlbResp:mptTlbRespBundle = 0.U.asTypeOf(new mptTlbRespBundle)): Bool = {    require(vpn.getWidth == vpnLen)
    require(allType)
    // require(this.asid.getWidth <= asid.getWidth)
    ...
    val isSuperPage =if(mergeMptLevel) (mpt_level.get=/= 0.U || mpt_NAPOT.get=/= 0.U) else (entry.level.getOrElse(0.U) =/= 0.U || entry.n.getOrElse(0.U) =/= 0.U)
...
class PtwRespS2(implicit p: Parameters) extends PtwBundle {
  ...
  def hit(vpn: UInt, asid: UInt, vasid: UInt, vmid: UInt, allType: Boolean = false, ignoreAsid: Boolean = false): Bool = {
    val noS2_hit = s1.hit(vpn, asid, vmid, allType, ignoreAsid, (if(HasMptCheck)(this.mpt.get.mptOnly) else false.B),HasMptCheck,this.mpt.getOrElse(0.U.asTypeOf(new mptTlbRespBundle)))
    ...
    val mpt_NAPOT= Option.when(HasMptCheck) (mpt_merge_n(level, allStage_n, mpt.get.mptLevel, mpt.get.permIsNAPOT))
    val mpt_NAPOT_onlys1 = Option.when(HasMptCheck) (mpt_merge_n(s1.entry.level.getOrElse(0.U) , s1.entry.n.getOrElse(0.U), mpt.get.mptLevel, mpt.get.permIsNAPOT))
    val mpt_level = Option.when(HasMptCheck) (level min mpt.get.mptLevel)  
    //(Mux(this.s2xlate === onlyStage1,mpt_NAPOT_onlys1, mpt_NAPOT)) 
    val n = if(HasMptCheck) (Mux(this.s2xlate === onlyStage1,mpt_NAPOT_onlys1.get,mpt_NAPOT.get)) else (Mux(this.s2xlate === onlyStage1, s1.entry.n.getOrElse(0.U), allStage_n)) 
