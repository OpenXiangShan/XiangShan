...
class PtwCache()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  if (HasBitmapCheck) {
    ...
    val wakeup_req_delay_valid = io.bitmap_wakeup.get.fire
    val wakeup_req_delay = RegEnable(io.bitmap_wakeup.get.bits, wakeup_req_delay_valid)

    io.bitmap_wakeup.get.ready := !(io.bitmap_wakeup.get.valid && io.refill.valid)

    val wakeup_ridx = genPtwL0SetIdx(wakeup_vpn_search)
    l0.io.r.req.valid := io.bitmap_wakeup.get.fire
    ...
    val wakeup_vVec_delay = RegEnable(wakeup_vVec_req, wakeup_req_delay_valid)
    val wakeup_hVec_delay = RegEnable(wakeup_hVec_req, wakeup_req_delay_valid)
    val wakeup_hitVec_delay = VecInit(wakeup_data_resp.zip(wakeup_vVec_delay.asBools).zip(wakeup_hVec_delay).map { case ((wayData, v), h) =>
      wayData.entries.hit(wakeup_delay_vpn, io.csr_dup(0).satp.asid, io.csr_dup(0).vsatp.asid, io.csr_dup(0).hgatp.vmid, s2xlate = wakeup_delay_h =/= noS2xlate) && v && (wakeup_delay_h === h)})
  ...
  val spPte = Cat(spHitData.n.getOrElse(0.U), spHitData.pbmt.asUInt, 0.U(pteResLen.W) ,spHitData.ppn, 0.U(pteRswLen.W), spHitPerm.asUInt,spHitData.v.asUInt) // Super-page Page Table Entry
  ...
  if (HasBitmapCheck) {
    ...
    io.resp.bits.toFsm.bitmapCheck.get.toLLPTW := stageResp.bits.req_info.s2xlate === noS2xlate || stageResp.bits.req_info.s2xlate === onlyStage1
