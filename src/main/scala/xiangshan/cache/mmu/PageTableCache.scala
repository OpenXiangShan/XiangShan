  ...
  val refill = Flipped(ValidIO(new Bundle {
    ...
    val levelOH = new Bundle {
      ...
      def apply(levelUInt: UInt, valid: Bool) = {
        sp := GatedValidRegNext((levelUInt === 1.U || levelUInt === 2.U) && valid, false.B)
...
class PtwCache()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  // sp: level 1/2 leaf pte of 1GB/2MB super pages
  ...
  val (l2Hit, l2HitPPN, l2Pre) = {
    ...
    for (i <- 0 until l2tlbParams.l2Size) {
      XSDebug(stageReq.fire, p"[l2] l2(${i.U}) ${l2(i)} hit:${l2(i).hit(vpn_search, io.csr_dup(0).satp.asid, io.csr_dup(0).vsatp.asid, io.csr_dup(0).hgatp.asid, s2xlate = h_search =/= noS2xlate)}\n")
    }
  ...
  io.resp.bits.toFsm.ppn   := Mux(resp_res.l1.hit, resp_res.l1.ppn, resp_res.l2.ppn)
  ...
  io.resp.bits.toHptw.ppn := Mux(resp_res.l1.hit, resp_res.l1.ppn, resp_res.l2.ppn)
  ...
  io.resp.bits.toHptw.resp.entry.ppn := Mux(resp_res.l0.hit, resp_res.l0.ppn(idx), resp_res.sp.ppn)
  ...
  for (i <- 0 until tlbcontiguous) {
    io.resp.bits.toTlb.entry(i).ppn := Mux(resp_res.l0.hit, resp_res.l0.ppn(i)(ppnLen - 1, sectortlbwidth), resp_res.sp.ppn(ppnLen - 1, sectortlbwidth))
  ...
  when (!flush_dup(0) && refill.levelOH.l2 && !memPte(0).isLeaf() && !memPte(0).isPf(refill.level_dup(0)) && !memPte(0).isAf()) {
    ...
    l2v := l2v | rfOH
    l2g := (l2g & ~rfOH) | Mux(memPte(0).perm.g, rfOH, 0.U)
    l2h(refillIdx) := refill.req_info_dup(0).s2xlate

    for (i <- 0 until l2tlbParams.l2Size) {
      l2RefillPerf(i) := i.U === refillIdx
    }

    XSDebug(p"[l2 refill] refillIdx:${refillIdx} refillEntry:${l2(refillIdx).genPtwEntry(refill.req_info_dup(0).vpn, Mux(refill.req_info_dup(0).s2xlate =/= noS2xlate, io.csr_dup(0).vsatp.asid, io.csr_dup(0).satp.asid), memSelData(0), 0.U, prefetch = refill_prefetch_dup(0))}\n")
    XSDebug(p"[l2 refill] l2v:${Binary(l2v)}->${Binary(l2v | rfOH)} l2g:${Binary(l2g)}->${Binary((l2g & ~rfOH) | Mux(memPte(0).perm.g, rfOH, 0.U))}\n")
  ...
  when (!flush_dup(2) && refill.levelOH.l0 && !memPte(2).isAf()) {
    val refillIdx = genPtwL0SetIdx(refill.req_info_dup(2).vpn)
    val victimWay = replaceWrapper(getl0vSet(refill.req_info_dup(2).vpn), ptwl0replace.way(refillIdx))
    val victimWayOH = UIntToOH(victimWay)
    val rfvOH = UIntToOH(Cat(refillIdx, victimWay))
    ...
    l0h(refillIdx)(victimWay) := refill.req_info_dup(2).s2xlate
  ...
  // sfence for l0
  val sfence_valid_l0 = sfence_dup(3).valid && !sfence_dup(3).bits.hg && !sfence_dup(3).bits.hv
  ...
  val hfencev_valid_l0 = sfence_dup(3).valid && sfence_dup(3).bits.hv
  ...
  val hfenceg_valid_l0 = sfence_dup(3).valid && sfence_dup(3).bits.hg
  ...
  val l2asidhit = VecInit(l2asids.map(_ === sfence_dup(0).bits.id)).asUInt
  ...
  when (sfence_valid) {
    val l2vmidhit = VecInit(l2vmids.map(_.getOrElse(0.U) === io.csr_dup(0).hgatp.asid)).asUInt
    val spvmidhit = VecInit(spvmids.map(_.getOrElse(0.U) === io.csr_dup(0).hgatp.asid)).asUInt
    val l2hhit = VecInit(l2h.map{a => io.csr_dup(0).priv.virt && a === onlyStage1 || !io.csr_dup(0).priv.virt && a === noS2xlate}).asUInt
    val sphhit = VecInit(sph.map{a => io.csr_dup(0).priv.virt && a === onlyStage1 || !io.csr_dup(0).priv.virt && a === noS2xlate}).asUInt
    val l1hhit = VecInit(l1h.flatMap(_.map{a => io.csr_dup(0).priv.virt && a === onlyStage1 || !io.csr_dup(0).priv.virt && a === noS2xlate})).asUInt
    ...
    when (sfence_dup(0).bits.rs1/*va*/) {
      when (sfence_dup(0).bits.rs2) {
        // all va && all asid
        l1v := l1v & ~l1hhit
        l2v := l2v & ~(l2hhit & VecInit(l2vmidhit.asBools.map{a => io.csr_dup(0).priv.virt && a || !io.csr_dup(0).priv.virt}).asUInt)
        spv := spv & ~(sphhit & VecInit(spvmidhit.asBools.map{a => io.csr_dup(0).priv.virt && a || !io.csr_dup(0).priv.virt}).asUInt)
      } .otherwise {
        // all va && specific asid except global
        l1v := l1v & (l1g | ~l1hhit)
        l2v := l2v & ~(~l2g & l2hhit & l2asidhit & VecInit(l2vmidhit.asBools.map{a => io.csr_dup(0).priv.virt && a || !io.csr_dup(0).priv.virt}).asUInt)
  ...
  when(hfenceg_valid) {
    val l2vmidhit = VecInit(l2vmids.map(_.getOrElse(0.U) === sfence_dup(0).bits.id)).asUInt
