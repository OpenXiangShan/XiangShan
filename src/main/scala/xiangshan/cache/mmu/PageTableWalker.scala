...
class PTW()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  io.resp.bits.resp.apply(pageFault && !accessFault && !ppn_af, accessFault || ppn_af, Mux(accessFault, af_level,level), memPte, vpn, satp.asid, vpn(sectortlbwidth - 1, 0), all_valid = false)
