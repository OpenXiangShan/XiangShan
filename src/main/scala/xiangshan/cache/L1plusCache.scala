...
// stage 1
  ...
  (0 until nWays).map{ w => 
    XSPerf("hit way_" + Integer.toString(w, 10),  s2_valid && s2_hit && s2_hit_way === w.U)
    XSPerf("refill way_" + Integer.toString(w, 10), io.miss_meta_write.valid && wayNum === w.U)
    XSPerf("access way_" + Integer.toString(w, 10), (io.miss_meta_write.valid && wayNum === w.U) || (s2_valid && s2_hit && s2_hit_way === w.U))
  }
