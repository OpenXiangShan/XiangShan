...
class ICache extends ICacheModule
{
  ...
  (0 until nWays).map{ w => 
    XSPerf("hit way_" + Integer.toString(w, 10),  s2_hit && OHToUInt(hitVec)  === w.U)
    XSPerf("refill way_" + Integer.toString(w, 10), icacheMissQueue.io.meta_write.valid && wayNum === w.U)
    XSPerf("access way_" + Integer.toString(w, 10), (icacheMissQueue.io.meta_write.valid && wayNum === w.U) || (s2_hit && OHToUInt(hitVec)  === w.U))
  }
