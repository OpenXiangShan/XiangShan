...
class FtbInput(implicit p: Parameters) extends PredictorInput {
  val fauftb_entry_in     = new FTBEntry
  val fauftb_entry_hit_in = Bool()
  val updateMeta          = new FTBMeta
  val isRedirectFromIfu   = Bool()
}

class FtbOutput(implicit p: Parameters) extends PredictorOutput {
  val meta     = new FTBMeta
  val s1_ready = Bool()
}

class FtbIO(implicit p: Parameters) extends XSBundle {
  val in  = Input(new FtbInput)
  val out = Output(new FtbOutput)
}

class FTB(implicit p: Parameters) extends XSModule
    with FTBParams with HasBPUConst with BPUUtils with HasPerfEvents with HasCircularQueuePtrHelper {
  val io = IO(new FtbIO)
  ...
  val s2_pc = io.in.s2_pc
  val s3_pc = io.in.s3_pc

  io.out.resp.s1.pc := dup(s1_pc)
  io.out.resp.s2.pc := dup(s2_pc)
  io.out.resp.s3.pc := dup(s3_pc)

  // FTB switch register & temporary storage of fauftb prediction results
  val s0_close_ftb_req            = RegInit(false.B)
  val s1_close_ftb_req            = RegEnable(s0_close_ftb_req, false.B, io.in.s0_fire)
  val s2_close_ftb_req            = RegEnable(s1_close_ftb_req, false.B, io.in.s1_fire)
  val s2_fauftb_ftb_entry_dup     = dup(io.in.s1_fire).map(f => RegEnable(io.in.fauftb_entry_in, f))
  val s2_fauftb_ftb_entry_hit_dup = dup(io.in.s1_fire).map(f => RegEnable(io.in.fauftb_entry_hit_in, f))

  val ftbBank = Module(new FTBBank(numSets, numWays))

  // for close ftb read_req
  ftbBank.io.req_pc.valid := io.in.s0_fire && !s0_close_ftb_req
  ftbBank.io.req_pc.bits  := io.in.s0_pc

  val s2_multi_hit        = ftbBank.io.read_multi_hits.valid && io.in.s2_fire
...
//  val btb_enable_dup   = dup(RegNext(io.in.ctrl.btb_enable))
  val s1_read_resp     = Mux(s1_close_ftb_req, io.in.fauftb_entry_in, ftbBank.io.read_resp)
  val s2_ftbBank_dup   = dup(io.in.s1_fire).map(f => RegEnable(ftbBank.io.read_resp, f))
  ...
  val s3_ftb_entry_dup = dup(io.in.s2_fire).zip(s2_ftb_entry_dup).map { case (f, e) =>
  ...
  val real_s3_fallThroughErr_dup = dup(io.in.s2_fire).map(f => RegEnable(real_s2_fallThroughErr, f))
  ...
  val s2_ftb_hit_dup = dup(io.in.s1_fire).map(f => RegEnable(s1_hit, 0.B, f))
  ...
  val s3_hit_dup = dup(io.in.s2_fire).zip(s2_hit_dup).map { case (f, h) =>
    RegEnable(Mux(s2_multi_hit_enable, s2_multi_hit, h), 0.B, f)
  }
  val s3_multi_hit_dup  = dup(io.in.s2_fire).map(f => RegEnable(s2_multi_hit_enable, f))
  val writeWay          = Mux(s1_close_ftb_req, 0.U, ftbBank.io.read_hits.bits)
  val s2_ftb_meta       = RegEnable(FTBMeta(writeWay.asUInt, s1_hit, GTimer()).asUInt, io.in.s1_fire)
  val s2_multi_hit_meta = FTBMeta(s2_multi_hit_way.asUInt, s2_multi_hit, GTimer()).asUInt

  val s2_writeWay  = RegEnable(writeWay, io.in.s1_fire)
  val s2_hit       = RegEnable(s1_hit, io.in.s1_fire)
  val s2_predCycle = RegEnable(GTimer(), io.in.s1_fire)

  val s2_realWriteWay = Mux(s2_multi_hit_enable, s2_multi_hit_way, s2_writeWay)
  val s2_realHit      = Mux(s2_multi_hit_enable, s2_multi_hit, s2_hit)

  val respMeta = WireDefault(0.U.asTypeOf(new FTBMeta))
  respMeta.writeWay := RegEnable(s2_realWriteWay, io.in.s2_fire)
  respMeta.hit      := RegEnable(s2_realHit, io.in.s2_fire)
  if (respMeta.pred_cycle.isDefined) {
    respMeta.pred_cycle.get := RegEnable(s2_predCycle, io.in.s2_fire)
  }
  ...
  when(io.in.s2_fire && s2_fauftb_ftb_entry_hit_dup(0) && s2_ftb_hit_dup(0)) {
    fauftb_ftb_entry_consistent_counter := Mux(
      fauftb_ftb_entry_consistent,
      fauftb_ftb_entry_consistent_counter + 1.U,
      0.U
    )
  }.elsewhen(io.in.s2_fire && !s2_fauftb_ftb_entry_hit_dup(0) && s2_ftb_hit_dup(0)) {
    fauftb_ftb_entry_consistent_counter := 0.U
  }

  when((fauftb_ftb_entry_consistent_counter >= FTBCLOSE_THRESHOLD) && io.in.s0_fire) {
  ...
  val updateMeta = RegEnable(io.in.updateMeta, io.in.update.valid)
  ...
  // so io.in.update.bits.pc is used directly here
...
//  update.meta := RegEnable(io.update.bits.meta, io.update.valid && !io.update.bits.old_entry)
  ...
  XSPerfAccumulate("this_cycle_is_close", s2_close_ftb_req && io.in.s2_fire)
  XSPerfAccumulate("this_cycle_is_open", !s2_close_ftb_req && io.in.s2_fire)

  // io.out.resp.bits.resp := RegEnable(io.in.bits.resp_in(0), 0.U.asTypeOf(new BranchPredictionResp), dup(io.in.s1_fire))

  io.out.resp.s2.full_pred.map { case fp => fp.multiHit := false.B }

  io.out.resp.s2.full_pred.zip(s2_hit_dup).map { case (fp, h) => fp.hit := h }
  for (
    full_pred & s2_ftb_entry & s2_pc & s1_pc & s1_fire <-
      io.out.resp.s2.full_pred zip s2_ftb_entry_dup zip dup(s2_pc) zip dup(s1_pc) zip dup(io.in.s1_fire)
  ) {
  ...
  io.out.resp.s3.full_pred.zip(s3_hit_dup).map { case (fp, h) => fp.hit := h }
  io.out.resp.s3.full_pred.zip(s3_multi_hit_dup).map { case (fp, m) => fp.multiHit := m }
  for (
    full_pred & s3_ftb_entry & s3_pc & s2_pc & s2_fire <-
      io.out.resp.s3.full_pred zip s3_ftb_entry_dup zip dup(s3_pc) zip dup(s2_pc) zip dup(io.in.s2_fire)
  )
    full_pred.fromFtbEntry(s3_ftb_entry, s3_pc, Some((s2_pc, s2_fire)))

  // Overwrite the fallThroughErr value
  io.out.resp.s3.full_pred.zipWithIndex.map { case (fp, i) => fp.fallThroughErr := real_s3_fallThroughErr_dup(i) }

  io.out.resp.last_stage_ftb_entry := s3_ftb_entry_dup(0)
  io.out.meta                      := respMeta
  io.out.resp.s1_ftbCloseReq       := s1_close_ftb_req
  io.out.resp.s1_uftbHit           := io.in.fauftb_entry_hit_in
  val s1_uftbHasIndirect = io.in.fauftb_entry_in.jmpValid &&
    io.in.fauftb_entry_in.isJalr && !io.in.fauftb_entry_in.isRet // uFTB determines that it's real JALR, RET and JAL are excluded
  io.out.resp.s1_uftbHasIndirect := s1_uftbHasIndirect

  // always taken logic
  for (i <- 0 until numBr) {
    for (
      out_fp & in_fp & s2_hit & s2_ftb_entry <-
        io.out.resp.s2.full_pred zip io.in.resp.s2.full_pred zip s2_hit_dup zip s2_ftb_entry_dup
    )
      out_fp.br_taken_mask(i) := in_fp.br_taken_mask(i) || s2_hit && s2_ftb_entry.strong_bias(i)
    for (
      out_fp & in_fp & s3_hit & s3_ftb_entry <-
        io.out.resp.s3.full_pred zip io.in.resp.s3.full_pred zip s3_hit_dup zip s3_ftb_entry_dup
    )
  ...
  XSError(
    s3_ftb_entry_dup(0).valid && s3_hit_dup(0) && io.in.s3_fire && fallThroughErr,
    "FTB read sram entry in s3 fallThrough address error!"
  )

  // Update logic
  val u_meta  = updateMeta
  ...
  ftbBank.io.s1_fire       := io.in.s1_fire
  ...
  XSDebug(
    ...
    io.in.resp.s2.full_pred(0).br_taken_mask.asUInt,
    io.out.resp.s2.full_pred(0).real_slot_taken_mask().asUInt
  )
  XSDebug("s2_target=%x\n", io.out.resp.s2.getTarget(0).toUInt)

  s2_ftb_entry_dup(0).display(true.B)

  XSPerfAccumulate("ftb_read_hits", RegNext(io.in.s0_fire) && s1_hit)
  XSPerfAccumulate("ftb_read_misses", RegNext(io.in.s0_fire) && !s1_hit)
