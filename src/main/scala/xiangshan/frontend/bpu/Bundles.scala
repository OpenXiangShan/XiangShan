...
object BpuPredictionSource {
  object Stage1 extends EnumUInt(5) {
    ...
    def UbtbMicroTage: UInt = 2.U(width.W)
    def AbtbMicroTage: UInt = 3.U(width.W)
...
class BpuResolveMeta(implicit p: Parameters) extends BpuBundle {
  ...
  // TODO: use Option.when to warpper these debug metas
  val debug_utage: MicroTageMeta = new MicroTageMeta
  val debug_phr:   PhrMeta       = new PhrMeta
}
