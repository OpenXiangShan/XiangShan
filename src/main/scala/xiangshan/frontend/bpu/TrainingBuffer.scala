...
class TrainingBuffer[T <: Data](gen: T, numBanks: Int, numEntries: Int)(implicit p: Parameters) extends XSModule {
  val io = IO(new Bundle {
    val predictionReadValid   = Input(Bool())
    val predictionReadBankIdx = Input(UInt(log2Ceil(numBanks).W))
  ...
  private val candidates = VecInit(entries.zipWithIndex.map { case (entry, i) =>
    valid(i) && (!io.predictionReadValid || entry.bankIdx =/= io.predictionReadBankIdx)
  })
  ...
  private val incomingConflict = io.enq.valid && io.enq.bits.needRead &&
    io.predictionReadValid && io.enq.bits.bankIdx === io.predictionReadBankIdx
  ...
  assert(!(enq && full && !deq), "bank-conflict training buffer must not drop incoming training")
  when(io.enq.valid) {
  ...
  assert(
    !(io.deq.valid && io.deq.bits.needRead && io.predictionReadValid && !forceTraining &&
      io.deq.bits.bankIdx === io.predictionReadBankIdx),
