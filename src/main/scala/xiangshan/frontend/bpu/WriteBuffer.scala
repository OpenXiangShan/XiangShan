...
class WriteBuffer[T <: WriteReqBundle](
  ...
  class WriteBufferIO extends Bundle {
    ...
    val lookupSetIdx: Option[Vec[UInt]] =
      Option.when(hasLookup)(Input(Vec(numLookupPorts, UInt(gen.setIdx.getWidth.W))))
    val lookup: Option[Vec[Vec[Valid[T]]]] =
      Option.when(hasLookup)(Output(Vec(numLookupPorts, Vec(numPorts, Valid(gen)))))
  }
