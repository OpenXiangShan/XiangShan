...
class ScMeta(implicit p: Parameters) extends ScBundle with HasScParameters {
  ...
  val debug_predPathIdx:      Option[Vec[UInt]] = Some(Vec(NumPathTables, UInt(log2Ceil(NumPathSets).W)))
  val debug_predGlobalIdx:    Option[Vec[UInt]] = Some(Vec(NumGlobalTables, UInt(log2Ceil(NumGlobalSets).W)))
  val debug_predBWIdx:        Option[Vec[UInt]] = Some(Vec(NumBWTables, UInt(log2Ceil(NumBWSets).W)))
  val debug_predImliIdx:      Option[UInt]      = Some(UInt(log2Ceil(NumImliSets).W))
  val debug_predBiasIdx:      Option[UInt]      = Some(UInt(log2Ceil(NumBiasSets).W))
}
