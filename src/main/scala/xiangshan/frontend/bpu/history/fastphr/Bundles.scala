...
//   - override: internally re-apply a corrected group from the snapshot that group started from
class FastPhrIO(implicit p: Parameters) extends FastPhrBundle with HasFastPhrParameters {
  ...
  // which stage corrected the group, and therefore which snapshot the replay starts from; s3 wins over s2, as it
