...
class TableUpdateReq(numSets: Int)(implicit p: Parameters) extends TageBundle {
...
class TableAllocateReq(numSets: Int)(implicit p: Parameters) extends TageBundle {
  val wayMask:  UInt            = UInt(NumWays.W)
  val tag:      UInt            = UInt(TagWidth.W)
  val takenCtr: SaturateCounter = new SaturateCounter(TakenCtrWidth)
}
...
class TableSramWriteReq(numSets: Int)(implicit p: Parameters) extends WriteReqBundle
    with HasTageParameters {
  val setIdx:                   UInt                    = UInt(log2Ceil(numSets / NumBanks).W)
  val updateReq:                Valid[TableUpdateReq]   = Valid(new TableUpdateReq(numSets))
  val allocateReq:              Valid[TableAllocateReq] = Valid(new TableAllocateReq(numSets))
