...
case class TageParameters(
    ...
    BaseTableNumBanks:      Int = 4, // to alleviate read-write conflicts in single-port SRAM
    BaseTableTakenCtrWidth: Int = 2,
    TableInfos: Seq[TageTableInfo] = Seq(
      // NumSets, NumWays, HistoryLength
      new TageTableInfo(1024, 3, 4),
      new TageTableInfo(1024, 3, 9),
      new TageTableInfo(1024, 3, 17),
      new TageTableInfo(1024, 3, 31),
      new TageTableInfo(1024, 3, 58),
      new TageTableInfo(1024, 3, 109),
      new TageTableInfo(1024, 3, 211),
      new TageTableInfo(1024, 3, 407)
    ),
    NumBanks:        Int = 2, // to alleviate read-write conflicts in single-port SRAM
    TagWidth:        Int = 13,
    TakenCtrWidth:   Int = 3,
...
trait HasTageParameters extends HasBpuParameters {
  ...
  def BaseTableNumBanks:      Int = tageParameters.BaseTableNumBanks
  def BaseTableBankIdxWidth:  Int = log2Ceil(BaseTableNumBanks)
  def BaseTableNumSets:       Int = tageParameters.BaseTableSize / BaseTableNumBanks / FetchBlockInstNum
