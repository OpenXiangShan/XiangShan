...
class TageTable(val numSets: Int)(implicit p: Parameters) extends TageModule with Helpers {
  class TageTableIO extends TageBundle {
    ...
    val updateReq:                Valid[TableUpdateReq]   = Flipped(Valid(new TableUpdateReq(numSets)))
    val allocateReq:              Valid[TableAllocateReq] = Flipped(Valid(new TableAllocateReq(numSets)))
  ...
  entrySram.zip(writeBuffers).zip(allocFailCtrSram).foreach {
    case ((bank, buffer), allocFailCtr) =>
      val valid                    = buffer.io.read.head.valid && !bank.head.io.r.req.valid
      val setIdx                   = buffer.io.read.head.bits.setIdx
      val updateReq                = buffer.io.read.head.bits.updateReq
      val allocateReq              = buffer.io.read.head.bits.allocateReq
      val needResetUsefulCtr       = buffer.io.read.head.bits.needResetUsefulCtr
      val needIncreaseAllocFailCtr = buffer.io.read.head.bits.needIncreaseAllocFailCtr
      bank.zipWithIndex.foreach {
        case (way, wayIdx) =>
          ...
          entry.takenCtr := Mux(needAllocate, allocateReq.bits.takenCtr, updateReq.bits.newTakenCtr(wayIdx))
          entry.usefulCtr :=
            Mux(
              needAllocate,
              ...
              Mux(
                needResetUsefulCtr,
                0.U,
                updateReq.bits.newUsefulCtr(wayIdx)
              )
            )
          val bitMask =
            Mux(
      ...
      allocFailCtr.io.w.req.valid       := valid && (needResetUsefulCtr || needIncreaseAllocFailCtr)
      allocFailCtr.io.w.req.bits.setIdx := setIdx
      allocFailCtr.io.w.req.bits.data :=
        Mux(
          needResetUsefulCtr,
          0.U,
          buffer.io.read.head.bits.oldAllocFailCtr.getIncrease
        )

      buffer.io.read.head.ready := bank.head.io.w.req.ready && !bank.head.io.r.req.valid
  }

  entrySram.zipWithIndex.foreach {
    case (bank, bankIdx) =>
      bank.foreach { way =>
  ...
  io.readResp.entries := Mux1H(readBankMaskNext, entrySram.map(bank => VecInit(bank.map(way => way.io.r.resp.data))))
  io.readResp.allocFailCtr := Mux1H(readBankMaskNext, allocFailCtrSram.map(_.io.r.resp.data))

  writeBuffers.zipWithIndex.foreach {
    case (buffer, bankIdx) =>
      buffer.io.write.head.valid            := (io.updateReq.valid || io.allocateReq.valid) && io.writeBankMask(bankIdx)
