...
class RasStack(implicit p: Parameters) extends RasModule
  ...
  private val topEntry    = getTop(ssp, sctr, tosr, tosw, allowBypass = true, tosrInSpec)
  ...
  writeNosEntry.inSpec := Mux(
    io.redirect.valid && io.redirect.isCall,
    tosrInRange(io.redirect.meta.tosr, io.redirect.meta.tosw, io.redirect.meta.tosrInSpec),
    tosrInRange(tosr, tosw, tosrInSpec)
  )
  ...
  when(writeBypassValidWire) {
    ...
    timingTop := getTop(popSsp, popSctr, popTosr, popTosw, allowBypass = false, io.redirect.meta.tosrInSpec)
    ...
    val popTosr = tosr
    val popTosw = tosw
    timingTop := getTop(popSsp, popSctr, popTosr, popTosw, allowBypass = false, tosrInSpec)
  }
  ...
  when(io.spec.popValid) {
    specPop(ssp, sctr, tosr, tosw, topNos, topNosEntry.inSpec, tosrInSpec)
  }
