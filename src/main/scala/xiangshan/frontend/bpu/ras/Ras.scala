...
class Ras(implicit p: Parameters) extends BasePredictor with HasCircularQueuePtrHelper {
  ...
  private val specBrHighPc    = specIn.startPc(VAddrBits - 1, log2Ceil(PredictWidth)) + specIn.cfiOffset.carry
  stack.spec.pushValid      := specPush && !stackNearOverflow
  stack.spec.popValid       := specPop && !stackNearOverflow
  stack.spec.pushAddr       := Cat(specBrHighPc, specIn.cfiOffset.offset) + Mux(specIn.isRvc, 2.U, 4.U)
  ...
  io.hit                    := false.B
  io.prediction.taken       := io.pass.taken
  io.prediction.cfiPosition := io.pass.cfiPosition
  io.prediction.target      := Mux(io.rasOverride, io.topRetAddr, io.specIn.bits.target)
  io.prediction.attribute   := io.pass.attribute
  ...
  private val commitBrHighPc    = commitInfo.startPc(VAddrBits - 1, log2Ceil(PredictWidth)) + commitInfo.cfiOffset.carry
  ...
  stack.commit.pushAddr     := Cat(commitBrHighPc, commitInfo.cfiOffset.offset) + Mux(commitInfo.isRvc, 2.U, 4.U)
