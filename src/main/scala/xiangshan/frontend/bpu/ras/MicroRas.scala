...
class MicroRas(implicit p: Parameters) extends BasePredictor with HasRasParameters with HalfAlignHelper {
  ...
  private val s2_retAddr = RegInit(0.U.asTypeOf(GuardedPc()))
  private val s3_retAddr = RegInit(0.U.asTypeOf(GuardedPc()))
  ...
  when(io.hasRedirect) {
    ...
    when(s2_hasPush) {
      ...
      topRetAddr := Mux(
        s3_hasPush,
        s3_retAddr,
        Mux(s3_hasPop, 0.U.asTypeOf(GuardedPc()), io.fullRetAddr)
      )
    }.elsewhen(s2_hasPop) {
