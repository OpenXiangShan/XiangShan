...
class MicroRas(implicit p: Parameters) extends BasePredictor with HasRasParameters with HalfAlignHelper {
  class MicroRasIO(implicit p: Parameters) extends BasePredictorIO {
    ...
    class MicroRasSpecOut extends Bundle {
      val retTarget: PrunedAddr = PrunedAddr(GuardedVAddrBits) // Predicted return address
    ...
    val fullRetAddr:  PrunedAddr            = Input(PrunedAddr(GuardedVAddrBits)) // Current top of the main RAS
  }
  ...
  private val s2_retAddr = RegInit(0.U.asTypeOf(PrunedAddr(GuardedVAddrBits)))
  private val s3_retAddr = RegInit(0.U.asTypeOf(PrunedAddr(GuardedVAddrBits)))

  // Final output: whether a valid return target is available, and what its target is.
  private val isCanUse   = RegInit(false.B)
  private val topRetAddr = RegInit(0.U.asTypeOf(PrunedAddr(GuardedVAddrBits)))
  ...
  when(io.hasRedirect) {
    ...
    when(s2_hasPush) {
      ...
      topRetAddr := Mux(
        s3_hasPush,
        s3_retAddr,
        Mux(s3_hasPop, 0.U.asTypeOf(PrunedAddr(GuardedVAddrBits)), io.fullRetAddr)
      )
    }.elsewhen(s2_hasPop) {
    ...
    when(s2_hasPush) {
      ...
      topRetAddr := Mux(s3_hasPush, io.fullRetAddr, 0.U.asTypeOf(PrunedAddr(GuardedVAddrBits)))
    }.elsewhen(s3_hasPop) {
      ...
      topRetAddr := Mux(redirectDelay1, 0.U.asTypeOf(PrunedAddr(GuardedVAddrBits)), io.fullRetAddr)
    }
  }
