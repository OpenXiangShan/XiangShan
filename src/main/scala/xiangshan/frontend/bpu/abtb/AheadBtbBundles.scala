...
class AheadBtbInput(implicit p: Parameters) extends XSBundle {
  val startPc       = Decoupled(PrunedAddr(VAddrBits))
  val bpuOverride   = Bool() // only s3_override
  val redirectValid = Bool()
  val update        = Valid(new AheadBtbUpdateBundle)
}

class AheadBtbOutput(implicit p: Parameters) extends XSBundle {
  val prediction = Decoupled(new Prediction)
}

class AheadBtbIO(implicit p: Parameters) extends XSBundle {
  val in  = Input(new AheadBtbInput)
  val out = Output(new AheadBtbOutput)
}

class Prediction(implicit p: Parameters) extends XSBundle {
  val startPc       = PrunedAddr(VAddrBits)
  val target        = PrunedAddr(VAddrBits)
  val cfiPosition   = Valid(UInt(log2Ceil(PredictWidth).W))
