...
class DummyMicroBtb(implicit p: Parameters) extends XSModule with HasAheadBtbParams with AheadBtbHelpers {
  ...
  val fallThroughAddr = s1_pc + FetchBlockSize.U

  io.out.prediction.startPc           := s1_pc
  io.out.prediction.target            := fallThroughAddr
  io.out.prediction.cfiPosition.valid := false.B
  io.out.prediction.cfiPosition.bits  := (PredictWidth - 1).U
  io.out.prediction.fallThruError     := false.B
}
