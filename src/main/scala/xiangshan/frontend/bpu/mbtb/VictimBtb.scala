...
class VictimBtb(implicit p: Parameters) extends MainBtbModule {
  ...
  for (setIdx <- 0 until NumVictimBtbSets) {
    when(trainEntry.req.valid && trainEntry.req.bits.setIdx.take(VictimBtbSetIdxLen) === setIdx.U) {
      for (w <- 0 until NumWay) {
        when(trainEntry.req.bits.counterWayMask(w)) {
          entries(setIdx)(w).counter := trainEntry.req.bits.counters(w)
        }
        when(trainEntry.req.bits.entryWayMask(w)) {
          entries(setIdx)(w).entry := trainEntry.req.bits.entry
        }
      }
    }
  }
  ...
  for (setIdx <- 0 until NumVictimBtbSets) {
    when(writeEntry.req.valid && writeEntry.req.bits.setIdx.take(VictimBtbSetIdxLen) === setIdx.U) {
      for (w <- 0 until NumWay) {
        when(writeEntry.req.bits.flushMask(w)) {
          entries(setIdx)(w).entry.attribute := BranchAttribute.None
        }
        // write entry has higher priority than flush
        when(writeEntry.req.bits.wayMask(w)) {
          entries(setIdx)(w).entry   := writeEntry.req.bits.entry
          entries(setIdx)(w).setIdx  := writeEntry.req.bits.setIdx
          entries(setIdx)(w).counter := TakenCounter.WeakPositive
        }
      }
    }
  }
  ...
  for (setIdx <- 0 until NumVictimBtbSets) {
    when(flush.req.valid && flush.req.bits.setIdx === setIdx.U) {
      for (w <- 0 until NumWay) {
        when(flush.req.bits.wayMask(w)) {
          entries(setIdx)(w).entry.attribute := BranchAttribute.None
        }
      }
    }
