...
import xiangshan.frontend.bpu.tage.ResolveTrain
...
class Bpu(implicit p: Parameters) extends BpuModule with HalfAlignHelper with SelectFirstTakenBranchHelper {
  ...
  // FIXME: remove it
  tage.io.resolveTrain.valid := false.B
  tage.io.resolveTrain.bits  := 0.U.asTypeOf(new ResolveTrain)
...
//  s3_override := s3_valid && !(s3_prediction === s3_s1Prediction)
  ...
  // tell ftq s3_ftqPtr for meta enqueue and s3 override
