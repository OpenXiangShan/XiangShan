...
class Bpu(implicit p: Parameters) extends BpuModule with HalfAlignHelper {
  ...
  redirect.bits.target := Mux(io.fromFtq.redirectFromIFU, ras.io.specRead.retAddr, io.fromFtq.redirect.bits.target)
  private val diffRetAddr = io.fromFtq.redirectFromIFU && (ras.io.specRead.retAddr =/= io.fromFtq.specRetAddr)
