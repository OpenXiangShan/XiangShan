...
//      abtb.io.prediction.taken -> abtb.io.prediction
  ...
  private val s2_otherTakenMask = VecInit(s2_mbtbResult.hitMask.zip(s2_mbtbResult.attributes).map {
    case (hit, attribute) => hit && (attribute.isDirect || attribute.isIndirect)
  })
  private val s2_takenMask = s2_condTakenMask.zip(s2_otherTakenMask).map {
    case (condTaken, otherTaken) => condTaken || otherTaken
  }
