...
//        (s3_taken && s3_firstTakenBranchIsReturn)                               -> ras.io.topRetAddr,
  ...
  XSPerfAccumulate(
    "finalPred_s1",
    s3_fire && !s3_override,
    BpuPredictionSource.Stage1.getValidSeq(s3_debugMeta.bpSource.s1Source)
  )

  XSPerfAccumulate(
    "finalPred_s3",
    s3_fire && s3_override,
    BpuPredictionSource.Stage3.getValidSeq(s3_debugMeta.bpSource.s3Source)
  )
  ...
  XSPerfAccumulate(
    ...
    io.toFtq.prediction.fire && s3_override && s3_debugMeta.bpSource.s1Fallthrough,
    perf_s1TakenSourceVec
  )

  XSPerfAccumulate(
    s"s3Override_takenMismatch_s3fall",
    io.toFtq.prediction.fire && s3_override && s3_debugMeta.bpSource.s3Fallthrough,
    perf_s3TakenSourceVec
  )

  XSPerfAccumulate(
    s"s3Override_takenMismatch_s3fallTage",
    io.toFtq.prediction.fire && s3_override && s3_debugMeta.bpSource.s3FallthroughTage,
    perf_s3TakenSourceVec
  )
