...
class Ftq(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper
  ...
  val perfEvents = Seq(
    ("bpu_s2_redirect"     , bpu_s2_redirect).withDescription("Branch-predictor stage-2 redirects accepted by the FTQ."),
    ("bpu_s3_redirect"     , bpu_s3_redirect).withDescription("Branch-predictor stage-3 redirects accepted by the FTQ."),
    ("bpu_to_ftq_stall"    , enq.valid && ~enq.ready).withDescription("Cycles in which the branch predictor has an FTQ enqueue that is not accepted."),
    ("mispredictRedirect"  , perf_redirect.valid && RedirectLevel.flushAfter === perf_redirect.bits.level).withDescription("Redirects that flush instructions after the redirecting instruction."),
    ("replayRedirect"      , perf_redirect.valid && RedirectLevel.flushItself(perf_redirect.bits.level)).withDescription("Replay redirects that also flush the redirecting instruction."),
    ("predecodeRedirect"   , fromIfuRedirect.valid).withDescription("Redirects generated by IFU predecode."),
    ("to_ifu_bubble"       , io.toIfu.req.ready && !io.toIfu.req.valid).withDescription("Cycles in which the IFU is ready for an FTQ request but none is valid."),
    ("from_bpu_real_bubble", !enq.valid && enq.ready && allowBpuIn).withDescription("Cycles in which the FTQ can accept BPU output but no prediction is valid."),
    ("BpInstr"             , PopCount(mbpInstrs)).withDescription("Committed instructions covered by branch-prediction accuracy checking."),
    ("BpBInstr"            , PopCount(mbpBRights | mbpBWrongs)).withDescription("Committed conditional branches covered by branch-prediction accuracy checking."),
    ("BpRight"             , PopCount(mbpRights)).withDescription("Correct branch predictions of all checked control-flow types."),
    ("BpWrong"             , PopCount(mbpWrongs)).withDescription("Incorrect branch predictions of all checked control-flow types."),
    ("BpBRight"            , PopCount(mbpBRights)).withDescription("Correct conditional-branch predictions."),
    ("BpBWrong"            , PopCount(mbpBWrongs)).withDescription("Incorrect conditional-branch predictions."),
    ("BpJRight"            , PopCount(mbpJRights)).withDescription("Correct direct-jump predictions."),
    ("BpJWrong"            , PopCount(mbpJWrongs)).withDescription("Incorrect direct-jump predictions."),
    ("BpIRight"            , PopCount(mbpIRights)).withDescription("Correct indirect-jump predictions."),
    ("BpIWrong"            , PopCount(mbpIWrongs)).withDescription("Incorrect indirect-jump predictions."),
    ("BpCRight"            , PopCount(mbpCRights)).withDescription("Correct call predictions."),
    ("BpCWrong"            , PopCount(mbpCWrongs)).withDescription("Incorrect call predictions."),
    ("BpRRight"            , PopCount(mbpRRights)).withDescription("Correct return predictions."),
    ("BpRWrong"            , PopCount(mbpRWrongs)).withDescription("Incorrect return predictions."),
    ("ftb_false_hit"       , PopCount(ftb_false_hit)).withDescription("FTB hits whose prediction does not match the committed control flow."),
    ("ftb_hit"             , PopCount(ftb_hit)).withDescription("Committed control-flow entries that hit in the FTB prediction metadata.")
  )
  generatePerfEvent()
}
