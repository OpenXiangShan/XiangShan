...
class Pc(val length: Int)(implicit p: Parameters) extends XSBundle {
  val pc: UInt = UInt((length - instOffsetBits).W)

  def apply(): UInt = Cat(pc, 0.U(instOffsetBits.W))
  ...
  def :=(x: UInt): Unit = {
    ...
    pc := x(length - instOffsetBits, instOffsetBits)
  }
...
object Pc {
  def apply(length: Int)(implicit p: Parameters): Pc = new Pc(length)
}
