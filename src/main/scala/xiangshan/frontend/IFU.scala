...
class NewIFU(implicit p: Parameters) extends XSModule
  ...
  val f2_crossPageFault = VecInit((0 until PredictWidth).map(i =>
    isLastInLine(f2_pc(i)) && (f2_exception(0) =/= ExceptionType.pf) && f2_doubleLine && (f2_exception(
      1
    ) === ExceptionType.pf) && !f2_pd(i).isRVC
  ))
  val f2_crossGuestPageFault = VecInit((0 until PredictWidth).map(i =>
    isLastInLine(f2_pc(i)) && (f2_exception(0) =/= ExceptionType.gpf) && f2_doubleLine && (f2_exception(
      1
    ) === ExceptionType.gpf) && !f2_pd(i).isRVC
  ))
  ...
  io.toIbuffer.bits.crossPageIPFFix := (0 until PredictWidth).map(i =>
    f3_crossPageFault(i) || f3_crossGuestPageFault(i)
  )
  ...
  when(f3_req_is_mmio) {
    ...
    io.toIbuffer.bits.crossPageIPFFix(
      0
    ) := mmio_resend_exception === ExceptionType.pf // || mmio_resend_exception === ExceptionType.gpf
