  ...
  with HasPerfEvents
{
  ...
  val f1_pc_adder_cut_point = (VAddrBits/2) - 1 // same as overflow bit
  ...
  val f1_pc                 = VecInit(f1_pc_lower_result.map{ i =>  
    Mux(i(f1_pc_adder_cut_point), Cat(f1_pc_high_plus1,i(f1_pc_adder_cut_point,0)), Cat(f1_pc_high,i(f1_pc_adder_cut_point,0)))})
  ...
  if (env.FPGAPlatform){
    ...
    XSError(f1_pc.zip(f1_pc_diff).map{ case (a,b) => a.asUInt =/= b.asUInt }.reduce(_||_), "IFU f1_pc adder cut fail")
    XSError(f1_half_snpc.zip(f1_half_snpc_diff).map{ case (a,b) => a.asUInt =/= b.asUInt }.reduce(_||_), "IFU f1_half_snpc adder cut fail")
  }
