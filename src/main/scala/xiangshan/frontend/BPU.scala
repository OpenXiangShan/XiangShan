...
class BPU(implicit p: Parameters) extends XSModule with HasBPUConst with HasPerfEvents
  ...
  // pc which BPU starts to predict
  ...
  // a priority mux to select next pc
  ...
  val redirect          = io.ftq_to_bpu.redirect
  val redirect_dup      = dup_seq(RegNextWithEnable(redirect))
  ...
  s3_flush_dup.map(_ := redirect.valid)
  ...
  for ((((s0_fire, s1_flush), s1_fire), s1_valid) <- s0_fire_dup zip s1_flush_dup zip s1_fire_dup zip s1_valid_dup) {
    when(redirect.valid)(s1_valid := false.B)
  ...
  s0_stall_dup.zip(s1_valid_dup).zip(s2_redirect_dup).zip(s3_redirect_dup).zip(redirect_dup).foreach {
    case ((((s0_stall, s1_valid), s2_redirect), s3_redirect), redirect) => {
      s0_stall := !(s1_valid || s2_redirect || s3_redirect || redirect.valid)
    }
  }
  // Power-on reset
  ...
  predictors.io.redirect := redirect_dup(0)

  // val updatedGh = oldGh.update(shift, taken && addIntoHist)
  for ((npcGen, redirect) <- npcGen_dup zip redirect_dup)
    npcGen.register(redirect.valid, redirect.bits.cfiUpdate.target, Some("redirect_target"), 2)
  ...
  controlRedirectBubble := redirect_dup(0).valid && redirect_dup(0).bits.ControlRedirectBubble
  ControlBTBMissBubble  := redirect_dup(0).bits.ControlBTBMissBubble
  TAGEMissBubble        := redirect_dup(0).bits.TAGEMissBubble
  SCMissBubble          := redirect_dup(0).bits.SCMissBubble
  ITTAGEMissBubble      := redirect_dup(0).bits.ITTAGEMissBubble
  RASMissBubble         := redirect_dup(0).bits.RASMissBubble

  memVioRedirectBubble                 := redirect_dup(0).valid && redirect_dup(0).bits.MemVioRedirectBubble
  otherRedirectBubble                  := redirect_dup(0).valid && redirect_dup(0).bits.OtherRedirectBubble
  btbMissBubble                        := redirect_dup(0).valid && redirect_dup(0).bits.BTBMissBubble
