...
trait PreDecodeHelper extends HasXSParameter {
  ...
  def getJalOffset(inst: UInt, isRvc: Bool): UInt = {
    ...
    SignExt(Mux(isRvc, SignExt(rvcOffset, maxWidth), SignExt(rviOffset, maxWidth)), XLEN)
  }

  def getBrOffset(inst: UInt, isRvc: Bool): UInt = {
