...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  /**
    ...
    ***********************************************************************
    */
  private val s3_halfSnpc = Wire(Vec(PredictWidth, UInt(VAddrBits.W)))
  ...
  private val s3_ftqReqStartAddr     = Cat(0.U(1.W), s3_ftqReq.startAddr)
  private val s3_ftqReqNextStartAddr = Cat(0.U(1.W), s3_ftqReq.nextStartAddr)
  ...
  private val mmioResendAddr = RegInit(0.U(PAddrBits.W))
  // NOTE: we don't use GPAddrBits here, refer to ICacheMainPipe.scala L43-48 and PR#3795
  private val mmioResendGpAddr            = RegInit(0.U(PAddrBitsMax.W))
  ...
  switch(mmioState) {
    ...
    is(MmioFsmState.TlbResp) {
      when(io.itlb.resp.fire) {
        ...
        mmioResendGpAddr            := io.itlb.resp.bits.gpaddr(0)
  ...
  io.itlb.req.bits.vaddr              := s3_resendVAddr
  io.itlb.req.bits.debug.pc           := s3_resendVAddr
  ...
  io.pmp.req.bits.addr := mmioResendAddr
  ...
  io.toBackend.gpaddrMem_wdata.gpaddr := Mux(s3_reqIsMmio, mmioResendGpAddr, s3_gpAddr)
...
//  private val lastTaken = wbCheckResultStage1.fixedTaken.last
  ...
  fetchIBufferDumpData.startAddr  := s3_ftqReq.startAddr
  ...
  ifuWbToFtqDumpData.startAddr         := wbFtqReq.startAddr
