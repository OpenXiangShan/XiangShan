...
// used for s3_prevLastHalfData calculation.
  ...
  private val s4_mmioMissEndOffset = Wire(ValidUndirectioned(UInt(log2Ceil(PredictWidth).W)))
  s4_mmioMissEndOffset.valid := s4_reqIsMmio
  s4_mmioMissEndOffset.bits  := Mux(s4_prevLastIsHalfRvi || mmioIsRvc, 0.U, 1.U)
  ...
  mmioFlushWb.bits.misEndOffset := s4_mmioMissEndOffset
