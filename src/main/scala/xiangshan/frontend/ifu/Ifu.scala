...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  private val fromICache                    = io.fromICache.resp
  ...
  private val s1_iCacheAllRespWire =
    fromICache.valid &&
      fromICache.bits.vAddr(0) === s1_ftqReq.startAddr &&
      (fromICache.bits.doubleline && fromICache.bits.vAddr(1) === s1_ftqReq.nextlineStart || !s1_doubleline)
  ...
  private val s1_exceptionIn        = fromICache.bits.exception
  private val s1_isBackendException = fromICache.bits.isBackendException
  // paddr and gpAddr of [startAddr, nextLineAddr]
  private val s1_pAddr             = fromICache.bits.pAddr
  private val s1_gpAddr            = fromICache.bits.gpAddr
  private val s1_isForVSnonLeafPTE = fromICache.bits.isForVSnonLeafPTE
  ...
  private val s1_mmioMismatchException = VecInit(Seq(
  ...
  // merge exceptions
  private val s1_exception = ExceptionType.merge(s1_exceptionIn, s1_mmioMismatchException)

  // we need only the first port, as the second is asked to be the same
  private val s1_pmpMmio  = fromICache.bits.pmpMmio(0)
  private val s1_itlbPbmt = fromICache.bits.itlbPbmt(0)
  ...
  private val s2_exception          = RegEnable(s1_exception, s1_fire)
  private val s2_isBackendException = RegEnable(s1_isBackendException, s1_fire)
  private val s2_pAddr              = RegEnable(s1_pAddr, s1_fire)
  private val s2_gpAddr             = RegEnable(s1_gpAddr, s1_fire)
  private val s2_isForVSnonLeafPTE  = RegEnable(s1_isForVSnonLeafPTE, s1_fire)
  private val s2_perfInfo           = RegEnable(s1_perfInfo, s1_fire)
  private val s2_pmpMmio            = RegEnable(s1_pmpMmio, s1_fire)
  private val s2_itlbPbmt           = RegEnable(s1_itlbPbmt, s1_fire)
