...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  private val s1_iCacheRespValidReg = ValidHold(s1_valid && s1_iCacheRespValid && !s2_ready, s1_fire, s1_flush)

  icacheRespAllValid := s1_iCacheRespValidReg || s1_iCacheRespValid
