...
class IfuPerfAnalysis(implicit p: Parameters) extends IfuModule {
  ...
  // Ftq had no block to hand over this cycle, so there is nothing to fetch and no later stage will name a reason for
  // the slots that go missing because of it.
  when(io.ifuPerfCtrl.fromFtqBubble) {
