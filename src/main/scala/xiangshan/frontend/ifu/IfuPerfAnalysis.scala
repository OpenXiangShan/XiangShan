...
class IfuPerfAnalysis(implicit p: Parameters) extends IfuModule {
  ...
  // A Bpu override cuts the entries the earlier stage had already handed on. Only the second block of a pair waits
  // for Ftq's point of no return; the first may still be overridden and is flushed from here. Stage 0 alone, because
  // that is all an override reaches: the later stages are flushed only by a backend or write-back redirect. With
  // Ftq's own stage that comes to the two cycles an override costs.
