...
class FauFTB(implicit p: Parameters) extends BasePredictor with FauFTBParams {
  ...
  val s1_possible_full_preds = Wire(Vec(numWays, Vec(numDup, new FullBranchPrediction)))
  
  val s1_all_entries = VecInit(ways.map(_.io.resp))
  for (c & fps & e <- ctrs zip s1_possible_full_preds zip s1_all_entries) {
    fps.map(_.hit := DontCare)
    fps.zip(s1_pc_dup).map {case (f, pc) => f.fromFtbEntry(e, pc)}
    for (i <- 0 until numBr) {
      fps.map(_.br_taken_mask(i) := c(i)(1) || e.always_taken(i))
    }
  }
  ...
  io.out.s1.full_pred.zip(fauftb_enable_dup).map {case (fp, en) => fp.hit := s1_hit && en}
  ...
  // s1
  val u_s1_valid_dup = dup_seq(RegNext(u.valid), numWays+1) // reduce fanouts
  ...
  val u_s1_ways_write_valid = VecInit((0 until numWays).map(w => u_s1_write_way_oh(w).asBool && u_s1_valid_dup(w)))
