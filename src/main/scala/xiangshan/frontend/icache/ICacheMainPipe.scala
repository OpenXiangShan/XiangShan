...
//  assert(!(assert_valid && assert_val))
  ...
  io.perfInfo.only_0_hit      := s2_rawHits(0) && !s2_doubleline
  io.perfInfo.only_0_miss     := !s2_rawHits(0) && !s2_doubleline
  io.perfInfo.hit_0_hit_1     := s2_rawHits(0) && s2_rawHits(1) && s2_doubleline
  io.perfInfo.hit_0_miss_1    := s2_rawHits(0) && !s2_rawHits(1) && s2_doubleline
  io.perfInfo.miss_0_hit_1    := !s2_rawHits(0) && s2_rawHits(1) && s2_doubleline
  io.perfInfo.miss_0_miss_1   := !s2_rawHits(0) && !s2_rawHits(1) && s2_doubleline
  io.perfInfo.hit_0_except_1  := s2_rawHits(0) && ExceptionType.hasException(s2_exception(1)) && s2_doubleline
  io.perfInfo.miss_0_except_1 := !s2_rawHits(0) && ExceptionType.hasException(s2_exception(1)) && s2_doubleline
  io.perfInfo.bank_hit(0)     := s2_rawHits(0)
  io.perfInfo.bank_hit(1)     := s2_rawHits(1) && s2_doubleline
  io.perfInfo.except_0        := ExceptionType.hasException(s2_exception(0))
  io.perfInfo.hit             := s2_rawHits(0) && (!s2_doubleline || s2_rawHits(1))
