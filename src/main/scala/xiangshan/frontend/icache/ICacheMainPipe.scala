...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule
  ...
  private val s1_metaCorrupt = checkMetaEcc(s1_pTags, s1_metaCodes, s1_waymasks, eccEnable)
  ...
  (0 until PortNumber).foreach { i =>
    ...
    io.errors(i).bits.paddr         := s2_pAddr(i)
...
//  assert(!(assert_valid && assert_val))
  ...
  private val s2_l2Exception = VecInit(s2_l2Corrupt.map(ExceptionType.fromECC(true.B, _)))
  ...
  (0 until PortNumber).foreach { i =>
    when(RegNext(s2_fire && s2_l2Corrupt(i))) {
      ...
      io.errors(i).bits.paddr         := RegNext(s2_pAddr(i))
