...
class ICacheAXICtrlUnit(implicit p: Parameters) extends LazyModule
    with HasICacheCtrlUnitParameters {
  lazy val module = new ICacheAXICtrlUnitImp(this)
  ...
  class ICacheAXICtrlUnitImp(wrapper: LazyModule) extends LazyModuleImp(wrapper)
      with HasICacheCtrlUnitParameters
      with ICacheMetaHelper {
    class ICacheAXICtrlUnitIO(implicit p: Parameters) extends ICacheBundle {
      // ecc control
    ...
    val io: ICacheAXICtrlUnitIO = IO(new ICacheAXICtrlUnitIO)
