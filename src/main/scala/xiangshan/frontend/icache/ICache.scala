...
class ICacheDataArray(implicit p: Parameters) extends ICacheArray
{
  ...
  val dataArrays = (0 until partWayNum).map{ bank =>
    (0 until 2).map { i =>
      ...
      if (i == 0)
        ...
        sramBank.io.r.req.bits.apply(
          setIdx= Mux(io.read.bits(bank).isDoubleLine, io.read.bits(bank).vSetIdx(1), io.read.bits(bank).vSetIdx(0)) 
        )
  ...
  val codeArrays = (0 until 2) map { i =>
    ...
    if (i == 0)
      ...
      codeArray.io.r.req.bits.apply(
        setIdx= Mux(io.read.bits.last.isDoubleLine, io.read.bits.last.vSetIdx(1), io.read.bits.last.vSetIdx(0)) 
      )
    // SRAM write logic
    codeArray.io.w.req.valid := io.read.valid
