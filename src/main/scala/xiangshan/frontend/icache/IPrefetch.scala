...
class PrefetchBuffer(implicit p: Parameters) extends IPrefetchModule
{
  ...
  /**
    ...
    ******************************************************************************
    */
  val isWriteICacheChangeTable = WireInit(Constantin.createRecord("isWriteICacheChangeTableIPF" + p(XSCoreParamsKey).HartId.toString))
  ...
  icacheChangeDumpData.virIdx  := toICacheMeta.bits.virIdx
  icacheChangeDumpData.phyTag  := toICacheMeta.bits.phyTag
  icacheChangeDumpData.waymask := toICacheMeta.bits.waymask
  icacheChangeDumpData.bankIdx := toICacheMeta.bits.bankIdx

  icacheChangeTable.log(
    data = icacheChangeDumpData,
    en = isWriteICacheChangeTable.orR && toICacheMeta.fire,
    site = "PrefetchBufferWrite" + p(XSCoreParamsKey).HartId.toString,
    clock = clock,
    reset = reset
  )
...
class PrefetchQueue(edge: TLEdgeOut)(implicit p: Parameters) extends IPrefetchModule {
  ...
  */
  ...
  val cancelHandleEntry = (((0 until PortNumber).map(i=> fromMainPipe(i).valid && r_hit_handle(i))).reduce(_||_) || io.fencei) && !handleEntry.issue && !handleEntry.finish
  val piq_hit = (0 until PortNumber).map(i => r_hit_handle(i) && (handleEntry.issue || handleEntry.finish) && !io.fencei)
  toMainPipe := DontCare
  if (prefetchToL1) {
    (0 until PortNumber).foreach (i => {
  ...
  /**
    ...
    ******************************************************************************
    */
  toIPF.valid          := handleEntry.valid && handleEntry.finish && !io.fencei
