...
class ResolveQueue(implicit p: Parameters) extends FtqModule with HalfAlignHelper with HasCircularQueuePtrHelper {
  ...
  private val dropResolveCounter = RegInit(0.U.asTypeOf(new SaturateCounter(8)))

  private object DropResolveFsmState extends EnumUInt(2) {
    def Idle: UInt = 0.U(width.W)
    def Drop: UInt = 1.U(width.W)
  }

  private val dropResolveState = RegInit(DropResolveFsmState.Idle)
  ...
  when(hasResolve && !hasMispredict) {
    dropResolveCounter.increase(1)
  }.elsewhen(hasResolve && hasMispredict) {
    dropResolveCounter.decrease(64)
  }

  switch(dropResolveState) {
    is(DropResolveFsmState.Idle) {
      when(dropResolveCounter.isSaturatePositive) {
        dropResolveState := DropResolveFsmState.Drop
      }
    }
    is(DropResolveFsmState.Drop) {
      when(dropResolveCounter.value < 192.U) {
        dropResolveState := DropResolveFsmState.Idle
      }
    }
  }

  private val resolve = filteredResolve.map { filteredResolve =>
    val mispredicted = filteredResolve.bits.mispredict
    val isDirect     = filteredResolve.bits.attribute.isDirect
    val stateIsDrop  = dropResolveState === DropResolveFsmState.Drop
    val shouldDrop   = !mispredicted && (stateIsDrop || isDirect)
