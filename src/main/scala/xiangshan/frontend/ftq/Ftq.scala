...
class Ftq(implicit p: Parameters) extends FtqModule
  ...
  private val ifuRedirectMetaEarlyByOneCycle = ifuFtqPtrsEarlyByTwoCycles.map {
    case ftqIdx => RegNext(metaQueueRedirect(ftqIdx.value))
  }
  ...
  specReadReq.tosr       := ifuAdvanceRedirectMeta.ras.tosr
  specReadReq.ssp        := ifuAdvanceRedirectMeta.ras.ssp
  specReadReq.tosrInSpec := ifuAdvanceRedirectMeta.ras.topInSpec

  private val specRead    = io.fromBpu.specRead
  private val specRetAddr = RegNext(ifuAdvanceRedirectMeta.ras.topRetAddr)
