...
//  XSError(ifuWbPtr < commitPtr && !isFull(ifuWbPtr(0), commitPtr(0)), "ifuWbPtr runs ahead of commitPtr")
  ...
  io.toICache.prefetchReq.bits.takenCfiOffset := cfiQueue(ifuPtr(0).value).bits
  ...
  io.toICache.fetchReq.bits.startVAddr         := entryQueue(ifuPtr(0).value)
  io.toICache.fetchReq.bits.nextCachelineVAddr := entryQueue(ifuPtr(0).value) + (CacheLineSize / 8).U
  io.toICache.fetchReq.bits.ftqIdx             := ifuPtr(0)
  io.toICache.fetchReq.bits.takenCfiOffset     := cfiQueue(ifuPtr(0).value).bits
