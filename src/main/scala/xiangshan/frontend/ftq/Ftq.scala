...
class Ftq(implicit p: Parameters) extends FtqModule
  ...
  // An override threw away what the earlier stage predicted, so the entries behind it are cut and fetched again. Name
  // the reason here rather than in Bpu: what the override costs is the fetch already in flight for those entries, and
  // this stage and Ifu's are the ones carrying it.
  when(bpuS2Redirect || bpuS3Redirect) {
