...
class Tage(implicit p: Parameters) extends BaseTage {
  ...
  // Debug signal
  val debug_s0_pc = dontTouch(s0_pc_dup(1))
  val debug_s1_pc = dontTouch(RegEnable(s0_pc_dup(1), io.s0_fire(1)))
  val debug_s2_pc = dontTouch(RegEnable(debug_s1_pc, io.s1_fire(1)))
