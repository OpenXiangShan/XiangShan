...
(
  ...
  if (EnableGHistDiff) {
    ...
    XSError1(idx_history =/= idx_fh.folded_hist, p"tage table $tableIdx has different fh," +
  ...
  if (EnableGHistDiff) {
    ...
    XSError1(update_idx_history =/= update_idx_fh.folded_hist && io.update.mask.reduce(_||_),
