...
object TraceRTLParameters {
  ...
  def generateVerilogHeader(implicit p: Parameters): Unit = {
    val trtl = p(TraceRTLParamKey)
    val tOutBundle = new TraceInstrFpgaBundle
    val tCollectBundle = new TraceFPGACollectBundle
    ...
    result += s"""
         |// generated by TraceParameters.scala
         |// DO NOT EDIT
         |
         |`define TRACERTL_GATEWAY_IN_BIT_WIDTH ${tOutBundle.getWidth * p(TraceRTLParamKey).TraceFpgaRecvWidth}
         |`define TRACERTL_GATEWAY_OUT_BIT_WIDTH ${TraceFPGACollectBundle.alignWidth() * p(TraceRTLParamKey).TraceFpgaCollectWidth}
         |
         |""".stripMargin

    streamToFile(result, "tracertl_define.vh", append=false)
  }
