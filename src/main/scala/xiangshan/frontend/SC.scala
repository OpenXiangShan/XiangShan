...
class SCTable(val nRows: Int, val ctrBits: Int, val histLen: Int,parentName:String = "Unknown")(implicit p: Parameters)
  ...
  val table = Module(new SRAMTemplate(SInt(ctrBits.W), set=nRows, way=2*TageBanks, shouldReset=true, singlePort=false, bypassWrite=true, parentName = parentName + "table_"))
  ...
  val per_br_ctrs_unshuffled = table.io.r.resp.data.sliding(2,2).toSeq.map(x => HoldUnless(VecInit(x), RegNext(io.req.valid)))
