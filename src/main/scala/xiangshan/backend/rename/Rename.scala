...
class Rename(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper {
  ...
  val raw_bypass = Wire(MixedVec(List.tabulate(RenameWidth-1)(i => UInt((i+1).W)))) // if move instruction has raw dependency with former instruction
  ...
  for (i <- 0 until RenameWidth) {
    ...
    if (i == 0) {
      ...
      // src0 bypass
      raw_bypass(i-1) := Cat((0 until i).map(j => {
        needIntDest(j) && io.in(i).bits.ctrl.srcType(0) === SrcType.reg && io.in(j).bits.ctrl.ldest === io.in(i).bits.ctrl.lsrc(0) && io.in(i).bits.ctrl.isMove
      }))

      // calculate meEnable
      meEnable(i) := isMove(i) && !(raw_bypass(i-1).orR | psrc_cmp(i-1).orR | isMax(uops(i).psrc(0)))
    }
  ...
  XSPerfAccumulate("move_elim_cancelled_psrc_bypass", PopCount(Seq.tabulate(RenameWidth)(i => io.out(i).fire() && io.in(i).bits.ctrl.isMove && !meEnable(i) && { if (i == 0) false.B else raw_bypass(i-1).orR })))
  ...
  for (i <- 0 until RenameWidth) {
    ...
    XSDebug(io.out(i).fire() && io.in(i).bits.ctrl.isMove && !meEnable(i) && { if (i == 0) false.B else raw_bypass(i-1).orR },
