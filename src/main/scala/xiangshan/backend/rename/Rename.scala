...
class Rename(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  val traceEliminateOoO = VecInit(io.in.map(r =>
    r.bits.traceDynaInfo.eliminateOoO))
  ...
  for (i <- 0 until RenameWidth) {
    ...
    when(isMove(i) || hasExceptionExceptFlushPipe || traceEliminateOoO(i)) {
      uops(i).numUops := 0.U
      uops(i).numWB := 0.U
    }
    if (i > 0) {
      when(!needRobFlags(i - 1)) {
        ...
        uops(i).numUops := instrSizesVec(i) - PopCount(compressMasksVec(i) & (Cat(isMove.reverse) | Cat(traceEliminateOoO.reverse)))
        uops(i).numWB := instrSizesVec(i) - PopCount(compressMasksVec(i) & (Cat(isMove.reverse) | Cat(traceEliminateOoO.reverse)))
      }
    }
    when(!needRobFlags(i)) {
...
//    }
  ...
  // }
  // val instTypeMap = ChiselMap.createTable("instType", Vec(RenameWidth, new FuTypeBundle))
