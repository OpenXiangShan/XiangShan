...
class RobImp(override val wrapper: Rob)(implicit p: Parameters, params: BackendParams) extends LazyModuleImp(wrapper)
  ...
  val globalTickCounter = Module(new GlobalSimClock)
  ...
  for (i <- 0 until CommitWidth) {
    ...
    PerfCCT.CommitInstMeta(deqDebugInst.debug_seqNum, io.commits.isCommit && io.commits.commitValid(i), clock, reset)
