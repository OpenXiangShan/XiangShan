...
class RobImp(outer: Rob)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  if (env.EnableDifftest) {
    for (i <- 0 until CommitWidth) {
      ...
      difftest.nFused   := CommitType.isFused(io.commits.info(i).commitType)
      ...
      runahead_commit.valid := difftest.valid && (commitBranchValid(i) || commitIsStore(i))
      // TODO: is branch or store
      runahead_commit.pc    := difftest.pc
    }
  }
