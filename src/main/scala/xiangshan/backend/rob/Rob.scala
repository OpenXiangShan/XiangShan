...
//    val pRobSize = Input(UInt(log2Up(RobSize + 1).W))
  ...
  for (fuType <- FuType.functionNameMap.keys) {
    ...
    if (fuType == FuType.fmac.litValue) {
      ...
      HardenXSPerfAccumulate(s"${fuName}_instr_cnt_fma", ifCommit(PopCount(commitIsFma)))
  ...
  io.commits.commitValid.zip(commitDebugUop).zipWithIndex.foreach{
    case ((v, uop), i) =>
      ...
      HardenXSPerfAccumulate(s"fuType", uop.ctrl.fuType, i)
      HardenXSPerfAccumulate(s"fuOpType", uop.ctrl.fuOpType, i)
      HardenXSPerfAccumulate(s"fpu", uop.ctrl.fpu.asUInt, i)
      HardenXSPerfAccumulate(s"FetchCacheLine", uop.debugInfo.fetchCacheTime, i)
      HardenXSPerfAccumulate(s"ProcessCacheCompletion", uop.debugInfo.cacheCompTime, i)
      ...
      HardenXSPerfAccumulate(s"InsertReadyList", max(uop.debugInfo.readyIssueTime, uop.debugInfo.readyIssueTime_std), i)
      HardenXSPerfAccumulate(s"Select", max(uop.debugInfo.selectTime, uop.debugInfo.selectTime_std), i)
      HardenXSPerfAccumulate(s"Issue", max(uop.debugInfo.issueTime, uop.debugInfo.issueTime_std), i)
      ...
      HardenXSPerfAccumulate(s"LQ", uop.lqIdx.value, i)
      HardenXSPerfAccumulate(s"SQ", uop.sqIdx.value, i)
      HardenXSPerfAccumulate(s"RS", uop.debugInfo.rsIdx, i)
      HardenXSPerfAccumulate(s"FU", uop.debugInfo.fuIdx, i)
      HardenXSPerfAccumulate(s"SRC0", uop.psrc(0), i)
  ...
  val perfEvents = Seq(
    ...
    ("rob_1_4_valid          ", validEntries <= (pRobSize / 4.U)                                      ),
    ("rob_2_4_valid          ", validEntries >  (pRobSize / 4.U)   && validEntries <= (pRobSize / 2.U)),
    ("rob_3_4_valid          ", validEntries >  (pRobSize / 2.U)   && validEntries <= (pRobSize * 3.U / 4.U)),
    ("rob_4_4_valid          ", validEntries >  (pRobSize * 3.U / 4.U)                                ),
  )
  generatePerfEvent()
}
