...
class RobImp(override val wrapper: Rob)(implicit p: Parameters, params: BackendParams) extends LazyModuleImp(wrapper)
  ...
  */
  ...
  val enqUpdateDirtyFsSeq = io.enq.req.map(req => req.valid && (req.bits.fpWen || req.bits.fflagsWen)).zip(canEnqueue).map(x => x._1 && x._2)
...
//  }
  ...
  for (i <- 0 until fflagsWidth) {
    ...
    val wbUpdateFflags = fflagsWBs.map(wb => wb.valid && wb.bits.fflagsWen.get && wb.bits.fflags.get(i)).reduce(_ | _)

    val wbUpdateFflagsSeq = fflagsWBs.map(wb =>
      {
        val s = Wire(new WbFflags)
        s.set := wb.valid && wb.bits.fflagsWen.get && wb.bits.fflags.get(i)
        s.robIdx := wb.bits.robIdx
        s
      }
    )
