...
class ExeUnitImp(implicit p: Parameters, val exuParams: ExeUnitParams) extends XSModule with HasXSParameter with HasCriticalErrors {
  ...
  def pipelineReg(init: NewExuInput, valid: Bool, latency: Int, flush: ValidIO[Redirect]): (Seq[NewExuInput], Seq[Bool]) = {
    ...
    val robIdxVec = inVec.map(_.robIdx.bits)
  ...
  in1ToN.io.out.zip(funcUnits.map(_.io.in)).foreach {
    case (source: DecoupledIO[NewExuInput], sink: DecoupledIO[FuncUnitInput]) =>
      ...
      sink.bits.ctrl.toRobValid  := source.bits.robIdx.valid
      sink.bits.ctrl.robIdx      := source.bits.robIdx.bits
  ...
  funcUnits.filter(_.cfg.latency.latencyVal.nonEmpty).map{ fu =>
    ...
    for (i <- 0 until (latency+1)) {
      ...
      sink.toRobValid := source.robIdx.valid
  ...
  private val outIntDataValid = OptionWrapper(funcUnits.exists(_.cfg.writeIntRf), WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeIntRf))(false.B))))
  private val outFpDataValid  = OptionWrapper(funcUnits.exists(_.cfg.writeFpRf),  WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeFpRf)) (false.B))))
  private val outVecDataValid = OptionWrapper(funcUnits.exists(_.cfg.writeVecRf), WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeVecRf))(false.B))))
  private val outV0DataValid  = OptionWrapper(funcUnits.exists(_.cfg.writeV0Rf),  WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeV0Rf)) (false.B))))
  private val outVlDataValid  = OptionWrapper(funcUnits.exists(_.cfg.writeVlRf),  WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeVlRf)) (false.B))))
  private val outIntData = OptionWrapper(funcUnits.exists(_.cfg.writeIntRf), WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeIntRf))(0.U(XLEN.W)))))
  private val outFpData  = OptionWrapper(funcUnits.exists(_.cfg.writeFpRf),  WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeFpRf)) (0.U(XLEN.W)))))
  private val outVecData = OptionWrapper(funcUnits.exists(_.cfg.writeVecRf), WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeVecRf))(0.U(VLEN.W)))))
  private val outV0Data  = OptionWrapper(funcUnits.exists(_.cfg.writeV0Rf),  WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeV0Rf)) (0.U(VLEN.W)))))
  private val outVlData  = OptionWrapper(funcUnits.exists(_.cfg.writeVlRf),  WireInit(VecInit(Seq.fill(funcUnits.count(_.cfg.writeVlRf)) (0.U(VLEN.W)))))
  ...
  Option.when(funcUnits.exists(_.cfg.writeIntRf)) {
    ...
    outIntDataValid.get.zip(outIntData.get).zip(vld).zip(data).foreach { case (((outVld, outData), inVld), inData) =>
      outVld  := inVld
      outData := inData
    }
  }
  Option.when(funcUnits.exists(_.cfg.writeFpRf)) {
    ...
    outFpDataValid.get.zip(outFpData.get).zip(vld).zip(data).foreach { case (((outVld, outData), inVld), inData) =>
      outVld  := inVld
      outData := inData
    }
  }
  Option.when(funcUnits.exists(_.cfg.writeVecRf)) {
    val vld = (funcUnits.zip(fuOutValidOH).filter { case (fu, _) => fu.cfg.writeVecRf }.map { case (_, fuoutOH) => fuoutOH })
    val data = (funcUnits.zip(fuOutresVec).filter { case (fu, _) => fu.cfg.writeVecRf }.map { case (_, fuout) => fuout.data })
    outVecDataValid.get.zip(outVecData.get).zip(vld).zip(data).foreach { case (((outVld, outData), inVld), inData) =>
      outVld  := inVld
      outData := inData
    }
  }
  Option.when(funcUnits.exists(_.cfg.writeV0Rf)) {
    val vld = (funcUnits.zip(fuOutValidOH).filter { case (fu, _) => fu.cfg.writeV0Rf }.map { case (_, fuoutOH) => fuoutOH })
    val data = (funcUnits.zip(fuOutresVec).filter { case (fu, _) => fu.cfg.writeVlRf }.map { case (_, fuout) => fuout.data })
    outV0DataValid.get.zip(outV0Data.get).zip(vld).zip(data).foreach { case (((outVld, outData), inVld), inData) =>
      outVld  := inVld
      outData := inData
    }
  }
  Option.when(funcUnits.exists(_.cfg.writeVlRf)) {
    val vld = (funcUnits.zip(fuOutValidOH).filter { case (fu, _) => fu.cfg.writeVlRf }.map { case (_, fuoutOH) => fuoutOH })
    val data = (funcUnits.zip(fuOutresVec).filter { case (fu, _) => fu.cfg.writeVlRf }.map { case (_, fuout) => fuout.data })
    outVlDataValid.get.zip(outVlData.get).zip(vld).zip(data).foreach { case (((outVld, outData), inVld), inData) =>
      outVld  := inVld
      outData := inData
    }
  }
  ...
  private val outValid = Cat(fuOutValidOH).orR
  ...
  io.out.bits.toIntRf.          foreach(x => x.bits  := Mux1H(outIntDataValid.get, outIntData.get))
  io.out.bits.toFpRf.           foreach(x => x.valid := Mux1H(fuOutValidOH, fuFpWenVec))
  io.out.bits.toFpRf.           foreach(x => x.bits  := Mux1H(outFpDataValid.get, outFpData.get))
  io.out.bits.toVecRf.          foreach(x => x.valid := Mux1H(fuOutValidOH, fuVecWenVec))
  io.out.bits.toVecRf.          foreach(x => x.bits  := Mux1H(outVecDataValid.get, outVecData.get))
  io.out.bits.toV0Rf.           foreach(x => x.valid := Mux1H(fuOutValidOH, fuV0WenVec))
  io.out.bits.toV0Rf.           foreach(x => x.bits  := Mux1H(outV0DataValid.get, outV0Data.get))
  io.out.bits.toVlRf.           foreach(x => x.valid := Mux1H(fuOutValidOH, fuVlWenVec))
  io.out.bits.toVlRf.           foreach(x => x.bits  := Mux1H(outVlDataValid.get, outVlData.get))
