...
class ExeUnitImp(implicit p: Parameters, val exuParams: ExeUnitParams) extends XSModule with HasXSParameter with HasCriticalErrors {
  ...
  in1ToN.io.out.zip(funcUnits.map(_.io.in)).foreach {
    case (source: DecoupledIO[ExuInput], sink: DecoupledIO[FuncUnitInput]) =>
      ...
      sink.bits.ctrl.pdestV0     .foreach(x => x := source.bits.pdestV0.get)
  ...
  funcUnits.filter(_.cfg.latency.latencyVal.nonEmpty).map{ fu =>
    ...
    for (i <- 0 until (latency+1)) {
      ...
      sink.pdestV0.foreach(_ := source.pdestV0.get)
      ...
      sinkData.v0.foreach(_ := sourceData.v0.get)
  ...
  funcUnits.zip(exuParams.idxCopySrc).map{ case(fu, idx) =>
    ...
    fu.io.in.bits.data.v0.foreach(_ := io.in.bits.v0.get)
  ...
  io.out.bits.pdestV0.foreach(_ := Mux1H(fuOutValidOH, funcUnits.map(_.io.out.bits.ctrl.pdestV0.getOrElse(0.U))))
