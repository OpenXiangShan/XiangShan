...
case class ExeUnitParams(
  ...
  val fuLatencyMap = if (latencyCertain) Some(fuConfigs.map(y => List(y.fuType, y.latency.latencyVal.get))) else None
  val latencyValMax = fuLatencyMap.map(x => x.map(_.last).max)
