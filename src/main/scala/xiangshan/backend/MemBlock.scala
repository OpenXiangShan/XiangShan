...
//    }
    ...
    for (j <- 0 until 3) {
      ...
      hit(j) := loadUnits(i).io.trigger(j).addrHit // Mux(tdata(j + 3).select, loadUnits(i).io.trigger(j).lastDataHit, loadUnits(i).io.trigger(j).addrHit) // this comes 1 cycle after wb
    ...
    XSDebug(io.writeback(i).bits.uop.cf.trigger.getHitBackend, p"Debug Mode: Load Inst No. ${i}" +
...
//          hit(j) := TriggerCmp(io.stOut(i).bits.data, tdata(sTriggerMapping(j)).tdata2, tdata(sTriggerMapping(j)).matchType, tEnable(sTriggerMapping(j)))
//          io.stOut(i).bits.uop.cf.trigger.backendHit(sTriggerMapping(j)) := hit(j)
//          io.stOut(i).bits.uop.cf.trigger.backendTiming(sTriggerMapping(j)) := tdata(sTriggerMapping(j)).timing
////          if (sChainMapping.contains(j)) io.stOut(i).bits.uop.cf.trigger.triggerChainVec(sChainMapping(j)) := hit && tdata(j + 3).chain
//        }
//
//        when(io.stOut(i).bits.uop.cf.trigger.backendEn(0)) {
//          io.stOut(i).bits.uop.cf.trigger.backendHit(4) := Mux(io.stOut(i).bits.uop.cf.trigger.backendConsiderTiming(0),
//            tdata(4).timing === io.writeback(i).bits.uop.cf.trigger.backendChainTiming(0), true.B) && hit(2)
//        } .elsewhen{
//          io.stOut(i).bits.uop.cf.trigger.backendHit(4) := false.B
//        }
//      }
//    }
    // store data
