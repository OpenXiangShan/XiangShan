  ...
  with HasL1PrefetchSourceParameter
{
  ...
  private val VlduCnt = backendParams.VlduCnt
  private val VstuCnt = backendParams.VstuCnt
  private val VecMemExuCnt = VlduCnt + VstuCnt
  println("VlduCnt = " + VlduCnt)
  println("VstuCnt = " + VstuCnt)
  require(VlduCnt == 1)
  require(VstuCnt == 1)
  ...
  // io.mem_to_ooo.writeback <> MixedVecInit(
  //   // scalar writeback
  //   loadUnits.map(_.io.ldout) ++
  //   storeUnits.map(_.io.stout) ++
  //   stdExeUnits.map(_.io.out) ++
  //   // vector writeback
  //   Seq(vlWrapper.io.uopWriteback) ++
  //   Seq(vsUopQueue.io.uopWriteback)
  // )
  ...
  dtlb.flatMap(a => a.ptw.req)
    ...
    val vector_hit = if (refillBothTlb) Cat(ptw_resp_next.vector).orR
      ...
      else Cat(ptw_resp_next.vector.drop(LduCnt + StaCnt + 1)).orR
  ...
  if (refillBothTlb) {
    ...
    dtlb_prefetch.foreach(_.ptw.resp.valid := ptw_resp_v && Cat(ptw_resp_next.vector.drop(LduCnt + StaCnt + 1)).orR)
  }
  ...
  for (i <- 0 until LduCnt) {
    ...
    io.mem_to_ooo.writeback(i) <> loadUnits(i).io.ldout
...
//      io.writeback(i).bits.uop.cf.trigger.backendTiming(lTriggerMapping(j)) := tdata(lTriggerMapping(j)).timing
  ...
  val PrefetcherDTLBPortIndex = LduCnt + StaCnt + 1
  prefetcherOpt match {
  ...
  for (i <- 0 until StaCnt) {
    ...
    io.mem_to_ooo.writeback(i + LduCnt + StaCnt) <> stdExeUnits(i).io.out
    ...
    io.mem_to_ooo.writeback(LduCnt + i) <> stu.io.stout
...
//  lsq.io.rob            <> io.lsqio.rob
  ...
  // via different ports.
  val vecIssuePort = io.ooo_to_mem.issue.drop(MemExuCnt).head
  vlWrapper.io.loadRegIn.valid := vecIssuePort.valid && isVLoad(vecIssuePort.bits.uop.fuType)
  vlWrapper.io.loadRegIn.bits := vecIssuePort.bits
  vsUopQueue.io.storeIn.valid := vecIssuePort.valid && isVStore(vecIssuePort.bits.uop.fuType)
  vsUopQueue.io.storeIn.bits := vecIssuePort.bits
  vecIssuePort.ready := vlWrapper.io.loadRegIn.ready && isVLoad(vecIssuePort.bits.uop.fuType) ||
    vsUopQueue.io.storeIn.ready && isVStore(vecIssuePort.bits.uop.fuType)
  io.ooo_to_mem.issue.drop(MemExuCnt + VlduCnt).foreach(_.ready := false.B) // idle issue ports
  ...
  val stDeqCount = PopCount(io.ooo_to_mem.issue.drop(LduCnt).take(StaCnt).map(_.valid))
