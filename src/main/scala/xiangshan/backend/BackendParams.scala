...
case class BackendParams(
  ...
  def NewJmpCnt = allSchdParams.map(_.NewJmpCnt).sum
    ...
    SchdBlockParams(Seq(
      IssueBlockParams(Seq(
        ExeUnitParams(
          ...
          Seq(Seq(VfRD(0, 0), IntRD(11, 0)), Seq(VfRD(1, 0)), Seq(VfRD(2, 0)), Seq(V0RD(0, 0))),
      ...
      IssueBlockParams(Seq(
        ExeUnitParams(
          ...
          Seq(Seq(VfRD(3, 0), IntRD(12, 0)), Seq(VfRD(4, 0)), Seq(VfRD(5, 0)), Seq(V0RD(1, 0))),
          vlRD = VlRD(1, 0),
        ),
      ), numEntries = 16, numEnq = 2, numComp = 12),
      IssueBlockParams(Seq(
