...
class Dispatch2RsDistinctImp(outer: Dispatch2Rs)(implicit p: Parameters) extends Dispatch2RsImp(outer) {
  ...
  require(isPow2(LoadPipelineWidth))
  // in: to deal with lsq
  ...
  if (io.enqLsq.isDefined) {
    ...
    for (i <- io.in.indices) {
      ...
      if (i < enqLsq.req.length) {
        ...
        enqLsq.req(i).valid := in(i).valid && VecInit(out.map(_.ready)).asUInt.andR
      }
    }
  }
  ...
  for ((config, i) <- outer.exuConfigCases) {
    ...
    for ((idx, j) <- outIndices.zipWithIndex) {
      ...
      if (config.contains(StdExeUnitCfg)) {
        val sta = out(idx - StorePipelineWidth)
        sta.valid := out(idx).valid
        out(idx).bits.ctrl.srcType(0) := out(idx).bits.ctrl.srcType(1)
        out(idx).bits.psrc(0) := out(idx).bits.psrc(1)
        XSPerfAccumulate(s"st_rs_not_ready_$idx", selectValid && (!sta.ready || !out(idx).ready))
        XSPerfAccumulate(s"sta_rs_not_ready_$idx", selectValid && !sta.ready && out(idx).ready)
        XSPerfAccumulate(s"std_rs_not_ready_$idx", selectValid && sta.ready && !out(idx).ready)
      }
      else {
