...
object Bundles {
  ...
  class ExuInput(val params: ExeUnitParams, copyWakeupOut:Boolean = false, copyNum:Int = 0, hasCopySrc: Boolean = false)(implicit p: Parameters) extends XSBundle {
    ...
    def toDynInst(): DynInst = {
      ...
      uop.loadWaitBit    := Mux(enableMdp, this.loadWaitBit.getOrElse(false.B), false.B)
      uop.waitForRobIdx  := Mux(enableMdp, this.waitForRobIdx.getOrElse(0.U.asTypeOf(new RobPtr)), 0.U.asTypeOf(new RobPtr))
      uop.storeSetHit    := Mux(enableMdp, this.storeSetHit.getOrElse(false.B), false.B)
      uop.loadWaitStrict := Mux(enableMdp, this.loadWaitStrict.getOrElse(false.B), false.B)
      uop.ssid           := Mux(enableMdp, this.ssid.getOrElse(0.U(SSIDWidth.W)), 0.U(SSIDWidth.W))
