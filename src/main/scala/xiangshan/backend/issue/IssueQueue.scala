...
class IssueQueueImp(implicit p: Parameters, params: IssueBlockParams) extends XSModule with HasXSParameter {
  ...
  deqBeforeDly.zipWithIndex.foreach { case (deq, i) =>
    ...
    deq.bits.common.pdestV0.foreach(_ := deqEntryVec(i).bits.payload.pdestV0.get)
    ...
    deq.bits.common.v0.foreach(_ := DontCare) // will connect it in datapath, DontCare here
    ...
    deq.bits.rfV0 lazyZip deqEntryVec(i).bits.status.srcStatusV0.map(_.psrc) foreach {
      case (rf, psrc) =>
        rf.addr := psrc
        rf.srcType := SrcType.v0 // this is vl
        rf.robIdx := deqEntryVec(i).bits.status.robIdx
        rf.issueValid := deqEntryVec(i).valid
    }
