...
class StatusArray(params: RSParams)(implicit p: Parameters) extends XSModule
  ...
  val io = IO(new Bundle {
    ...
    val stIssue = if (params.checkWaitBit) Flipped(Vec(exuParameters.StuCnt, ValidIO(new ExuInput))) else null
  })
  ...
  for (((status, statusNext), i) <- statusArray.zip(statusArrayNext).zipWithIndex) {
    ...
    if (params.checkWaitBit) {
      ...
      val storeWaitforIsIssuing = VecInit((0 until StorePipelineWidth).map(i => {
        io.stIssue(i).valid && io.stIssue(i).bits.uop.sqIdx.value === statusNext.waitForSqIdx.value
      })).asUInt.orR
      statusNext.blocked := Mux(updateValid(i), updateVal(i).blocked, status.blocked) && 
        blockNotReleased && 
        storeWaitforIsIssuing
