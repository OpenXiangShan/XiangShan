...
import xiangshan.backend.fu.FuConfig.VSetRvfWvfCfg
...
class VTypeGen(implicit p: Parameters) extends XSModule{
  ...
  /** first vset instruction is the keep-vl form: vsetvli x0, x0, imm */
  private val isKeepVlVsetvli = isVsetvli &&
    (firstVsetInstField.RD === 0.U) && (firstVsetInstField.RS1 === 0.U)
   ...
   *
   * For the keep-vl form (vsetvli x0, x0, imm) the new vtype still depends on the old vtype:
   * the encoding is reserved and vill must be raised when vill was already set or when the
   * new SEW/LMUL ratio changes VLMAX. The vtype generated here is the one committed for
   * vsetvli, so the check has to be done in decode as well.
   */
  private val vsetModule = Module(new VsetModule(VSetRvfWvfCfg))
  vsetModule.io.in.avl := 0.U
  vsetModule.io.in.vtype := vtypei
  vsetModule.io.in.func := Mux(isKeepVlVsetvli, VSETOpType.keepVl(VSETOpType.uvsetvcfg_xi),
                                              VSETOpType.uvsetvcfg_xi)
  /** speculative vtype before this instruction, used by the keep-vl check */
  vsetModule.io.in.oldVt.get := vtypeSpec
