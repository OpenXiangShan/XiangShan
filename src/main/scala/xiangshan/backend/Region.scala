      ...
      for (j <- toMem(i).indices) {
        ...
        NewPipelineConnect(
          ...
          Mux(
            ...
            toMem(i)(j).bits.robIdx.needFlush(flushCopyReg2) || toMem(i)(j).valid && !toMem(i)(j).ready
          ),
        ...
        val s0RespFinalSuccess = (if (thisIQ.param.isStAddrIQ) !FuType.isStore(toMem(i)(j).bits.fuType)
                                  else if (thisIQ.param.isVecStuIQ) !FuType.isVStore(toMem(i)(j).bits.fuType)
                                  else true.B)
        if (thisIQ.io.s0Resp.nonEmpty) {
          thisIQ.io.s0Resp.get(j).failed := toMem(i)(j).valid && !toMem(i)(j).ready
          thisIQ.io.s0Resp.get(j).finalSuccess := toMem(i)(j).fire && s0RespFinalSuccess
