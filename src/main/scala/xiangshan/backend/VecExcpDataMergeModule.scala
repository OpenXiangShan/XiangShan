...
class VecExcpDataMergeModule(implicit p: Parameters) extends XSModule {
  ...
  private val sNoExcp_idxRangeVec: Vec[HWRange] = Module(new NfMappedElemIdx(VLEN))(sNoExcp_dvemulNoLessThanM1, sNoExcp_deewOH)
...
class NfMappedElemIdx(vlen: Int) extends Module {
  ...
  val in = IO(Input(new Bundle {
    val eewOH = SewOH()
    val dvemulNoLessThanM1 = UInt(2.W)
  }))
  ...
  private val rangeTable: Vec[Vec[HWRange]] = VecInit.tabulate(4, 8) { case(dvemul, vdIdx) =>
    val vdIdxInGroup = vdIdx / (1 << dvemul)
    HWRange(idxWidth)((maxElemNumPerVreg * vdIdxInGroup).U, (maxElemNumPerVreg * (vdIdxInGroup + 1)).U)
  }

  val e8Range: Vec[HWRange] = rangeTable(in.dvemulNoLessThanM1)

  out.idxRangeVec := Mux1H(
    (0 until 4).map(
      i => in.eewOH(i) -> VecInit(e8Range.map(x => HWRange(idxWidth)(x.from >> i, x.until >> i)))
    )
  )

  dontTouch(out.idxRangeVec)

  def apply(dvemulNoLessThanM1: UInt, eewOH: UInt): Vec[HWRange] = {
    this.in.dvemulNoLessThanM1 := dvemulNoLessThanM1
    this.in.eewOH := eewOH
