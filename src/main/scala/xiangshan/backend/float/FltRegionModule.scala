...
class FltRegionImp(
  ...
  val i2fWbPort = backendParams.allExuParams.filter(_.hasi2fFu).head.getFpWBPort.get.port
  val i2fIdx0 = backendParams.intSchdParams.get.issueBlockParams.indexWhere(_.writeFpRf)
  val i2fIdx1 = backendParams.intSchdParams.get.issueBlockParams.filter(_.writeFpRf).head.exuBlockParams.indexWhere(_.hasi2fFu)
  val i2fFpWbNext = in.fromIntRegion.fpWbNext(i2fIdx0)(i2fIdx1)
  out.toIntRegion.fpExuOut.flatten.zip(fpExuOut.flatten).zipWithIndex.foreach { case ((sink, source), i) =>
    ...
    // for i2f -> std data forward
    if (i == i2fWbPort) {
      sink.valid := source.wen || i2fFpWbNext.wen
      sink.bits.toFpRf.get.valid := source.wen || i2fFpWbNext.wen
      sink.bits.toFpRf.get.bits := Mux(source.wen, source.data, i2fFpWbNext.data)
      sink.bits.pdest := Mux(source.wen, source.pdest, i2fFpWbNext.pdest)
    }
  }
