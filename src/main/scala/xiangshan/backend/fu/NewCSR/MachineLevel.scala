...
trait MachineLevel { self: NewCSR =>
  ...
  val mcycle = Module(new CSRModule("Mcycle", new CounterValueBundle("Machine cycle counter value.")) with HasMachineCounterControlBundle {
    when(w.wen) {
      reg := w.wdata
  ...
  val minstret = Module(new CSRModule("Minstret", new CounterValueBundle("Machine retired-instruction counter value.")) with HasMachineCounterControlBundle with HasRobCommitBundle {
    when(w.wen) {
      reg := w.wdata
...
class MstatusBundle extends CSRBundle {
  ...
  // Keep reset compatible with the standard flash reset vector. The first
  // M-mode trap sets MDT through TrapEntryMEvent as required.
  val MDT  = CSRRWField     (42).withReset(0.U).withDescription("M-mode disable-trap bit used by the Smdbltrp extension.")
...
class MnstatusBundle extends CSRBundle {
  // OpenSBI does not initialize Smrnmi yet, so leave NMI handling enabled.
