  ...
  with IpIeAliasConnect
{
  ...
  if (env.AlwaysBasicDiff || env.EnableDifftest) {
    ...
    val meipReg = RegInit(false.B)
    val mtipReg = RegInit(false.B)
    val msipReg = RegInit(false.B)
    val seipReg = RegInit(false.B)
    val stipReg = RegInit(false.B)
    val vseipReg = RegInit(false.B)
    val hgeipVgeinReg = RegInit(false.B)
    val vstipReg = RegInit(false.B)
    val aiaMeipReg = RegInit(false.B)
    val aiaSeipReg = RegInit(false.B)
    val lcofiReqReg = RegInit(false.B)
    meipReg := platformIRP.MEIP
    mtipReg := platformIRP.MTIP
    msipReg := platformIRP.MSIP
    seipReg := platformIRP.SEIP
    stipReg := sstcIRGen.o.STIP
    vseipReg := platformIRP.VSEIP
    hgeipVgeinReg := hgeip.rdata.asUInt(hstatus.regOut.VGEIN.asUInt)
    vstipReg := sstcIRGen.o.VSTIP
    ...
    diffMhpmeventOverflowEvent.valid  := Cat(mhpmevents.zipWithIndex.map{ case (event, i) =>
      val ofFromPerfCntVecReg = RegInit(false.B)
      ofFromPerfCntVecReg := ofFromPerfCntVec(i)
