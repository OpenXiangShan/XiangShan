...
class SbpctlBundle extends CSRBundle {
  val LOOP_ENABLE = RW(6).withReset(true.B).withDescription("Enable loop prediction.")
  val RAS_ENABLE  = RW(5).withReset(true.B).withDescription("Enable the return-address stack predictor.")
  ...
  val BIM_ENABLE  = RW(2).withReset(true.B).withDescription("Enable the bimodal predictor.")
  val BTB_ENABLE  = RW(1).withReset(true.B).withDescription("Enable the branch target buffer.")
