...
class TrapEntryHSEventModule(implicit val p: Parameters) extends Module with CSREventBase with HasXSParameter {
  ...
  out.scause.bits.ExceptionCode := (if (HasShadowStack) trapExceptionCode else highPrioTrapNO)
