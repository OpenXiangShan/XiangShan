...
class JumpUnit(cfg: FuConfig)(implicit p: Parameters) extends PipedFuncUnit(cfg) {
  ...
  redirectValid := io.in.valid && !jumpDataModule.io.isAuipc && TraceRTLChoose(true.B, !io.in.bits.ctrl.traceInfo.isWrongPath && !io.in.bits.ctrl.traceInfo.hasTriggeredExuRedirect)
  ...
  redirect.specifyField(
    ...
    _.fullTarget := TraceRTLChoose(jumpDataModule.io.target, SignExt(io.in.bits.ctrl.traceInfo.target, XLEN)),
    _.cfiUpdate.predTaken := true.B,
    _.cfiUpdate.taken := true.B,
    _.cfiUpdate.target := TraceRTLChoose(jumpDataModule.io.target, SignExt(io.in.bits.ctrl.traceInfo.target, XLEN)),
    ...
    _.cfiUpdate.backendIAF := io.instrAddrTransType.get.checkAccessFault(jumpDataModule.io.target),
    _.cfiUpdate.backendIPF := io.instrAddrTransType.get.checkPageFault(jumpDataModule.io.target),
    _.cfiUpdate.backendIGPF := io.instrAddrTransType.get.checkGuestPageFault(jumpDataModule.io.target),
