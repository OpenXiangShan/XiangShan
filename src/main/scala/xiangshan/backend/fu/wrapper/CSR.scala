  ...
  with HasCircularQueuePtrHelper with HasCriticalErrors with HasSoCParameter
{
  ...
  val preDecodeReg = RegEnable(io.in.bits.ctrl.isRVC.get, io.in.fire)
  io.out.bits.ctrl.isRVC.foreach(_ := Mux(isXRetReg, preDecodeReg, DelayNWithValid(preDecodeReg, csrModOutValid, 3)._2))
