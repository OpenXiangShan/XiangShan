...
import xiangshan.backend.decode.opcode.Opcode.{VSha256cOpcodes, VSha256msOpcodes}
...
class VSha256cWrapper(cfg: FuConfig)(implicit p: Parameters) extends VecPipedFuncUnit(cfg) {
  ...
  cMod.in match {
    case in =>
      in.pipe0.valid := valid && VSha256cOpcodes.isLegal
      in.pipe0.vs1 := vs1
      in.pipe0.vs2 := vs2
      in.pipe0.vs3 := oldVd
      in.pipe0.uop.vsha512c := false.B
      in.pipe0.uop.vsha256c := true.B // only support vsha256c[lh]
      in.pipe0.uop.vsha2cl := VSha256cOpcodes.isCL
