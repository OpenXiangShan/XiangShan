...
object FuType extends OHEnumeration {
  ...
  val vcrypnp = addType(name = "vcrypnp") // vector crypto non-pipelined
  ...
  val vcrypto = Seq(vcrypp2, vcrypp5, vcrypnp)
  ...
  def isVCryptoNonPiped(fuType: UInt): Bool = FuTypeOrR(fuType, vcrypnp)
