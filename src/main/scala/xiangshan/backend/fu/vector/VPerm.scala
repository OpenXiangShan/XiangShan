...
class VPermWrapper(implicit p: Parameters)  extends VPUDataModule {
  ...
  val vs1Mux = Mux(VpermType.notNeedSew(in.uop.ctrl.fuOpType)&&(!SrcType.isVp(in.uop.ctrl.srcType(0))),
                    Mux(SrcType.isFp(in.uop.ctrl.srcType(0)), in.src(0)(63,0), ctrl.imm(4,0)), 
                    vs1)
