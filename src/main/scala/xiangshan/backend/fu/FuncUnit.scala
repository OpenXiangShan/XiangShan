...
abstract class FuncUnit(val cfg: FuConfig)(implicit p: Parameters) extends XSModule with HasCriticalErrors {
  ...
  PerfCCT.updateInstPos(io.in.bits.seqNum, PerfCCT.InstPos.AtFU.id.U, io.in.valid, clock, reset)
  PerfCCT.updateInstPos(io.out.bits.seqNum, PerfCCT.InstPos.AtBypassVal.id.U, io.out.valid, clock, reset)
  ...
  def connectNonPipedCtrlSingal: Unit = {
    ...
    io.out.bits.seqNum := RegEnable(io.in.bits.seqNum, io.in.fire)
  }
  ...
  def connectNonPipedCtrlSingalForCSR: Unit = {
    ...
    io.out.bits.seqNum := DataHoldBypass(io.in.bits.seqNum, io.in.fire)
  }
...
trait HasPipelineReg { this: FuncUnit =>
  ...
  def pipelineReg(init: FuncUnitInput , valid:Bool, ready: Bool,latency: Int, flush:ValidIO[Redirect]): (Seq[FuncUnitInput],Seq[Bool],Seq[Bool])={
    ...
    val seqNumVec = init.seqNum +: Seq.fill(latency)(Reg(chiselTypeOf(io.in.bits.seqNum)))
  ...
  val seqNumVec = pipeReg.map(_.seqNum)
  val robIdxVec = ctrlVec.map(_.robIdx)
  ...
  fixtiminginit.seqNum := seqNumVec.last
  ...
  val fixSeqNumVec = fixpipeReg.map(_.seqNum)
  ...
  io.out.bits.seqNum := fixSeqNumVec.last
