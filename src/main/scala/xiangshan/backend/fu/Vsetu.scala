...
class VsetModule(cfg: FuConfig)(implicit p: Parameters) extends XSModule {
  ...
  // csrrvl uses the same function bit as the internal keep-vl encoding, but
  // only reads vl and must not participate in vset legality checking.
  private val isCsrrVl = func === VSETOpType.csrrvl
  private val keepVlIllegal: Bool = io.in.oldVt match {
    case Some(_) => isKeepVl && !isCsrrVl && (oldVt.illegal || log2NewRatio =/= log2OldRatio)
    case None => 0.B
  }
