...
class VecNonFixedLatFunc(cfg: VecFuConfig)(implicit p: Parameters) extends Func(cfg)
  with VecFuncAlias {val outFuLat = IO(Output(Valid(UInt(WbFuBusyTable.NonFixedLatencyWidth.W))))
