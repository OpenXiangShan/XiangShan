...
class BufferSelectModule[T1 <: Data, T2 <: Data](
  ...
  val in = IO(Input(new Bundle {
    ...
    val mopAcceptVec = Vec(mopWidth, Bool())
  }))
  ...
  val mopAcceptVec = in.mopAcceptVec
  val mopFromInput = in.mopFromInput
  ...
  for (i <- 0 until uopBufferLength) {
    ...
    // The last accept mop will be buffered in MopBuffer
    out.mopToBuffer := PriorityMux((mopAcceptVec zip mopFromInput).reverse)
  }
}
