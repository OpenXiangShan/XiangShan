...
import xiangshan.backend.vector.Decoder.Uop.UopTrait.UopTrait
...
sealed trait InstPattern extends DecodePattern {
  ...
  private var traits: Set[UopTrait] = Set()

  def getTraits: Set[UopTrait] = traits

  def setTraits(_traits: UopTrait*): this.type = {
    this.traits = _traits.toSet
    this
  }
}
...
case class PseudoInstPattern(rawInst: BitPat) extends InstPattern
...
case class MptFenceInstPattern()(implicit rawInst: BitPat) extends SystemInstPattern
        ...
        func3.rawString match {
          case "000" =>
            imm12.rawString match {
              ...
              case s if rawStringMatches(s, "1011111?????") || rawStringMatches(s, "0111111?????") => MptFenceInstPattern()
...
object VecInstPattern {
  ...
  object Category {
    ...
    val arithSet = Set(OPIVV, OPFVV, OPMVV, OPIVI, OPIVX, OPFVF, OPMVX)
  }
}
