...
class FuTypeField(uopIdx: Int) extends DecodeField[InstSewLmulNfPattern, UInt] {
  ...
  private def decodeFuType(uop: Opcode): UInt = {
    if (uop.getTraits.contains(ForceVfAlu)) {
      ...
      uop.factory match {
        ...
        case _: Opcode.VFMacOpcodes.type => FuType.vfma.U
        case _: Opcode.VFDivOpcodes.type => FuType.vfdiv.U
        case _: Opcode.VFCvtOpcodes.type => FuType.vfcvt.U
        case _: Opcode.VFMiscOpcodes.type => FuType.vfalu.U
