...
class BypassNetwork(implicit val param: ExuParam, val p: Parameters) extends Module with HasXSParameter {
  ...
  for (i <- out.src.indices) {
    val gpSeq: Seq[(Bool, UInt)] = Option.when(param.readPortCfgs(i).exists(_.isInstanceOf[IntRD])){
      ...
      Seq(
        (gpRen(i) && delay(i) === BypassDelay.delay1) -> Mux1H(Seq(
          in.sewOH.is8  -> Fill(VLEN /  8, gpData.take( 8)),
          in.sewOH.is16 -> Fill(VLEN / 16, gpData.take(16)),
          in.sewOH.is32 -> Fill(VLEN / 32, gpData.take(32)),
          in.sewOH.is64 -> Fill(VLEN / 64, gpData.take(64)),
        )),
        (gpRen(i) && delay(i) >= BypassDelay.delay2) -> in.gpRdData(i),
      )
    }.getOrElse(Seq())
    ...
    val fpSeq: Seq[(Bool, UInt)] = Option.when(param.readPortCfgs(i).exists(_.isInstanceOf[FpRD])){
      ...
      Seq(
        (fpRen(i) && delay(i) === BypassDelay.delay1) -> Mux1H(Seq(
          in.sewOH.is8  -> Fill(VLEN /  8, fpData.take( 8)),
          in.sewOH.is16 -> Fill(VLEN / 16, fpData.take(16)),
          in.sewOH.is32 -> Fill(VLEN / 32, fpData.take(32)),
          in.sewOH.is64 -> Fill(VLEN / 64, fpData.take(64)),
        )),
