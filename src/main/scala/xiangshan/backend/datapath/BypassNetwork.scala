    ...
    def connectExuOutput(
      ...
      getSinkVecN(this).zip(sourceVecN).foreach { case (sinkVec, sourcesVec) =>
        sinkVec.zip(sourcesVec).foreach { case (sink, source) =>
          ...
          sink.bits.data := {if (source.bits.params.writeIntRf) { source.bits.toIntRf.map(_.bits).getOrElse(0.U(source.bits.params.destDataBitsMax.W)) }
                        else if (source.bits.params.writeFpRf)  { source.bits.toFpRf.map(_.bits).getOrElse(0.U(source.bits.params.destDataBitsMax.W))  }
                        else if (source.bits.params.writeVecRf) { source.bits.toVecRf.map(_.bits).getOrElse(0.U(source.bits.params.destDataBitsMax.W)) }
                        else if (source.bits.params.writeV0Rf)  { source.bits.toV0Rf.map(_.bits).getOrElse(0.U(source.bits.params.destDataBitsMax.W))  }
                        else if (source.bits.params.writeVlRf)  { source.bits.toVlRf.map(_.bits).getOrElse(0.U(source.bits.params.destDataBitsMax.W))  }
                        else { 0.U(source.bits.params.destDataBitsMax.W) }} 
        }
      }
    }
  }
