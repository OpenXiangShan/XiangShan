    ...
    exuInput.bits.src.zipWithIndex.foreach { case (src, srcIdx) =>
      ...
      if (exuParm.hasAluFu && srcIdx == 0) {
        when(isAlu) {
          ...
          val isAdd = ALUOpType.isAdd(fuOpType)
          val isAdduw = ALUOpType.isAdduw(fuOpType)
          val isSradd = ALUOpType.isSradd(fuOpType)
          val isSr29add = ALUOpType.isSr29add(fuOpType)
          val isSr30add = ALUOpType.isSr30add(fuOpType)
          val isSr31add = ALUOpType.isSr31add(fuOpType)
          val isSr32add = ALUOpType.isSr32add(fuOpType)
          val isShadd = ALUOpType.isShadd(fuOpType)
          val isSh1add = ALUOpType.isSh1add(fuOpType)
          val isSh2add = ALUOpType.isSh2add(fuOpType)
          val isSh3add = ALUOpType.isSh3add(fuOpType)
          val isSh4add = ALUOpType.isSh4add(fuOpType)
          val isOthers = ALUOpType.isOthers(fuOpType)
          ...
          val sh1addSrc = Cat(originSrc(62, 0), 0.U(1.W))
          val sh2addSrc = Cat(originSrc(61, 0), 0.U(2.W))
          val sh3addSrc = Cat(originSrc(60, 0), 0.U(3.W))
          ...
          aluSrc := Mux1H(Seq(
            isAdduw -> adduwSrc,
            isOddadd -> ZeroExt(originSrc(0), XLEN),
            isAdd -> Mux1H(Seq(
              isSradd -> Mux1H(Seq(
