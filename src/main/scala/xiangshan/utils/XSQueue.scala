...
class XSQueueIO[T <: Data](gen: T, enqnum: Int, deqnum: Int) extends XSBundle {
  val enq = Vec(enqnum, Flipped(DecoupledIO(gen)))
  val deq = Vec(deqnum, DecoupledIO(gen))
}

class XSQueue[T <: Data](gen: T, size: Int, enqnum: Int, deqnum: Int) extends XSModule {
  val io = IO(new XSQueueIO(gen, enqnum, deqnum))
...
object XSQueueTop extends App {
  Driver.execute(args, () => new XSQueue(UInt(32.W), 16, 6, 4))
}
