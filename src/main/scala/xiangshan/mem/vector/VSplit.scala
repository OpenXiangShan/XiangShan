...
class VLSplitBufferImp(implicit p: Parameters) extends VSplitBuffer(isVStore = false){
  ...
  switch(stopWaitState) {
    is(StopAndWaitState.idle) {
      when(isOrderIndexed(issueInstType) && io.out.valid) { // have one active element issue, only order index need it
        stopWaitStateNext := StopAndWaitState.active
      }
    }
    is(StopAndWaitState.active) {
      when(splitFinish) {
  ...
  lazy val pipelineWbValid = io.fromPipeline.get.map{case port =>
  ...
  override lazy val splitCanGo = RegNext(pipelineWbValid) || stopWaitState === StopAndWaitState.idle

  XSError(stopWaitState === StopAndWaitState.active && !isOrderIndexed(issueInstType), "normal vector load instruction do not need to wait!\n")
}
