...
class VSplitPipeline(isVStore: Boolean = false)(implicit p: Parameters) extends VLSUModule {
  ...
  val s0_emul = Mux(
    us_whole_reg(s0_fuOpType),
    GenUSWholeEmul(s0_uop.vpu.nf),
  ...
  val evl =
    Mux(isUsWholeReg, GenUSWholeRegVL(io.in.bits.uop.vpu.nf +& 1.U, s0_eew), Mux(isMaskReg, GenUSMaskRegVL(vvl), vvl))
  val vvstart     = io.in.bits.uop.vpu.vstart
  val alignedType = Mux(isIndexed(instType), s0_sew(1, 0), s0_eew)
  val broadenAligendType =
    Mux(s0_preIsSplit, Cat("b0".U, alignedType), "b100".U) // if is unit-stride, use 128-bits memory access
  ...
  val indexedSplitOffset =
    Mux(isSpecialIndexed, flowsPrevThisUop - flowsPrevThisVd, 0.U) // only for index instructions of emul > lmul
  ...
  val stride =
    Mux(isIndexed(s1_instType), s1_stride, s1_notIndexedStride).asUInt // if is index instructions, get index when split
...
abstract class VSplitBuffer(isVStore: Boolean = false)(implicit p: Parameters) extends VLSUModule {
  ...
  val canAccept =
    !allocated || allocated && splitFinish && (activeIssue || inActiveIssue) // if is valid entry, need split finish and send last uop
  ...
  val usMaskInSingleUop =
    genUSSplitMask(issueUsMask, 1.U) === 0.U // if second splited Mask is zero, means this uop is unnecessary to split
  ...
  val issueCount =
    Mux(
      usNoSplit,
      2.U,
      PopCount(inActiveIssue) + PopCount(activeIssue)
    ) // for dont need split unit-stride, issue two flow
    ...
    when(!splitFinish) {
      when(activeIssue || inActiveIssue) {
        ...
        strideOffsetReg := Mux(
          !issuePreIsSplit,
          strideOffsetReg,
          strideOffsetReg + issueEntry.stride
        ) // when normal unit-stride, don't use strideOffsetReg
      }
    }.otherwise {
      when(activeIssue || inActiveIssue) {
  ...
  XSPerfAccumulate(
    "unitstride_vlenAlign",
    io.out.fire && !issuePreIsSplit && getCheckAddrLowBits(io.out.bits.vaddr, maxMemByteNum) === 0.U
  )
  XSPerfAccumulate(
    "unitstride_invalid",
    io.out.ready && issueValid && !issuePreIsSplit && PopCount(io.out.bits.mask).orR
  )
}
...
class VLSplitImp(implicit p: Parameters) extends VLSUModule {
  ...
  // skid buffer
  skidBuffer(
    splitPipeline.io.out,
    splitBuffer.io.in,
    Mux(
      splitPipeline.io.out.fire,
