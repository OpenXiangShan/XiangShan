  ...
  with HasLoadHelper
{
  ...
  * */
  ...
  when(state === s_idle){
    ...
    when(io.rdcache.resp.fire) {
      when(io.rdcache.resp.bits.miss || io.rdcache.resp.bits.cacheRetry || io.rdcache.s2_bank_conflict) {
  ...
  * */
  ...
  io.rdcache.dcacheUse := state === s_cache_resp && io.rdcache.resp.fire && isVSegLoad &&
    io.rdcache.resp.bits.baseValid && !io.rdcache.resp.bits.replay && !exceptionVec.orR
  if (env.FPGAPlatform){
