...
class VSegmentUnit(implicit p: Parameters) extends VLSUModule
  ...
  **/
  ...
  val issueUopFlowNumLog2 =
    GenRealFlowLog2(issueInstType, issueEmul, issueLmul, issueEew, issueSew, true) // max element number log2 in vd
  val issueVlMax = instMicroOp.uopFlowNum // max elementIdx in vd
  val issueMaxIdxInIndex =
    GenVLMAX(Mux(issueEmul.asSInt > 0.S, 0.U, issueEmul), issueEew(1, 0)) // index element index in index register
   ...
   * */
  val s_idle :: s_flush_sbuffer_req :: s_wait_flush_sbuffer_resp :: s_tlb_req :: s_wait_tlb_resp :: s_pm :: s_cache_req :: s_cache_resp :: s_misalign_merge_data :: s_latch_and_merge_data :: s_send_data :: s_wait_to_sbuffer :: s_finish :: s_fof_fix_vl :: Nil =
    Enum(14)
  ...
  when(state === s_idle) {
    ...
    stateNext := Mux(io.dtlb.resp.fire, Mux(!io.dtlb.resp.bits.miss, s_pm, s_tlb_req), s_wait_tlb_resp)
    ...
    when(exception_pa || exception_va || exception_gpa) {
      ...
      when(
        canHandleMisalign && isMisalignWire && !notCross16ByteWire || (isMisalignReg && !notCross16ByteReg && isFirstSplit && isVSegStore)
      ) {
        stateNext := s_tlb_req
      }.otherwise {
    ...
    when(io.rdcache.resp.fire) {
      when(io.rdcache.resp.bits.miss || io.rdcache.s2_bank_conflict) {
        ...
        stateNext := Mux(
          isVSegLoad,
          Mux(isMisalignReg && !notCross16ByteReg, s_misalign_merge_data, s_latch_and_merge_data),
          s_send_data
        )
      }
    }.otherwise {
    ...
    when(!sbufferOut.fire && segmentActive || (isMisalignReg && !notCross16ByteReg && isFirstSplit)) {
      ...
      segmentIdx === maxSegIdx && (fieldIdx === maxNfields && sbufferOut.fire || !segmentActive && io.sbuffer.valid && !io.sbuffer.ready)
    ) {
  ...
  val realSegmentOffset = Mux(isIndexed(issueInstType), indexStride, segmentOffset)
...
//  val misalignVaddr                   = Mux(notCross16ByteReg, notCross16ByteVaddr, Mux(isFirstSplit, misalignLowVaddr, misalignHighVaddr))
  ...
  val exceptionWithPf = exceptionVec(storePageFault) || exceptionVec(loadPageFault) || exceptionVec(
    storeGuestPageFault
  ...
  when(state === s_pm) {
    ...
    exceptionVec(loadAccessFault) := (exceptionVec(
      loadAccessFault
    ) || pmp.ld || pmp.mmio) && isVSegLoad && canTriggerException
    exceptionVec(storeAccessFault) := (exceptionVec(
      storeAccessFault
    ) || pmp.st || pmp.mmio) && isVSegStore && canTriggerException
  ...
  * */
  when(state === s_finish || state === s_latch_and_merge_data || state === s_send_data && stateNext =/= s_send_data) {
    ...
    when(isVSegLoad) {
      ...
      when(isMisalignReg && !notCross16ByteReg && state === s_pm) {
        ...
        isMisalignReg && !notCross16ByteReg && state === s_send_data && stateNext === s_send_data && sbufferOut.fire
      ) {
        curPtr := !curPtr
      }
    }
  ...
  val misalignLowData = LookupTree(
    latchVaddr(3, 0),
    List(
      "b1001".U -> io.rdcache.resp.bits.data_delayed(127, 72),
      "b1010".U -> io.rdcache.resp.bits.data_delayed(127, 80),
  ...
  val misalignHighData = LookupTree(
    latchVaddr(3, 0),
    List(
      "b1001".U -> io.rdcache.resp.bits.data_delayed(63, 8),
      "b1010".U -> io.rdcache.resp.bits.data_delayed(63, 16),
      "b1011".U -> io.rdcache.resp.bits.data_delayed(63, 24),
      "b1100".U -> io.rdcache.resp.bits.data_delayed(63, 32),
      "b1101".U -> io.rdcache.resp.bits.data_delayed(63, 40),
      "b1110".U -> io.rdcache.resp.bits.data_delayed(63, 48),
      "b1111".U -> io.rdcache.resp.bits.data_delayed(63, 56)
    )
  )

  val misalignCombinedData = LookupTree(
    latchVaddr(3, 0),
    List(
  ...
  val dcacheReqPaddr = Mux(
    isMisalignReg,
    Cat(instMicroOp.paddr(instMicroOp.paddr.getWidth - 1, PageOffsetWidth), misalignVaddr(PageOffsetWidth - 1, 0)),
    instMicroOp.paddr
  )
  ...
  private val segmentInactiveFinish =
    ((state === s_latch_and_merge_data) || (state === s_send_data && stateNext =/= s_send_data)) && !segmentActive
  ...
  splitPtrNext :=
    Mux(
      fieldIdx === maxNfields || !segmentActive, // if segment is active, need to complete this segment, otherwise jump to next segment
  ...
  // update splitPtr
  when(
    state === s_latch_and_merge_data || (state === s_send_data && stateNext =/= s_send_data && (fieldActiveWirteFinish || !segmentActive))
  ) {
  ...
  when(io.in.fire && !instMicroOpValid) {
    ...
    fieldIdx === maxNfields && (state === s_latch_and_merge_data || (state === s_send_data && stateNext =/= s_send_data && fieldActiveWirteFinish)) &&
      segmentIdx =/= maxSegIdx
  ) { // next segment, only if segment is active
  ...
  when(
    ...
    segmentOffset := segmentOffset + Mux(
      isUnitStride(issueInstType),
      (maxNfields +& 1.U) << issueEew(1, 0),
      stride(stridePtr.value)
    )
  }
   ...
   *************************************************************************/
  val vdIdxInField =
    GenUopIdxInField(Mux(isIndexed(instType), issueLmul, issueEmul), uopq(deqPtr.value).uop.vpu.vuopIdx)
  ...
  when(fofFixVlValid) {
    ...
    writebackOut.uop.vpu.vstart := Mux(
      instMicroOp.uop.exceptionVec.asUInt.orR || TriggerAction.isDmode(instMicroOp.uop.trigger),
      instMicroOp.exceptionVstart,
      instMicroOp.vstart
    )
