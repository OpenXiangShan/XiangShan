...
class VfofBuffer(implicit p: Parameters) extends VLSUModule{
  ...
  val enqIsFixVl = enqBits.uop.vpu.isVleff && enqBits.uop.vpu.lastUop && enqValid
  ...
  when(enqValid && !enqNeedCancel && !(entriesIsFixVl && valid)) { entries.uop := enqBits.uop }
  ...
  when(needRedirect) { entries := 0.U.asTypeOf(new VfofDataBundle) }
  ...
  val wbUpdateValid = wbIsfof.reduce(_ || _) && wbUpdateBits.vpu.vl > entries.vl && valid && !needRedirect
  ...
  when(io.uopWriteback.fire) { entries := 0.U.asTypeOf(new VfofDataBundle) }
  when(io.uopWriteback.fire) { valid   := false.B }


//  dontTouch(io.uopWriteback.valid)
