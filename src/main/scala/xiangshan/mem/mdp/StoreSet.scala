...
class SSIT(implicit p: Parameters) extends XSModule {
  ...
  XSPerfAccumulate(
    "ssit_update_should_strict",
    s2_mempred_update_req_valid && s2_ssidIsSame && s2_loadAssigned && s2_storeAssigned
  )
  XSPerfAccumulate(
    "ssit_update_strict_failed",
  ...
  when(s2_mempred_update_req.valid) {
    XSDebug(
      ...
      GTimer(),
      s2_mempred_update_req.ldpc,
      s2_mempred_update_req.stpc
    )
    XSDebug(
...
class LFST(implicit p: Parameters) extends XSModule {
  ...
  (0 until LFSTSize).map(i => valid(i) := validVec(i).asUInt.orR)
  ...
  for (i <- 0 until RenameWidth) {
    ...
    io.dispatch.resp(i).bits.robIdx := robIdxVec(io.dispatch.req(i).bits.ssid)(allocPtr(
      io.dispatch.req(i).bits.ssid
    ) - 1.U)
    if (i > 0) {
      (0 until i).map(j =>
  ...
  (0 until backendParams.StaExuCnt).map { i =>
    ...
    (0 until LFSTWidth).map { j =>
      when(io.storeIssue(i).valid && io.storeIssue(i).bits.storeSetHit && io.storeIssue(
        i
      ).bits.robIdx.value === robIdxVec(io.storeIssue(i).bits.ssid)(j).value) {
