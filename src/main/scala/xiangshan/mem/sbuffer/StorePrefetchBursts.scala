...
trait HasStorePrefetchHelper extends HasCircularQueuePtrHelper with HasDCacheParameters {
  ...
  def can_burst(st_count: UInt, N: UInt, sa_count: SInt): Bool =
    ...
    trigger_check(st_count, N) && get_store_count_divided_by_8(st_count) === sa_count.asUInt && sa_count(
      sa_count.getWidth - 1
    ) === false.B
}
...
class PrefetchBurstGenerator(is_store: Boolean)(implicit p: Parameters) extends DCacheModule
  ...
  (deq_valids zip deq_decoupled zip datas zip datas_next zip datas_next_next zip pagebits zip valids).foreach {
    case ((((((deq_valid, out_decouple), data), data_next), data_next_next), pg_bit), v) => {
...
class StorePrefetchBursts(implicit p: Parameters) extends DCacheModule with HasStorePrefetchHelper {
  ...
  val next_saturate_count = (saturate_counter + Mux(
    sbuffer_fire,
    cache_block_addr_difference(sbuffer_vaddr, last_st_block_addr).asSInt,
    0.S
  )).asSInt
