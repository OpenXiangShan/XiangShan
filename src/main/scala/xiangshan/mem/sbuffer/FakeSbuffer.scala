...
import utility.XSInfo
import xiangshan._
import xiangshan.cache.DCacheLineIO
import xiangshan.cache.DCacheWordReq
...
class FakeSbuffer(implicit p: Parameters) extends XSModule {
  ...
  for (i <- 0 until LoadPipelineWidth) {
    ...
    io.forward(i).forwardMask := Mux(need_forward, VecInit(mask.asBools), VecInit(0.U(8.W).asBools))
    io.forward(i).forwardData := VecInit((0 until 8) map { i => req.data((i + 1) * 8 - 1, i * 8) })
  }

  XSInfo(
    io.in(0).fire,
    "ensbuffer addr 0x%x wdata 0x%x mask %b\n",
    io.in(0).bits.addr,
    io.in(0).bits.data,
    io.in(0).bits.mask
  )
