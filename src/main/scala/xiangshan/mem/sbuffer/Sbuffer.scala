...
class Sbuffer(implicit p: Parameters)
  ...
  val io = IO(new Bundle() {
    ...
    val in =
      Vec(
        EnsbufferWidth,
        Flipped(Decoupled(new DCacheWordReqWithVaddrAndPfFlag))
  ...
  assert(
    !(candidateVec.asUInt.orR && replaceAlgoNotDcacheCandidate),
    "we have way to select, but replace algo selects invalid way"
  )
  ...
  val missqReplayTimeOutMask =
    VecInit(widthMap(i => missqReplayCount(i)(MissqReplayCountBits - 1) && stateVec(i).w_timeout))
  ...
  val sameTag =
    inptags(0) === inptags(1) && io.in(0).valid && io.in(1).valid && io.in(0).bits.vecValid && io.in(1).bits.vecValid
  ...
  def getFirstOneOH(input: UInt): UInt = {
    ...
    (1 until input.getWidth).map(i => output(i) := !input(i - 1, 0).orR && input(i))
    output.asUInt
  }
  ...
  val secondInsertIdx =
    Mux(sameTag, firstInsertIdx, Mux(~enbufferSelReg, evenInsertIdx, oddInsertIdx)) // slow to generate, for debug only
  val firstInsertVec = Mux(enbufferSelReg, evenInsertVec, oddInsertVec)
  val secondInsertVec =
    Mux(sameTag, firstInsertVec, Mux(~enbufferSelReg, evenInsertVec, oddInsertVec)) // slow to generate, for debug only
  val firstCanInsert = sbuffer_state =/= x_drain_sbuffer && Mux(enbufferSelReg, evenCanInsert, oddCanInsert)
  val secondCanInsert = sbuffer_state =/= x_drain_sbuffer && Mux(
    sameTag,
    firstCanInsert,
  ...
  val do_uarch_drain =
    GatedValidRegNext(forward_need_uarch_drain) || GatedValidRegNext(GatedValidRegNext(merge_need_uarch_drain))
  ...
  for (i <- 0 until EnsbufferWidth) {
    ...
    if (EnableStorePrefetchAtCommit) {
      if (EnableAtCommitMissTrigger) {
        io.store_prefetch(i).valid := prefetcher.io.prefetch_req(i).valid || (io.in(i).fire && io.in(
          i
        ).bits.vecValid && io.in(i).bits.prefetch)
      } else {
      ...
      io.store_prefetch(i).bits.vaddr := Mux(
        prefetcher.io.prefetch_req(i).valid,
        prefetcher.io.prefetch_req(i).bits.vaddr,
        io.in(i).bits.vaddr
      )
  ...
  def wordReqToBufLine( // allocate a new line in sbuffer
    ...
    (0 until StoreBufferSize).map { entryIdx =>
      when(insertVec(entryIdx)) {
        ...
        stateVec(
          entryIdx
        ).w_sameblock_inflight := sameBlockInflightMask.orR // set w_sameblock_inflight when a line is first allocated
  ...
  for (i <- 0 until StoreBufferSize) {
    XSDebug(stateVec(i).isValid(), p"[$i] timeout:${cohCount(i)(EvictCountBits - 1)} state:${stateVec(i)}\n")
  }

  for ((req, i) <- io.in.zipWithIndex) {
    XSDebug(
      req.fire && req.bits.vecValid,
      p"accept req [$i]: " +
  ...
  val do_eviction = GatedValidRegNext(
    ActiveCount >= forceThreshold || ActiveCount === (StoreBufferSize - 1).U || ValidCount === StoreBufferSize.U,
    init = false.B
  )
  ...
  val sbuffer_out_s0_evictionIdx = Mux(
    missqReplayHasTimeOut,
    missqReplayTimeOutIdx,
    Mux(need_drain, drainIdx, Mux(cohHasTimeOut, cohTimeOutIdx, replaceIdx))
  )
  ...
  XSDebug(
    p"sbuffer_out_s0_valid:$sbuffer_out_s0_valid evictIdx:$sbuffer_out_s0_evictionIdx dcache ready:${io.dcache.req.ready}\n"
  )
  ...
  accessIdx(EnsbufferWidth).valid := invalidMask(replaceIdx) || (
    need_replace && !need_drain && !cohHasTimeOut && !missqReplayHasTimeOut && sbuffer_out_s0_cango && activeMask(
      replaceIdx
    )
  )
  ...
  XSDebug(sbuffer_out_s1_fire, p"send buf [$sbuffer_out_s1_evictionIdx] to Dcache, req fire\n")
  ...
  io.dcache.hit_resps.zip(dataModule.io.maskFlushReq).map {
    case (resp, maskFlush) => {
  ...
  if (env.EnableDifftest) {
    // hit resp
    io.dcache.hit_resps.zipWithIndex.map {
      case (resp, index) => {
  ...
  for ((forward, i) <- io.forward.zipWithIndex) {
    ...
    val ptag_matches =
      VecInit(widthMap(w => RegEnable(ptag(w), forward.valid) === RegEnable(getPTag(forward.paddr), forward.valid)))
    ...
    val selectedInflightMaskFast =
      Mux1H(line_offset_mask, Mux1H(inflight_tag_matches, mask).asTypeOf(Vec(CacheLineVWords, Vec(VDataBytes, Bool()))))
    val selectedValidMaskFast =
      Mux1H(line_offset_mask, Mux1H(valid_tag_matches, mask).asTypeOf(Vec(CacheLineVWords, Vec(VDataBytes, Bool()))))
  ...
  for (i <- 0 until StoreBufferSize) {
    XSDebug(
      "sbf entry " + i + " : ptag %x vtag %x valid %x active %x inflight %x w_timeout %x\n",
      ptag(i) << OffsetWidth,
      vtag(i) << OffsetWidth,
      stateVec(i).isValid(),
      activeMask(i),
  ...
  **********************************************************
  ...
  XSPerfAccumulate(
    "sbuffer_req_fire_vecinvalid",
    PopCount(VecInit(io.in.map(data => data.fire && !data.bits.vecValid)).asUInt)
  )
  XSPerfAccumulate(
    "sbuffer_merge",
  ...
  val perfEvents = Seq(
    ...
    (
      "sbuffer_2_4_valid ",
      (perf_valid_entry_count > (StoreBufferSize.U / 4.U)) & (perf_valid_entry_count <= (StoreBufferSize.U / 2.U))
    ),
    (
      "sbuffer_3_4_valid ",
      (perf_valid_entry_count > (StoreBufferSize.U / 2.U)) & (perf_valid_entry_count <= (StoreBufferSize.U * 3.U / 4.U))
    ),
