...
class Sbuffer(implicit p: Parameters)
  ...
  */
  ...
  for ((forward, i) <- io.forward.zipWithIndex) {
    ...
    val valid_tag_match_reg = valid_tag_matches.map(RegEnable(_, false.B, forward.valid))
    val inflight_tag_match_reg = inflight_tag_matches.map(RegEnable(_, false.B, forward.valid))
