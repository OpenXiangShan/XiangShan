  ...
  */
  ...
  if (env.EnableDifftest) {
    ...
    for (i <- 0 until EnsbufferWidth) {
      ...
      val pma             = io.diffStore.pmaStore(i)
      ...
      difftestCommon.valid        := pma.fire && pma.bits.mask.orR && pma.bits.vecValid
      difftestCommon.wLine        := pma.bits.wline
      difftestCommon.offset       := io.diffStore.diffInfo(i).offset
      difftestCommon.pc           := uop.pc
      difftestCommon.robidx       := uop.robIdx.value

      difftestCommon.isNonCacheable := false.B
      difftestCommon.isVStore := isVStore(uop.fuType)
      difftestCommon.isUStride := LSUOpType.isUStride(uop.fuOpType)
      difftestCommon.isMasked := VstuType.isMasked(uop.fuOpType)
      difftestCommon.isWhole := VstuType.isWhole(uop.fuOpType)
      difftestCommon.veew := uop.vpu.veew
      difftestCommon.nf := uop.vpu.nf

      difftestCommon.rawDataLow := pma.bits.data(63,0)
      difftestCommon.rawDataHigh := pma.bits.data(127,64)
      difftestCommon.rawMask := pma.bits.mask
      difftestCommon.rawAddr := pma.bits.addr
      difftestCommon.isHighPart := pma.bits.diffIsHighPart

      // All address and data operations are handled within difftest.
    }
    ...
    val diffInfo0 = io.diffStore.diffInfo(0)
    ...
    ncmmStoreEvent.pc := diffInfo0.uop.pc
    ncmmStoreEvent.robidx := diffInfo0.uop.robIdx.value

    ncmmStoreEvent.isNonCacheable := true.B
    ncmmStoreEvent.isVStore := false.B
    ncmmStoreEvent.isUStride := false.B
    ncmmStoreEvent.isMasked := false.B
    ncmmStoreEvent.isWhole := false.B
    ncmmStoreEvent.veew := 0.U
    ncmmStoreEvent.nf := 0.U

    ncmmStoreEvent.rawDataLow := ncStore.bits.data(63,0)
    ncmmStoreEvent.rawDataHigh := 0.U
    ncmmStoreEvent.rawMask := ncStore.bits.mask
    ncmmStoreEvent.rawAddr := ncStore.bits.addr
    ncmmStoreEvent.isHighPart := false.B
  }
