...
class Sbuffer(implicit p: Parameters)
  ...
  */
  ...
  if (env.EnableDifftest) {
    ...
    // resplit handling is in difftest submodule
    for (i <- 0 until EnsbufferWidth) {
      ...
      val storeCommitValid = cacStore.fire && rawMask.orR
      ...
      when (isVSLine || isWLine) {
        ...
        assert(!storeCommitValid || rawData === 0.U || isVSLine, "wline only supports whole zero write now")
      }.otherwise { // Normal scalar store
        val isHighPart        = cacStore.bits.diffIsHighPart
        val waddr             = ZeroExt(Cat(rawAddr(PAddrBits - 1, 4), isHighPart, 0.U(3.W)), 64)
    ...
    val noncacStore    = io.diffStore.ncStore
    val dataMask = MaskExpand(noncacStore.bits.mask)
    ncmmStoreEvent.coreid := io.hartId
    ncmmStoreEvent.index := diffStoreEventCount.U
    ncmmStoreEvent.valid := noncacStore.valid && noncacStore.bits.memBackTypeMM
    ncmmStoreEvent.addr := Cat(noncacStore.bits.addr(PAddrBits - 1, DCacheWordOffset), 0.U(DCacheWordOffset.W)) // aligned to 8 bytes
    ncmmStoreEvent.data := noncacStore.bits.data & dataMask // data align
    ncmmStoreEvent.highData := 0.U
    ncmmStoreEvent.mask := noncacStore.bits.mask
