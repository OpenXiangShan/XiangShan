...
object SelectOldest {
  ...
  class SelectOldest[T <: Data, SelectT <: Data](gen: T, selectGen: SelectT, numIn: Int, fn: (SelectT, SelectT) => Bool, groupSize: Int, latch: Boolean) extends Module {
  ...
  def apply[T <: Data, SelectT <: Data](
    ...
    require(ins.length == sels.length)
    val selectOldest = Module(new SelectOldest(ins.head.bits, sels.head, ins.length, fn, groupSize, latch)).suggestName(moduleName.getOrElse("SelectOldest"))
    selectOldest.io.in <> ins
    selectOldest.io.sel <> sels
