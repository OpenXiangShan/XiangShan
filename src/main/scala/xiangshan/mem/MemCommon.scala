...
class LdPrefetchTrainBundle(implicit p: Parameters) extends LsPipelineBundle {
  ...
  def fromLsPipelineBundle(input: LsPipelineBundle, latch: Boolean = false, enable: Bool = true.B) = {
    ...
    if (latch) isForVSnonLeafPTE   := RegEnable(input.isForVSnonLeafPTE, enable)
    else isForVSnonLeafPTE         := input.isForVSnonLeafPTE
    ...
    if (latch) isFrmMisAlignBuf    := RegEnable(input.isFrmMisAlignBuf, enable)
    else isFrmMisAlignBuf          := input.isFrmMisAlignBuf
    ...
    if (latch) dcacheRequireReplay := RegEnable(input.dcacheRequireReplay, enable)
    else dcacheRequireReplay       := input.dcacheRequireReplay
    ...
    if (latch) uop_unit_stride_fof := RegEnable(input.uop_unit_stride_fof, enable)
    else uop_unit_stride_fof       := input.uop_unit_stride_fof
...
class LqWriteBundle(implicit p: Parameters) extends LsPipelineBundle {
  ...
  def fromLsPipelineBundle(input: LsPipelineBundle, latch: Boolean = false, enable: Bool = true.B) = {
    ...
    if (latch) forward_tlDchannel  := RegEnable(input.forward_tlDchannel, enable)
    else forward_tlDchannel        := input.forward_tlDchannel
    ...
    if (latch) replacementUpdated  := RegEnable(input.replacementUpdated, enable)
    ...
    if (latch) delayedLoadError    := RegEnable(input.delayedLoadError, enable)
    else delayedLoadError          := input.delayedLoadError
