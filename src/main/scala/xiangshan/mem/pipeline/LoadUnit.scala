...
class LoadUnit(implicit p: Parameters) extends XSModule
  ...
  io.tlb.req.bits.cmd         := Mux(s0_sel_src.prf, Mux(s0_sel_src.prf_wr, TlbCmd.write, TlbCmd.read), TlbCmd.read)
...
//    out.isFirstIssue  := src.isFirstIssue
  ...
  s0_dcache_vaddr := Mux(
    ...
    Mux(
      ...
      Mux(
        s0_src_select_vec(nc_idx),
        io.lsq.nc_ldin.bits.vaddr, // not for dcache access, but for address alignment check
        s0_tlb_vaddr
      )
    )
  )
  ...
  // address align check
  XSError(
    s0_sel_src.isvec && s0_dcache_vaddr(3, 0) =/= 0.U && s0_sel_src.alignedType(2),
    "unit-stride 128 bit element is not aligned!"
  )

  val s0_check_vaddr_low = s0_dcache_vaddr(4, 0)
  val s0_check_vaddr_Up_low = LookupTree(
    s0_alignType,
    List(
      "b00".U -> 0.U,
      "b01".U -> 1.U,
  ...
  s0_out.paddr :=
    Mux(
      s0_src_valid_vec(nc_idx),
      io.lsq.nc_ldin.bits.paddr,
      Mux(
        s0_src_valid_vec(fast_rep_idx),
        io.fast_rep_in.bits.paddr,
        Mux(s0_src_select_vec(int_iss_idx) && s0_sel_src.prf_i, 0.U, io.prefetch_req.bits.paddr)
      )
  ...
  s0_out.uop.exceptionVec(loadAddrMisaligned) := (!s0_addr_aligned || s0_sel_src.uop.exceptionVec(
    loadAddrMisaligned
  ...
  io.replay.ready := (s0_can_go && io.dcache.req.ready && (s0_src_ready_vec(
    lsq_rep_idx
  ) && !s0_rep_stall || s0_src_select_vec(super_rep_idx)))
  ...
  io.dcache.replacementUpdated := Mux(
    s0_src_select_vec(lsq_rep_idx) || s0_src_select_vec(super_rep_idx),
    io.replay.bits.replacementUpdated,
    false.B
  )
  ...
  val s1_exception =
    ExceptionNO.selectByFu(s1_out.uop.exceptionVec, LduCfg).asUInt.orR // af & pf exception were modified below.
  ...
  private val s1_isMatch128 =
    io.stld_nuke_query.map(x => x.bits.matchLine || ((s1_in.isvec || s1_in.misalignWith16Byte) && s1_in.is128bit))
  val s1_nuke_paddr_match = VecInit((0 until StorePipelineWidth).zip(s1_isMatch128).map {
    case (w, s) => {
      Mux(
        s,
  ...
  when(!s1_dly_err) {
    ...
    s1_out.uop.exceptionVec(loadPageFault) := io.tlb.resp.bits.excp(
      0
    ).pf.ld && s1_vecActive && !s1_tlb_miss && !s1_in.tlbNoQuery
    s1_out.uop.exceptionVec(loadGuestPageFault) := io.tlb.resp.bits.excp(0).gpf.ld && !s1_tlb_miss && !s1_in.tlbNoQuery
    s1_out.uop.exceptionVec(loadAccessFault) := io.tlb.resp.bits.excp(
      0
    ).af.ld && s1_vecActive && !s1_tlb_miss && !s1_in.tlbNoQuery
  ...
  val s2_only_misalign_exception =
    !ExceptionNO.selectByFuAndUnSelect(
      s2_exception_vec,
      LduCfg,
      Seq(loadAddrMisaligned)
    ).asUInt.orR && !s2_trigger_debug_mode &&
  ...
  //  4. Data contains.
  private val s2_isMatch128 =
    io.stld_nuke_query.map(x => x.bits.matchLine || ((s2_in.isvec || s2_in.misalignWith16Byte) && s2_in.is128bit))
  val s2_nuke_paddr_match = VecInit((0 until StorePipelineWidth).zip(s2_isMatch128).map {
    case (w, s) => {
      Mux(
        s,
  ...
  val s2_safe_wakeup =
    !s2_out.rep_info.need_rep && !s2_mmio && (!s2_in.nc || s2_nc_with_data) && !s2_mis_align && !s2_exception || s2_in.misalignNeedWakeUp // don't need to replay and is not a mmio\misalign no data
  ...
  io.lsq.ldld_nuke_query.req.bits.data_valid := Mux(
    s2_full_fwd || s2_fwd_data_valid || s2_nc_with_data,
    true.B,
    !s2_dcache_miss
  )
  ...
  io.lsq.stld_nuke_query.req.bits.data_valid := Mux(
    s2_full_fwd || s2_fwd_data_valid || s2_nc_with_data,
    true.B,
    !s2_dcache_miss
  )
  ...
  for (i <- 0 until VLEN / 8) {
    ...
    s2_fwd_data(i) :=
      Mux(
        ...
        Mux(s2_nc_with_data, io.ubuffer.forwardData(i), io.sbuffer.forwardData(i))
      )
  }
  ...
  XSDebug(
    ...
    s2_in.uop.pc,
    io.lsq.forward.forwardData.asUInt,
    io.lsq.forward.forwardMask.asUInt,
    s2_in.forwardData.asUInt,
  ...
  s2_out.uop.vpu.vstart := Mux(
    s2_in.isLoadReplay || s2_in.isFastReplay,
    s2_in.uop.vpu.vstart,
    s2_in.vecVaddrOffset >> s2_in.uop.vpu.veew
  )
  ...
  io.fast_uop.valid := GatedValidRegNext(
    s1_fast_uop_valid
  ) && (s2_valid && !s2_out.rep_info.need_rep && !s2_uncache && !(s2_prf && !s2_hw_prf)) && !s2_isvec && !s2_frm_mabuf
  ...
  io.prefetch_train.bits.miss := RegEnable(
  ...
  val s3_fast_rep_canceled =
    io.replay.valid && io.replay.bits.forward_tlDchannel || io.misalign_ldin.valid || !io.dcache.req.ready
  ...
  io.lsq.ldin.bits.missDbUpdated := GatedValidRegNext(
    s2_fire && s2_in.hasROBEntry && !s2_in.tlbMiss && !s2_in.missDbUpdated
  )
  ...
  when(s3_exception || s3_dly_ld_err || s3_rep_frm_fetch || s3_frm_mabuf) {
    s3_replayqueue_rep_cause := 0.U.asTypeOf(s3_lrq_rep_info.cause.cloneType)
    s3_replayqueue_rep_cause(
      LoadReplayCauses.C_MF
    ) := s3_onlyMisalignException && !s3_frm_mabuf && s3_lrq_rep_info.misalign_nack
  }.otherwise {
  ...
  s3_out.bits.uop.exceptionVec(loadAccessFault) := (s3_dly_ld_err || s3_in.uop.exceptionVec(
    loadAccessFault
  )) && s3_vecActive
...
//  io.lsq.ldin.bits.uop.exceptionVec(loadAddrMisaligned) := Mux(s3_in.onlyMisalignException, false.B, s3_in.uop.exceptionVec(loadAddrMisaligned))
  ...
  val s3_picked_data_frm_mmio = LookupTree(
    s3_ld_raw_data_frm_mmio.addrOffset,
    List(
  ...
  XSError(
    s3_valid && s3_in.misalignNeedWakeUp && !s3_frm_mabuf,
    "Only the needwakeup from the misalignbuffer may be high"
  )
  // TODO: check this --hx
  ...
  io.vecldout.bits.vecdata.get := Mux(
    s3_in.misalignWith16Byte,
    vecShiftData,
    Mux(s3_in.is128bit, s3_merged_data_frm_pipe, vecdata)
  )
  ...
  io.vecldout.valid := s3_out.valid && !s3_out.bits.uop.robIdx.needFlush(
    io.redirect
  ...
  io.debug_ls.s3_isReplayRS := RegNext(
    io.feedback_fast.valid && !io.feedback_fast.bits.hit
  ...
  XSPerfAccumulate(
    "s0_addr_spec_success",
    s0_fire && s0_dcache_vaddr(VAddrBits - 1, 12) === io.ldin.bits.src(0)(VAddrBits - 1, 12)
  )
  XSPerfAccumulate(
    "s0_addr_spec_failed",
    s0_fire && s0_dcache_vaddr(VAddrBits - 1, 12) =/= io.ldin.bits.src(0)(VAddrBits - 1, 12)
  ...
  XSPerfAccumulate(
    ...
    s0_fire && s0_dcache_vaddr(VAddrBits - 1, 12) === io.ldin.bits.src(0)(VAddrBits - 1, 12) && s0_sel_src.isFirstIssue
  )
  XSPerfAccumulate(
    "s0_addr_spec_failed_once",
    s0_fire && s0_dcache_vaddr(VAddrBits - 1, 12) =/= io.ldin.bits.src(0)(VAddrBits - 1, 12) && s0_sel_src.isFirstIssue
  )
  ...
  XSPerfAccumulate(
    "s2_prefetch_ignored",
    s2_fire && s2_prf && io.dcache.s2_mq_nack
  ) // ignore prefetch for mshr full / miss req port conflict
  ...
  XSPerfAccumulate(
    "load_to_load_forward_fail_wakeup_mismatch",
    s1_cancel_ptr_chasing && !s1_ptr_chasing_canceled && s1_not_fast_match
  )
  XSPerfAccumulate(
    "load_to_load_forward_fail_op_not_ld",
    s1_cancel_ptr_chasing && !s1_ptr_chasing_canceled && !s1_not_fast_match && s1_fu_op_type_not_ld
  )
  XSPerfAccumulate(
    "load_to_load_forward_fail_addr_align",
    s1_cancel_ptr_chasing && !s1_ptr_chasing_canceled && !s1_not_fast_match && !s1_fu_op_type_not_ld && s1_addr_misaligned
  )
  XSPerfAccumulate(
    "load_to_load_forward_fail_set_mismatch",
    s1_cancel_ptr_chasing && !s1_ptr_chasing_canceled && !s1_not_fast_match && !s1_fu_op_type_not_ld && !s1_addr_misaligned && s1_addr_mismatch
  )
