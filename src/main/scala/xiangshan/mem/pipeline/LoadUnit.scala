  ...
  with SdtrigExt
{
  ...
  io.dcache.req.bits.replayCarry  := s0_sel_src.replayCarry
  ...
  def fromMisAlignBufferSource(src: LsPipelineBundle): FlowSource = {
    ...
    out.replayCarry     := src.replayCarry
  ...
  def fromMmioSource(src: MemExuOutput) = {
    ...
    out.try_l2l       := false.B
    out.has_rob_entry := false.B
    out.replayCarry     := 0.U.asTypeOf(out.replayCarry)
    out.mshrid        := 0.U
...
//    out.isFirstIssue  := src.isFirstIssue
  ...
  //for Svpbmt Nc
  s0_out.isNoncacheable := s0_sel_src.isnc
  s0_out.data := s0_sel_src.data
  s0_out.misalignWith16Bytes    := s0_misalignWith16Byte
  ...
  val s1_nc               = s1_in.isNoncacheable
  ...
  io.sbuffer.req.bits.pc    := s1_in.uop.pc // FIXME: remove it
  ...
  io.ubuffer.req.bits.pc    := s1_in.uop.pc // FIXME: remove it
  ...
  private val s1_isMatch128 = io.stld_nuke_query.map(x => (x.bits.matchLine || ((s1_in.isVector || s1_in.misalignWith16Bytes) && s1_in.is128bit)))
  ...
  s1_out.isNoncacheable  := s1_nc || Pbmt.isNC(s1_pbmt)
  ...
  val s2_actually_uncache = Pbmt.isPMA(s2_pbmt) && s2_pmp.mmio || s2_in.isNoncacheable || s2_in.mmio
  ...
  val s2_mis_align = s2_valid && GatedValidRegNext(io.csrCtrl.hd_misalign_ld_enable) &&
                     s2_out.misalign && !s2_in.misalignWith16Bytes && !s2_exception_vec(breakPoint) && !s2_trigger_debug_mode && !s2_check_mmio
  ...
  val s2_dcache_miss   = io.dcache.resp.bits.miss &&
                         ...
                         !s2_full_fwd && !s2_in.isNoncacheable
  ...
  //  4. Data contains.
  private val s2_isMatch128 = io.stld_nuke_query.map(x => (x.bits.matchLine || ((s2_in.isVector || s2_in.misalignWith16Bytes) && s2_in.is128bit)))
  ...
  val s2_nuke          = VecInit((0 until StorePipelineWidth).map(w => {
                        ...
                        })).asUInt.orR && !s2_tlb_miss || s2_in.cause(ReplayCauseNO.C_NK)
  ...
  val s2_dcache_should_resp = !(s2_in.tlbMiss || s2_exception || s2_in.delayedError || s2_uncache || s2_prf)
  ...
  s2_real_exceptionVec(loadAddrMisaligned) := s2_out.misalign && s2_check_mmio
  ...
  val s2_safe_wakeup = !s2_out.needReplay && !s2_mmio && (!s2_in.isNoncacheable || s2_nc_with_data) && !s2_mis_align && !s2_real_exception || s2_in.misalignNeedWakeUp // don't need to replay and is not a mmio\misalign no data
  ...
  io.lsq.ldld_nuke_query.req.bits.isNoncacheable := s2_nc_with_data
  ...
  io.lsq.stld_nuke_query.req.bits.isNoncacheable := s2_nc_with_data
  ...
  s2_out.isNoncacheable      := s2_in.isNoncacheable
  s2_out.mmio                := s2_mmio
  ...
  io.prefetch_train.bits.misalignWith16Bytes := false.B
  ...
  io.prefetch_train_l1.bits.misalignWith16Bytes := false.B
  ...
  io.fast_rep_out.bits.delayedError := s3_hw_err
  ...
  val s3_lrq_rep_info = WireInit(s3_in.cause)
  s3_lrq_rep_info(ReplayCauseNO.C_MF) := toMisalignBufferValid && !io.misalign_buf.ready
  val s3_lrq_sel_rep_cause = PriorityEncoderOH(s3_lrq_rep_info.asUInt)
  val s3_replayqueue_rep_cause = WireInit(0.U.asTypeOf(s3_in.cause))
  s3_replayqueue_rep_cause(ReplayCauseNO.C_MF) := s3_mis_align && s3_lrq_rep_info(ReplayCauseNO.C_MF)

  val s3_mab_rep_info = WireInit(s3_in.cause)
  val s3_mab_sel_rep_cause = PriorityEncoderOH(s3_mab_rep_info.asUInt)
  val s3_misalign_rep_cause = WireInit(0.U.asTypeOf(s3_in.cause))

  s3_misalign_rep_cause := Mux(
    s3_in.misalignNeedWakeUp,
    0.U.asTypeOf(s3_mab_rep_info.cloneType),
    VecInit(s3_mab_sel_rep_cause.asBools)
  )

  when (s3_exception || s3_hw_err || s3_rep_frm_fetch || s3_frm_mabuf) {
    s3_replayqueue_rep_cause := 0.U.asTypeOf(s3_lrq_rep_info.cloneType)
  } .otherwise {
  ...
  s3_out.bits.debug.isNC      := s3_in.isNoncacheable
  ...
  val s3_frm_mis_flush     = s3_frm_mabuf &&
    (io.misalign_ldout.bits.cause(ReplayCauseNO.C_FF) || io.misalign_ldout.bits.cause(ReplayCauseNO.C_MA) || io.misalign_ldout.bits.cause(ReplayCauseNO.C_NK))
...
//  io.lsq.ldin.bits.uop.exceptionVec(loadAddrMisaligned) := Mux(s3_in.onlyMisalignException, false.B, s3_in.uop.exceptionVec(loadAddrMisaligned))
  ...
  val s3_revoke = s3_exception || io.lsq.ldin.bits.needReplay || s3_mis_align
  ...
  io.feedback_slow.bits.hit              := !s3_lrq_rep_info.asUInt.orR || io.lsq.ldin.ready
  ...
  s3_ld_raw_data_frm_pipe.dcacheData       := Mux(s2_nc_with_data, s2_ld_data_frm_nc, io.dcache.resp.bits.data)
  s3_ld_raw_data_frm_pipe.forwardDchan            := s2_fwd_frm_d_chan && !s2_nc_with_data
  s3_ld_raw_data_frm_pipe.forwardDataDchan        := s2_fwd_data_frm_d_chan
  s3_ld_raw_data_frm_pipe.forwardMshr         := s2_fwd_frm_mshr && !s2_nc_with_data
  s3_ld_raw_data_frm_pipe.forwardDataMshr     := s2_fwd_data_frm_mshr
  s3_ld_raw_data_frm_pipe.forwardResultValid := s2_fwd_data_valid
  ...
  val vecFeedback = s3_valid && s3_fb_no_waiting && s3_lrq_rep_info.asUInt.orR && !io.lsq.ldin.ready && s3_isvec
  ...
  io.vecldout.bits.vecdata.get := Mux(s3_in.misalignWith16Bytes, vecShiftData, Mux(s3_in.is128bit, s3_merged_data_frm_pipe, vecdata))
  ...
  io.vecldout.bits.hit := !s3_lrq_rep_info.asUInt.orR || io.lsq.ldin.ready
  ...
  io.misalign_ldout.bits.data := Mux(s3_in.misalignWith16Bytes, s3_merged_data_frm_pipe, s3_picked_data_frm_pipe(2))
  io.misalign_ldout.bits.cause := s3_misalign_rep_cause

  // fast load to load forward
  if (EnableLoadToLoadForward) {
    io.l2l_fwd_out.valid      := s3_valid && !s3_in.mmio && !s3_in.isNoncacheable && !s3_lrq_rep_info.asUInt.orR
  ...
  io.debug_ls.s3_isReplay := s3_valid && s3_lrq_rep_info.asUInt.orR // include fast+slow+rs replay
  io.debug_ls.replayCause := s3_lrq_rep_info
  ...
  XSPerfAccumulate("nc_stld_vio", s3_valid && s3_nc_with_data && s3_in.cause(ReplayCauseNO.C_NK))
  XSPerfAccumulate("nc_ldld_vioNack", s3_valid && s3_nc_with_data && s3_in.cause(ReplayCauseNO.C_RARF))
  XSPerfAccumulate("nc_stld_vioNack", s3_valid && s3_nc_with_data && s3_in.cause(ReplayCauseNO.C_RAWF))
