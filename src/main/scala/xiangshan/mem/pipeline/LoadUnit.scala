  ...
  with HasCircularQueuePtrHelper
{
  ...
  // src6: vec read first issue from RS (TODO)
  ...
  .elsewhen (s0_ld_mmio_select)      { fromMmioSource(io.lsq.uncache.bits)        }
  ...
  val s3_ld_wb_meta = Mux(s3_out.valid, s3_out.bits, s3_mmio.bits)
