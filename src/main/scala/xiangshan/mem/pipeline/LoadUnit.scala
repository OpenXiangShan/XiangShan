...
import xiangshan.frontend.tracertl.{TraceInstrBundle, TraceRTLChoose, TraceRTLDontCareValue, TraceRTLDontCare}
...
//    out.isFirstIssue  := src.isFirstIssue
  ...
  val int_issue_vaddr = TraceRTLChoose(
    io.ldin.bits.src(0) + SignExt(io.ldin.bits.uop.imm(11, 0), VAddrBits),
    io.ldin.bits.uop.traceInfo.memoryAddrVA
  )
  ...
  s0_out.uop.exceptionVec(loadAddrMisaligned) :=  TraceRTLDontCareValue((!s0_addr_aligned || s0_sel_src.uop.exceptionVec(loadAddrMisaligned)) && s0_sel_src.vecActive)
  ...
  val s1_exception        = TraceRTLDontCareValue(ExceptionNO.selectByFu(s1_out.uop.exceptionVec, LduCfg).asUInt.orR)   // af & pf exception were modified below.
  ...
  s1_paddr_dup_lsu    := Mux(s1_in.tlbNoQuery, s1_in.paddr,
    TraceRTLChoose(io.tlb.resp.bits.paddr(0), Mux(s1_in.isHWPrefetch, io.tlb.resp.bits.paddr(0), s1_in.uop.traceInfo.memoryAddrPA)))
  s1_paddr_dup_dcache := Mux(s1_in.tlbNoQuery, s1_in.paddr,
    TraceRTLChoose(io.tlb.resp.bits.paddr(1), Mux(s1_in.isHWPrefetch, io.tlb.resp.bits.paddr(1), s1_in.uop.traceInfo.memoryAddrPA)))
  s1_gpaddr_dup_lsu   := Mux(s1_in.isFastReplay, s1_in.paddr,
    TraceRTLChoose(io.tlb.resp.bits.gpaddr(0), Mux(s1_in.isHWPrefetch, io.tlb.resp.bits.gpaddr(0), s1_in.uop.traceInfo.memoryAddrPA)))
  ...
  TraceRTLDontCare(s1_out.uop.exceptionVec)
  ...
  val s2_exception =  TraceRTLDontCareValue(s2_vecActive &&
  ...
  TraceRTLDontCare(s2_exception_vec)
  ...
  TraceRTLDontCare(s3_in.uop.exceptionVec)
  ...
  s3_out.bits.data            := TraceRTLDontCareValue(s3_in.data)
  ...
  val s3_outexception = TraceRTLDontCareValue(ExceptionNO.selectByFu(s3_out.bits.uop.exceptionVec, LduCfg).asUInt.orR && s3_vecActive)
  io.ldout.bits        := s3_ld_wb_meta
  io.ldout.bits.data   := TraceRTLDontCareValue(Mux(s3_valid, s3_ld_data_frm_cache, s3_ld_data_frm_uncache))
