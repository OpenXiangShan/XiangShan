...
class LoadUnit_S0(implicit p: Parameters) extends XSModule with HasDCacheParameters{
  ...
  // query DTLB
  io.dtlbReq.valid := io.in.valid || tryFastpath || io.prefetch_in.valid
  ...
  // query DCache
  io.dcacheReq.valid := io.in.valid || tryFastpath || io.prefetch_in.valid
  when (isPrefetchRead) {
...
class LoadUnit(implicit p: Parameters) extends XSModule with HasLoadHelper with HasPerfEvents {
  ...
  val s0_tryPointerChasing = !io.ldin.valid && io.fastpathIn.valid && !io.prefetch_req.valid
