...
class LoadToLsqIO(implicit p: Parameters) extends XSBundle {
  ...
  val loadReplay = ValidIO(new LqWriteBundle)
  ...
  with HasDCacheParameters 
{
  val io = IO(new Bundle() {
    ...
    val replay = ValidIO(new LqWriteBundle)
  })
  ...
  // * dcache miss
  val cause = Wire(Vec(LoadReplayCauses.allCauses, Bool()))
  cause := io.in.bits.replayInfo.cause
  cause(LoadReplayCauses.waitStore) := s2_wait_store && !s2_mmio && !s2_is_prefetch
  cause(LoadReplayCauses.tlbMiss) := s2_tlb_miss
  cause(LoadReplayCauses.schedError) := (io.in.bits.replayInfo.cause(LoadReplayCauses.schedError) || s2_schedError) && !s2_mmio && !s2_is_prefetch
  cause(LoadReplayCauses.dcacheMiss) := io.out.bits.miss 
  if (EnableFastForward) {
    cause(LoadReplayCauses.dcacheReplay) := !(!s2_cache_replay || s2_is_prefetch || s2_mmio || s2_exception || fullForward)
  }else {
    cause(LoadReplayCauses.dcacheReplay) := !(!s2_cache_replay || s2_is_prefetch || s2_mmio || s2_exception || io.dataForwarded) 
  }
  cause(LoadReplayCauses.forwardFail) := s2_data_invalid && !s2_mmio && !s2_is_prefetch
  cause(LoadReplayCauses.rejectEnq) := s2_rejectEnq && !s2_mmio && !s2_is_prefetch && !s2_exception

  val sel_cause = PriorityEncoderOH(cause.asUInt)
  io.out.bits.replayInfo.cause := sel_cause.asBools
  ...
  io.replay.valid := io.out.valid 
  io.replay.bits := io.out.bits
  ...
  with HasCircularQueuePtrHelper
{
  ...
  load_s2.io.replay <> io.lsq.loadReplay
