...
class StoreUnit(implicit p: Parameters) extends XSModule
  ...
  val s0_addr_aligned = LookupTree(s0_alignTpye, List(
    "b00".U   -> true.B,              //b
    "b01".U   -> (s0_vaddr_tmp(0) === 0.U),   //h
    "b10".U   -> (s0_vaddr_tmp(1,0) === 0.U), //w
    "b11".U   -> (s0_vaddr_tmp(2,0) === 0.U)  //d
  ))
  // if vector store sends 128-bit requests, its address must be 128-aligned
  XSError(s0_use_flow_vec && s0_vaddr_tmp(3, 0) =/= 0.U && s0_vecstin.alignedType(2), "unit stride 128 bit element is not aligned!")
  ...
  val s0_addr_low = s0_vaddr_tmp(4, 0)
  ...
  val s0_vaddr = Mux(
    ...
    Cat(s0_vaddr_tmp(s0_vaddr_tmp.getWidth - 1, 4), 0.U(4.W)),
    s0_vaddr_tmp
  )
  s0_fullva := Mux(
    s0_use_flow_rs,
...
//  s0_out.uop.exceptionVec(storeAddrMisaligned) := false.B
  ...
  // goto misalignBuffer
  val toMisalignBufferValid = s1_valid && !s1_tlb_miss && !s1_in.isHWPrefetch && GatedValidRegNext(io.csrCtrl.hd_misalign_st_enable) && s1_in.isMisalign && !s1_in.misalignWith16Byte
  ...
  io.lsq.bits.vaddr := s1_out.fullva(VAddrBits - 1, 0)
  ...
  val s2_mis_align = GatedValidRegNext(io.csrCtrl.hd_misalign_st_enable) &&
                     s2_in.isMisalign && !s2_in.misalignWith16Byte && !s2_in.uop.exceptionVec(breakPoint) && !s2_trigger_debug_mode
  ...
  s2_out.uop.exceptionVec(storeAccessFault) := (s2_in.uop.exceptionVec(storeAccessFault) ||
                                                s2_pmp.st ||
                                                ((s2_in.isvec || s2_frm_mabuf)  && s2_pmp.mmio && RegNext(s1_feedback.bits.hit))
                                                ) && s2_vecActive
...
//  io.lsq_replenish.paddr   := Mux(s2_exception, Cat(s2_out.paddr(s2_out.paddr.getWidth -1, 4), s2_out.fullva(3, 0)), s2_out.paddr)
//  io.lsq_replenish.gpaddr  := Mux(s2_exception, Cat(s2_out.gpaddr(s2_out.gpaddr.getWidth -1, 4), s2_out.fullva(3, 0)), s2_out.gpaddr)

  io.lsq_replenish.paddr   := Cat(s2_out.paddr(s2_out.paddr.getWidth -1, 4), s2_out.fullva(3, 0))
  io.lsq_replenish.gpaddr  := Cat(s2_out.gpaddr(s2_out.gpaddr.getWidth -1, 4), s2_out.fullva(3, 0))
  // prefetch related
