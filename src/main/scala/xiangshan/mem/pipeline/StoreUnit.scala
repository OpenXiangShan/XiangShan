...
class StoreUnit(implicit p: Parameters) extends XSModule with HasDCacheParameters {
  ...
  // vector
  val s0_activative          = !s0_use_flow_vec || s0_vecstin.activative
  ...
  s1_out.uop.exceptionVec(storePageFault)   := io.tlb.resp.bits.excp(0).pf.st && s1_activative
  s1_out.uop.exceptionVec(storeAccessFault) := io.tlb.resp.bits.excp(0).af.st && s1_activative
  ...
  s2_out.uop.exceptionVec(storeAccessFault) := (s2_in.uop.exceptionVec(storeAccessFault) || s2_pmp.st) && s2_activative
