...
import xiangshan.cache.DCacheStoreIO
import xiangshan.cache.DcacheStoreRequestIO
import xiangshan.cache.HasDCacheParameters
import xiangshan.cache.MemoryOpConstants
import xiangshan.cache.StorePrefetchReq
...
class StoreUnit(implicit p: Parameters) extends XSModule
  ...
  XSError(
    s0_use_flow_vec && s0_vaddr(3, 0) =/= 0.U && s0_vecstin.alignedType(2),
    "unit stride 128 bit element is not aligned!"
  )

  val s0_isMisalign = Mux(
    s0_use_non_prf_flow,
    !s0_addr_aligned || s0_vecstin.uop.exceptionVec(storeAddrMisaligned) && s0_vecActive,
    false.B
  )
  val s0_addr_low = s0_vaddr(4, 0)
  val s0_addr_Up_low = LookupTree(
    s0_alignTpye,
    List(
      "b00".U -> 0.U,
      "b01".U -> 1.U,
  ...
  s0_is128bit := Mux(
    s0_use_flow_ma,
    io.misalign_stin.bits.is128bit,
    is128Bit(s0_vecstin.alignedType) || s0_misalignWith16Byte
  )
...
//  s0_out.uop.exceptionVec(storeAddrMisaligned) := Mux(s0_use_non_prf_flow, (!s0_addr_aligned || s0_vecstin.uop.exceptionVec(storeAddrMisaligned) && s0_vecActive), false.B) && !s0_misalignWith16Byte
  ...
  // goto misalignBuffer
  val toMisalignBufferValid = s1_valid && !s1_tlb_miss && !s1_in.isHWPrefetch && GatedValidRegNext(
    io.csrCtrl.hd_misalign_st_enable
  ) && s1_in.isMisalign && !s1_in.misalignWith16Byte && !s1_trigger_breakpoint && !s1_trigger_debug_mode
  ...
  XSDebug(s1_feedback.valid, "S1 Store: tlbHit: %d robIdx: %d\n", s1_feedback.bits.hit, s1_feedback.bits.robIdx.value)
  ...
  io.lsq.bits.isFinalSplit := !s1_frm_mabuf || s1_exception
  ...
  val s2_actually_uncache =
    (Pbmt.isPMA(s2_pbmt) && s2_pmp.mmio || s2_in.nc || s2_in.mmio) && RegNext(s1_feedback.bits.hit)
  ...
  s2_out.uop.exceptionVec(storeAccessFault) := (s2_in.uop.exceptionVec(storeAccessFault) ||
    s2_pmp.st ||
    ((s2_in.isvec || s2_frm_mabuf) && s2_actually_uncache && RegNext(s1_feedback.bits.hit))) && s2_vecActive
  ...
  feedback_slow_valid := s1_feedback.valid && !s1_out.uop.robIdx.needFlush(
    io.redirect
  ...
  val s2_vecFeedback =
    RegNext(!s1_out.uop.robIdx.needFlush(io.redirect) && s1_feedback.bits.hit && s1_feedback.valid) && s2_in.isvec
  ...
  for (i <- 0 until TotalDelayCycles + 1) {
    if (i == 0) {
      ...
      sx_ready(i) := !s3_valid(i) || sx_in(i).output.uop.robIdx.needFlush(io.redirect) || (if (TotalDelayCycles == 0)
                                                                                             io.stout.ready
                                                                                           else sx_ready(i + 1))
    } else {
  ...
  io.stout.valid := sx_last_valid && !sx_last_in.output.uop.robIdx.needFlush(
    io.redirect
  ) && !sx_last_in_vec // isStore(sx_last_in.output.uop.fuType)
  ...
  io.vecstout.valid := sx_last_valid && !sx_last_in.output.uop.robIdx.needFlush(
    io.redirect
  ) && sx_last_in_vec // isVStore(sx_last_in.output.uop.fuType)
  // TODO: implement it!
  ...
  XSPerfAccumulate(
    ...
    s0_fire && !s0_use_flow_vec && s0_saddr(VAddrBits - 1, 12) === s0_stin.src(0)(VAddrBits - 1, 12)
  )
  XSPerfAccumulate(
    "s0_addr_spec_failed",
    s0_fire && !s0_use_flow_vec && s0_saddr(VAddrBits - 1, 12) =/= s0_stin.src(0)(VAddrBits - 1, 12)
  ...
  XSPerfAccumulate(
    ...
    s0_fire && !s0_use_flow_vec && s0_saddr(VAddrBits - 1, 12) === s0_stin.src(0)(VAddrBits - 1, 12) && s0_isFirstIssue
  )
  XSPerfAccumulate(
