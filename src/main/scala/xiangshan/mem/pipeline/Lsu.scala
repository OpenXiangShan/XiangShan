...
class LdPipelineBundle extends XSBundle {
...
class LsuIO extends XSBundle {
  val ldin = Vec(2, Flipped(DecoupledIO(new LduReq)))
  val ldout = Vec(2, DecoupledIO(new ExuOutput))
  val stin = Vec(2, Flipped(DecoupledIO(new StuReq)))
  // val stout = Vec(2, DecoupledIO(new ExuOutput))
...
//-------------------------------------------------------
  ...
  // val l2_in  = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  val l2_out = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  // val s3_in  = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  // val s3_out = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  // val s4_in  = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  val s4_out = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  val s5_in  = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
  val s5_out = Wire(Vec(2, DecoupledIO(new LdPipelineBundle)))
...
//-------------------------------------------------------
  ...
  (0 until LoadPipelineWidth).map(i => {
    ...
    l2_out(i).lsroqId := io.ldin(i).lsroqId
...
//-------------------------------------------------------
  ...
  // result from dcache
  (0 until LoadPipelineWidth).map(i => {
    s4_out(i).bits := DontCare
    // s4_out(i).bits.paddr := io.dmem.resp(i).bits.addr
    // s4_out(i).bits.data := io.dmem.resp(i).bits.data
    // s4_out(i).bits.func := io.dmem.resp(i).bits.user ...
    // s4_out(i).bits.pc := io.dmem.resp(i).bits.user ...
    // s4_out(i).lsroqId := io.dmem.resp(i).bits.user ...
    // s4_out(i).valid := io.dmem(i).resp.valid
  })
  ...
  val forwardWmask = Wire(UInt(8.W))
  ...
  // TODO: add brIdx
  (0 until LoadPipelineWidth).map(i => {
    PipelineConnect(s4_out(i), s5_in(i), s5_out(i).fire(), io.redirect.valid)
  })
...
//-------------------------------------------------------
  ...
  (0 until LoadPipelineWidth).map(i => {
    ...
    val rdata = VecInit((0 until 8).map(j => {
      Mux(s5_in(i).bits.forwardMask(j), 
        s5_in(i).bits.forwardData(8*(j+1)-1, 8*j), 
        s5_in(i).bits.data(8*(j+1)-1, 8*j)
      )
    })).asUInt
    val func = s5_in(i).bits.func
    val rdataSel = LookupTree(rdata(2, 0), List(
...
//-------------------------------------------------------
  ...
  (0 until StorePipelineWidth).map(i => {
    ...
    s2_out(i).lsroqId := io.stin(i).lsroqId
...
//-------------------------------------------------------
  ...
  (0 until StorePipelineWidth).map(i => {
    ...
    s3_in(i).bits.paddr := io.dtlb.store(i).bits.paddr
    s3_in(i).bits.data := io.dtlb.store(i).bits.data
    s3_in(i).bits.func := io.dtlb.store(i).bits.func
    s3_in(i).bits.pc := io.dtlb.store(i).bits.pc
    s3_in(i).lsroqId := io.dtlb.store(i).bits.lsroqId
    s3_in(i).valid := io.dtlb.store(i).valid
  })
