...
import xiangshan.cache.mmu.TlbHintReq
...
class HybridUnit(implicit p: Parameters) extends XSModule
  ...
  io.tlb.req.bits.size := Mux(
    s0_isvec,
    io.vec_stu_io.in.bits.alignedType(1, 0),
  ...
  io.ldu_io.dcache.req.bits.cmd := Mux(
    s0_prf_rd,
  ...
  io.ldu_io.dcache.is128Req := is128Bit(
    io.vec_stu_io.in.bits.alignedType
  ) && io.vec_stu_io.in.valid && s0_src_select_vec(vec_iss_idx)
  ...
  val s0_addr_aligned = LookupTree(
    Mux(s0_isvec, io.vec_stu_io.in.bits.alignedType(1, 0), s0_uop.fuOpType(1, 0)),
    List(
      "b00".U -> true.B,                   // b
  ...
  io.ldu_io.replay.ready := (s0_can_go && io.ldu_io.dcache.req.ready && (s0_src_ready_vec(
    lsq_rep_idx
  ...
  io.lsin.ready := (s0_can_go &&
    Mux(
      FuType.isLoad(io.lsin.bits.uop.fuType),
      io.ldu_io.dcache.req.ready,
      if (StorePrefetchL1Enabled) io.stu_io.dcache.req.ready else true.B
  ...
  io.ldu_io.dcache.replacementUpdated := Mux(
    s0_src_select_vec(lsq_rep_idx) || s0_src_select_vec(super_rep_idx),
    io.ldu_io.replay.bits.replacementUpdated,
    false.B
  )
  ...
  // load debug
  XSDebug(
    io.ldu_io.dcache.req.fire && s0_ld_flow,
  ...
  // store debug
  XSDebug(
    io.stu_io.dcache.req.fire && !s0_ld_flow,
  ...
  val s1_st_exception =
    ExceptionNO.selectByFu(s1_out.uop.exceptionVec, StaCfg).asUInt.orR // af & pf exception were modified below.
  ...
  // mmio cbo decoder
  val s1_mmio_cbo =
    (s1_in.uop.fuOpType === LSUOpType.cbo_clean ||
  ...
  s1_kill := s1_late_kill ||
    ...
    RegEnable(
      s0_kill,
      false.B,
      io.lsin.valid || io.ldu_io.replay.valid || io.ldu_io.l2l_fwd_in.valid || io.ldu_io.fast_rep_in.valid || io.vec_stu_io.in.valid
    )
  ...
  if (EnableLoadToLoadForward) {
    ...
    s1_addr_misaligned := LookupTree(
      s1_in.uop.fuOpType(1, 0),
      List(
        "b00".U -> false.B,                  // b
    ...
    when(!s1_cancel_ptr_chasing) {
      s0_ptr_chasing_canceled := s1_try_ptr_chasing && !io.ldu_io.replay.fire && !io.ldu_io.fast_rep_in.fire && !(s0_src_valid_vec(
        high_pf_idx
      ) && io.canAcceptHighConfPrefetch)
      when(s1_try_ptr_chasing) {
  ...
  io.ldu_io.wakeup.valid := s0_fire && s0_ld_flow && (s0_src_select_vec(super_rep_idx) || s0_src_select_vec(
    fast_rep_idx
  ) || s0_src_select_vec(lsq_rep_idx) || s0_src_select_vec(int_iss_idx))
  ...
  val (s2_fwd_frm_d_chan, s2_fwd_data_frm_d_chan) =
    io.ldu_io.tl_d_channel.forward(s1_valid && s1_out.forward_tlDchannel, s1_out.mshrid, s1_out.paddr)
  ...
  for (i <- 0 until VLEN / 8) {
    s2_fwd_mask(i) := io.ldu_io.lsq.forward.forwardMask(i) || io.ldu_io.sbuffer.forwardMask(
      i
    ...
    s2_fwd_data(i) :=
      Mux(
        ...
        Mux(
          ...
          Mux(io.ldu_io.ubuffer.forwardMask(i), io.ldu_io.ubuffer.forwardData(i), io.ldu_io.sbuffer.forwardData(i))
        )
      )
  }
  ...
  XSDebug(
    ...
    s2_in.uop.pc,
    io.ldu_io.lsq.forward.forwardData.asUInt,
  ...
  s2_out.rep_info.data_inv_sq_idx := Mux(
    io.ldu_io.vec_forward.dataInvalid,
    s2_out.uop.sqIdx,
    io.ldu_io.lsq.forward.dataInvalidSqIdx
  )
  s2_out.rep_info.addr_inv_sq_idx := Mux(
    io.ldu_io.vec_forward.addrInvalid,
    s2_out.uop.sqIdx,
    io.ldu_io.lsq.forward.addrInvalidSqIdx
  )
  ...
  io.prefetch_train.bits.miss := Mux(
    s2_ld_flow,
    io.ldu_io.dcache.resp.bits.miss,
    io.stu_io.dcache.resp.bits.miss
  ...
  io.ldu_io.lsq.ldin.bits.missDbUpdated := RegNext(
  ...
  val s3_vp_match_fail = RegNext(
    io.ldu_io.lsq.forward.matchInvalid || io.ldu_io.sbuffer.matchInvalid || io.ldu_io.ubuffer.matchInvalid
  ) && s3_troublem
  ...
  val s3_picked_data_frm_cache = LookupTree(
    s3_ld_raw_data_frm_cache.addrOffset,
    List(
  ...
  for (i <- 0 until TotalDelayCycles + 1) {
    if (i == 0) {
      sx_valid(i) := s3_valid &&
        ...
        !s3_in.mmio
      sx_in(i) := s3_out.bits
      sx_ready(i) := !s3_valid(i) || sx_in(i).uop.robIdx.needFlush(io.redirect) || (if (TotalDelayCycles == 0)
                                                                                      io.stout.ready
                                                                                    else sx_ready(i + 1))
    } else {
  ...
  io.stout.valid := sx_last_valid && !sx_last_in.uop.robIdx.needFlush(io.redirect) && FuType.isStore(
    sx_last_in.uop.fuType
  )
  ...
  XSPerfAccumulate(
    ...
    s0_fire && s0_vaddr(VAddrBits - 1, 12) === io.lsin.bits.src(0)(VAddrBits - 1, 12)
  )
  XSPerfAccumulate(
    "s0_addr_spec_failed",
    s0_fire && s0_vaddr(VAddrBits - 1, 12) =/= io.lsin.bits.src(0)(VAddrBits - 1, 12)
  ...
  XSPerfAccumulate(
    ...
    s0_fire && s0_vaddr(VAddrBits - 1, 12) === io.lsin.bits.src(0)(VAddrBits - 1, 12) && s0_isFirstIssue
  )
  XSPerfAccumulate(
  ...
  XSPerfAccumulate(
    "s2_prefetch_ignored",
    s2_fire && s2_prf && io.ldu_io.dcache.s2_mq_nack
  ...
  XSPerfAccumulate(
    "s2_prefetch_accept",
    s2_fire && s2_prf && io.ldu_io.dcache.resp.bits.miss && !io.ldu_io.dcache.s2_mq_nack
