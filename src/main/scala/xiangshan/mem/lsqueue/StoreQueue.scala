...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  for (i <- 0 until StorePipelineWidth) {
    ...
    when (io.storeAddrIn(i).fire && io.storeAddrIn(i).bits.updateAddrValid) {
      ...
      nc(stWbIndex) := io.storeAddrIn(i).bits.isNoncacheable
    ...
    when (io.storeAddrIn(i).fire && !io.storeAddrIn(i).bits.isMisalignBuf) {
      ...
      cross16Byte(stWbIndex) := io.storeAddrIn(i).bits.misalign && !io.storeAddrIn(i).bits.misalignWith16Bytes
  ...
  for (i <- 0 until LoadPipelineWidth) {
    val req = io.forward(i).req
    val resp = io.forward(i).resp
    ...
    // val vaddrMatchFailed = vpmaskNotEqual && req.valid
    val vpmaskNotEqual = (
