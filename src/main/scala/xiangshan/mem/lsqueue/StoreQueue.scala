...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  for (i <- 0 until LoadPipelineWidth) {
    ...
    val addrInvalidMask1 = (~addrValidVec.asUInt & mdpMatchVec.asUInt & forwardMask1.asUInt)
    val addrInvalidMask2 = (~addrValidVec.asUInt & mdpMatchVec.asUInt & forwardMask2.asUInt)
    // make chisel happy
