...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  val io = IO(new Bundle() {
    ...
    val cboZerotout = DecoupledIO(new MemExuOutput)
  ...
  io.cboZerotout.valid                := cboZeroValid && !cboZeroWaitFlushSb
  io.cboZerotout.bits.uop             := cboZeroUop
  io.cboZerotout.bits.data            := DontCare
  io.cboZerotout.bits.isFromLoadUnit  := DontCare
  io.cboZerotout.bits.debug.isMMIO    := false.B
  io.cboZerotout.bits.debug.isNC      := false.B
  io.cboZerotout.bits.debug.paddr     := DontCare
  io.cboZerotout.bits.debug.isPerfCnt := false.B
  io.cboZerotout.bits.debug.vaddr     := DontCare

  when (cboZeroWaitFlushSb && io.flushSbuffer.empty) {
    cboZeroWaitFlushSb    := false.B
  }
  when (io.cboZerotout.fire) {
