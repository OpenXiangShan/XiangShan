...
maskModule.clock := gate_clock
  ...
  val stFtqIdx_resp = (0 until StorePipelineWidth).map(w => DelayNWithValid(io.storeIn(w).bits.uop.ftqPtr, io.storeIn(w).valid, TotalSelectCycles))
  val stFtqOffset_resp = (0 until StorePipelineWidth).map(w => DelayNWithValid(io.storeIn(w).bits.uop.ftqOffset, io.storeIn(w).valid, TotalSelectCycles))
