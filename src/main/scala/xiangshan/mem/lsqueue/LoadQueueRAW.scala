  ...
  with HasPerfEvents
{
  ...
  def detectRollback(i: Int) = {
    ...
    val lqViolationSel = lqViolationSelVec.zip(lqViolationSelUopExts).map {
      case (v, bits) =>
        val wrapper = Wire(ValidIO(new XSBundleWithMicroOp))
        wrapper.valid := v
        wrapper.bits := bits
        wrapper
    }

    val lqSelect = SelectOldest(
      ins         = lqViolationSel,
      sels        = lqViolationSel.map(_.bits.uop.robIdx),
      fn          = (a: RobPtr, b: RobPtr) => isBefore(a, b), // `fn` return true choose `a`, otherwise choose `b`
      groupSize   = RollbackGroupSize,
      latch       = true,
      moduleName  = Some("LoadQueueRAW rollback select")
    )

    // select one inst
    val lqViolation = lqSelect.valid && !lqSelect.bits.uop.robIdx.needFlush(DelayN(io.redirect, TotalSelectCycles))
    val lqViolationUop = lqSelect.bits.uop
