  ...
  with HasPerfEvents
{
  ...
  def detectRollback(i: Int) = {
    ...
    XSDebug(
      ...
      storeIn(i).bits.uop.cf.pc, storeIn(i).bits.uop.robIdx.asUInt, lqViolationUop.robIdx.asUInt
    )
  ...
  for (w <- 0 until StorePipelineWidth) {
    ...
    stFtqIdx(w) := DelayN(storeIn(w).bits.uop.cf.ftqPtr, TotalSelectCycles)
    stFtqOffset(w) := DelayN(storeIn(w).bits.uop.cf.ftqOffset, TotalSelectCycles)
  }
