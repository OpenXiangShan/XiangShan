...
class SQAddrModule(dataWidth: Int, numEntries: Int, numRead: Int, numWrite: Int, numForward: Int)(implicit
    p: Parameters
) extends XSModule with HasDCacheParameters {
  ...
  for (i <- 0 until numForward) {
    for (j <- 0 until numEntries) {
      ...
      val hit128bit =
        (io.forwardMdata(i)(DCacheLineOffset - 1, DCacheVWordOffset) === data(j)(
          DCacheLineOffset - 1,
          DCacheVWordOffset
        )) &&
...
class SQData8Module(numEntries: Int, numRead: Int, numWrite: Int, numForward: Int)(implicit p: Parameters)
    extends XSModule
  ...
  (0 until numRead).map(i => io.rdata(i) := data(GatedRegNext(io.raddr(i))))
...
class SQDataModule(numEntries: Int, numRead: Int, numWrite: Int, numForward: Int)(implicit p: Parameters)
    extends XSModule with HasDCacheParameters with HasCircularQueuePtrHelper {
  val io = IO(new Bundle() {
    // sync read port
    val raddr = Vec(numRead, Input(UInt(log2Up(numEntries).W)))
