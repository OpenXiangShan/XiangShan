...
abstract class LqRawDataModule[T <: Data](
  ...
  for (bank <- 0 until numWBank) {
    ...
    val sx_bankWriteAddrDec_resp =
      (0 until numWrite).map(w => DelayNWithValid(s0_bankWriteAddrDec(w), io.wen(w), numWDelay - 1))
...
class LqPAddrModule[T <: UInt](
    ...
    numWDelay:  Int = 1,
    numCamPort: Int = 1
)(implicit p: Parameters) extends LqRawDataModule(gen, numEntries, numRead, numWrite, numWBank, numWDelay, numCamPort)
    with HasDCacheParameters {
  // content addressed match
  // 128-bits aligned
  for (i <- 0 until numCamPort) {
    for (j <- 0 until numEntries) {
      io.violationMmask(i)(j) := io.violationMdata(i)(PAddrBits - 1, DCacheVWordOffset) === data(j)(
        PAddrBits - 1,
        DCacheVWordOffset
      )
    }
  ...
  for (i <- 0 until numCamPort) {
    for (j <- 0 until numEntries) {
      io.releaseViolationMmask(i)(j) := io.releaseViolationMdata(i)(PAddrBits - 1, DCacheLineOffset) === data(j)(
        PAddrBits - 1,
        DCacheLineOffset
      )
    }
  ...
  for (i <- 0 until numCamPort) {
    for (j <- 0 until numEntries) {
      io.releaseMmask(i)(j) := io.releaseMdata(i)(PAddrBits - 1, DCacheLineOffset) === data(j)(
        PAddrBits - 1,
        DCacheLineOffset
      )
    }
...
class LqVAddrModule[T <: UInt](
  ...
  for (i <- 0 until numCamPort) {
    for (j <- 0 until numEntries) {
      io.violationMmask(i)(j) := io.violationMdata(i)(VAddrBits - 1, DCacheVWordOffset) === data(j)(
        VAddrBits - 1,
        DCacheVWordOffset
      )
    }
  ...
  for (i <- 0 until numCamPort) {
    for (j <- 0 until numEntries) {
      io.releaseMmask(i)(j) := io.releaseMdata(i)(VAddrBits - 1, DCacheLineOffset) === data(j)(
        VAddrBits - 1,
        DCacheLineOffset
      )
    }
