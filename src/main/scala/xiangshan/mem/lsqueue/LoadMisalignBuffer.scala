  ...
  with HasTlbConst
{
  ...
  when(!req_valid && !select_req_bit.uop.robIdx.needFlush(io.redirect) && select_req_valid) {
    req := select_req_bit
    req_valid := true.B
  }
  ...
  when (bufferState === s_comb_wakeup_rep) {
    when (!cross16BytesBoundary) {
      ...
      val truncateData = LookupTree(alignedType(1, 0), List(
        LB -> shiftData(7,  0), // lb
      ...
      assert(false.B, s"There should be no non-aligned data combine that does not cross 16Byte boundaries.")
    } .otherwise {
