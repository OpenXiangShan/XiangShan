...
class LoadMisalignBuffer(implicit p: Parameters) extends XSModule
  ...
  val data_select =
    RegEnable(genRdataOH(select_req_bit.uop), 0.U(genRdataOH(select_req_bit.uop).getWidth.W), canEnqValid)
  ...
  io.splitLoadReq.bits.uop.fuOpType := Mux(
    req.isvec,
    req.uop.fuOpType,
    Cat(reqIsHlv, reqIsHlvx, 0.U(1.W), splitLoadReqs(curPtr).uop.fuOpType(1, 0))
  )
