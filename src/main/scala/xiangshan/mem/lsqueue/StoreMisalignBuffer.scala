...
class StoreMisalignBuffer(implicit p: Parameters) extends XSModule
  ...
  def getMask(sizeEncode: UInt) = LookupTree(
    sizeEncode,
    List(
      SB -> 0x1.U,
      SH -> 0x3.U,
  ...
  def selectOldest[T <: LsPipelineBundle](
      valid: Seq[Bool],
      bits:  Seq[T],
      index: Seq[UInt]
    ...
    if (valid.length == 0 || valid.length == 1) {
      ...
      val oldest = Mux(
        ...
        Mux(
          isAfter(bits(0).uop.robIdx, bits(1).uop.robIdx) ||
            (isNotBefore(bits(0).uop.robIdx, bits(1).uop.robIdx) && bits(0).uop.uopIdx > bits(1).uop.uopIdx),
          res(1),
          res(0)
        ),
      ...
      val right = selectOldest(
        ...
        index.takeRight(index.length - (index.length / 2))
      )
  ...
  when(cross4KBPageBoundary && !reqRedirect) {
    when(reqSelValid && (isAfter(req.uop.robIdx, reqSelBits.uop.robIdx) || (isNotBefore(
      req.uop.robIdx,
      reqSelBits.uop.robIdx
  ...
  io.req.zipWithIndex.map {
    case (reqPort, index) =>
      reqPort.ready := reqSelCanEnq(index) && (!req_valid || cross4KBPageBoundary && cross4KBPageEnq)
  }
  ...
  val hasException = ExceptionNO.selectByFu(
    io.splitStoreResp.bits.uop.exceptionVec,
    StaCfg
  ).asUInt.orR && !io.splitStoreResp.bits.need_rep
  ...
  val highPageAddress = LookupTree(
    alignedType,
    List(
      SB -> 0.U,
      SH -> 1.U,
  ...
  io.splitStoreReq.bits.uop.fuOpType := Mux(
    req.isvec,
    req.uop.fuOpType,
    Cat(reqIsHsv, 0.U(2.W), splitStoreReqs(curPtr).uop.fuOpType(1, 0))
  )
  ...
  StaCfg.exceptionOut.map(no =>
    io.writeBack.bits.uop.exceptionVec(no) := (globalMMIO || globalException) && exceptionVec(no)
  )
  ...
  io.vecWriteBack.zipWithIndex.map {
    case (wb, index) => {
      wb.valid := req_valid && (bufferState === s_wb) && req.isvec && !io.storeVecOutValid && UIntToOH(req.portIndex)(
        index
      )
