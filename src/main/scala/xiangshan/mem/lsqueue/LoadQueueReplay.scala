  ...
  with HasPerfEvents
{
  ...
  val block = RegInit(VecInit(List.fill(LoadQueueReplaySize)(false.B)))
  ...
  (0 until LoadQueueReplaySize).map(i => {
    // case C_MA:
    when (block(i) && cause(i)(LoadReplayCauses.C_MA) && stAddrDeqVec(i)) {
      block(i) := false.B
    }
    // case C_TM:
    when (block(i) && cause(i)(LoadReplayCauses.C_TM) && creditUpdate(i) === 0.U) {
      block(i) := false.B
    }
    // case C_FF:
    when (block(i) && cause(i)(LoadReplayCauses.C_FF) && stDataDeqVec(i)) {
      block(i) := false.B
    }
    // case C_DM:
    when (block(i) && cause(i)(LoadReplayCauses.C_DM) && io.tl_d_channel.valid && io.tl_d_channel.mshrid === missMSHRId(i)) {
      block(i) := false.B
    }
    // case C_RAR:
    when (block(i) && cause(i)(LoadReplayCauses.C_RAR) && (!io.rarFull || !isAfter(uop(i).lqIdx, io.ldWbPtr))) {
      block(i) := false.B
    }
    // case C_RAW:
    when (block(i) && cause(i)(LoadReplayCauses.C_RAW) && (!io.rawFull || !isAfter(uop(i).sqIdx, io.stAddrReadySqPtr))) {
      block(i) := false.B
    }
  })
  ...
  val s0_loadHintWakeMask = VecInit((0 until LoadQueueReplaySize).map(i => {
    allocated(i) && !scheduled(i) && cause(i)(LoadReplayCauses.C_DM) && missMSHRId(i) === io.l2_hint.bits.sourceId && io.l2_hint.valid
  })).asUInt()
  ...
  val s0_loadHigherPriorityReplaySelMask = VecInit((0 until LoadQueueReplaySize).map(i => {
    val blocked = selBlocked(i) || block(i)
  ...
  for ((enq, w) <- io.enq.zipWithIndex) {
    ...
    when (needEnqueue(w) && enq.ready) {
      ...
      creditUpdate(enqIndex) := Mux(replayInfo.cause(LoadReplayCauses.C_TM), blockCyclesTlb(blockCyclesTlbPtr), 0.U)

      // update block pointer
      when (replayInfo.cause(LoadReplayCauses.C_BC) || replayInfo.cause(LoadReplayCauses.C_NK) || replayInfo.cause(LoadReplayCauses.C_DR)) {
        // normal case: bank conflict or schedule error or dcache replay
        // can replay next cycle
        block(enqIndex) := false.B
      }
      ...
      when (replayInfo.cause(LoadReplayCauses.C_DM) && enq.bits.handledByMSHR) {
        block(enqIndex) := !replayInfo.full_fwd && //  dcache miss
