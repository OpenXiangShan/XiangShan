...
class AgeDetector(numEntries: Int, numEnq: Int, regOut: Boolean = true)(implicit p: Parameters) extends XSModule {
  ...
  def getOldest(get: (Int, Int) => Bool): UInt =
    VecInit((0 until numEntries).map { i =>
      io.ready(i) & VecInit(
        (0 until numEntries).map(j => if (i != j) !io.ready(j) || get(i, j) else true.B)
      ).asUInt.andR
    }).asUInt
...
class LoadQueueReplay(implicit p: Parameters) extends XSModule
  ...
  val allocated =
    RegInit(
      VecInit(List.fill(LoadQueueReplaySize)(false.B))
  ...
  val hasExceptions =
    io.enq.map(enq => ExceptionNO.selectByFu(enq.bits.uop.exceptionVec, LduCfg).asUInt.orR && !enq.bits.tlbMiss)
  ...
  for (i <- 0 until LoadQueueReplaySize) {
    ...
    dataNotBlockVec(i) := isNotBefore(io.stDataReadySqPtr, blockSqIdx(i)) || stDataReadyVec(
      blockSqIdx(i).value
    ) || io.sqEmpty // for better timing
    addrNotBlockVec(i) := isNotBefore(io.stAddrReadySqPtr, blockSqIdx(i)) || !strict(i) && stAddrReadyVec(
      blockSqIdx(i).value
    ) || io.sqEmpty // for better timing
  ...
  (0 until LoadQueueReplaySize).map(i => stAddrDeqVec(i) := allocated(i) && storeAddrValidVec(i))

  // store data issue check
  val stDataDeqVec = Wire(Vec(LoadQueueReplaySize, Bool()))
  (0 until LoadQueueReplaySize).map(i => stDataDeqVec(i) := allocated(i) && storeDataValidVec(i))
  ...
  val s0_remLoadEnqFireVec =
    s0_loadEnqFireMask.map(x => VecInit((0 until LoadPipelineWidth).map(rem => getRemBits(x)(rem))))
  ...
  val s0_loadHintWakeMask = VecInit((0 until LoadQueueReplaySize).map { i =>
    allocated(i) && !scheduled(i) && cause(i)(LoadReplayCauses.C_DM) && blocking(i) && missMSHRId(
      i
    ) === io.l2_hint.bits.sourceId && io.l2_hint.valid
  }).asUInt
  ...
  val s0_remLoadHigherPriorityReplaySelMask =
    VecInit((0 until LoadPipelineWidth).map(rem => getRemBits(s0_loadHigherPriorityReplaySelMask)(rem)))
  ...
  val s0_remLoadLowerPriorityReplaySelMask =
    VecInit((0 until LoadPipelineWidth).map(rem => getRemBits(s0_loadLowerPriorityReplaySelMask)(rem)))
  val s0_loadNormalReplaySelMask =
    s0_loadLowerPriorityReplaySelMask | s0_loadHigherPriorityReplaySelMask | s0_loadHintSelMask
  val s0_remNormalReplaySelVec = VecInit((0 until LoadPipelineWidth).map(rem =>
    s0_remLoadLowerPriorityReplaySelMask(rem) | s0_remLoadHigherPriorityReplaySelMask(rem) | s0_remLoadHintSelMask(rem)
  ))
  val s0_remPriorityReplaySelVec = VecInit((0 until LoadPipelineWidth).map { rem =>
    Mux(
      s0_remHintSelValidVec(rem),
      s0_remLoadHintSelMask(rem),
      Mux(
        ParallelORR(s0_remLoadHigherPriorityReplaySelMask(rem)),
  ...
  val s0_oldestMatchMaskVec = (0 until LoadQueueReplaySize).map(i =>
    (0 until OldestSelectStride).map(j => s0_loadNormalReplaySelMask(i) && uop(i).lqIdx === oldestPtrExt(j))
  )
  val s0_remOldsetMatchMaskVec =
    (0 until LoadPipelineWidth).map(rem => getRemSeq(s0_oldestMatchMaskVec.map(_.take(1)))(rem))
  val s0_remOlderMatchMaskVec =
    (0 until LoadPipelineWidth).map(rem => getRemSeq(s0_oldestMatchMaskVec.map(_.drop(1)))(rem))
  val s0_remOldestSelVec = VecInit(Seq.tabulate(LoadPipelineWidth) { rem =>
    VecInit((0 until LoadQueueReplaySize / LoadPipelineWidth).map { i =>
      Mux(
        ParallelORR(s0_remOldsetMatchMaskVec(rem).map(_(0))),
        s0_remOldsetMatchMaskVec(rem)(i)(0),
        s0_remOlderMatchMaskVec(rem)(i).reduce(_ | _)
      )
    }).asUInt
  })
  ...
  s0_oldestSel := VecInit((0 until LoadPipelineWidth).map { rport =>
    ...
    val ageOldest = AgeDetector(
      ...
      s0_remEnqSelVec(rport),
      s0_remFreeSelVec(rport),
      s0_remPriorityReplaySelVec(rport)
    )
    ...
    val issOldestIndexOH =
      Mux(l2HintFirst, PriorityEncoderOH(s0_remOldestHintSelVec(rport)), PriorityEncoderOH(s0_remOldestSelVec(rport)))
  ...
  for (i <- 0 until LoadPipelineWidth) {
    ...
    s2_oldestSel(i).valid := RegEnable(
      Mux(s1_can_go(i), s1_oldestSelV, false.B),
      false.B,
      s1_can_go(i) || replay_req(i).fire
    )
  ...
  for (i <- 0 until LoadQueueReplaySize) {
    ...
    for (j <- 0 until VecLoadPipelineWidth) {
      vecLdCanceltmp(i)(j) := allocated(i) && fbk(j).valid && fbk(j).bits.isFlush && uop(i).robIdx === fbk(
        j
      ).bits.robidx && uop(i).uopIdx === fbk(j).bits.uopidx
      vecLdCommittmp(i)(j) := allocated(i) && fbk(j).valid && fbk(j).bits.isCommit && uop(i).robIdx === fbk(
        j
      ).bits.robidx && uop(i).uopIdx === fbk(j).bits.uopidx
    }
    ...
    XSError(
      (vecLdCancel(i) || vecLdCommit(i)) && allocated(i),
      s"vector load, should not have replay entry $i when commit or flush.\n"
    )
  }
  ...
  val rob_head_lq_match = ParallelOperation(
    ...
    (a: Tuple2[Bool, XSBundleWithMicroOp], b: Tuple2[Bool, XSBundleWithMicroOp]) => {
      val (a_v, a_uop) = (a._1, a._2)
      val (b_v, b_uop) = (b._1, b._2)

      val res = Mux(
        a_v && b_v,
        Mux(isAfter(a_uop.uop.robIdx, b_uop.uop.robIdx), b_uop, a_uop),
        Mux(a_v, a_uop, Mux(b_v, b_uop, a_uop))
      )
      (a_v || b_v, res)
    }
  )
  ...
  val replayTlbMissCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_TM)))
  val replayMemAmbCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_MA)))
  val replayNukeCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_NK)))
  val replayRARRejectCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_RAR)))
  val replayRAWRejectCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_RAW)))
  val replayBankConflictCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_BC)))
  val replayDCacheReplayCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_DR)))
  val replayForwardFailCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_FF)))
  val replayDCacheMissCount =
    PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.rep_info.cause(LoadReplayCauses.C_DM)))
