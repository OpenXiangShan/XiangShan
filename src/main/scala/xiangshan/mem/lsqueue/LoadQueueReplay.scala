  ...
  with HasPerfEvents
{
  ...
  val trueCacheMissReplay = WireInit(VecInit(cause.map(_(ReplayCauseNO.C_DM))))
  ...
  (0 until LoadQueueReplaySize).map(i => {
    // case C_MA
    when (cause(i)(ReplayCauseNO.C_MA)) {
      blocking(i) := Mux(stAddrDeqVec(i), false.B, blocking(i))
    }
    // case C_TM
    when (cause(i)(ReplayCauseNO.C_TM)) {
  ...
  val s0_loadHintWakeMask = VecInit((0 until LoadQueueReplaySize).map(i => {
    allocated(i) && !scheduled(i) && cause(i)(ReplayCauseNO.C_DM) && blocking(i) && missMSHRId(i) === io.l2_hint.bits.sourceId && io.l2_hint.valid
  })).asUInt
  ...
  val s0_loadHigherPriorityReplaySelMask = VecInit((0 until LoadQueueReplaySize).map(i => {
    val hasHigherPriority = cause(i)(ReplayCauseNO.C_DM) || cause(i)(ReplayCauseNO.C_FF)
  ...
  val s0_loadLowerPriorityReplaySelMask = VecInit((0 until LoadQueueReplaySize).map(i => {
    val hasLowerPriority = !cause(i)(ReplayCauseNO.C_DM) && !cause(i)(ReplayCauseNO.C_FF)
  ...
  for ((enq, w) <- io.enq.zipWithIndex) {
    ...
    when (needEnqueue(w) && enq.ready) {
      ...
      when (replayInfo.cause(ReplayCauseNO.C_BC) ||
            replayInfo.cause(ReplayCauseNO.C_NK) ||
            replayInfo.cause(ReplayCauseNO.C_DR) ||
            replayInfo.cause(ReplayCauseNO.C_WPF)) {
      ...
      when (replayInfo.cause(ReplayCauseNO.C_TM)) {
      ...
      when (replayInfo.cause(ReplayCauseNO.C_DM) && enq.bits.mshrHandled) {
  ...
  val rob_head_tlb_miss        = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_TM)
  val rob_head_nuke            = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_NK)
  val rob_head_mem_amb         = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_MA)
  val rob_head_confilct_replay = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_BC)
  val rob_head_forward_fail    = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_FF)
  val rob_head_mshrfull_replay = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_DR)
  val rob_head_dcache_miss     = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_DM)
  val rob_head_rar_nack        = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_RARF)
  val rob_head_raw_nack        = lq_match && cause(lq_match_idx)(ReplayCauseNO.C_RAWF)
  ...
  val replayTlbMissCount      = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_TM)))
  val replayMemAmbCount       = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_MA)))
  val replayNukeCount         = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_NK)))
  val replayRARRejectCount    = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_RARF)))
  val replayRAWRejectCount    = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_RAWF)))
  val replayBankConflictCount = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_BC)))
  val replayDCacheReplayCount = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_DR)))
  val replayForwardFailCount  = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_FF)))
  val replayDCacheMissCount   = PopCount(io.enq.map(enq => enq.fire && !enq.bits.isLoadReplay && enq.bits.cause(ReplayCauseNO.C_DM)))
