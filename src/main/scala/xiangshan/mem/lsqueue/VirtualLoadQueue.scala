...
class VirtualLoadQueue(implicit p: Parameters) extends XSModule
  ...
  val allocated =
    RegInit(
      VecInit(List.fill(VirtualLoadQueueSize)(false.B))
  ...
  val validVLoadFlow = vLoadFlow.zipWithIndex.map { case (vLoadFlowNumItem, index) =>
    Mux(canEnqueue(index), vLoadFlowNumItem, 0.U)
  }
  val validVLoadOffset = vLoadFlow.zip(io.enq.needAlloc).map { case (flow, needAllocItem) =>
  ...
  val deqLookup = VecInit(deqLookupVec.map(ptr =>
    allocated(ptr.value)
      && ((datavalid(ptr.value) && addrvalid(ptr.value) && !isvec(ptr.value)) || (isvec(ptr.value) && veccommitted(
        ptr.value
      )))
      && ptr =/= enqPtrExt(0)
  ))
  ...
  for (i <- 0 until VirtualLoadQueueSize) {
    ...
    for (j <- 0 until VecLoadPipelineWidth) {
      vecLdCommittmp(i)(j) := allocated(i) && cmt(j).valid && uop(i).robIdx === cmt(j).bits.robidx && uop(
        i
      ).uopIdx === cmt(j).bits.uopidx
    }
