...
class LoadQueueRAR(implicit p: Parameters) extends XSModule
  ...
  val allocated =
    RegInit(
      VecInit(List.fill(LoadQueueRARSize)(false.B))
  ...
  (0 until LoadQueueRARSize).map { i =>
    val bypassMatch = VecInit((0 until LoadPipelineWidth).map(j =>
      lastCanAccept(j) && lastAllocIndexOH(j)(i) && lastReleasePAddrMatch(j)
    )).asUInt.orR
    when(
      RegNext((paddrModule.io.releaseMmask.takeRight(1)(0)(i) || bypassMatch) && allocated(i) && release1Cycle.valid)
    ) {
