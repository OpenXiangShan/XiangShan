  ...
  with HasLoadHelper
{
  ...
  val writeback = Mux(req.isNoncacheable, io.ncOut.fire, io.mmioOut.fire)
  ...
  io.uncache.req.bits.nc := req.isNoncacheable
  ...
  when(req.isNoncacheable){
    ...
    io.ncOut.bits.isNoncacheable := true.B
    io.ncOut.bits.mask := Mux(req.paddr(3), req.mask(15, 8), req.mask(7, 0))
    io.ncOut.bits.schedIdx := req.schedIdx
    io.ncOut.bits.isVector := req.isVector
  ...
  with HasMemBlockParameters
{
  ...
  for (w <- 0 until LoadPipelineWidth) {
    s2_enqueue(w) := s2_valid(w) && !s2_has_exception(w) && !s2_need_replay(w) && (s2_req(w).mmio || s2_req(w).isNoncacheable)
  }
