  ...
  with HasLoadHelper
{
  val io = IO(new Bundle() {
    ...
    // to ldu: mmio/nc data
    val forward = (new ForwardQueryIO)
    // <=> uncache
  ...
  with HasMemBlockParameters
{
  ...
  for (w <- 0 until LoadPipelineWidth) {
    ...
    entries.zipWithIndex.foreach {
      case (e, i) =>
        val ncMatch = e.io.ncOut.valid &&
                      io.forward(w).req.lduStage0ToLq.valid &&
                      io.forward(w).req.lduStage0ToLq.bits.isnc &&
                      (e.io.ncOut.bits.uop.lqIdx === io.forward(w).req.lduStage0ToLq.bits.lqIdx)
        
        val mmioMatch = e.io.mmioOut.valid &&
                        io.forward(w).req.lduStage0ToLq.valid &&
                        !io.forward(w).req.lduStage0ToLq.bits.isnc &&
                        (e.io.mmioOut.bits.uop.lqIdx === io.forward(w).req.lduStage0ToLq.bits.lqIdx)
    ...
    io.forward(w).resp.lqToLduStage1.valid := RegNext(hasMatch)
    io.forward(w).resp.lqToLduStage1.bits.paddr := RegEnable(matchedPaddr, hasMatch)
    io.forward(w).resp.lqToLduStage2.valid := RegNext(RegNext(hasMatch))
    io.forward(w).resp.lqToLduStage2.bits.forwardData := RegNext(RegEnable(matchedData, hasMatch))
    io.forward(w).resp.lqToLduStage2.bits.nderr := RegNext(RegEnable(matchednderr, hasMatch))
    io.forward(w).resp.lqToLduStage2.bits.addrOffset := RegNext(RegEnable(matchedAddrOffset, hasMatch))
  }
