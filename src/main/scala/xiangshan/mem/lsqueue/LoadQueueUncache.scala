  ...
  with HasMemBlockParameters
{
  val io = IO(new Bundle() {
    ...
    // to LRQ
    val uncacheReplay = Output(Vec(LoadUncacheBufferSize, Valid(new UncacheReplay)))
    // <=>uncache
    val uncache = new UncacheWordIO
    // writeback: to ldu s1,s2: mmio/nc data
  ...
  for (w <- 0 until LoadPipelineWidth) {
    io.uncacheReplay(w).valid := false.B
    io.uncacheReplay(w).bits := DontCare
  }
  ...
  // TODO: default value
  val uncacheReplay = Wire(Vec(LoadUncacheBufferSize, Valid(new UncacheReplay)))
  ...
  for (i <- 0 until LoadUncacheBufferSize) {
    ...
    uncacheReplay(i).valid := false.B
    uncacheReplay(i).bits := DontCare
  }
  ...
  entries.zipWithIndex.foreach {
    case (e, i) =>
      ...
      when (e.io.mmioSelect) {
        ...
        uncacheReplay(i).valid := e.io.mmioOut.valid
        uncacheReplay(i).bits.lqIdx := e.io.mmioOut.bits.uop.lqIdx
      }.otherwise{
        ...
        uncacheReplay(i).valid := e.io.ncOut.valid
        uncacheReplay(i).bits.lqIdx := e.io.ncOut.bits.uop.lqIdx
      }
  ...
  // uncache Replay
  io.uncacheReplay := uncacheReplay
   ...
   * Forward Logic
   * 
   * s1 response paddr, s2 response data
   ******************************************************************/
  ...
  for (w <- 0 until LoadPipelineWidth) {
    ...
    entries.zipWithIndex.foreach {
      case (e, i) =>
        val ncMatch = e.io.ncOut.valid &&
                      io.bypass(w).s0Req.valid &&
                      io.bypass(w).s0Req.bits.isNCReplay &&
        ...
        when (entryMatch) {
          ...
          when (ncMatch) {
            ...
            matchednderr := e.io.ncOut.bits.uop.exceptionVec(hardwareError)
            e.io.ncOut.ready := true.B
          }.otherwise { // mmioMatch
            matchedPaddr := e.io.mmioOut.bits.debug.paddr
            matchedData := e.io.mmioRawData.lqData
            matchednderr := e.io.mmioOut.bits.uop.exceptionVec(hardwareError)
    ...
    io.bypass(w).s1Resp.valid := RegNext(hasMatch)
    io.bypass(w).s1Resp.bits.paddr := RegEnable(matchedPaddr, hasMatch)
    io.bypass(w).s2Resp.valid := RegNext(RegNext(hasMatch))
    io.bypass(w).s2Resp.bits.data := RegNext(RegEnable(matchedData, hasMatch))
    io.bypass(w).s2Resp.bits.nderr := RegNext(RegEnable(matchednderr, hasMatch))
    io.bypass(w).s2Resp.bits.addrOffset := RegNext(RegEnable(matchedAddrOffset, hasMatch))
  }
