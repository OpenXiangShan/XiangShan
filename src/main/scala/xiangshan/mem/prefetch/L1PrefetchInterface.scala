...
trait HasL1PrefetchSourceParameter {
  ...
  // V0.5 adds detailed MDP source attribution. Four bits are required because
  // the original 0..7 encoding was already full.
  def L1PfSourceBits = 4
  // Stream 0, Stride 1
  ...
  def L1_HW_PREFETCH_MDP_STREAM = 8.U
  def L1_HW_PREFETCH_MDP_CHASING_STRIDE = 9.U
  def L1_HW_PREFETCH_MDP_CHASING_STREAM = 10.U
  def L1_HW_PREFETCH_MDP_CHASING_CHAIN = 11.U
  // Reserved for todo item 4. The V0.5 iteration intentionally does not
  // generate history chasing requests.
  def L1_HW_PREFETCH_MDP_CHASING_HISTORY = 12.U
  ...
  def isFromMdpLegacyChasing(value: UInt) = value === L1_HW_PREFETCH_MDP_CHASING
  def isFromMdpStride(value: UInt) = value === L1_HW_PREFETCH_MDP_STRIDE
  def isFromMdpStream(value: UInt) = value === L1_HW_PREFETCH_MDP_STREAM
  def isFromMdpChasingStride(value: UInt) = value === L1_HW_PREFETCH_MDP_CHASING_STRIDE
  def isFromMdpChasingStream(value: UInt) = value === L1_HW_PREFETCH_MDP_CHASING_STREAM
  def isFromMdpChasingChain(value: UInt) = value === L1_HW_PREFETCH_MDP_CHASING_CHAIN
  def isFromMdpChasingHistory(value: UInt) = value === L1_HW_PREFETCH_MDP_CHASING_HISTORY
  def isFromMdpChasing(value: UInt) =
    isFromMdpLegacyChasing(value) || isFromMdpChasingStride(value) ||
      isFromMdpChasingStream(value) || isFromMdpChasingChain(value) ||
      isFromMdpChasingHistory(value)
  def isFromMDP(value: UInt) = isFromMdpStride(value) || isFromMdpStream(value) || isFromMdpChasing(value)
...
class L1PrefetchReq(implicit p: Parameters) extends XSBundle with HasDCacheParameters {
  ...
  // BaseChain metadata. mdpOrigin classifies the chasing request generated by
  // this hinted load as stride, stream, chain, or the reserved history class.
