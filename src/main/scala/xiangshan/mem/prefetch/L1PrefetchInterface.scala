...
trait HasL1PrefetchSourceParameter {
  ...
  // MDP sources are split so PrefetcherMonitor can report chasingPf and
  // stridePf accuracy/coverage/timeliness independently.
  def L1_HW_PREFETCH_MDP_CHASING = 6.U
  def L1_HW_PREFETCH_MDP_STRIDE = 7.U
  ...
  def isFromMdpChasing(value: UInt) = value === L1_HW_PREFETCH_MDP_CHASING
  def isFromMdpStride(value: UInt) = value === L1_HW_PREFETCH_MDP_STRIDE
  def isFromMDP(value: UInt) = isFromMdpChasing(value) || isFromMdpStride(value)
...
class L1PrefetchReq(implicit p: Parameters) extends XSBundle with HasDCacheParameters {
  ...
  // MDP writes this metadata, PrefetcherWrapper/LDU/LoadPipe forward it, and a
  // missing stridePf stores it in MissQueue until refill emits chasingPf.
  val mdpPfHint = Bool()
  val mdpImm = UInt(12.W)
  val mdpVaddr = UInt(VAddrBits.W) // exact (not cache-line-aligned) load address
