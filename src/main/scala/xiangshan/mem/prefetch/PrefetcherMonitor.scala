...
class PrefetcherMonitor()(implicit p: Parameters) extends XSModule with HasPrefetcherMonitorHelper with HasStreamPrefetchHelper {
  ...
  depth_const := Constantin.createRecord("depth", initValue = 32.U)
  ...
  val enableDynamicPrefetcher_const = WireInit(Constantin.createRecord("enableDynamicPrefetcher", initValue = 1.U))
