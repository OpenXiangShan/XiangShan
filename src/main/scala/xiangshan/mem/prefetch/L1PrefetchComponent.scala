...
class L1PrefetchRetryArbiter(
  ...
  val arb = Module(new Arbiter(new L1PrefetchReq, sourceCount))
  io.in.zip(arb.io.in).foreach { case (source, sink) =>
    sink.valid := source.valid && !io.mshrFull
    sink.bits := source.bits
    source.ready := sink.ready && !io.mshrFull
  }
  io.out <> arb.io.out

  XSPerfAccumulate("l1PfThrottleMshrFullCycles", io.mshrFull)
  XSPerfAccumulate("l1PfThrottleBlockCycles", io.mshrFull && io.in.map(_.valid).reduce(_ || _))

  when(io.mshrFull) { io.in.foreach(source => assert(!source.fire)) }
}
