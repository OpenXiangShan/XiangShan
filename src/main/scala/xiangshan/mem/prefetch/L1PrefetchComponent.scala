...
trait HasTrainFilterHelper extends HasCircularQueuePtrHelper {
  def reorder[T <: LdPrefetchTrainBundle](source: Vec[ValidIO[T]]): Vec[ValidIO[T]] =
    if (source.length == 1) {
      ...
      val source_1_older =
        Mux(Cat(source_v).andR, isBefore(source(1).bits.uop.robIdx, source(0).bits.uop.robIdx), false.B)
      when(source_1_older) {
        res(0) := source(1)
        res(1) := source(0)
      }.otherwise {
...
class TrainFilter(size: Int, name: String)(implicit p: Parameters) extends XSModule with HasL1PrefetchHelper
  ...
  for (i <- 0 until enqLen) {
    ...
    val prev_enq_match = if (i == 0) false.B
    else Cat(reqs_l.zip(reqs_vl).take(i).map {
...
class MutiLevelPrefetchFilter(implicit p: Parameters) extends XSModule with HasL1PrefetchHelper {
  ...
  val l1_pf_req_arb = Module(new RRArbiterInit(
    new Bundle {
  ...
  val l2_pf_req_arb = Module(new RRArbiterInit(
    new Bundle {
  ...
  val l2_real_replace_vec =
    Mux(Cat(l2_opt_replace_vec).orR, l2_opt_replace_vec, VecInit(Seq.fill(MLP_L2L3_SIZE)(true.B)))
  ...
  s0_l1_index := Mux(
    s0_l1_hit,
    OHToUInt(VecInit(s0_l1_match_vec).asUInt),
  ...
  XSPerfAccumulate(
    "l1_hash_conflict",
    s0_l1_valid && RegNext(s1_l1_valid) && (s0_l1_region =/= RegNext(s1_l1_region)) && (s0_l1_region_hash === RegNext(
      s1_l1_region_hash
    ))
  )
  XSPerfAccumulate(
    "s1_l1_enq_evict_useful_entry",
  ...
  XSPerfAccumulate(
    "l2_hash_conflict",
    s0_l2_valid && RegNext(s1_l2_valid) && (s0_l2_region =/= RegNext(s1_l2_region)) && (s0_l2_region_hash === RegNext(
      s1_l2_region_hash
    ))
  )
  XSPerfAccumulate(
  ...
  val stream_pf_trace_debug_table = ChiselDB.createTable(
    "StreamPFTrace" + p(XSCoreParamsKey).HartId.toString,
    new StreamPFTraceInEntry,
    basicDB = false
  )
  ...
  for (i <- 0 until MLP_SIZE) {
    ...
    if (i < MLP_L1_SIZE) {
      tlb_req_arb.io.in(i).valid := l1_valids(i) && l1_array(i).is_vaddr && !s1_tlb_fire_vec(i) && !s2_tlb_fire_vec(
        i
      ) && !l1_evict
      tlb_req_arb.io.in(i).bits.vaddr := l1_array(i).get_tlb_va()
    } else {
      tlb_req_arb.io.in(i).valid := l2_valids(i - MLP_L1_SIZE) && l2_array(
        i - MLP_L1_SIZE
      ).is_vaddr && !s1_tlb_fire_vec(i) && !s2_tlb_fire_vec(i) && !l2_evict
  ...
  when(s2_tlb_resp.valid && !s2_tlb_evict) {
    when(s2_tlb_update_index < MLP_L1_SIZE.U) {
      ...
      when(!s2_tlb_resp.bits.miss) {
        l1_array(s2_tlb_update_index).region := Cat(
          0.U((VAddrBits - PAddrBits).W),
          s2_tlb_resp.bits.paddr.head(s2_tlb_resp.bits.paddr.head.getWidth - 1, REGION_TAG_OFFSET)
        )
        when(
          s2_tlb_resp.bits.excp.head.pf.ld || s2_tlb_resp.bits.excp.head.gpf.ld || s2_tlb_resp.bits.excp.head.af.ld
        ) {
      ...
      when(!s2_tlb_resp.bits.miss) {
        l2_array(inner_index).region := Cat(
          0.U((VAddrBits - PAddrBits).W),
          s2_tlb_resp.bits.paddr.head(s2_tlb_resp.bits.paddr.head.getWidth - 1, REGION_TAG_OFFSET)
        )
        when(
  ...
  XSPerfAccumulate(
    "s2_tlb_resp_page_fault",
    s2_tlb_resp.valid && !s2_tlb_evict && !s2_tlb_resp.bits.miss && s2_tlb_resp.bits.excp.head.pf.ld
  )
  XSPerfAccumulate(
    "s2_tlb_resp_guestpage_fault",
    s2_tlb_resp.valid && !s2_tlb_evict && !s2_tlb_resp.bits.miss && s2_tlb_resp.bits.excp.head.gpf.ld
  )
  XSPerfAccumulate(
    "s2_tlb_resp_access_fault",
    s2_tlb_resp.valid && !s2_tlb_evict && !s2_tlb_resp.bits.miss && s2_tlb_resp.bits.excp.head.af.ld
  )
  ...
  assert(
    !((s1_l1_alloc || s1_l1_update) && s1_pf_fire && (s1_l1_index === s1_pf_index)),
    "pf pipeline & enq pipeline bit_vec harzard!"
  )
  ...
  for (i <- 0 until MLP_L2L3_SIZE) {
    ...
    l2_pf_req_arb.io.in(i).bits.req.source := MuxLookup(l2_array(i).source.value, MemReqSource.Prefetch2L2Unknown.id.U)(
      Seq(
  ...
  when(l2_pf_req_arb.io.out.valid) {
    l2_array(l2_pf_req_arb.io.chosen).sent_vec := l2_array(l2_pf_req_arb.io.chosen).sent_vec | get_candidate_oh(
      l2_pf_req_arb.io.out.bits.req.addr
    )
  }

  val stream_out_debug_table = ChiselDB.createTable(
    "StreamPFTraceOut" + p(XSCoreParamsKey).HartId.toString,
    new StreamPFTraceOutEntry,
    basicDB = false
  )
  ...
  when(l3_pf_req_arb.io.out.valid) {
    l2_array(l3_pf_req_arb.io.chosen).sent_vec := l2_array(l3_pf_req_arb.io.chosen).sent_vec | get_candidate_oh(
      l3_pf_req_arb.io.out.bits
    )
  }
  ...
  XSPerfHistogram(
    "l3_filter_active",
    PopCount(VecInit(l2_array.zip(l2_valids).map { case (e, v) => e.can_send_pf(v) && (e.sink === SINK_L3) }).asUInt),
    true.B,
    0,
    MLP_L2L3_SIZE,
    1
...
class L1Prefetcher(implicit p: Parameters) extends BasePrefecher with HasStreamPrefetchHelper
  ...
  val l2_in_pmem =
    PmemRanges.map(range => pf_queue_filter.io.l2_pf_addr.bits.addr.inRange(range._1.U, range._2.U)).reduce(_ || _)
  ...
  val l3_in_pmem =
    PmemRanges.map(range => pf_queue_filter.io.l3_pf_addr.bits.inRange(range._1.U, range._2.U)).reduce(_ || _)
