...
class StrideMetaArray(implicit p: Parameters) extends XSModule with HasStridePrefetchHelper {
  ...
  val always_update =
    Constantin.createRecord(s"always_update${p(XSCoreParamsKey).HartId}", initValue = ALWAYS_UPDATE_PRE_VADDR)
  ...
  // s3: send l1 pf out
  val s3_valid =
    if (LOOK_UP_STREAM) GatedValidRegNext(s2_valid) && !io.stream_lookup_resp else GatedValidRegNext(s2_valid)
  ...
  XSPerfHistogram(
    "high_conf_num",
    PopCount(VecInit(array.map(_.confidence === MAX_CONF.U))).asUInt,
    true.B,
    0,
    STRIDE_ENTRY_NUM,
    1
