...
class CounterFilter()(implicit p: Parameters) extends DCacheModule {
  ...
  for (i <- 0 until enqLen) {
    ...
    assert(
      !needAlloc(i) || canAlloc(i),
      s"port${i} can not accept CounterFilter enq request, check if SIZE >= (Ldu stages - 2) * LduCnt"
    )
  }
...
class FDPrefetcherMonitor()(implicit p: Parameters) extends XSModule {
  ...
  val prev_cnts = Seq(
    total_prefetch_prev_cnt,
    useful_prefetch_prev_cnt,
    late_prefetch_prev_cnt,
    demand_miss_prev_cnt,
    pollution_prev_cnt
  )
  ...
  val interval_cnts = Seq(
    total_prefetch_interval_cnt,
    useful_prefetch_interval_cnt,
    late_prefetch_interval_cnt,
    demand_miss_interval_cnt,
    pollution_interval_cnt
  )

  val interval_trigger = refill_cnt === INTERVAL.U

  val io_ens = Seq(
    io.accuracy.total_prefetch,
    io.accuracy.useful_prefetch,
    io.timely.late_prefetch,
    io.pollution.demand_miss,
    io.pollution.cache_pollution
  )
  ...
  XSPerfRolling(
    ...
    PopCount(io.accuracy.useful_prefetch),
    PopCount(io.accuracy.total_prefetch),
    1000,
    io.debugRolling.robTrueCommit,
    clock,
    reset
  )
  ...
  XSPerfRolling(
    ...
    PopCount(io.timely.late_prefetch),
    PopCount(io.accuracy.total_prefetch),
    1000,
    io.debugRolling.robTrueCommit,
    clock,
  ...
  XSPerfRolling(
    ...
    PopCount(io.pollution.cache_pollution),
