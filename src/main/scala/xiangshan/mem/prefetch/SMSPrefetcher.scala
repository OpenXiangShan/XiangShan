...
class PhtEntry()(implicit p: Parameters) extends XSBundle with HasSMSModuleHelper {
  ...
  val region_offset = UInt(REGION_BLKS.W)
  val decr_mode = Bool()
}
...
class PatternHistoryTable()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s2_region_offset_update = s2_hit_region_offset | UIntToOH(s2_region_offset)
  val s2_ram_region_offset = Mux(s2_pht_hit, s2_region_offset_update, UIntToOH(s2_region_offset))
  ...
  val s3_offset_match = s3_hit_region_offset(s3_region_offset).asBool
