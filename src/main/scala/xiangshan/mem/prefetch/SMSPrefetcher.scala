...
trait HasSMSModuleHelper extends HasCircularQueuePtrHelper with HasDCacheParameters { this: HasXSParameter =>
...
class StridePF()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s2_pf_block_addr = Mux(
    s2_pf_gen_paddr_valid,
    Cat(
...
class ActiveGenerationTable()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s0_pf_gen_match_vec = valids.indices.map { i =>
    Mux(
      ...
      Mux(any_region_m1_match, region_m1_match_vec_s0(i), region_p1_match_vec_s0(i))
    )
  }
  ...
  for (i <- entries.indices) {
    ...
    update_entry.access_cnt := Mux(
      entries(i).access_cnt === (REGION_BLKS - 1).U,
    ...
    update_entry.has_been_signal_updated := entries(
      i
    ).has_been_signal_updated || (!((s1_alloc || s1_do_dcache_evict) && s1_evict_valid)) && s1_update
  ...
  val s1_pf_gen_decr_mode = Mux(s1_update, s1_hited_entry_decr, s1_agt_entry.decr_mode)
  ...
  val s1_pf_gen_offset =
    Mux(s1_pf_gen_decr_mode, s1_pf_gen_vaddr_dec(REGION_OFFSET - 1, 0), s1_pf_gen_vaddr_inc(REGION_OFFSET - 1, 0))
  ...
  val s1_pf_gen_valid = prev_lookup_valid && (s1_alloc && s1_cross_region_match || s1_update) && Mux(
    s1_pf_gen_decr_mode,
  ...
  val s1_pf_gen_region_addr = Mux(
    s1_pf_gen_paddr_valid,
  ...
  val s1_pf_gen_region_bits = Mux(s1_pf_gen_decr_mode, s1_pf_gen_decr_region_bits, s1_pf_gen_incr_region_bits)
  ...
  XSPerfAccumulate("sms_agt_pf_gen_paddr_valid", io.s2_pf_gen_req.valid && io.s2_pf_gen_req.bits.paddr_valid)
  XSPerfAccumulate("sms_agt_pf_gen_decr_mode", io.s2_pf_gen_req.valid && io.s2_pf_gen_req.bits.decr_mode)
  for (i <- 0 until smsParams.active_gen_table_size) {
    XSPerfAccumulate(s"sms_agt_access_entry_$i", s1_alloc && s1_replace_mask(i) || s1_update && s1_update_mask(i))
  }
...
class PatternHistoryTable()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val pht_ram = Module(new SRAMTemplate[PhtEntry](
    new PhtEntry,
  ...
  val s0_ram_raddr              = Mux(evict.valid, evict.bits.pht_index, lookup.bits.pht_index)
  ...
  val s1_pht_valids = pht_valids_reg.map(way =>
    Mux1H(
  ...
  val signal_update_write =
    Mux(!s2_single_update, true.B, s2_pht_hit || s2_single_update && (s2_access_cnt_signal > 4.U))
  ...
  val s3_way_mask = Mux(s3_hit, VecInit(s3_hit_vec).asUInt, s3_repl_way_mask).asUInt
...
class PrefetchFilter()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  assert(
    !io.tlb_req.resp.fire || Cat(s2_tlb_fire_vec).orR,
...
class SMSTrainFilter()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper with HasTrainFilterHelper {
  ...
  class Ptr(implicit p: Parameters) extends CircularQueuePtr[Ptr](p => smsParams.train_filter_size) {}
  ...
  for (i <- 0 until enqLen) {
    ...
    else Cat(reqs_ls.zip(reqs_vls).take(i).map {
...
class SMSPrefetcher()(implicit p: Parameters) extends BasePrefecher with HasSMSModuleHelper
  ...
  stride.io.s0_lookup.bits.vaddr := Cat(
    train_region_vaddr_s0,
    train_region_offset_s0,
    0.U(log2Up(dcacheParameters.blockBytes).W)
  )
  stride.io.s0_lookup.bits.paddr := Cat(
    train_region_paddr_s0,
    train_region_offset_s0,
    0.U(log2Up(dcacheParameters.blockBytes).W)
  )
  ...
  val pf_gen_req = Mux(
    agt_gen_valid || stride_gen_valid,
    Mux1H(Seq(
  ...
  val is_valid_address =
    PmemRanges.map(range => pf_filter.io.l2_pf_addr.bits.inRange(range._1.U, range._2.U)).reduce(_ || _)
  ...
  XSPerfAccumulate("sms_pf_gen_conflict", pht_gen_valid && agt_gen_valid)
