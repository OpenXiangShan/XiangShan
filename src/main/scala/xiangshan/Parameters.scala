...
(
  ...
  val intExuConfigs = (Seq.fill(exuParameters.AluCnt)(AluExeUnitCfg) ++
    ...
    Seq.fill(exuParameters.VduCnt)(VduExeUnitCfg)
...
trait HasXSParameter {
  ...
  val NRIntWritePorts = exuParameters.AluCnt + exuParameters.MduCnt + exuParameters.LduCnt + exuParameters.VduCnt
  ...
  val NumRs = (exuParameters.JmpCnt+1)/2 + (exuParameters.AluCnt+1)/2 + (exuParameters.MulCnt+1)/2 +
              ...
              ((exuParameters.StuCnt+1)/2) + ((exuParameters.StuCnt+1)/2) + ((exuParameters.VduCnt+1)/2)
