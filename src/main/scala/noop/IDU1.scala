...
class IDU1 extends NOOPModule with HasInstrType {
  ...
  val redirectPC = Cat(io.in.bits.pc(AddrBits-1,3), 0.U(3.W))+"b1010".U // IDU can got get full inst from a single inst line  
  ...
  when(!io.flush){
    switch(state){
      ...
      is(s_extra){//get 16 aligned inst, pc controled by this FSM
        canGo := rvcFinish || rvcNext
        canIn := rvcFinish || rvcForceLoadNext
        pcOut := Cat(io.in.bits.pc(AddrBits-1,3), pcOffsetR(2,0)) 
