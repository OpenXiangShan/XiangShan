...
sealed class CacheStage2(ro: Boolean, name: String, userBits: Int = 0) extends CacheModule {
  ...
  Debug(){
    printf("[L1$] " +name+" stage 2: addr %x, io.in.valid: %x, io.in.ready: %x, io.out.valid: %x, io.out.ready: %x\n", req.addr, io.in.valid, io.in.ready, io.out.valid, io.out.ready)
  }
...
sealed class CacheStage3(ro: Boolean, name: String, userBits: Int = 0) extends CacheModule {
  ...
  Debug(){
    when(io.mem.req.valid && io.mem.req.ready){
      printf("[L1$] " +name+" mem access addr: %x\n", io.mem.req.bits.addr)
    }
  }
