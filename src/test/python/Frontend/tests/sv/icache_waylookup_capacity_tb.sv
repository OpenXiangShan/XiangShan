...
module icache_waylookup_capacity_tb;
  ...
  initial begin
    ...
    io_toMainPipe_ready = 1;
    #1;
    if (!io_toMainPipe_valid) $fatal(1, "31-entry queue did not allow read");
    if (io_write_0_ready) $fatal(1, "same-cycle read leaked capacity to dual write");
    tick(); check_count(30);
    io_toMainPipe_ready = 0;
    io_write_0_valid = 0;
    io_write_1_valid = 0;
    $display("PASS read_write_boundary_blocks_dual_at_31");
    ...
    io_toMainPipe_ready = 0;
    check_count(31);
    $display("PASS push_pop_same_cycle_at_30");
    repeat (31) pop(0);
    check_count(0);
