...
def test_frontend_fixture_has_one_funcov_path_and_keeps_code_coverage(tmp_path):
    ...
    assert len(recorder.definitions) == 240
...
def test_backannotation_can_skip_testpoint_structure_errors(tmp_path, capsys):
    ...
    with pytest.raises(ValueError, match="leaf must bind exactly one group/point/bin"):
    ...
    assert "line 3: leaf must bind exactly one group/point/bin" in capsys.readouterr().err
    with testpoint_path.open(encoding="utf-8-sig", newline="") as handle:
