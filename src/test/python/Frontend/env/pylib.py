from __future__ import annotations

import os
from pathlib import Path


def frontend_pylib_path() -> Path:
    override = os.getenv("TB_FRONTEND_PYLIB", "").strip()
    if override:
        return Path(override).expanduser()

    sim = os.getenv("TB_FRONTEND_SIM", "verilator").strip().lower()
    if sim not in {"verilator", "vcs"}:
        raise RuntimeError("TB_FRONTEND_SIM must be one of: verilator vcs")

    repo_root = Path(__file__).resolve().parents[5]
    build_root = repo_root / "build-frontend"
    simulator_pylib = build_root / f"pylib-{sim}"
    legacy_pylib = build_root / "pylib"
    # New Makefile targets always populate pylib-<sim>.  This fallback only
    # permits reading an already-built legacy DUT while the migration is in
    # progress; it never changes the output location of a new build.
    if sim == "verilator" and not simulator_pylib.exists() and legacy_pylib.exists():
        return legacy_pylib
    return simulator_pylib


def frontend_offset_path() -> Path:
    return frontend_pylib_path() / "Frontend" / "Frontend_offset.yaml"
