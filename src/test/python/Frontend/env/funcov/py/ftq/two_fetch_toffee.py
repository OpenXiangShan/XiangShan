...
def _point_definitions() -> dict[tuple[str, str], tuple[str, ...]]:
    ...
    for group_name, bin_name in sorted(TWO_FETCH_SAMPLER_BIN_KEYS):
        point_name = TWO_FETCH_COVERPOINTS[group_name]
        definitions.setdefault((group_name, point_name), []).append(bin_name)
    return {key: tuple(value) for key, value in definitions.items()}


class TwoFetchToffeeCoverage:
    ...
    def __init__(self, runtime, *, sink=None, audit_recorder=None) -> None:
        ...
        self._view = _TwoFetchCycleView(
            flags={key: False for key in TWO_FETCH_SAMPLER_BIN_KEYS}
        )
    ...
    def sample(self, cycle: int) -> None:
        flags, evidence = evaluate_two_fetch_coverage(self, self.env, int(cycle))
        self._view.flags = {
        ...
        if self._sink is not None:
            self._sink.record_native_hits(
                self._view.flags, TWO_FETCH_COVERPOINTS, cycle, evidence
            )
        if self._audit_recorder is not None:
            for group_name, bin_name in TWO_FETCH_SAMPLER_BIN_KEYS:
                ...
                self._audit_recorder.mark(
                    ...
                    cycle,
                    evidence,
                    coverpoint=TWO_FETCH_COVERPOINTS[group_name],
