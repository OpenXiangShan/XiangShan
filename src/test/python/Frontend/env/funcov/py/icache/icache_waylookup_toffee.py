...
class ICacheWaylookupToffeeCoverage:
    ...
    def __init__(self, runtime, *, sink=None, audit_recorder=None) -> None:
        ...
        self._view = _WaylookupCycleView(
    ...
    def on_cycle(self, cycle: int) -> None:
        ...
        if self._runtime._read_dut_signal(dut, "reset", 0) == 1:
            ...
            self._view.flags = {
                bin_name: False for _, bin_name in ICACHE_WAYLOOKUP_SAMPLER_BIN_KEYS
            }
            return
        self.sample(cycle)

    def sample(self, cycle: int) -> None:
        flags, evidence = evaluate_icache_waylookup_coverage(self, self.env, int(cycle))
        self._view.flags = {
            bin_name: bool(flags.get(bin_name, False))
            for _, bin_name in ICACHE_WAYLOOKUP_SAMPLER_BIN_KEYS
        }
            ...
            self._sink.record_native_hits(
                {
                    ...
                    for group_name, bin_name in ICACHE_WAYLOOKUP_SAMPLER_BIN_KEYS
                },
                ICACHE_WAYLOOKUP_COVERPOINTS,
                cycle,
                evidence,
        ...
        if self._audit_recorder is not None:
            for group_name, bin_name in ICACHE_WAYLOOKUP_SAMPLER_BIN_KEYS:
                ...
                self._audit_recorder.mark(
                    ...
                    cycle,
                    evidence,
                    coverpoint=ICACHE_WAYLOOKUP_COVERPOINTS[group_name],
