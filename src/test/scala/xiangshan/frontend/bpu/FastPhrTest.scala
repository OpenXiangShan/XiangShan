...
class FastPhrTest extends AnyFlatSpec {
  ...
  class TestModule(implicit p: Parameters) extends FastPhrModule with PhrHelper {
    class CtrlIO extends Bundle {
      ...
      val overrideFromS3:      Bool      = Input(Bool())
    ...
    fastPhr.io.overrideFromS3      := ctrl.overrideFromS3
  ...
  private def clearControls(dut: TestModule): Unit = {
    ...
    dut.ctrl.overrideFromS3.poke(false.B)
  ...
  // an override at either stage replays from the snapshot belonging to the group that stage holds,
  // so the same scenario is run twice with only the stage selector changed
  for (fromS3 <- Seq(true, false)) {
    val stage = if (fromS3) "s3" else "s2"
    it should s"restore the snapshot belonging to the group in $stage and replay a corrected group on override" in {
      implicit val p: Parameters = config
      simulate(new TestModule) { dut =>
        dut.reset.poke(true.B)
        dut.clock.step(5)
        ...
        // from-scratch window built by re-advancing the stage's own snapshot, independent of
        // FastPhr's own incremental fold maintenance
        val correctedNumBlocks = Random.nextInt(3)
        val correctedToken     = BigInt(tokenWidth, Random)
        val expectedWindow     = advance(if (fromS3) snapS3 else snapS2, correctedNumBlocks, correctedToken)

        dut.ctrl.valid.poke(false.B)
        dut.ctrl.s2Fire.poke(false.B)
        dut.ctrl.overrideValid.poke(true.B)
        dut.ctrl.overrideFromS3.poke(fromS3.B)
