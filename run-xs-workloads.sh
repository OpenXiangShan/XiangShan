...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/coremark-riscv64-xs.bin" \
    >"$LOG_DIR/coremark-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/dhrystone-riscv64-xs.bin" \
    >"$LOG_DIR/dhrystone-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/hello-riscv64-xs.bin" \
    >"$LOG_DIR/hello-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/hpmdriver-riscv64-xs.bin" \
    >"$LOG_DIR/hpmdriver-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/litenes-riscv64-xs.bin" \
    >"$LOG_DIR/litenes-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/maprobe-riscv64-xs.bin" \
    >"$LOG_DIR/maprobe-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/mem_test_bw-riscv64-xs.bin" \
    >"$LOG_DIR/mem_test_bw-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/build-mem_test_latency--riscv64-xs.bin" \
    >"$LOG_DIR/build-mem_test_latency--riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/microbench-riscv64-xs.bin" \
    >"$LOG_DIR/microbench-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/rva-trigger-riscv64-xs.bin" \
    >"$LOG_DIR/rva-trigger-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/rvv-trigger-riscv64-xs.bin" \
    >"$LOG_DIR/rvv-trigger-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/slider-riscv64-xs.bin" \
    >"$LOG_DIR/slider-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/stream-riscv64-xs.bin" \
    >"$LOG_DIR/stream-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/typing-riscv64-xs.bin" \
    >"$LOG_DIR/typing-riscv64-xs.log" 2>&1; then
...
if cd "$NOOP_HOME" &&
  ./build/emu -i "$WORKLOAD_DIR/zcb-test-riscv64-xs.bin" \
    >"$LOG_DIR/zcb-test-riscv64-xs.log" 2>&1; then
