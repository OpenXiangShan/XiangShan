...
//=== UPDATE {{{
    ...
    always@(*) begin
        mtlb_refill_tag_ori     = 'd0;
        mtlb_refill_itag_ori    = 'd0;
        mtlb_refill_content_ori = 'd0;
        for(int unsigned ori=0; ori<MICRO_TLB_DEPTH; ori++) begin
            if(plru_idx[ori]==1'b1) begin
                mtlb_refill_tag_ori     = entry_hit_tag_o[ori];
                mtlb_refill_itag_ori    = entry_inv_tag_o[ori];
                mtlb_refill_content_ori = entry_content_o[ori];
            end
        end
    end
// check if any entry hit with this original swap-out entry, but lvl higher than it
genvar sout;
generate
    for(sout=0; sout<MICRO_TLB_DEPTH; sout++) begin : swap_out_hit_gen
        if(CACHE_TYPE==0) begin : ddtc_tag_hit_gen
            ...
            /*input  logic [23:0]               */  .device_id_i                (mtlb_refill_tag_ori.device_id  ),
            /*input  logic [19:0]               */  .process_id_i               (20'd0                          ),
            /*input  logic                      */  .valid_i                    (entry_valid_o[sout]            ),
            /*input  logic [23:0]               */  .hit_tag_device_id          (entry_hit_tag_o[sout].device_id),
            /*input  logic [19:0]               */  .hit_tag_process_id         (20'd0                          ),
            /*input  logic [1:0]                */  .content_lvl                (entry_content_o[sout].lvl      ),
            /*output logic                      */  .tag_hit                    (swap_out_hit_list[sout]        ) 
            );
        end
            ...
            /*input  logic [23:0]               */  .device_id_i                (mtlb_refill_tag_ori.device_id  ),
            /*input  logic [19:0]               */  .process_id_i               (mtlb_refill_tag_ori.process_id ),
            /*input  logic                      */  .valid_i                    (entry_valid_o[sout]            ),
            /*input  logic [23:0]               */  .hit_tag_device_id          (entry_hit_tag_o[sout].device_id),
            /*input  logic [19:0]               */  .hit_tag_process_id         (entry_hit_tag_o[sout].process_id),
            /*input  logic [1:0]                */  .content_lvl                (entry_content_o[sout].lvl      ),
            /*output logic                      */  .tag_hit                    (swap_out_hit_list[sout]        ) 
            );
        end
...
endgenerate
    ...
    always@(*) begin
        swap_out_hit_list_l[0] = 'd0;
        for(int sohl0=0; sohl0<MICRO_TLB_DEPTH; sohl0++) begin
            if(swap_out_hit_list[sohl0] & entry_content_o[sohl0].lvl=='d0)
                swap_out_hit_list_l[0][sohl0] = 1'b1;
        end
    end

    always@(*) begin
        swap_out_hit_list_l[1] = 'd0;
        for(int sohl1=0; sohl1<MICRO_TLB_DEPTH; sohl1++) begin
            if(swap_out_hit_list[sohl1] & entry_content_o[sohl1].lvl=='d1)
                swap_out_hit_list_l[1][sohl1] = 1'b1;
        end
    end

    always@(*) begin
        swap_out_hit_list_l[2] = 'd0;
        for(int sohl2=0; sohl2<MICRO_TLB_DEPTH; sohl2++) begin
            if(swap_out_hit_list[sohl2] & entry_content_o[sohl2].lvl=='d2)
                swap_out_hit_list_l[2][sohl2] = 1'b1;
        end
    end

    always@(*) begin
        plru_idx_lvl_masked = plru_idx;
        if(all_entry_valid) begin
            if(swap_out_hit_list_l[2]!='d0) begin
                plru_idx_lvl_masked = swap_out_hit_list_l[2];
            end
            else if(swap_out_hit_list_l[1]!='d0) begin
                plru_idx_lvl_masked = swap_out_hit_list_l[1];
            end
            else if(swap_out_hit_list_l[0]!='d0) begin
                plru_idx_lvl_masked = swap_out_hit_list_l[0];
            end
            else
                plru_idx_lvl_masked = plru_idx;
        end
...
genvar eurh_masked;
generate
    for(eurh_masked=0; eurh_masked<MICRO_TLB_DEPTH; eurh_masked++) begin : entry_update_req_hit_o_lvl_maksed_gen
        assign entry_update_req_hit_o_lvl_masked[eurh_masked] = entry_update_req_hit_o[eurh_masked] & (entry_content_o[eurh_masked].lvl==update_req_muxed.lvl);
    end
endgenerate
...
//=== multi_hit {{{
    ...
    iommu_multi_bit_1_check#(MICRO_TLB_DEPTH) U_multi_hit_check(.in(lu_hit_masked), .ok(multi_hit_flag));
// in DDTC/PDTC may hit multi lvl entry, so no multi_fault gen anymore
//    assign multi_hit_fault_o = multi_hit_check_i ? multi_hit_flag : 0;
...
//=== LOOKUP_ACK & MAIN_TLB_LOOKUP{{{
    ...
    always@(*) begin
        lookup_ack_l[0]       = 'd0;
        lookup_ack_l_valid[0] = 'd0;
        for(int unsigned ll0=0; ll0<MICRO_TLB_DEPTH; ll0++) begin
            if(lu_hit_masked[ll0] & entry_lookup_ack_o[ll0].lvl=='d0) begin
                lookup_ack_l[0]       = entry_lookup_ack_o[ll0];
                lookup_ack_l_valid[0] = 1'b1;
            end
        end
    end
    ...
    always@(*) begin
        lookup_ack_l[1]       = 'd0;
        lookup_ack_l_valid[1] = 'd0;
        for(int unsigned ll1=0; ll1<MICRO_TLB_DEPTH; ll1++) begin
            if(lu_hit_masked[ll1] & entry_lookup_ack_o[ll1].lvl=='d1) begin
                lookup_ack_l[1]       = entry_lookup_ack_o[ll1];
                lookup_ack_l_valid[1] = 1'b1;
            end
        end
    end
    ...
    always@(*) begin
        lookup_ack_l[2]       = 'd0;
        lookup_ack_l_valid[2] = 'd0;
        for(int unsigned ll2=0; ll2<MICRO_TLB_DEPTH; ll2++) begin
            if(lu_hit_masked[ll2] & entry_lookup_ack_o[ll2].lvl=='d2) begin
                lookup_ack_l[2]       = entry_lookup_ack_o[ll2];
                lookup_ack_l_valid[2] = 1'b1;
            end
        end
    end
...
//            for(int unsigned kk=0; kk<MICRO_TLB_DEPTH; kk++) begin
            for(int unsigned kk=0; kk<=2; kk++) begin
//                if(lu_hit_masked[kk]) begin
                if(lookup_ack_l_valid[2-kk]) begin
//                    lookup_ack                      = entry_lookup_ack_o[kk]     ;
                    lookup_ack                      = lookup_ack_l[2-kk]     ;
...
// iommu_atd_ddtc_micro_tlb_entry
////////////////////////////////////////////////////////
module iommu_atd_ddtc_micro_tlb_entry #( //{{{
//{{{ PARAM
    parameter   CACHE_TYPE                  = 0, // 0:DDTC, 1:PDTC
    parameter type          INVALID_REQ_TYPE            = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
    parameter type          LOOKUP_REQ_TYPE             = iommu_atd_cache_pkg::ddtc_lookup_req_t,
