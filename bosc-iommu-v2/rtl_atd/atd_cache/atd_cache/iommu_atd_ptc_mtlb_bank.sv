...
////////////////////////////////////////////////////////////////////////////////
module iommu_atd_ptc_mtlb_bank_top #( //{{{
    parameter   CACHE_TYPE                                          = 0, // 0:S2PTC, 1:S1PTC
    parameter   BANK_TYPE                                           = 3'b00,
    ...
    parameter   BANK_IDX_WIDTH                                      = iommu_atd_cache_pkg::S2PTC_MAX_BANK_IDX_WIDTH,
    parameter   BANK_SET_IDX_WIDTH                                  = iommu_atd_cache_pkg::S2PTC_MAX_BANK_SET_IDX_WIDTH,
    parameter   BANK_WAY_IDX_WIDTH                                  = iommu_atd_cache_pkg::S2PTC_MAX_BANK_WAY_IDX_WIDTH,
...
generate
    for(i=0; i<BANK_NUM; i++) begin : bank_inst_gen
        ...
        iommu_atd_ptc_mtlb_bank #(
        /*parameter  */ .CACHE_TYPE                         (CACHE_TYPE                 ), // = 0, // 0:S2PTC, 1:S1PTC
        /*parameter  */ .BANK_TYPE                          (BANK_TYPE                  ), // = 3'b000,    // 00:l0  01:l1  10:l2
        ...
        /*parameter  */ .BANK_IDX_WIDTH                     (BANK_IDX_WIDTH             ), // = iommu_atd_cache_pkg::S2PTC_MAX_BANK_IDX_WIDTH,
        /*parameter  */ .BANK_SET_IDX_WIDTH                 (BANK_SET_IDX_WIDTH         ), // = iommu_atd_cache_pkg::S2PTC_MAX_BANK_SET_IDX_WIDTH,
        /*parameter  */ .BANK_WAY_IDX_WIDTH                 (BANK_WAY_IDX_WIDTH         ), // = iommu_atd_cache_pkg::S2PTC_MAX_BANK_WAY_IDX_WIDTH,
        /*parameter  */ .TECC_WIDTH                         (TECC_WIDTH                 ), // = 6,
        /*parameter  */ .DECC_WIDTH                         (DECC_WIDTH                 ), // = 7,
...
// iommu_atd_ptc_mtlb_bank
////////////////////////////////////////////////////////////////
module iommu_atd_ptc_mtlb_bank #( //{{{
    parameter   CACHE_TYPE                          = 0, // 0:S2PTC, 1:S1PTC
    parameter   BANK_TYPE                           = 3'b000,    // 00:l0  01:l1  10:l2
    parameter type          INVALID_REQ_TYPE                    = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
    parameter type          LOOKUP_REQ_TYPE                     = iommu_atd_cache_pkg::s2ptc_lookup_req_t,
    parameter type          LOOKUP_ACK_TYPE                     = iommu_atd_cache_pkg::s2ptc_lookup_ack_t,
    parameter type          UPDATE_REQ_TYPE                     = iommu_atd_cache_pkg::s2ptc_update_req_t,
...
generate //{{{
    for(i=0; i<BANK_WAY_NUM; i++) begin : way_gen
        if(TECC_WIDTH==0 | DECC_WIDTH==0) begin
            ...
            ///*uparameter int unsigned */ .ECC_WIDTH          (TECC_WIDTH              ), // = 8,
            ///*parameter  */ .SPARE_PARAM        (                       )  // = 0
            //) U_iram_ecc_enc(
...
//=== RD0 inst {{{
    ...
    iommu_atd_ptc_mtlb_bank_rd0 #(
    /*parameter  */ .BANK_TYPE                          (BANK_TYPE                          ), // = 3'b00,
    /*parameter type         */ .BANK_REQ_TYPE                      (bank_req_t                         ), // = iommu_atd_cache_pkg::s2ptc_bank_req_t,
    /*parameter type         */ .INVALID_REQ_TYPE                   (INVALID_REQ_TYPE                   ), // = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
    ...
    /*parameter type         */ .BANK_UPD_REQ_TYPE                  (BANK_UPD_REQ_TYPE                  ), // = iommu_atd_cache_pkg::s2ptc_bank_upd_req_t,
    /*parameter type         */ .BANK_UPD_ACK_TYPE                  (BANK_UPD_ACK_TYPE                  ), // = iommu_atd_cache_pkg::s2ptc_bank_upd_ack_t,
    /*parameter type         */ .BANK_INV_REQ_TYPE                  (BANK_INV_REQ_TYPE                  ), // = iommu_atd_cache_pkg::s2ptc_bank_inv_req_t,
    /*parameter type         */ .BANK_INV_ACK_TYPE                  (BANK_INV_ACK_TYPE                  ), // = iommu_atd_cache_pkg::s2ptc_bank_inv_ack_t,
...
endgenerate
    iommu_atd_ptc_mtlb_bank_match1 #(
    /*parameter  */ .CACHE_TYPE                         (CACHE_TYPE                         ), // = 0, // 0:S2PTC, 1:S1PTC
    /*parameter  */ .BANK_TYPE                          (BANK_TYPE                          ), // = 3'b00,
    /*parameter type         */ .INVALID_REQ_TYPE                   (INVALID_REQ_TYPE                   ), // = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
    /*parameter type         */ .BANK_REQ_TYPE                      (bank_req_t                         ), // = iommu_atd_cache_pkg::s2ptc_bank_req_t,
...
//=== WR2 inst {{{
    ...
    iommu_atd_ptc_mtlb_bank_wr2 #(
    /*parameter  */ .CACHE_TYPE                         (CACHE_TYPE                         ), //= 0, // 0:S2PTC, 1:S1PTC
    /*parameter  */ .BANK_TYPE                          (BANK_TYPE                          ), // = 3'b000,
    /*parameter type         */ .BANK_REQ_TYPE                      (bank_req_t                         ), // = iommu_atd_cache_pkg::s2ptc_bank_req_t,
    /*parameter type         */ .INVALID_REQ_TYPE                   (INVALID_REQ_TYPE                   ), // = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
    /*parameter type         */ .LOOKUP_REQ_TYPE                    (LOOKUP_REQ_TYPE                    ), // = iommu_atd_cache_pkg::s2ptc_lookup_req_t,
    /*parameter type         */ .LOOKUP_ACK_TYPE                    (LOOKUP_ACK_TYPE                    ), // = iommu_atd_cache_pkg::s2ptc_lookup_ack_t,
...
module iommu_atd_ptc_mtlb_bank_rd0 #( //{{{
...
module iommu_atd_ptc_mtlb_bank_match1 #( //{{{
    parameter   CACHE_TYPE                          = 0, // 0:S2PTC, 1:S1PTC
    parameter   BANK_TYPE                           = 3'b000,
    parameter type          INVALID_REQ_TYPE                    = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
    parameter type          BANK_REQ_TYPE                       = iommu_atd_cache_pkg::s2ptc_bank_req_t,
...
generate
    for(i=0; i<BANK_WAY_NUM; i++) begin : tag_match_gen
        iommu_atd_ptc_mtlb_tag_hit #(
        /*parameter  */ .CACHE_TYPE                 (CACHE_TYPE         ), // = 0, // 0:S2PTC, 1:S1PTC
        /*parameter  */ .BANK_TYPE                  (BANK_TYPE          ), // = 2'b00,
        /*parameter type         */ .INVALID_REQ_TYPE           (INVALID_REQ_TYPE   ), // = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
        /*parameter type         */ .BANK_REQ_TYPE              (BANK_REQ_TYPE      ), // = iommu_atd_cache_pkg::s2ptc_bank_req_t.
        /*parameter type         */ .MTLB_TAG_TYPE              (MTLB_TAG_TYPE      ), // = iommu_atd_cache_pkg::s2ptc_mtlb_tag_t,
        /*parameter type         */ .MTLB_ITAG_TYPE             (MTLB_ITAG_TYPE     ), // = iommu_atd_cache_pkg::s2ptc_mtlb_itag_t,
        ...
        /*input  logic                                      */  .csr_fctl_gxl       (csr_fctl_gxl_i     ),
        /*input  logic                                      */  .spare_in           (1'b0               ) 
        );
...
module iommu_atd_ptc_mtlb_bank_wr2 #( //{{{
...
//=== Declare === {{{
    ...
    MTLB_TAG_TYPE [BANK_WAY_NUM-1:0]                            lkp_tram_wdata_int;
    MTLB_TAG_TYPE [BANK_WAY_NUM-1:0]                            upd_tram_wdata_int;
//}}}
...
genvar t0;
generate
    for(t0=0; t0<BANK_WAY_NUM; t0++) begin : tram_wdata_gen
        if(CACHE_TYPE=='d0) begin : s2ptc_tram_wdata_gen
            always@(*) begin
                lkp_tram_wdata_int[t0].valid        = tag[t0].valid;                // valid
                lkp_tram_wdata_int[t0].prefetched   = 1'b0;                         // prefetched
                lkp_tram_wdata_int[t0].addr         = tag[t0].addr;                 // addr
                lkp_tram_wdata_int[t0].gscid        = tag[t0].gscid;                // gscid
                lkp_tram_wdata_int[t0].g            = tag[t0].g;                    // g
                lkp_tram_wdata_int[t0].n            = tag[t0].n;                    // n
                lkp_tram_wdata_int[t0].leaf         = tag[t0].leaf;                 // leaf
                lkp_tram_wdata_int[t0].lvl          = {BANK_TYPE=='d0 ? tag[t0].lvl : 3'(BANK_TYPE)};  // lvl

                upd_tram_wdata_int[t0].valid        = 1'b1;                          // valid
                upd_tram_wdata_int[t0].prefetched   = req.upd.req.prefetched;        // prefetched
                upd_tram_wdata_int[t0].addr         = req.upd.req.addr;              // addr
                upd_tram_wdata_int[t0].gscid        = req.upd.req.gscid;             // gscid
                upd_tram_wdata_int[t0].g            = req.upd.req.PERM[4];           // g
                upd_tram_wdata_int[t0].n            = req.upd.req.N;                 // n
                upd_tram_wdata_int[t0].leaf         = (req.upd.req.PERM[2] | req.upd.req.PERM[0]);   // leaf
                upd_tram_wdata_int[t0].lvl          = {BANK_TYPE=='d0 ? req.upd.req.lvl : 3'(BANK_TYPE)}; // lvl

            end
        end
        else begin : s1ptc_tram_wdata_gen
            always@(*) begin
                lkp_tram_wdata_int[t0].valid        = tag[t0].valid;                // valid
                lkp_tram_wdata_int[t0].prefetched   = 1'b0;                         // prefetched
                lkp_tram_wdata_int[t0].addr         = tag[t0].addr;                 // addr
                lkp_tram_wdata_int[t0].gv           = tag[t0].gv;                   // gv
                lkp_tram_wdata_int[t0].gscid        = tag[t0].gscid;                // gscid
                lkp_tram_wdata_int[t0].g            = tag[t0].g;                    // g
                lkp_tram_wdata_int[t0].n            = tag[t0].n;                    // n
                lkp_tram_wdata_int[t0].leaf         = tag[t0].leaf;                 // leaf
                lkp_tram_wdata_int[t0].pscid        = tag[t0].pscid;                // pscid
                lkp_tram_wdata_int[t0].sxl          = tag[t0].sxl;
                lkp_tram_wdata_int[t0].lvl          = {BANK_TYPE=='d0 ? tag[t0].lvl : 3'(BANK_TYPE)};  // lvl

                upd_tram_wdata_int[t0].valid        = 1'b1;                          // valid
                upd_tram_wdata_int[t0].prefetched   = req.upd.req.prefetched;        // prefetched
                upd_tram_wdata_int[t0].addr         = req.upd.req.addr;              // addr
                upd_tram_wdata_int[t0].gv           = req.upd.req.gv;
                upd_tram_wdata_int[t0].gscid        = req.upd.req.gscid;             // gscid
                upd_tram_wdata_int[t0].g            = req.upd.req.PERM[4];           // g
                upd_tram_wdata_int[t0].n            = req.upd.req.N;                 // n
                upd_tram_wdata_int[t0].leaf         = (req.upd.req.PERM[2] | req.upd.req.PERM[0]);   // leaf
                upd_tram_wdata_int[t0].pscid        = req.upd.req.pscid;
                upd_tram_wdata_int[t0].sxl          = req.upd.req.sxl;
                upd_tram_wdata_int[t0].lvl          = {BANK_TYPE=='d0 ? req.upd.req.lvl : 3'(BANK_TYPE)}; // lvl
            end
        end
    end
...
//                                                                                  tag[t].n,                  // n
//                                                                                  tag[t].leaf,               // leaf
//                                                                                  {BANK_TYPE=='d0 ? tag[t].lvl : 3'(BANK_TYPE)}  // lvl
//                                                                                 }
                                                            }                                           :
                                    (req_do & req_is_inv) ? 'd0                                         :                       // wr inv_hit or err_entries
                                    (req_do & req_is_upd) ? {
                                                                upd_sel[t] ?     upd_tram_wdata_int[t] ://{1'b1,                      // valid
                                                                                 // req.upd.req.prefetched,    // prefetched
                                                                                 // req.upd.req.addr,          // addr
                                                                                 // req.upd.req.gscid,         // gscid
                                                                                 // req.upd.req.PERM[4],       // g
                                                                                 // req.upd.req.N,             // n
                                                                                 // (req.upd.req.PERM[2] | req.upd.req.PERM[0]),   // leaf
                                                                                 // {BANK_TYPE=='d0 ? req.upd.req.lvl : 3'(BANK_TYPE)} // lvl
                                                                                 //} :
...
generate
    if(BANK_TYPE=='d0) begin : s2ptc_dram_wdata_gen
        always@(*) begin
            dram_wdat_int.N     = req.upd.req.N;
            dram_wdat_int.PBMT  = req.upd.req.PBMT;
            dram_wdat_int.D     = req.upd.req.D;
            dram_wdat_int.A     = req.upd.req.A;
            dram_wdat_int.PPN   = req.upd.req.PPN;
            dram_wdat_int.PERM  = req.upd.req.PERM;
            dram_wdat_int.V     = req.upd.req.V;
        end
    end
    else begin : s1ptc_dram_wdata_gen
        always@(*) begin
            dram_wdat_int.PBMT  = req.upd.req.PBMT;
            dram_wdat_int.D     = req.upd.req.D;
            dram_wdat_int.A     = req.upd.req.A;
...
module iommu_atd_ptc_mtlb_bank_ack3 #( //{{{
    parameter   CACHE_TYPE                          = 0, // 0:DDTC, 1:PDTC
    parameter   BANK_TYPE                           = 2'b00,
    parameter type          BANK_REQ_TYPE                       = iommu_atd_cache_pkg::s2ptc_bank_req_t,
    parameter type          INVALID_REQ_TYPE                    = iommu_atd_cache_pkg::INVALID_REQ_TYPE,
...
generate
    if(BANK_TYPE=='d0) begin: l0_lkp_ack_gen
        if(CACHE_TYPE==0) begin : s2ptc_lkp_ack_gen
            always@(posedge clk or negedge rstn) begin
                if(~rstn) begin
                    lkp_ack_valid <= 1'b0;
                    lkp_ack       <= 'd0;
                    ...
                    if(bank_req_valid_i & req_is_lkp) begin
                        ...
                        lkp_ack.ack.idx             <= {4'd0, req.lkp.req.idx}      ;
                        lkp_ack.ack.lvl             <= 3'(BANK_TYPE)        ;
                        lkp_ack.ack.prefetched      <= tag_i.prefetched     ;
                        lkp_ack.ack.addr            <= tag_i.n ? {tag_i.addr[63:16], req.lkp.req.addr[15:12]} : tag_i.addr; // if svnapot active, replace ppn[15:12] to req.addr's vpn0[15:12] as this indicates the pte_positon for this translation
                        lkp_ack.ack.gscid           <= tag_i.gscid          ;
//                        lkp_ack.ack.pscid           <= tag_i.pscid          ;
//                        lkp_ack.ack.sxl             <= tag_i.sxl            ;
                        lkp_ack.ack.N               <= tag_i.n              ;
                        lkp_ack.ack.PBMT            <= dram_rdata_muxed.PBMT;
                        lkp_ack.ack.D               <= dram_rdata_muxed.D   ;
                        lkp_ack.ack.A               <= dram_rdata_muxed.A   ;
                        lkp_ack.ack.PPN             <= dram_rdata_muxed.PPN ;
                        lkp_ack.ack.PERM            <= dram_rdata_muxed.PERM;
            ...
            always@(posedge clk or negedge rstn) begin
                if(~rstn) begin
                    ...
                    if(bank_req_valid_i & req_is_lkp) begin
                        ...
                        lkp_ack.ack.lvl             <= 3'(BANK_TYPE)        ;
                        lkp_ack.ack.prefetched      <= tag_i.prefetched     ;
                        lkp_ack.ack.addr            <= tag_i.n ? {tag_i.addr[63:16], req.lkp.req.addr[15:12]} : tag_i.addr;
                        lkp_ack.ack.gv              <= tag_i.gv             ;
                        lkp_ack.ack.gscid           <= tag_i.gscid          ;
                        lkp_ack.ack.pscid           <= tag_i.pscid          ;
                        lkp_ack.ack.sxl             <= tag_i.sxl            ;
                        lkp_ack.ack.N               <= tag_i.n              ;
