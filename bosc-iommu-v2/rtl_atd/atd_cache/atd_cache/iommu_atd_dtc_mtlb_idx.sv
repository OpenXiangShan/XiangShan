...
module iommu_atd_dtc_mtlb_idx #(
    ...
    assign device_id_masked = CACHE_TYPE!='d0 ? device_id                   :   // PDTC, use all did for idx cal
                              lvl=='d0        ? device_id                   :   // DDTC lvl0, use all did for idx cal
                              lvl=='d1        ? {device_id[23:6], 6'd0}     :   // DDTC lvl1, use did1/2 for idx cal
                                                {device_id[23:15], 15'd0}   ;   // DDTC lvl2, use did2 for idx cal
    assign process_id_masked= CACHE_TYPE=='d0 ? 'd0                         :   // DDTC, no process_id for idx cal
                              lvl=='d0        ? process_id                  :   // PDTC lvl0, use all pid for idx cal
                              lvl=='d1        ? {process_id[19:8], 8'd0}    :   // PDTC lvl1, use pid1/2 for idx cal
                                                {process_id[19:17], 17'd0}  ;   // PDTC lvl2, use pid2 for idx cal

    assign idx_in_vec = {53'b0, process_id_masked, device_id_masked};
