...
module iommu_acd_mtlb_idx #(
    parameter   IDX_WIDTH       = 7
)(
    input  logic [23:0]                     device_id,
    input  logic                            process_id_valid,
    input  logic [19:0]                     process_id,
    input  logic [63:12]                    va,
    input  logic [1:0]                      page_size,
    output logic [IDX_WIDTH-1:0]            idx
);
    logic [63:12]                           va_masked;
    logic [96:0]                            idx_in_vec;
    logic [15:0]                            idx_raw;        // by now, only support max 16bit idx gen, enough for current design

    logic [23:0]                            device_id_masked;
    logic                                   process_id_valid_masked;
    logic [19:0]                            process_id_masked;

// when page size is 4k, va[15:12] is not used for idx cal, for svnapot lookup dont need [15:12] bits matching, 20241107
//    assign va_masked = (page_size==2'b00) ? va  :                // 4K
    assign va_masked = (page_size==2'b00) ? {va[63:16], 4'b0 } : // 4K
// when page size is 2M/4M, va[21] is not used for idx cal, for Sv32/Sv32x4, 20241210
                       (page_size==2'b01) ? {va[63:22], 10'b0 } : // 2M
                       (page_size==2'b10) ? {va[63:30], 18'b0} : // 1G
                       (page_size==2'b11) ? {va[63:39], 27'b0} : // 512G
                                            va;
    assign device_id_masked        = device_id;
    assign process_id_valid_masked = process_id_valid;
    assign process_id_masked       = process_id;

    assign idx_in_vec = {45'd0, va_masked}; //, device_id_masked, process_id_valid_masked, process_id_masked};   // mod 20241010, only use va for idx cal, cause invalid cmd can not carry all this info
