...
module iommu_acd_mtlb_bank #( //{{{
    ...
    parameter   ECC_WIDTH                   = 7,    // 0: disable ECC, other: enable ECC
...
//{{{ IO
    ...
    output logic [BANK_WAY_NUM-1:0] [ECC_WIDTH:0]               tram_wdata_ecc_o,
    output logic [BANK_WAY_NUM-1:0] [ECC_WIDTH:0]               iram_wdata_ecc_o,
    output logic [BANK_WAY_NUM-1:0] [ECC_WIDTH:0]               dram_wdata_ecc_o,
    input  logic [BANK_WAY_NUM-1:0] [ECC_WIDTH:0]               tram_rdata_ecc_i,
    input  logic [BANK_WAY_NUM-1:0] [ECC_WIDTH:0]               iram_rdata_ecc_i,
    input  logic [BANK_WAY_NUM-1:0] [ECC_WIDTH:0]               dram_rdata_ecc_i,
    // CFG                                                      
...
//=== Declare === {{{
    ...
    logic [BANK_WAY_NUM-1:0]                            ak3_dram_cs_o       ;
    logic [BANK_WAY_NUM-1:0]                            ak3_dram_wr_o       ;
    logic [BANK_WAY_NUM-1:0] [BANK_SET_IDX_WIDTH-1:0]   ak3_dram_addr_o     ;
    MTLB_DAT_TYPE [BANK_WAY_NUM-1:0]                    ak3_dram_wdata_o    ;
...
generate //{{{
    for(i=0; i<BANK_WAY_NUM; i++) begin : way_gen
        if(ECC_WIDTH==0) begin : ecc_zero_gen
...
//=== RD0 inst {{{
    ...
    iommu_acd_mtlb_bank_rd0 #(
    /*parameter  */ .BANK_TYPE                          (BANK_TYPE                          ), // = 2'b00,    // 00:4K  01:2M  10:1G  11:512G
    /*parameter type         */ .BANK_REQ_TYPE                      (bank_req_t                         ), // = iommu_acd_pkg::bank_req_t,
...
endgenerate
    ...
    /*parameter type         */ .MTLB_DAT_TYPE                      (MTLB_DAT_TYPE                     ), // = iommu_acd_pkg::mtlb_data_t,
...
//=== ACK3 inst {{{
    always@(posedge clk or negedge rstn) begin
        if(~rstn)
            ...
            ak3_ram_valid_i <= (~wr2_dram_cs_o) & wr2_dram_wr_o; // record with way is read, only one way read at the same time
    end
...
`ifdef IOMMU_ACD_SIMULATION_ASSERT
    assert property (@(negedge clk) $onehot({ak3_ram_valid_i}))
    else $error("Violation: Only one way-dram readed at a time, ak3_ram_valid_i has more than 1 bit active");
`endif
...
module iommu_acd_mtlb_bank_rd0 #( //{{{
    parameter   BANK_TYPE                           = 2'b00,    // 00:4K  01:2M  10:1G  11:512G
...
module iommu_acd_mtlb_bank_match1 #( //{{{
    parameter   BANK_TYPE                           = 2'b00,    // 00:4K  01:2M  10:1G  11:512G
    parameter type          BANK_REQ_TYPE                       = iommu_acd_pkg::bank_req_t,
    parameter type          INVALID_REQ_TYPE                    = iommu_acd_pkg::INVALID_REQ_TYPE,
    parameter type          LOOKUP_REQ_TYPE                     = iommu_acd_pkg::lookup_req_t,
...
generate
    for(i=0; i<BANK_WAY_NUM; i++) begin : tag_match_gen
        iommu_acd_mtlb_tag_hit #(
        /*parameter  */ .BANK_TYPE                  (BANK_TYPE      ), // = 2'b00,
        /*parameter type         */ .BANK_REQ_TYPE              (BANK_REQ_TYPE  ), // = iommu_acd_pkg::bank_req_t.
        /*parameter type         */ .MTLB_TAG_TYPE              (MTLB_TAG_TYPE  ), // = iommu_acd_pkg::mtlb_tag_t,
        /*parameter type         */ .MTLB_ITAG_TYPE             (MTLB_ITAG_TYPE ), // = iommu_acd_pkg::mtlb_itag_t,
        ...
        // force unhit when corresponding err happens
...
module iommu_acd_mtlb_bank_wr2 #( //{{{
    parameter   BANK_TYPE                           = 2'b00,    // 00:4K  01:2M  10:1G  11:512G
    parameter type          BANK_REQ_TYPE                       = iommu_acd_pkg::bank_req_t,
    parameter type          INVALID_REQ_TYPE                    = iommu_acd_pkg::INVALID_REQ_TYPE,
    parameter type          LOOKUP_REQ_TYPE                     = iommu_acd_pkg::lookup_req_t,
...
endgenerate
    ...
    assign do_ram_write     = (req_do & req_is_inv & (|inv_tag_hit))    |   // INV&hit
                              (req_do & req_is_upd & ~(|tag_hit))       |   // UPD&~hit
...
//=== TAG RAM {{{
    assign tram_cs_o = (req_do & req_is_lkp) ? ((do_ram_write & ~do_ram_write_cnt) ? ~any_ecc_err                   :   // wr err_entries
                                                                                     {BANK_WAY_NUM{1'b1}}
                                                )                                                                       :
    ...
    assign tram_wr_o = 'd0;/*(req_do & req_is_lkp) ? ((do_ram_write & ~do_ram_write_cnt) ? {BANK_WAY_NUM{1'b0}}   :           // wr err_entries
                                                                                     {BANK_WAY_NUM{1'b1}}
                                                ) :
...
generate
    for(t=0; t<BANK_WAY_NUM; t++) begin : tram_addr_wdata_gen
        assign tram_addr_o[t] = (req_do & req_is_lkp) ? req.lkp.bank_set_idx[BANK_SET_IDX_WIDTH-1:0]    :   // wr err_entries
        ...
        assign tram_wdata_o[t]= (req_do & req_is_lkp) ? 'd0                                             :   // wr err_entries
                                ...
                                (req_do & req_is_upd) ? {
                                                            upd_sel[t] ?    {1'b1,// valid                  // store upd to upd_sel entry
                                                                             ...
                                                                             req.upd.req.S1SIZE,
                                                                             req.upd.req.S2SIZE,
                                                                             req.upd.req.is_translated,
                                                                             req.upd.req.process_id_valid,
                                                                             req.upd.req.process_id,
                                                                             req.upd.req.device_id,
                                                                             req.upd.req.va
                                                                            }                       :
...
//=== ITAG RAM {{{
    ...
    assign iram_wr_o = 'd0;/*(req_do & req_is_lkp) ? ((do_ram_write & ~do_ram_write_cnt) ? {BANK_WAY_NUM{1'b0}}   :           // wr err_entries
                                                                                     {BANK_WAY_NUM{1'b1}}
                                                ) :
                       (req_do & req_is_inv) ? ((do_ram_write & ~do_ram_write_cnt) ? {BANK_WAY_NUM{1'b0}}   :           // wr inv_hit or err_entries
                                                                                     {BANK_WAY_NUM{1'b1}}
...
generate
    for(i=0; i<BANK_WAY_NUM; i++) begin : iram_addr_wdata_gen
        ...
        assign iram_wdata_o[i]= (req_do & req_is_lkp) ? 'd0                                             :   // wr err_entries
                                (req_do & req_is_inv) ? 'd0                                             :   // wr inv_hit or err_entries
                                (req_do & req_is_upd) ? {   upd_sel[i] ?    {req.upd.req.GPPN,              // wr upd to upd_sel entry
                                                                             req.upd.req.PSCID,
                                                                             req.upd.req.GSCID,
                                                                             req.upd.req.S1_PERM[4]
                                                                            }                       :
                                                                            'd0                             // wr err(without upd_sel valid) entries
                                                        }                                               :
                                                        'd0;
...
//=== UTAG RAM {{{
    assign uram_cs_o = (req_do & req_is_lkp) ? {BANK_WAY_NUM{1'b1}}                                     :   // nop  // by now, lookup hit will not update plru, as the hitted entry will updated to MicroTLB
                       ...
                       (req_do & req_is_upd) ? ((do_ram_write_upd & ~do_ram_write_cnt) ? 1'b0       :
                                                                                         {BANK_WAY_NUM{1'b1}}
                                               )                                                        :   // wr
                                               {BANK_WAY_NUM{1'b1}};
...
//=== DATA RAM {{{
    ...
    assign dram_cs_o = (req_do & req_is_lkp) ? ((do_ram_read & ~do_ram_read_cnt)    ? {~lkp_dram_cs_muxed & ~any_ecc_err}   :       // rd tag_hit and wr err entry
                                                (do_ram_write & ~do_ram_write_cnt)  ? ~any_ecc_err                          :       // wr err entry
                                                                                     {BANK_WAY_NUM{1'b1}}
                                                )                                                                               :
                       (req_do & req_is_inv) ? ((do_ram_write & ~do_ram_write_cnt) ? {~inv_tag_hit & ~any_ecc_err}          :       // wr inv_hit or err_entries
                                                                                     {BANK_WAY_NUM{1'b1}}
    ...
    assign dram_wr_o = (req_do & req_is_lkp) ? (lkp_dram_cs_muxed | ~any_ecc_err)   :   // rd tag_hit and wr err entry
                       (req_do & req_is_inv) ? {~inv_tag_hit & ~any_ecc_err}        :   // wr err entry
                       (req_do & req_is_upd) ? (~upd_sel & ~any_ecc_err )           :   // wr upd_sel and err entry
                                               {BANK_WAY_NUM{1'b1}} ;
    always@(*) begin
        dram_wdat_int.PBMT            = req.upd.req.PBMT        ;
        dram_wdat_int.ENATS           = req.upd.req.ENATS       ;
        ...
        dram_wdat_int.S1_D            = req.upd.req.S1_D        ;
        dram_wdat_int.S2_D            = req.upd.req.S2_D        ;
        dram_wdat_int.SADE            = req.upd.req.SADE        ;
        dram_wdat_int.GADE            = req.upd.req.GADE        ;
//        dram_wdat_int.N               = req.upd.req.N           ;
//        dram_wdat_int.S1_PERM_D       = req.upd.req.S1_PERM_D   ;
//        dram_wdat_int.S1_PERM_A       = req.upd.req.S1_PERM_A   ;
        dram_wdat_int.S1_PERM         = req.upd.req.S1_PERM     ;
//        dram_wdat_int.S2_PERM_D       = req.upd.req.S2_PERM_D   ;
//        dram_wdat_int.S2_PERM_A       = req.upd.req.S2_PERM_A   ;
        dram_wdat_int.S2_PERM         = req.upd.req.S2_PERM     ;
        dram_wdat_int.PDTMODE         = req.upd.req.PDTMODE     ;
...
generate
    for(d=0; d<BANK_WAY_NUM; d++) begin : dram_addr_wdata_gen
        assign dram_addr_o[d] = (req_do & req_is_lkp) ? req.lkp.bank_set_idx[BANK_SET_IDX_WIDTH-1:0]    :  // rd tag_hit and wr err_entries
                                (req_do & req_is_inv) ? req.inv.bank_set_idx[BANK_SET_IDX_WIDTH-1:0]    :  // wr err entry
                                (req_do & req_is_upd) ? req.upd.bank_set_idx[BANK_SET_IDX_WIDTH-1:0]    :  // wr upd_sel or err_entries
                                                        'd0;
...
module iommu_acd_mtlb_bank_ack3 #( //{{{
    parameter   BANK_TYPE                           = 2'b00,    // 00:4K  01:2M  10:1G  11:512G
    parameter type          BANK_REQ_TYPE                       = iommu_acd_pkg::bank_req_t,
    parameter type          INVALID_REQ_TYPE                    = iommu_acd_pkg::INVALID_REQ_TYPE,
    parameter type          LOOKUP_REQ_TYPE                     = iommu_acd_pkg::lookup_req_t,
    ...
    always@(posedge clk or negedge rstn) begin
        if(~rstn) begin
            ...
            if(bank_req_valid_i & req_is_lkp) begin
                ...
                lkp_ack.ack.idx             <= req.lkp.req.idx;
                lkp_ack.ack.is_translated   <= tag_i.is_translated;
                lkp_ack.ack.process_id_valid<= tag_i.process_id_valid;
                lkp_ack.ack.PBMT            <= dram_rdata_muxed.PBMT;
                lkp_ack.ack.GPPN            <= itag_i.GPPN;
                lkp_ack.ack.ENATS           <= dram_rdata_muxed.ENATS;
                ...
                lkp_ack.ack.S1_D            <= dram_rdata_muxed.S1_D;
                lkp_ack.ack.S2_D            <= dram_rdata_muxed.S2_D;
                lkp_ack.ack.SADE            <= dram_rdata_muxed.SADE;
                lkp_ack.ack.GADE            <= dram_rdata_muxed.GADE;
                lkp_ack.ack.N               <= tag_i.N;
//                lkp_ack.ack.S1_PERM_D         <= dram_rdata_muxed.S1_PERM_D;
//                lkp_ack.ack.S1_PERM_A         <= dram_rdata_muxed.S1_PERM_A;
                lkp_ack.ack.S1_PERM         <= dram_rdata_muxed.S1_PERM;
//                lkp_ack.ack.S2_PERM_D         <= dram_rdata_muxed.S2_PERM_D;
//                lkp_ack.ack.S2_PERM_A         <= dram_rdata_muxed.S2_PERM_A;
                lkp_ack.ack.S2_PERM         <= dram_rdata_muxed.S2_PERM;
                lkp_ack.ack.S1SIZE          <= tag_i.S1SIZE;
                lkp_ack.ack.S2SIZE          <= tag_i.S2SIZE;
                lkp_ack.ack.S1MODE          <= tag_i.S1MODE;
                lkp_ack.ack.S2MODE          <= tag_i.S2MODE;
                lkp_ack.ack.PDTMODE         <= dram_rdata_muxed.PDTMODE;
                lkp_ack.ack.PSCID           <= itag_i.PSCID;
                lkp_ack.ack.GSCID           <= itag_i.GSCID;
                lkp_ack.ack.PPN             <= dram_rdata_muxed.PPN;
                lkp_ack.ack.process_id      <= tag_i.process_id;
                lkp_ack.ack.device_id       <= tag_i.device_id;
                lkp_ack.ack.va              <= tag_i.va;
            end
            else begin
                lkp_ack_valid               <= 1'b0;
            end
