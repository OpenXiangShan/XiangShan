...
FRONTEND_SIGNAL_INVENTORY = $(FRONTEND_PYLIB_DIR)/Frontend_offset.yaml
FRONTEND_VCS_INVENTORY_EXPORTER = src/test/python/Frontend/tools/export_vcs_signal_inventory.py
...
FRONTEND_FUNCOV_FILELIST = $(FRONTEND_BUILD_DIR)/full-rtl-picker.funcov.f
...
frontend:
	@if [ -L "$(FRONTEND_RTL_DIR)" ]; then rm -f "$(FRONTEND_RTL_DIR)"; fi
	$(MAKE) verilog BUILD_DIR=$(FRONTEND_BUILD_DIR)
	$(MAKE) frontend-full-rtl-picker BUILD_DIR=$(FRONTEND_BUILD_DIR)
.PHONY: frontend
...
FRONTEND_FULL_RTL_PICKER_BUILD_DIR = $(FRONTEND_BUILD_DIR)
FRONTEND_FULL_RTL_PICKER_RTL_DIR = $(FRONTEND_FULL_RTL_PICKER_BUILD_DIR)/rtl
FRONTEND_FULL_RTL_PICKER_TOP_V = $(FRONTEND_FULL_RTL_PICKER_RTL_DIR)/$(TOP).$(RTL_SUFFIX)
FRONTEND_FULL_RTL_PICKER_BASE_FILELIST = $(FRONTEND_BUILD_DIR)/full-rtl-picker.f
FRONTEND_FULL_RTL_PICKER_FILELIST = $(if $(filter vcs,$(FRONTEND_SIM)),$(FRONTEND_FUNCOV_FILELIST),$(FRONTEND_FULL_RTL_PICKER_BASE_FILELIST))
FRONTEND_FULL_RTL_PICKER_PYLIB_ROOT = $(FRONTEND_PYLIB_ROOT)
FRONTEND_FULL_RTL_PICKER_PYLIB_DIR = $(FRONTEND_PYLIB_DIR)
FRONTEND_FULL_RTL_PICKER_PYLIB = $(FRONTEND_PYLIB)
FRONTEND_FULL_RTL_PICKER_MANIFEST = $(FRONTEND_BUILD_MANIFEST)
FRONTEND_FULL_RTL_PICKER_BUILD_TARGET = $(if $(filter vcs,$(FRONTEND_SIM)),frontend-vcs,frontend)
FRONTEND_FULL_RTL_PICKER_EXTRA_TARGETS =
FRONTEND_FULL_RTL_PICKER_CCACHE_DIR = $(FRONTEND_CCACHE_DIR)
FRONTEND_FULL_RTL_PICKER_CCACHE_TMP = $(FRONTEND_CCACHE_TMP)
FRONTEND_FULL_RTL_PICKER_TMP = $(abspath $(FRONTEND_FULL_RTL_PICKER_BUILD_DIR)/.tmp)
FRONTEND_FULL_RTL_PICKER_CACHE = $(abspath $(FRONTEND_FULL_RTL_PICKER_BUILD_DIR)/.cache)
# VCS FSDB paths are selected per testcase at runtime; -w would embed one shared startup file.
FRONTEND_FULL_RTL_PICKER_WAVEFORM_ARG = $(if $(filter vcs,$(FRONTEND_SIM)),,-w $(abspath $(FRONTEND_FULL_RTL_PICKER_BUILD_DIR))/frontend.$$frontend_waveform_format)
FRONTEND_FULL_RTL_PICKER_BUILD_ENV = CCACHE_DIR=$(FRONTEND_FULL_RTL_PICKER_CCACHE_DIR) CCACHE_TEMPDIR=$(FRONTEND_FULL_RTL_PICKER_CCACHE_TMP) TMPDIR=$(FRONTEND_FULL_RTL_PICKER_TMP) XDG_CACHE_HOME=$(FRONTEND_FULL_RTL_PICKER_CACHE) NPROC=$(FRONTEND_BUILD_JOBS) MAKEFLAGS=-e $(if $(FRONTEND_TOOL_PATH),PATH=$(FRONTEND_TOOL_PATH)$$PATH) $(if $(FRONTEND_SWIG_LIB),SWIG_LIB=$(FRONTEND_SWIG_LIB)) $(FRONTEND_SIM_ENV)

$(FRONTEND_FULL_RTL_PICKER_BASE_FILELIST): Makefile $(FRONTEND_FULL_RTL_PICKER_TOP_V) $(FRONTEND_FULL_RTL_PICKER_RTL_DIR)/Frontend.sv $(FRONTEND_FULL_RTL_PICKER_RTL_DIR)/filelist.f
	@sed 's|^|rtl/|' $(FRONTEND_FULL_RTL_PICKER_RTL_DIR)/filelist.f > $@

ifeq ($(FRONTEND_SIM),vcs)
$(FRONTEND_FUNCOV_FILELIST): FORCE $(FRONTEND_FULL_RTL_PICKER_BASE_FILELIST) $(FRONTEND_FUNCOV_SV_FILES)
	@tmp="$@.tmp"; \
	{ \
		cat "$(FRONTEND_FULL_RTL_PICKER_BASE_FILELIST)"; \
...
$(FRONTEND_FULL_RTL_PICKER_PYLIB): $(FRONTEND_FULL_RTL_PICKER_FILELIST) $(FRONTEND_WAVEFORM_FORMAT_FILE) $(FRONTEND_CONFIG_FILE) $(FRONTEND_FUNCOV_SV_FILES)
	...
	rm -rf $(FRONTEND_FULL_RTL_PICKER_PYLIB_DIR)
	mkdir -p $(FRONTEND_FULL_RTL_PICKER_CCACHE_DIR) \
		$(FRONTEND_FULL_RTL_PICKER_CCACHE_TMP) \
		$(FRONTEND_FULL_RTL_PICKER_TMP) \
		$(FRONTEND_FULL_RTL_PICKER_CACHE)
	@frontend_waveform_format="$$(cat "$(FRONTEND_WAVEFORM_FORMAT_FILE)")"; \
	cd $(FRONTEND_FULL_RTL_PICKER_BUILD_DIR) && \
		$(FRONTEND_FULL_RTL_PICKER_BUILD_ENV) time $(FRONTEND_PICKER) export \
			$(abspath $(FRONTEND_FULL_RTL_PICKER_RTL_DIR))/Frontend.sv \
			--filelist $(abspath $(FRONTEND_FULL_RTL_PICKER_FILELIST)) \
			--sname Frontend --tname Frontend \
			...
			$(FRONTEND_FULL_RTL_PICKER_WAVEFORM_ARG) \
...
ifeq ($(FRONTEND_SIM),vcs)
$(FRONTEND_SIGNAL_INVENTORY): $(FRONTEND_FULL_RTL_PICKER_PYLIB) $(FRONTEND_VCS_INVENTORY_EXPORTER)
	@LD_PRELOAD="$(abspath $(FRONTEND_FULL_RTL_PICKER_PYLIB))$${LD_PRELOAD:+:$$LD_PRELOAD}" \
		$(if $(PYTHON),$(PYTHON),python3) $(FRONTEND_VCS_INVENTORY_EXPORTER) \
		--pylib $(FRONTEND_FULL_RTL_PICKER_PYLIB_ROOT) --output $@

FRONTEND_FULL_RTL_PICKER_EXTRA_TARGETS += $(FRONTEND_SIGNAL_INVENTORY)
endif

frontend-full-rtl-picker: $(FRONTEND_FULL_RTL_PICKER_PYLIB) $(FRONTEND_FULL_RTL_PICKER_FILELIST) $(FRONTEND_FULL_RTL_PICKER_EXTRA_TARGETS)
	@frontend_waveform_format="$$(cat "$(FRONTEND_WAVEFORM_FORMAT_FILE)")"; \
	python3 src/test/python/Frontend/tools/write_frontend_build_manifest.py \
		--repo-root . \
		--build-root $(FRONTEND_FULL_RTL_PICKER_BUILD_DIR) \
		--output $(FRONTEND_FULL_RTL_PICKER_MANIFEST) \
		--sim $(FRONTEND_SIM) \
		--build-config "CONFIG=$(CONFIG);ISSUE=$(ISSUE);NUM_CORES=$(NUM_CORES);CHISEL_TARGET=$(CHISEL_TARGET);WAVEFORM=$$frontend_waveform_format;PROFILE=full-rtl-picker" \
		--build-command "make $(FRONTEND_FULL_RTL_PICKER_BUILD_TARGET) CONFIG=$(CONFIG) ISSUE=$(ISSUE) NUM_CORES=$(NUM_CORES) CHISEL_TARGET=$(CHISEL_TARGET) FRONTEND_WAVEFORM_FORMAT=$$frontend_waveform_format"
.PHONY: frontend-full-rtl-picker
