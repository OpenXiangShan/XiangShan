...
FRONTEND_BUILD_MANIFEST_DEPS = $(FRONTEND_PYLIB) $(FRONTEND_WAVEFORM_FORMAT_FILE) $(FRONTEND_CONFIG_FILE)
...
ifeq ($(FRONTEND_SIM),vcs)
$(FRONTEND_SIGNAL_INVENTORY): $(FRONTEND_PYLIB) $(FRONTEND_VCS_INVENTORY_EXPORTER)
	@LD_PRELOAD="$(abspath $(FRONTEND_PYLIB))$${LD_PRELOAD:+:$$LD_PRELOAD}" \
		$(if $(PYTHON),$(PYTHON),python3) $(FRONTEND_VCS_INVENTORY_EXPORTER) \
		--pylib $(FRONTEND_PYLIB_ROOT) --output $@

FRONTEND_BUILD_MANIFEST_DEPS += $(FRONTEND_SIGNAL_INVENTORY)
endif

$(FRONTEND_BUILD_MANIFEST): $(FRONTEND_BUILD_MANIFEST_DEPS)
	@frontend_waveform_format="$$(cat "$(FRONTEND_WAVEFORM_FORMAT_FILE)" 2>/dev/null || printf '%s' '$(FRONTEND_WAVEFORM_FORMAT)')"; \
