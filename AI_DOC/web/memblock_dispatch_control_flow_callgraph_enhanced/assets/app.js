(function () {
  ...
  const functionCatalog = {
    ...
    "dut_ready_fire_boundary": fn("dut_ready_fire_boundary", {
      ...
      input: "issue driver valid/payload 与 DUT ready",
      output: "接口 fire 或等待/abort",
      purpose: "表示真实 DUT 接口握手边界。",
      role: "该节点只表示lintsissue valid/ready fire；V2 LSQ enqueue使用独立clock-first sample边界。",
      external: true,
    }),
    ...
    "tc_base::build_phase": fn("tc_base::build_phase", {
      ...
      purpose: "测试环境构建入口。",
      role: "配置真实 dispatch flow 需要的 LSQENQ、LINTSISSUE、LSQCOMMIT、REDIRECT、L2TLB 默认 sequence；当前 scalar-only flow 不绑定 vecissue 默认 sequence。",
    ...
    "random_amo_fuoptype": fn("random_amo_fuoptype", { file: "memblock_dispatch_base_sequence.sv", input: "plus 权重", output: "AMO fuOpType", purpose: "保留AMO编码生成helper。", role: "本轮scalar-only主表校验拒绝AMO/MOU；该helper不能表示当前已支持AMO flow。" }),
    "apply_legal_addr_template": fn("apply_legal_addr_template", {
      file: "memblock_dispatch_base_sequence.sv",
      input: "main_control_transaction",
      output: "src_0/imm/vaddr 合法化",
      purpose: "在 MAIN_VADDR 窗口内生成完整访问跨度合法的 64B 对齐虚拟地址。",
      role: "给后续 TLB/LSQ 和 RAW 违例注入提供虚拟地址；物理地址仍由 TLB builder 独立映射。",
    }),
    ...
    "apply_addr_reuse_window": fn("apply_addr_reuse_window", {
      ...
      calls: ["select_addr_reuse_kind", "random_pick_recent_uid", "set_transaction_ls_kind", "fixup_after_addr_reuse", "ensure_normal_reused_addr_span", "validate_main_table_entry"],
    }),
    ...
    "fixup_after_addr_reuse": fn("fixup_after_addr_reuse", { file: "memblock_dispatch_base_sequence.sv", input: "transaction, ref_tr, copy_addr", output: "vaddr 更新并校验", purpose: "第二层复用后修正。", role: "负责复制 src_0/imm、重算 vaddr，并在 normal copy-address 场景调用最终跨度收口。" }),
    "ensure_normal_reused_addr_span": fn("ensure_normal_reused_addr_span", { file: "memblock_dispatch_base_sequence.sv", input: "transaction, ref_tr, caller", output: "合法最终访问跨度", purpose: "修正地址复用后可能越界的随机访问尺寸。", role: "normal 路径保留参考地址，必要时选择与参考访问大小一致的 load/store opcode；boundary/manual 不消费 MAIN_VADDR。" }),
    ...
    "memblock_lsqenq_dispatch_base_sequence::body": fn("memblock_lsqenq_dispatch_base_sequence::body", {
      ...
      role: "默认开启真实admission；launch后激活uid，下一driver边界设置issue_ready并路由；TLB entry由后续L2TLB request查/建。",
    ...
    "memblock_lsqenq_dispatch_base_sequence::configure_from_plus": fn("memblock_lsqenq_dispatch_base_sequence::configure_from_plus", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "seq_csr_common getter", output: "enable/no-progress参数", purpose: "读取admission运行期开关和warning阈值。", role: "V2本地ready_timeout固定0，不读取ready getter；无专用idle/start/max-cycle退出。" }),
    "memblock_lsqenq_dispatch_base_sequence::ensure_helpers": fn("memblock_lsqenq_dispatch_base_sequence::ensure_helpers", {
      ...
      input: "无",
      output: "data/lsq_ctrl/issue_sched/monitor_adapter 句柄",
      purpose: "准备 LSQ admission 所需 helper。",
      role: "确保后续reservation、issue route和CSR drain都有可用对象。",
    ...
    "drive_lsqenq_loop": fn("drive_lsqenq_loop", {
      ...
      role: "forever loop每拍尝试发送可入队uid；no-progress只warning，global stop时用trailing idle完成末批sample后退出。",
      calls: ["send_lsqenq_cycle", "send_idle_lsqenq_boundary"],
    }),
    "send_lsqenq_cycle": fn("send_lsqenq_cycle", {
      file: "memblock_lsqenq_dispatch_base_sequence.sv",
      input: "cycle_idx",
      output: "上一批完成sample，当前批launch并预留",
      purpose: "一拍 LSQ admission 的完整逻辑。",
      role: "先消费redirect留下的pending cancel；pending batch后遇non-LSQ时先发送idle完成sample，再执行non-LSQ admission；随后按6/4收集、clock-first launch并预留普通candidate。",
      calls: ["memblock_lsqenq_dispatch_base_sequence::apply_pending_lsq_cancels", "send_idle_lsqenq_boundary", "admit_non_lsq_if_ready", "collect_lsq_candidates", "clear_lsqenq_xaction", "assign_lsqenq_slot", "lsqenq_agent_agent_driver::main_phase", "complete_v2_pending_sample", "confirm_lsq_candidates"],
    ...
    if (pending_sample_valid &&
        next_uid_needs_lsq_admission(probe_uid, probe_tr, probe_behavior) &&
...
endtask:send_lsqenq_cycle`,
      logicNotes: [
        ...
        "driver launch后立即返回；sequence先完成上一pending batch，再由confirm_lsq_candidates为当前批建立reservation，下一driver边界才开放issue。",
      ],
    }),
    "send_idle_lsqenq_boundary": fn("send_idle_lsqenq_boundary", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "cycle_idx/reason/has_progress", output: "全零item和上一pending batch completion", purpose: "在无候选、non-LSQ切换或global-stop收尾时提供下一driver采样边界。", role: "发送全零xaction；finish_item返回后调用complete_v2_pending_sample开放上一批issue或按epoch丢弃。", calls: ["clear_lsqenq_xaction", "complete_v2_pending_sample"] }),
    "admit_non_lsq_if_ready": fn("admit_non_lsq_if_ready", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "next uid", output: "未来non-LSQ uid可直接active/route", purpose: "保留need_alloc=0的兼容入口。", role: "当前主表校验拒绝AMO/MOU/CBO，scalar load/store均需分配LSQ，因此本轮支持范围内不可达。" }),
    ...
    if (data.pending_lq_cancel_count != 0) begin
        // redirect flush后回退软件LSQ admission镜像，确保重入队仍与当前软件pointer一致。
    ...
    "collect_lsq_candidates": fn("collect_lsq_candidates", {
      file: "memblock_lsqenq_dispatch_base_sequence.sv",
      input: "data.get_next_new_admit_uid()、总slot目标、V2 6/4和free count",
      output: "本拍候选 uid/tr/behavior/lq/sq key",
      purpose: "选择本拍能入队的连续 uid。",
      role: "从公共连续uid起点采样一次总slot目标，再按load/store 6/4、实际free count和flush状态截断；随机0不消费uid。",
      calls: ["admission_blocked_by_flush", "common_data_transaction::get_status", "common_data_transaction::get_main_transaction", "lsq_ctrl_model::derive_op_behavior", "lsq_ctrl_model::advance_lq_key", "lsq_ctrl_model::advance_sq_key"],
    }),
    ...
    "assign_lsqenq_slot": fn("assign_lsqenq_slot", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "候选main_tr/behavior/lq_key/sq_key", output: "lsqenq xaction slot", purpose: "填 DUT enqLsq payload。", role: "把主表ROB/fuType/fuOpType和预览LQ/SQ key交给唯一setter；scalar字段由setter固定。", calls: ["set_need_alloc", "set_req_fields"] }),
    ...
    "set_req_fields": fn("set_req_fields", {
      file: "memblock_lsqenq_dispatch_base_sequence.sv",
      input: "tr/slot/valid/main_tr/behavior/lq_key/sq_key；tr非null且slot小于编译期slot数",
      output: "只写当前xaction slot的req qualifier/payload",
      purpose: "按fail-fast入口合同写入单个enqLsq req slot。",
      role: "写字段前先检查tr和slot。active要求main_tr非null、main/behavior均为scalar单元素且need_alloc与uses_lq/sq严格匹配load或store，key来自candidate preview；idle要求main_tr为null、behavior完整等于make_default_behavior、LQ/SQ key全0。直接合同失败先fatal；active key范围由driver在写VIF前检查，实际使用key漂移由confirm在公共reservation前检查。setter不修改其它slot或任何公共状态。",
      sideEffects: ["只修改tr的当前slot", "不修改主表、状态表、active map、pending-sample、pointer或free count"],
    }),
    "lsqenq_agent_agent_driver::main_phase": fn("lsqenq_agent_agent_driver::main_phase", {
      file: "mem_ut/ver/ut/memblock/agent/lsqenq_agent_agent/src/lsqenq_agent_agent_driver.sv",
      input: "sequencer item与当前flush epoch",
      output: "当前clocking边界的DUT驱动和launch/abort metadata",
      purpose: "每个clocking边界先让DUT采样上一拍，再决定launch或abort当前item。",
      role: "main_phase在写VIF前检查flush/epoch；有效时调用send_pkt，abort时驱idle；V2不等待ready、不采样response。",
      calls: ["lsqenq_agent_agent_driver::send_pkt"],
    }),
    "lsqenq_agent_agent_driver::send_pkt": fn("lsqenq_agent_agent_driver::send_pkt", {
      file: "mem_ut/ver/ut/memblock/agent/lsqenq_agent_agent/src/lsqenq_agent_agent_driver.sv",
      input: "lsqenq_agent_agent_xaction",
      output: "DUT valid/payload 一次写入",
      purpose: "校验batch合同后把V2 enqLsq request写入VIF。",
      role: "校验scalar字段和load/store 6/4上限，再执行机械字段驱动；flush/epoch和launch metadata由main_phase负责。",
      calls: [],
    }),
    "confirm_lsq_candidates": fn("confirm_lsq_candidates", {
      file: "memblock_lsqenq_dispatch_base_sequence.sv",
      input: "xaction launch metadata与候选/key列表",
      output: "LSQ reservation、uid active、pending sample",
      purpose: "确认当前item确实launch并立即预留软件资源。",
      role: "重新preview实际使用key，一致后调用唯一commit_allocate；本边界不开放issue。",
      calls: ["lsq_ctrl_model::preview_allocate", "lsq_ctrl_model::commit_allocate"],
    }),
    "complete_v2_pending_sample": fn("complete_v2_pending_sample", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "pending uid与pending_sample_flush_epoch", output: "issue-ready completion或丢弃", purpose: "在下一driver边界完成上一批sample。", role: "比较保存epoch与当前dispatch_flush_epoch；相等且flush未阻塞时逐uid调用complete_admission，不等或阻塞时不调用、不开放issue，由既有redirect/cancel owner回退reservation，最后清pending。", calls: ["complete_admission", "clear_v2_pending_sample"] }),
    "lsq_ctrl_model::can_allocate": fn("lsq_ctrl_model::can_allocate", { file: "lsq_ctrl_model.sv", input: "behavior/numLsElem", output: "bit", purpose: "预测本地 LQ/SQ 是否有空间。", role: "用于候选选择，并与V2 6/4 gate共同约束batch。" }),
    "lsq_ctrl_model::preview_allocate": fn("lsq_ctrl_model::preview_allocate", { file: "lsq_ctrl_model.sv", input: "behavior", output: "预期 lq/sq key", purpose: "预览本拍将分配的 key。", role: "填入enq payload，并在launch后确认pointer未漂移。" }),
    "lsq_ctrl_model::commit_allocate": fn("lsq_ctrl_model::commit_allocate", { file: "lsq_ctrl_model.sv", input: "uid/behavior/main transaction", output: "active map与本地LSQ指针推进", purpose: "V2 launch后预留软件LSQ资源。", role: "唯一allocation owner；写回主表key后直接调用data.activate_uid(uid, behavior.uses_lq, behavior.uses_sq)建立active map，再置enq并推进pointer/free count；issue-ready由下一边界pending completion负责。", calls: ["common_data_transaction::set_main_transaction", "common_data_transaction::activate_uid", "common_data_transaction::set_status_field"] }),
    "complete_admission": fn("complete_admission", {
      file: "memblock_lsqenq_dispatch_base_sequence.sv",
      input: "uid",
      output: "issue_ready状态和issue queue",
      purpose: "admission完成后的统一issue-ready后处理。",
      role: "普通LSQ由下一sample边界调用，non-LSQ兼容入口在commit后直接调用；active/enq已完成，这里drain CSR后调用prepare_issue_route_for_uid设置issue_ready并route。TLB entry由后续L2TLB request按key查/建。",
      calls: ["memblock_lsqenq_dispatch_base_sequence::drain_csr_runtime_events", "issue_queue_scheduler::prepare_issue_route_for_uid"],
    }),
    "common_data_transaction::activate_uid": fn("common_data_transaction::activate_uid", { file: "common_data_transaction.sv", input: "uid, map_lq, map_sq", output: "active ROB/LQ/SQ map", purpose: "按显式LQ/SQ映射位激活uid。", role: "commit_allocate直接传入behavior.uses_lq/uses_sq，建立ROB及所需LQ/SQ到uid的反查映射，用于monitor/commit/deq定位。" }),
    "memblock_lsqenq_dispatch_base_sequence::drain_csr_runtime_events": fn("memblock_lsqenq_dispatch_base_sequence::drain_csr_runtime_events", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "raw CSR queue", output: "runtime CSR镜像更新", purpose: "sample completion后同步CSR runtime snapshot。", role: "后续L2TLB request查/建entry时读取最新ASID/VMID/s2xlate。" }),
...
endtask:send_l2tlb_cycle`,
    ...
    "issue_queue_scheduler::route_all_ready_uids": fn("issue_queue_scheduler::route_all_ready_uids", { file: "issue_queue_scheduler.sv", input: "active窗口与status/main表", output: "issue queues更新", purpose: "按compile slot上限扫描并路由ready项。", role: "要求active/enq/issue_ready，过滤flush/redirect/exception和不匹配replay target；不以tlb_mapped作为route gate。", calls: ["issue_queue_scheduler::route_uid"] }),
    ...
    "lsq_ctrl_model::is_vector_ls_futype": fn("lsq_ctrl_model::is_vector_ls_futype", { file: "lsq_ctrl_model.sv", input: "fuType", output: "bit", purpose: "识别 vector LS FuType。", role: "scalar split issue 在字段写入前用它拒绝 vector LS。" }),
    "lsq_ctrl_model::is_load_fuoptype": fn("lsq_ctrl_model::is_load_fuoptype", { file: "lsq_ctrl_model.sv", input: "fuOpType", output: "bit", purpose: "识别普通 load FuOpType。", role: "check_target_futype_fuoptype 用它确认 LDU 的普通 load 合同。" }),
    "lsq_ctrl_model::is_prefetch_fuoptype": fn("lsq_ctrl_model::is_prefetch_fuoptype", { file: "lsq_ctrl_model.sv", input: "fuOpType", output: "bit", purpose: "识别 prefetch FuOpType。", role: "check_target_futype_fuoptype 用它确认 LDU 的 prefetch 合同。" }),
    "lsq_ctrl_model::is_store_fuoptype": fn("lsq_ctrl_model::is_store_fuoptype", { file: "lsq_ctrl_model.sv", input: "fuOpType", output: "bit", purpose: "识别 store FuOpType。", role: "check_target_futype_fuoptype 用它确认 STU 的 store 合同。" }),
    "lsq_ctrl_model::is_cbo_fuoptype": fn("lsq_ctrl_model::is_cbo_fuoptype", { file: "lsq_ctrl_model.sv", input: "fuOpType", output: "bit", purpose: "识别 CBO FuOpType。", role: "scalar split issue 对 CBO 直接 fail-fast。" }),
    ...
    "scalar_issue_compile_time_layout": fn("scalar_issue_compile_time_layout", { kind: "编译期合同", file: "memblock_compile_params.svh / memblock_dispatch_types.sv / lintsissue_agent_agent_xaction.sv", input: "LOAD/STA/STD port base 与 pipe 数", output: "target port base、总 port 数和 fired mask 宽度", purpose: "描述当前 scalar split issue 的静态布局合同。", role: "LOAD 使用 LOAD_PORT_BASE，STA/STD base 由前一组 base/count 推导；总 port 数和 fired mask 宽度随布局计算。当前 V2 物理信号是 issueLda[0..2]、issueSta[0..1]、issueStd[0..1]，这些展开数量不是通用逻辑编号；若要超过已生成字段扩展物理端口，仍需同步更新 interface/xaction/driver。", external: true }),
    "issue_queue_scheduler::has_pending_issue_work": fn("issue_queue_scheduler::has_pending_issue_work", { file: "issue_queue_scheduler.sv", input: "common_data 中的 LOAD/STA/STD issue queue", output: "bit", purpose: "判断是否仍有待发射 issue work。", role: "只检查三个 issue queue 是否非空；队列为空时不触发 no-progress error。", source: String.raw`function bit has_pending_issue_work();
    ensure_data();
    return data.load_issue_q.size() != 0 ||
           data.sta_issue_q.size()  != 0 ||
           data.std_issue_q.size()  != 0;
endfunction:has_pending_issue_work`, logicNotes: ["这是当前源码的近似骨架，省略了 data null fatal；该 helper 只观察待发射队列，不把 active map、writeback 等待或 terminal 状态误判为 issue pending。"] }),
    ...
    "memblock_issue_dispatch_base_sequence::body": fn("memblock_issue_dispatch_base_sequence::body", {
      ...
      purpose: "真实驱动 V2 scalar split 的 issueLda/issueSta/issueStd 接口。",
      role: "每拍 route、select、assign、drive，再只对 DUT 接收的 item 更新发射状态；vector issue 不属于当前默认 flow。",
      calls: ["seq_csr_common::init", "wait_for_main_table", "drive_dispatch_issue_loop"],
    }),
    "drive_dispatch_issue_loop": fn("drive_dispatch_issue_loop", { file: "memblock_issue_dispatch_base_sequence.sv", input: "无", output: "多拍 issue fire 和 no-progress 诊断", purpose: "issue 发射主循环。", role: "每拍 route、send、推进 delay 后查询 has_pending_issue_work；只有 issue queue 非空且持续无 fire 时才累计并按阈值报 error，队列为空时清零 idle 计数。", calls: ["issue_queue_scheduler::route_all_ready_uids", "send_issue_cycle", "issue_queue_scheduler::advance_issue_queue_delays", "issue_queue_scheduler::has_pending_issue_work"], source: String.raw`forever begin
    ...
    if (has_fire) idle_count = 0;
    else if (pending_issue_work) begin
        idle_count++;
        if ((idle_count % no_progress_warn_cycles) == 0) report_no_progress_error();
    end else idle_count = 0;
end`, logicNotes: ["这是当前源码的近似骨架，省略了日志格式和零阈值保护；关键语义是 no-progress error 只在 issue queue 非空且没有 fire 时触发。"] }),
    ...
    "send_issue_cycle": fn("send_issue_cycle", { file: "memblock_issue_dispatch_base_sequence.sv", input: "cycle_idx", output: "本拍 issue xaction", purpose: "一拍 scalar split issue 发射。", role: "选择候选并填入 issueLda/issueSta/issueStd xaction；finish_item 后按候选构造 compile-time 宽度 candidate_mask，拒绝 driver 返回候选外 bit，阻塞模式还要求全部 candidate fire，再标记 effective fired item。", calls: ["select_issue_candidates", "assign_issue_items", "lintsissue_agent_agent_driver::main_phase", "lintsissue_agent_agent_driver::wait_dispatch_issue_ready", "lintsissue_agent_agent_driver::drive_dispatch_issue_one_cycle", "memblock_issue_dispatch_base_sequence::port_idx_for_item", "mark_fired_items"], source: String.raw`candidate_mask = '0;
foreach (fired_items[idx])
    candidate_mask[port_idx_for_item(fired_items[idx])] = 1'b1;
effective_fired_mask = tr.memblock_dispatch_fired_mask & candidate_mask;
if ((tr.memblock_dispatch_fired_mask & ~candidate_mask) != '0)
    fatal("driver returned fired bits outside candidate mask");
if (!nonblocking && !abort_or_flush && effective_fired_mask != candidate_mask)
    fatal("blocking issue completed without all candidate fires");
if (effective_fired_mask != '0)
    mark_fired_items(fired_items, effective_fired_mask);`, logicNotes: ["这是当前源码的近似骨架，省略了具体日志文本并用 abort_or_flush 汇总两个条件；candidate_mask 由 port_idx_for_item() 逐个候选生成，避免把未被本拍选中的 mask bit 误当成 fire。"] }),
    ...
    "assign_issue_items": fn("assign_issue_items", { file: "memblock_issue_dispatch_base_sequence.sv", input: "selected items", output: "lintsissue xaction", purpose: "把候选 item 映射到 split issue pipe payload。", role: "LOAD/STA/STD 各自从 local pipe index 写入 issueLda/issueSta/issueStd；候选数量受对应 compile-time pipe count 限制。", calls: ["issue_field_assigner::assign_issue_item_fields"] }),
    "issue_field_assigner::assign_issue_item_fields": fn("issue_field_assigner::assign_issue_item_fields", {
      ...
      purpose: "发射字段赋值总入口和 scalar 合同校验入口。",
      role: "真实源码中字段赋值逻辑在 issue_field_assigner.sv；先确认 split style、main transaction、pipe 范围、scalar-only FuType/FuOpType 与 target route 一致，再写主字段、依赖字段和后端 meta 字段。",
      calls: ["issue_field_assigner::check_pipe_idx", "issue_field_assigner::check_target_futype_fuoptype", "assign_main_issue_fields", "assign_issue_dep_fields", "assign_backend_meta_fields"],
    ...
    if (!MEMBLOCK_DUT_ISSUE_PORT_STYLE_SPLIT) fatal("split style required");
    check_pipe_idx(item.target, pipe_idx, "assign_issue_item_fields");
    if (lsq_ctrl_model::is_vector_ls_futype(main_tr.fuType)) fatal("vector LS outside scalar scope");
    ...
    assign_backend_meta_fields(tr, item, pipe_idx);
      ...
      logicNotes: [
        "这是当前源码的近似骨架，省略了 null 检查和 fatal 文本；校验顺序先于三个字段赋值子函数。",
        "vector LS、atomic/MOU、CBO 或 FuType/FuOpType/target 不匹配会在进入字段写入前 fail-fast。",
        "网页统一使用 issue_field_assigner:: 前缀，避免误以为字段赋值主实现仍在 memblock_dispatch_base_sequence.sv。",
      ],
    }),
    "issue_field_assigner::get_target_pipe_limit": fn("issue_field_assigner::get_target_pipe_limit", { file: "issue_field_assigner.sv", input: "issue target", output: "编译期 pipe 数", purpose: "取得 target 对应的物理 pipe 数。", role: "分别返回 LOAD/STA/STD 的 MEMBLOCK_DUT_*_PIPE_NUM，不把当前 3/2/2 展开值写成运行期合同。", source: String.raw`function int unsigned get_target_pipe_limit(input memblock_issue_target_e target);
    case (target)
    ...
    endcase
endfunction:get_target_pipe_limit`, logicNotes: ["当前源码直接返回 compile-time pipe count；未知 target 走 fatal 分支，网页骨架省略该分支文本。"] }),
    "issue_field_assigner::check_pipe_idx": fn("issue_field_assigner::check_pipe_idx", { file: "issue_field_assigner.sv", input: "target, pipe_idx, caller", output: "合法性检查或 fatal", purpose: "校验 local pipe index。", role: "调用 get_target_pipe_limit() 做 compile-time count 范围检查。", calls: ["issue_field_assigner::get_target_pipe_limit"], source: String.raw`pipe_limit = get_target_pipe_limit(target);
if (pipe_idx >= pipe_limit)
    fatal(caller, target, pipe_idx, pipe_limit);`, logicNotes: ["这是近似骨架；实际源码使用 UVM fatal，并在错误信息中带 caller、target、pipe_idx 和 limit。"] }),
    "issue_field_assigner::check_target_futype_fuoptype": fn("issue_field_assigner::check_target_futype_fuoptype", { file: "issue_field_assigner.sv", input: "main transaction, op behavior, target", output: "scalar route 合法性或 fatal", purpose: "校验 scalar split issue 的 FuType/FuOpType 与 target 合同。", role: "拒绝 vector LS、atomic/MOU、CBO 和错误 route；LDU 只允许普通 load/prefetch 走 LOAD，STU 只允许合法 store 走 STA/STD。", calls: ["lsq_ctrl_model::is_vector_ls_futype", "lsq_ctrl_model::is_load_fuoptype", "lsq_ctrl_model::is_prefetch_fuoptype", "lsq_ctrl_model::is_store_fuoptype", "lsq_ctrl_model::is_cbo_fuoptype"], source: String.raw`if (is_vector_ls_futype(main_tr.fuType) ||
    main_tr.fuType == MEMBLOCK_FUTYPE_MOU || behavior.is_atomic)
    fatal("outside scalar split issue scope");
case (main_tr.fuType)
    MEMBLOCK_FUTYPE_LDU: check_load_route_and_fuoptype();
    MEMBLOCK_FUTYPE_STU: check_store_route_and_fuoptype();
    default: fatal("unsupported scalar split FuType");
endcase`, logicNotes: ["这是当前源码的近似骨架；LDU 分支区分 ordinary load 与 prefetch，STU 分支同时拒绝 CBO，并检查 target 必须是合法的 LOAD/STA/STD 路由。"] }),
    ...
    send_pkt(req);
    aborted_by_redirect = 1'b1;
end else begin
    send_pkt(req);
    if (nonblocking) drive_dispatch_issue_one_cycle(req);
    else if (wait_ready) wait_dispatch_issue_ready(req);
end`, logicNotes: ["这是当前源码的近似骨架；实际 main_phase 还处理 pre/post_pkt_gap、无 item idle 和 item_done。"] }),
    "lintsissue_agent_agent_driver::send_pkt": fn("lintsissue_agent_agent_driver::send_pkt", { file: "mem_ut/ver/ut/memblock/agent/lintsissue_agent_agent/src/lintsissue_agent_agent_driver.sv", input: "lintsissue xaction", output: "issueLda/issueSta/issueStd valid 与 payload", purpose: "真实驱动 scalar split issue 接口。", role: "只机械写 VIF；fire 采样由 ready 处理 helper 完成，flush/epoch 边界由对应 driver task 处理。", source: String.raw`task send_pkt(lintsissue_agent_agent_xaction tr);
    drive issueLda[0..2], issueSta[0..1], issueStd[0..1] fields and valid;
endtask:send_pkt`, logicNotes: ["这是近似骨架；实际源码逐字段驱动 VIF。物理字段由 V2 interface 固定，是否参与 pending/fire 则由 compile-time count 门控。"] }),
    "lintsissue_agent_agent_driver::clear_dispatch_issue_ports": fn("lintsissue_agent_agent_driver::clear_dispatch_issue_ports", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction", output: "所有 issue valid 清零", purpose: "清理 flush/abort 边界的待发送 valid。", role: "保留 payload，仅清掉各 issueLda/issueSta/issueStd valid，供 abort 或 idle 边界重新驱动。" }),
    "lintsissue_agent_agent_driver::drive_idle": fn("lintsissue_agent_agent_driver::drive_idle", { file: "lintsissue_agent_agent_driver.sv", input: "driver mode", output: "issue VIF 全零 idle", purpose: "保持没有可发送 item 时的接口空闲。", role: "main_phase 无 item或处理 pre/post packet gap 时驱动空值。" }),
    "lintsissue_agent_agent_driver::wait_dispatch_issue_ready": fn("lintsissue_agent_agent_driver::wait_dispatch_issue_ready", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction", output: "阻塞等待完成、fired mask 或 redirect abort", purpose: "阻塞模式等待所有启用 valid port 被 ready 接收。", role: "每拍先清理已 fire port，再检查 flush epoch，继续 send_pkt；timeout 且仍 pending 时 fatal。", calls: ["lintsissue_agent_agent_driver::has_dispatch_issue_pending", "lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports", "lintsissue_agent_agent_driver::send_pkt"], source: String.raw`while (has_dispatch_issue_pending(tr)) begin
    clear_ready_dispatch_issue_ports(tr);
    if (flush_or_epoch_changed()) abort_pending_ports();
    send_pkt(tr);
end`, logicNotes: ["这是近似骨架；实际源码还维护 wait_cycles，并在配置的 timeout 到达且仍 pending 时 fatal。"] }),
    "lintsissue_agent_agent_driver::drive_dispatch_issue_one_cycle": fn("lintsissue_agent_agent_driver::drive_dispatch_issue_one_cycle", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction", output: "单拍采样结果、fired mask 或 redirect abort", purpose: "非阻塞模式只采样一拍 ready。", role: "若没有 pending valid 直接返回；跨 flush 时清 valid 并 abort，否则清理本拍 ready port 后发送一次。", calls: ["lintsissue_agent_agent_driver::has_dispatch_issue_pending", "lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports", "lintsissue_agent_agent_driver::send_pkt"] }),
    "lintsissue_agent_agent_driver::has_dispatch_issue_pending": fn("lintsissue_agent_agent_driver::has_dispatch_issue_pending", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction", output: "bit", purpose: "判断 driver 是否还有待发送 valid。", role: "每个物理 valid 只在对应 compile-time PIPE_NUM 大于零时参与 pending 判断。", source: String.raw`return (LOAD_PIPE_NUM > 0 && tr.issueLda_0_valid) ||
       (LOAD_PIPE_NUM > 1 && tr.issueLda_1_valid) ||
       (LOAD_PIPE_NUM > 2 && tr.issueLda_2_valid) ||
       (STA_PIPE_NUM > 0 && tr.issueSta_0_valid) ||
       (STA_PIPE_NUM > 1 && tr.issueSta_1_valid) ||
       (STD_PIPE_NUM > 0 && tr.issueStd_0_valid) ||
       (STD_PIPE_NUM > 1 && tr.issueStd_1_valid);`, logicNotes: ["这是近似骨架；源码使用 compile-time 宏门控每个 V2 物理 valid，这里用去掉宏前缀的字段名展示逻辑。"] }),
    "lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports": fn("lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction 与 DUT ready", output: "清除已 fire valid，并更新 fired mask", purpose: "识别本拍 valid&&ready 的 issue port。", role: "对 compile-time count 启用且 valid 的物理 port，ready 为 X/Z 时立即 fatal；ready 严格等于 1 才按 base/count 计算逻辑 port并调用 record_dispatch_issue_fire。", calls: ["dut_ready_fire_boundary", "lintsissue_agent_agent_driver::record_dispatch_issue_fire"], source: String.raw`if (valid) begin
    if ($isunknown(ready)) fatal("ready is X/Z while valid");
    if (ready === 1'b1)
        record_dispatch_issue_fire(base + local_pipe, tr);
end`, logicNotes: ["这是近似骨架；实际源码对各个 V2 物理字段逐项执行 X/Z 检查、清 valid 和 fire 记录，并按 base/count 计算 port。"] }),
    "lintsissue_agent_agent_driver::record_dispatch_issue_fire": fn("lintsissue_agent_agent_driver::record_dispatch_issue_fire", { file: "lintsissue_agent_agent_driver.sv", input: "port_idx, issue xaction", output: "fired mask 对应 bit 置 1，并输出 fire 诊断", purpose: "记录一次 scalar issue fire。", role: "检查 port_idx 位于从 LOAD_PORT_BASE 开始的 compile-time scalar issue 区间后设置 mask，再调用 report_dispatch_issue_fire。", calls: ["lintsissue_agent_agent_driver::report_dispatch_issue_fire"], source: String.raw`if (port_idx < load_port_base || port_idx >= issue_port_num)
    fatal("fire port outside scalar issue range");
tr.memblock_dispatch_fired_mask[port_idx] = 1'b1;
report_dispatch_issue_fire(port_idx, tr);`, logicNotes: ["这是近似骨架；该 helper 是 fired mask 的唯一写入入口，未发生 valid&&ready 的 port 不会被记录。"] }),
    "lintsissue_agent_agent_driver::report_dispatch_issue_fire": fn("lintsissue_agent_agent_driver::report_dispatch_issue_fire", { file: "lintsissue_agent_agent_driver.sv", input: "port_idx, issue xaction", output: "按 target 分类的低级别诊断", purpose: "按 base 区间把逻辑 port 分类为 LOAD/STA/STD。", role: "分类边界由各 target base/count 推导，并对越界 port fatal。" }),
    "vecissue_agent_agent_driver::send_pkt": fn("vecissue_agent_agent_driver::send_pkt", { file: "mem_ut/ver/ut/memblock/agent/vecissue_agent_agent/src/vecissue_agent_agent_driver.sv", input: "vector issue xaction", output: "vector 接口 idle 或 fail-fast", purpose: "执行 scalar-only vector issue 边界保护。", role: "任一 issueVldu valid 非 0 都 fatal；合法的全零 transaction 只驱动 idle。当前 testcase 不绑定 vecissue 默认 sequence。", calls: ["vecissue_agent_agent_driver::drive_idle"], source: String.raw`if (issueVldu_0_valid !== 0 || issueVldu_1_valid !== 0)
    fatal("vector issue is outside current scalar scope");
drive_idle(DRV_0);`, logicNotes: ["这是近似骨架；当前 driver 不向 vector DUT 口发送有效请求，非法 vector transaction 在 VIF 驱动前 fail-fast。"] }),
    "vecissue_agent_agent_driver::drive_idle": fn("vecissue_agent_agent_driver::drive_idle", { file: "vecissue_agent_agent_driver.sv", input: "DRV_0", output: "vector issue VIF 全零", purpose: "保持未启用的 vector issue 接口 idle。", role: "scalar-only send_pkt 在通过 valid 检查后调用该 task。", source: String.raw`if (drv_mode == DRV_0)
    drive every vector issue field and valid to zero;`, logicNotes: ["这是近似骨架；实际源码逐字段把 issueVldu payload、valid 和相关控制信号驱为 0。"] }),
    "mark_fired_items": fn("mark_fired_items", { file: "memblock_issue_dispatch_base_sequence.sv", input: "fired_items, fired_mask", output: "status/queue 更新", purpose: "标记 DUT 已接受的发射项。", role: "防止未 ready 或 redirect abort 的 item 被错误删除；先按 target/local pipe 转换 port，再根据 fired mask 更新状态。", calls: ["memblock_issue_dispatch_base_sequence::port_idx_for_item", "issue_queue_scheduler::mark_issue_fire", "issue_queue_scheduler::mark_issue_fire_already_accepted", "submit_issue_accept_pass"] }),
    "memblock_issue_dispatch_base_sequence::port_idx_for_item": fn("memblock_issue_dispatch_base_sequence::port_idx_for_item", { file: "memblock_issue_dispatch_base_sequence.sv", input: "issue queue item", output: "逻辑 scalar issue port index", purpose: "把 target/local pipe 映射到 compile-time 布局中的 port。", role: "使用 LOAD/STA/STD 各自 base 和 PIPE_NUM 做范围校验，同时确认结果落在总 port 数和 mask 宽度内。", source: String.raw`case (item.target)
    LOAD: port_idx = LOAD_PORT_BASE + item.uop_index;
    STA:  port_idx = STA_PORT_BASE  + item.uop_index;
    STD:  port_idx = STD_PORT_BASE  + item.uop_index;
endcase
check local pipe < target PIPE_NUM;
check port_idx < SCALAR_ISSUE_PORT_NUM && port_idx < SCALAR_ISSUE_MASK_W;
return port_idx;`, logicNotes: ["这是当前源码的近似骨架；实际函数对未知 target、local pipe 越界和总 port/mask 越界分别 fatal。"] }),
...
endtask:process_pending_events`,
  ...
  const flowDefinitions = {
    overview: {
      ...
      stages: [
        ...
        stage("并行接口流", "agent default sequence 并行驱动 DUT 接口并通过 common_data 协同。", [
          ...
          lane("issue", ["scalar_issue_compile_time_layout", "memblock_issue_dispatch_base_sequence::body", "send_issue_cycle", "issue_field_assigner::assign_issue_item_fields", "lintsissue_agent_agent_driver::record_dispatch_issue_fire", "issue_queue_scheduler::mark_issue_fire", "submit_issue_accept_pass"]),
    ...
    lsq_admission: {
      ...
      summary: "LSQ admission sequence从主表取连续uid，按V2 6/4和free count组batch；clock-first driver launch后预留资源，下一边界才route issue queue。",
      stages: [
        ...
        stage("一拍 admission", "选候选、填slot、clock-first launch、完成上一批sample并预留当前批。", [
          lane("cycle", ["send_lsqenq_cycle", "memblock_lsqenq_dispatch_base_sequence::apply_pending_lsq_cancels", "admit_non_lsq_if_ready"]),
          lane("select", ["common_data_transaction::get_next_new_admit_uid", "collect_lsq_candidates", "lsq_ctrl_model::derive_op_behavior", "lsq_ctrl_model::advance_lq_key", "lsq_ctrl_model::advance_sq_key"]),
          lane("drive", ["assign_lsqenq_slot", "lsqenq_agent_agent_driver::send_pkt"]),
          lane("commit", ["complete_v2_pending_sample", "confirm_lsq_candidates", "lsq_ctrl_model::commit_allocate"]),
        ]),
        stage("入队后处理", "active/enq在commit_allocate reservation完成；下一driver边界由complete_v2_pending_sample开放issue。", [
          lane("active/enq", ["lsq_ctrl_model::commit_allocate", "common_data_transaction::set_status_field", "common_data_transaction::mark_uid_enqueued", "common_data_transaction::activate_uid"]),
          lane("issue-ready", ["complete_admission", "issue_queue_scheduler::prepare_issue_route_for_uid", "issue_queue_scheduler::route_uid"]),
    ...
    issue: {
      ...
      stages: [
        ...
        stage("select", "按优先级、ROB 年龄、pipe 数选择发射候选。", [
          lane("sequence", ["memblock_issue_dispatch_base_sequence::body", "drive_dispatch_issue_loop", "issue_queue_scheduler::has_pending_issue_work", "send_issue_cycle", "submit_issue_accept_pass"]),
        ...
        stage("assign/drive/mark", "填 payload、驱动 DUT、更新状态。", [
          lane("assign", ["assign_issue_items", "issue_field_assigner::assign_issue_item_fields", "issue_field_assigner::check_pipe_idx", "issue_field_assigner::check_target_futype_fuoptype", "assign_main_issue_fields", "assign_issue_dep_fields", "assign_backend_meta_fields"]),
          lane("drive", ["lintsissue_agent_agent_driver::main_phase", "lintsissue_agent_agent_driver::send_pkt", "lintsissue_agent_agent_driver::wait_dispatch_issue_ready", "lintsissue_agent_agent_driver::drive_dispatch_issue_one_cycle", "lintsissue_agent_agent_driver::clear_dispatch_issue_ports", "lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports", "lintsissue_agent_agent_driver::record_dispatch_issue_fire", "dut_ready_fire_boundary"]),
          lane("mark", ["mark_fired_items", "memblock_issue_dispatch_base_sequence::port_idx_for_item", "issue_queue_scheduler::mark_issue_fire", "issue_queue_scheduler::mark_issue_fire_already_accepted", "submit_issue_accept_pass", "common_data_transaction::mark_issue_snapshot", "common_data_transaction::delete_issue_queue_entry", "common_data_transaction::clear_replay_target_after_fire"]),
          lane("scalar boundary", ["vecissue_agent_agent_driver::send_pkt", "vecissue_agent_agent_driver::drive_idle"]),
        ]),
      ],
    },
    monitor_writeback: {
    ...
    final_check: {
      ...
      stages: [
        ...
        stage("uid 生命周期", "uid 从主表创建到 active、issue、writeback、commit/deq、retire。", [
          ...
          lane("active", ["common_data_transaction::activate_uid", "common_data_transaction::mark_uid_enqueued", "common_data_transaction::mark_issue_snapshot", "common_data_transaction::mark_target_normal_pass"]),
