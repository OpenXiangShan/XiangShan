(function () {
  ...
  const functionCatalog = {
    ...
    "memblock_lsqenq_dispatch_base_sequence::body": fn("memblock_lsqenq_dispatch_base_sequence::body", {
      ...
      role: "默认开启真实admission；显式关闭时保持idle并直接返回，不fallback父类随机sequence。",
    ...
    "send_lsqenq_cycle": fn("send_lsqenq_cycle", {
      ...
      role: "先消费redirect留下的pending cancel；pending batch后遇non-LSQ时先发送idle完成sample，再执行non-LSQ admission；随后按6/4收集、launch并预留普通candidate。",
    ...
    "memblock_lsqenq_dispatch_base_sequence::apply_pending_lsq_cancels": fn("memblock_lsqenq_dispatch_base_sequence::apply_pending_lsq_cancels", { file: "memblock_lsqenq_dispatch_base_sequence.sv", input: "pending LQ/SQ cancel count", output: "软件LSQ pointer/free count回退并清cancel count", purpose: "在新admission前消费redirect留下的资源回退。", role: "它是send_lsqenq_cycle的第一步，保证reissue使用回退后的LQ/SQ镜像。", calls: ["lsq_ctrl_model::cancel_lq", "lsq_ctrl_model::cancel_sq"] }),
    ...
    "issue_queue_scheduler::has_pending_issue_work": fn("issue_queue_scheduler::has_pending_issue_work", { file: "issue_queue_scheduler.sv", input: "common_data 中的 LOAD/STA/STD issue queue", output: "bit", purpose: "判断是否仍有待发射 issue work。", role: "只检查三个 issue queue 是否非空；队列为空时不触发 no-progress error。" }),
    ...
    "issue_field_assigner::assign_issue_item_fields": fn("issue_field_assigner::assign_issue_item_fields", { file: "issue_field_assigner.sv", input: "tr,item,pipe_idx", output: "完整 issue payload", purpose: "发射字段总入口和 scalar 合同校验入口。", role: "先确认 split style、main transaction、pipe 范围、scalar-only FuType/FuOpType 与 target route 一致，再写主字段、依赖字段和后端 meta。", calls: ["issue_field_assigner::check_pipe_idx", "issue_field_assigner::check_target_futype_fuoptype", "assign_main_issue_fields", "assign_issue_dep_fields", "assign_backend_meta_fields"] }),
    "issue_field_assigner::get_target_pipe_limit": fn("issue_field_assigner::get_target_pipe_limit", { file: "issue_field_assigner.sv", input: "issue target", output: "编译期 pipe 数", purpose: "取得 target 对应的物理 pipe 数。", role: "分别返回 LOAD/STA/STD 的 MEMBLOCK_DUT_*_PIPE_NUM，不把当前 3/2/2 展开值写成运行期合同。" }),
    "issue_field_assigner::check_pipe_idx": fn("issue_field_assigner::check_pipe_idx", { file: "issue_field_assigner.sv", input: "target, pipe_idx, caller", output: "合法性检查或 fatal", purpose: "校验 local pipe index。", role: "使用 get_target_pipe_limit() 做 compile-time count 范围检查。", calls: ["issue_field_assigner::get_target_pipe_limit"] }),
    "issue_field_assigner::check_target_futype_fuoptype": fn("issue_field_assigner::check_target_futype_fuoptype", { file: "issue_field_assigner.sv", input: "main transaction, op behavior, target", output: "scalar route 合法性或 fatal", purpose: "校验 scalar split issue 的 FuType/FuOpType 与 target 合同。", role: "拒绝 vector LS、atomic/MOU、CBO 和错误 route；LDU 只允许普通 load/prefetch 走 LOAD，STU 只允许合法 store 走 STA/STD。", calls: ["lsq_ctrl_model::is_vector_ls_futype", "lsq_ctrl_model::derive_op_behavior"] }),
    ...
    "lintsissue_agent_agent_driver::has_dispatch_issue_pending": fn("lintsissue_agent_agent_driver::has_dispatch_issue_pending", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction", output: "bit", purpose: "判断 driver 是否还有待发送 valid。", role: "每个物理 valid 只在对应 compile-time PIPE_NUM 大于零时参与 pending 判断。" }),
    "lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports": fn("lintsissue_agent_agent_driver::clear_ready_dispatch_issue_ports", { file: "lintsissue_agent_agent_driver.sv", input: "issue xaction 与 DUT ready", output: "清除已 fire valid，并更新 fired mask", purpose: "识别本拍 valid&&ready 的 issue port。", role: "对 compile-time count 启用且 valid 的物理 port，ready 为 X/Z 时立即 fatal；ready 严格等于 1 才按 base/count 计算逻辑 port 并调用 record_dispatch_issue_fire。", calls: ["dut_ready_fire_boundary", "lintsissue_agent_agent_driver::record_dispatch_issue_fire"] }),
    ...
    "vecissue_agent_agent_driver::drive_idle": fn("vecissue_agent_agent_driver::drive_idle", { file: "vecissue_agent_agent_driver.sv", input: "DRV_0", output: "vector issue VIF 全零", purpose: "保持未启用的 vector issue 接口 idle。", role: "scalar-only send_pkt 在通过 valid 检查后调用该 task。" }),
    ...
    overview: {
      ...
      stages: [
        ...
        stage("并行接口流", "agent default sequence 并行驱动 DUT 接口并通过 common_data 协同。", [
          ...
          lane("issue", ["scalar_issue_compile_time_layout", "memblock_issue_dispatch_base_sequence::body", "send_issue_cycle", "issue_field_assigner::check_target_futype_fuoptype", "lintsissue_agent_agent_driver::record_dispatch_issue_fire", "issue_queue_scheduler::mark_issue_fire", "submit_issue_accept_pass"]),
    ...
    lsq_admission: {
      ...
      stages: [
        ...
        stage("一拍 admission", "选候选、填slot、clock-first launch、完成上一批sample并预留当前批。", [
          ...
          lane("select", ["collect_lsq_candidates", "lsq_ctrl_model::derive_op_behavior", "lsq_ctrl_model::advance_lq_key", "lsq_ctrl_model::advance_sq_key"]),
        ...
        stage("入队后处理", "active/enq在commit_allocate reservation完成；下一driver边界由complete_v2_pending_sample开放issue。", [
          lane("active/enq", ["lsq_ctrl_model::commit_allocate", "common_data_transaction::set_main_transaction", "common_data_transaction::activate_uid"]),
    ...
    final_check: {
      ...
      stages: [
        ...
        stage("uid 生命周期", "uid 从主表创建到 active、issue、writeback、commit/deq、retire。", [
          ...
          lane("active", ["common_data_transaction::activate_uid", "common_data_transaction::mark_issue_snapshot", "common_data_transaction::mark_target_normal_pass"]),
