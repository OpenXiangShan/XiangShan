...
| `reason overlap` | 同一 global sample 内同一 flush 来源被重复发布。 | `sample_producer_reason_mask` | CSR 与 FENCE 可合并；同一 reason 重复立即 fatal。 |
...
启动 responder：
  ...
  每个新 event 必须是 owner cursor 的下一个连续 event_seq；缺号不允许静默跳过。
