...
2. 生成顺序、随机约束和 directed 控制。
3. sequence、handler、scheduler、adapter、driver/responder 的职责边界。
4. 测试框架自身的激励有效性检查机制，例如参数组合是否合法、是否成功生成目标 transaction、标签与实际生成字段是否一致、fixed 模式是否命中目标激励。

激励有效性检查只检查生成过程是否自洽，包括约束是否满足、目标 transaction 是否生成、fixed override 是否命中和字段/标签是否一致。不得根据 DUT observed result、RM expected value、scoreboard compare 或 coverage bin 命中情况判断该激励是否通过或正确。
...
plan 可以根据实际语义和项目约定选择复用已有能力或实现本地逻辑；具体实现取舍不作为 plan 的强制说明项。
...
### 5.1.1 首章术语表与函数抽象描述（强制）

正式 plan 的第一个正文一级章节（通常为第一个 `##` 章节）必须包含“专有名词与抽象功能说明”小节。该小节必须在任何主流程、功能描述、流程图或文字伪代码之前，解释全文后续使用的专有英文名词，例如 `batch`、`pending sample`、`epoch`、`sideband`。术语表至少写出当前中文含义、代码对象或状态落点，以及一个必要的场景示例；不能只在术语第一次出现的位置临时翻译。

纯函数名、字段名、信号名、路径、命令和日志原文可以保留英文，但其背后的概念如果会影响 plan 读者理解，仍必须在首章解释。术语表必须覆盖主 flow、差异章节、功能描述和所有文字伪代码中实际使用的专有英文名词。

凡 plan 展开实现逻辑的函数、task、helper 或 plan 明确要求实现的关键函数（包括新增和修改对象），在详细源码级伪代码或逐分支逻辑之前必须提供“抽象功能描述”。抽象描述只说明调用阶段和职责边界，不罗列内部 `if/else`、循环、逐字段赋值或子函数调用；这些内容由后续详细伪代码说明。

推荐格式：

```text
抽象功能描述：build_target_transaction() 在测试序列构造目标激励时，
根据当前配置生成供后续 driver 使用的 transaction；它不承担 DUT 正确性判断。
```

对容易混淆的时序或交互关系，建议用最小例子辅助说明。
...
1. Plan 定位、专有名词与抽象功能说明
   说明该 plan 是测试框架激励生成、调度、响应建模还是其它类型；
   在本章解释全文使用的专有英文名词，并给出关键函数的抽象功能职责。

2. 目标功能 Flow 总览
   用 Mermaid 或文字 flow 描述最终功能入口、主流程、关键分支和退出路径。

3. 字段、参数和标签语义
   解释新增 enum、profile、mode、参数、字段和标签的权威来源及使用方式；不重复首章术语表。

4. 参数与配置 Flow
   描述 plus/cfg 参数、默认值、合法性检查、权重选择和 directed 使用方式。

5. 主流程实现 Flow
   按真实执行顺序描述入口函数、关键 helper 和关键分支。

6. 关键 helper 细节
   每个新增或修改 helper 单独成节，包含职责和文字伪代码。
...
8. 正式 plan 不要求固定设置 “RM/覆盖率协同支持” 章节。
   只有当该功能确实需要给 RM、checker、scoreboard 或 coveragent 留后续衔接入口时，才在 plan 末尾增加对应协同支持章节。
   协同支持章节只能描述后续可使用的字段、事件或标签，不能实现 DUT 正确性 checker、RM 或 covergroup。
9. 正式 plan 的每个函数逻辑小节必须先给出抽象功能描述，再给出源码级伪代码和中文详细伪代码；抽象描述不能代替控制流说明，也不能把内部实现细节堆入抽象描述。
...
1. 内容完整性：
   ...
   是否遗漏参数、默认值、开关、权重、fixed/direct 模式或文档同步要求。
   是否错误删除了用户明确确认过的约束。

2. 测试框架整体角度：
   该方案是否破坏主表生成、sequence 调度、driver/responder 或参数管理的一致性。
   是否把 coverage/checker/RM 职责混入测试框架激励生成主流程。
   是否存在非法激励判断错误，把 DUT 合法异常激励误判为非法，或把框架不支持行为误放行为合法激励。

3. 功能本身角度：
   选择顺序、候选构造、权重随机、标签同步和校验策略是否自洽。
   每个关键分支是否说明触发条件和后续行为。
   新增 helper 的职责是否清楚。

4. 性能优化角度：
   是否引入每 transaction 全表扫描、重复大集合遍历或运行期高复杂度逻辑。
   是否可通过已有 helper 或更简单的实现降低复杂度。
   性能优化是否没有牺牲激励语义或可解释性。
...
5. 文档格式角度：
   ...
   正式 plan 的第一个正文一级章节是否已经解释全部后文会使用的专有英文名词，并给出当前语义、代码落点和必要示例。
   每个新增或修改函数在详细伪代码前是否有独立的抽象功能描述，且没有把内部循环和分支误写进抽象层。
...
2. 功能代码实现逻辑差异：
   关键入口、选择顺序、候选构造、标签同步等逻辑如何变化。

3. 函数/helper 差异：
   新增哪些函数/helper，删除或不再新增哪些函数/helper，复用哪些已有函数/helper。
   对新增函数/helper，必须先给出不展开内部实现细节的抽象功能描述，再说明该函数/helper 的职责。
...
修改前逻辑行为：
  按初步 plan 或当前实现的原始行为描述入口、关键分支和函数调用。
  如果描述中出现子函数调用，必须简短说明该子函数在旧逻辑中的功能和副作用。

修改后逻辑行为：
  按正式 plan 的最终行为描述入口、关键分支和函数调用。
  如果描述中出现子函数调用，必须简短说明该子函数在新逻辑中的功能和副作用。

差异影响：
  说明差异是否改变功能语义、参数语义、默认行为、函数/helper 落点、性能路径或文档同步范围。
```
...
抽象功能描述：
  用 1 到 3 句话说明该函数/helper 在当前 flow 中的职责和调用阶段；不罗列内部判断、循环和逐字段赋值。

添加原因：
  写明为什么必须新增该函数/helper，不能只写“为了整理 flow”。

功能说明：
  写明该函数/helper 与当前功能 flow 的关系。

详细文字伪代码：
  先说明该函数/helper 在当前功能 flow 中承担什么角色。
  再按执行顺序描述关键判断、循环、候选构造和 return。
...
抽象功能描述：
  先用 1 到 3 句话说明该函数/helper 在当前 flow 中的外部职责、调用阶段和可见状态结果；不罗列内部实现细节。

修改前文字伪代码：
  按旧逻辑执行顺序描述入口检查、关键分支和子函数调用。
  凡调用子函数，必须说明该子函数在旧逻辑中的功能和副作用。

修改后文字伪代码：
  按新逻辑执行顺序描述入口检查、关键分支和子函数调用。
  凡调用子函数，必须说明该子函数在新逻辑中的功能和副作用。

差异影响：
  写明该函数/helper 修改后对上层 flow、参数语义或性能路径的影响。
```

如果正式 plan 与初步 plan 没有功能差异，也必须写：

```text
本次整理不改变功能行为、实现逻辑、函数/helper 或参数。
```
...
1. 该 plan 的目标是激励生成、调度驱动、响应建模还是其它测试框架工作。
2. plan 是否把 coverage/checker/RM 职责混入测试框架主流程。
3. 如果某个激励组合被禁止，禁止原因是 DUT 输入层非法、DUT 行为模拟层非法、框架交互约束不满足、plan 目标/标签语义不一致，还是仅仅缺少 coverage/checker。
4. 新增参数和 fixed override 是否能稳定构造目标激励。
5. plan 是否把 DUT 正确行为 check 放入末尾 `RM 协同支持`，并明确本 plan 不实现。
6. plan 是否把 coveragent/功能覆盖率内容放入末尾 `功能覆盖率协同支持`，并明确本 plan 不实现。
7. 测试框架本体是否只实现激励有效性检查，而非 DUT 结果正确性判断。
8. 新增字段是否只有一个权威来源，是否避免同一语义多字段重复维护。
9. 新增 helper 是否优先复用 UVM/SystemVerilog 或项目已有 helper。
10. 如果 plan 明确模拟某段 DUT/Scala 行为，是否列出对应源码依据，并说明测试框架不会把源码中不可能出现的行为组合标记为合法行为激励。
11. 第一个正文一级章节是否包含覆盖全文的专有英文名词表，并说明每个术语的当前含义、代码落点和必要示例。
12. 每个新增或修改 function、task、class method、helper 是否在详细伪代码前提供抽象功能描述，且抽象层没有罗列内部实现细节。
13. 如果 plan 新增 function、task、class method、helper 或参数，是否同步满足对应外部规则文件要求。
