...
正式 plan 的第一个正文一级章节（通常为第一个 `##` 章节）必须包含“专有名词与抽象功能说明”小节。该小节必须在任何主流程、功能描述、流程图或文字伪代码之前，解释全文后续使用的专有英文名词，例如 `batch`、`reservation`、`pending sample`、`epoch`、`owner`、`cursor`、`sideband`。术语表至少写出当前中文含义、代码对象或状态落点，以及一个必要的场景示例；不能只在术语第一次出现的位置临时翻译。

纯函数名、字段名、信号名、路径、命令和日志原文可以保留英文，但其背后的概念如果会影响 plan 读者理解，仍必须在首章解释。术语表必须覆盖主 flow、差异章节、失败策略、功能描述和所有文字伪代码中实际使用的专有英文名词。

凡 plan 展开实现逻辑的函数、task、helper 或 plan 明确要求实现的关键函数（包括新增和修改对象），在详细源码级伪代码或逐分支逻辑之前必须提供“抽象功能描述”。抽象描述只说明调用阶段、输入来源、对外输出/状态转换和职责边界，不罗列内部 `if/else`、循环、逐字段赋值或子函数调用；这些内容由后续详细伪代码说明。

推荐格式：

```text
抽象功能描述：complete_v2_pending_sample() 在下一次 driver 采样边界后，
把上一批已发送但尚未开放 issue 的 uid 转为可 issue 状态；redirect/flush 使 epoch 失效时，
它只抑制旧批次的 issue 开放，不负责回退 reservation。
```

对容易混淆的时序、所有权或生命周期，建议用最小例子辅助说明；例如明确“C0 发送 batch A，C1 DUT 采样 A，C1 才开放 A 的 issue”。
...
3. 字段、参数和标签语义
   解释新增 enum、profile、mode、参数、状态字段和标签的权威来源；不重复首章术语表，
   重点说明字段的写者、读者、生命周期和状态副作用。
...
3. 函数/helper 差异：
   ...
   对新增函数/helper，必须先给出不展开内部实现细节的抽象功能描述，再说明该函数/helper 的输入、输出和副作用。
...
抽象功能描述：
  用 1 到 3 句话说明该函数/helper 在当前 flow 中的职责、调用阶段、输入来源和对外状态结果；不罗列内部判断、循环和逐字段赋值。
...
18. 第一个正文一级章节是否包含覆盖全文的专有英文名词表，并说明每个术语的当前含义、代码落点和必要示例。
19. 每个新增或修改 function、task、class method、helper 是否在详细伪代码前提供抽象功能描述，且抽象层没有罗列内部实现细节。
20. 如果 plan 新增 function、task、class method、helper 或参数，是否同步满足对应外部规则文件要求。
