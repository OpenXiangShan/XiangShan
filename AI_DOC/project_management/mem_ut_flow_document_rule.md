...
Flow 文档的第一个正文一级章节（通常为第一个 `##` 章节）必须包含“术语与抽象功能说明”小节，并在主流程图和“函数调用 Flow 图整体文字伪代码”之前，解释全文会使用的专有英文名词。后文首次出现时再补充说明不能替代首章定义。

术语表必须按当前测试框架语义说明英文名词，而不是给通用词典翻译。至少包含：英文术语、中文含义、代码对象/状态落点、典型场景或示例。`batch`、`reservation`、`pending sample`、`epoch`、`active map`、`owner`、`cursor` 等带有时序、所有权或生命周期含义的词，必须明确说明它们在本 flow 中具体代表什么。

纯函数名、信号名、路径、命令和日志原文可以保留英文；但如果函数名或信号名背后的概念会影响 flow 理解，必须在首章用中文解释其语义。术语表应覆盖流程图、表格、功能描述、整体伪代码和端到端总结中的术语。

推荐格式：

| 英文术语 | 当前 flow 中的中文含义 | 代码对象/状态落点 | 示例 |
|---|---|---|---|
| `batch` | 本拍一起进入 request interface 的一组 transaction | sequence 的 transaction 和 uid 列表 | `batch A` 在下一采样边界被 DUT 采样 |
| `reservation` | 软件状态先占用 LQ/SQ key，防止下一批重复使用 | allocation owner、pointer、free count | launch 后 reservation，sample 后才开放 issue |

### 1.2 函数抽象功能描述（强制）

每个 flow 中有源码实现的函数、task、handler、adapter 或关键 helper，在源码片段和详细伪代码之前必须提供“抽象功能描述”。抽象描述只说明该函数处于哪个阶段、由谁调用、处理什么输入、产生什么对外可见结果或状态转换，以及它是否拥有某项状态；不罗列内部判断、循环、逐字段赋值或子函数调用细节。

详细伪代码随后再按源码顺序展开内部控制流。推荐格式：

```text
抽象功能描述：complete_v2_pending_sample() 负责在下一次 driver 时钟边界后，
把上一批已发送且已完成软件 reservation 的 uid 转为可 issue 状态；如果 redirect epoch 已失效，
它只抑制旧批次的 issue 开放，不拥有 LQ/SQ 资源回退。
```

当术语或时序容易混淆时，应使用一个最小例子说明输入、边界、状态变化和后续消费者，例如“C0 发送 batch A，C1 DUT 采样 A，C1 开放 A 的 issue”。
...
用 1 到 3 句话说明该函数在完整 flow 中的外部职责、调用阶段、输入来源、可见输出或状态转换；不要在这里罗列内部判断、循环和逐字段赋值。
...
- 每个函数章节的源码片段前必须先出现“抽象功能描述”；该描述只说明外部职责，不替代后续详细逻辑。
...
本节规则建立在首章术语表和函数抽象功能描述之上：伪代码中可以使用已经在首章解释过的专有英文名词，但不能重新引入未解释的 `batch`、`reservation`、`epoch`、`owner` 等概念。详细伪代码负责解释实现顺序和副作用，不应把抽象功能描述改写成一串内部语句。
...
- 是否有函数调用 flow 图。
- 第一个一级章节是否包含覆盖全文的专有英文名词表，并给出当前 flow 语义、代码落点和必要示例。
- Flow 图下面是否有整体文字伪代码，并按图和源码顺序解释主链路、关键分支、队列和状态变化。
- 是否按函数调用顺序展开。
- 每个关键函数是否有源码位置。
- 每个实现函数/task/handler/helper 的源码片段前是否有不展开内部细节的“抽象功能描述”。
- 每个关键函数是否有源码摘要。
- 每个关键函数是否有文字伪代码。
