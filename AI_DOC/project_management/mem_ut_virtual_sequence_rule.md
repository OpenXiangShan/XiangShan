...
2. testcase 只负责搭建 env、配置 cfg、解析 `+VSEQ_MAIN`，并在 `main_phase()` 中把目标 vseq
   显式启动到 `env.vsqr`。
...
顶层 vseq 由 `basicTest::main_phase()` 显式启动，不再通过
`env.vsqr.main_phase.default_sequence` 或 `virtual_base_sequence` factory override 间接启动：

```text
build_phase:
  uvm_factory::get().find_wrapper_by_name(main_vseq_name)

main_phase:
  uvm_factory::get().create_object_by_type(main_vseq_wrapper, ...)
  cast 为 virtual_base_sequence
...
场景内部的 orchestration child sequence 运行在 virtual sequencer 上：
...
本节中的 agent `default_sequence` 指 agent 目录中既有 sequence class，不表示允许向
`<agent>.main_phase.default_sequence` 写 UVM phase resource。testcase 和 env 均不得用 phase
`default_sequence` 分散启动 agent；是否启动某个 agent sequence 必须由当前顶层 vseq 统一决定。
...
这类 responder sequence 仍必须由 vseq 通过 `uvm_do_on(responder_seq, p_sequencer.<agent>_sqr)` 启动，
不允许回到 testcase 的 agent phase `default_sequence` 分散配置。
...
无论启动哪类 agent default sequence，都必须由 vseq 通过
`uvm_do_on(default_seq, p_sequencer.<agent>_sqr)` 启动，不允许再通过 testcase 的 agent phase
`default_sequence` 分散配置。
...
basicTest 是否仍使用 `find_wrapper_by_name()`、`create_object_by_type()` 和 `main_phase()` 显式 `start()`。
```
...
```text
basicTest 按名称找到并创建 virtual_base_sequence。
basicTest::main_phase() 在 env.vsqr 上显式 start 该对象。
...
`basicTest` 只负责选择、创建并显式启动顶层 vseq，不直接实现具体场景激励。
...
basicTest::build_phase:
  ...
  调用 find_wrapper_by_name() 查找目标类的真实 wrapper。
  wrapper 未注册时 BASIC_VSEQ_FACTORY fatal。

basicTest::main_phase:
  检查 env、env.vsqr 和 wrapper 非空。
  调用 create_object_by_type() 创建目标对象。
  要求对象可 cast 为 virtual_base_sequence，否则 fatal。
  testcase 在 start() 前 raise objection。
  设置 env.vsqr、reseed 和 starting phase，再 randomize 目标 vseq。
  输出 VSEQ_BODY starting 日志。
  调用 main_vseq.start(env.vsqr)，同步等待 pre_body/body/post_body 全部返回。
  输出 VSEQ_BODY completed 日志。
  testcase 在 start() 返回后 drop objection。
```

testcase objection 从调用 `start()` 前持续到 `start()` 返回后，覆盖目标 vseq 的完整生命周期。
派生 vseq 可以保留自身 `pre_body()`/`post_body()` 或 automatic objection 用于局部 drain/兼容行为，
但顶层 phase 保活不依赖这些派生 callback 的 objection。

`basicTest` 不应：

```text
直接 start 场景 base_sequence。
通过 factory override 或 env.vsqr phase default_sequence 间接选择顶层 vseq。
...
find_wrapper_by_name 找到目标 vseq wrapper。
env.vsqr 已创建。
create_object_by_type 创建的对象可 cast 为 virtual_base_sequence。
basicTest::main_phase 持有 testcase objection 并显式调用 start(env.vsqr)。
<scenario_vseq>.body 已启动。
本场景需要的 p_sequencer.<agent>_sqr 非 null。
关键 child sequence 已通过 p_sequencer.<agent>_sqr 或 p_sequencer 启动。
VSEQ_BODY starting/completed 成对出现。
场景结束时 active 标志清零。
```
...
4. find_wrapper_by_name 是否找到已注册目标类型。
5. env.vsqr 是否创建并连接 agent sqr。
6. create_object_by_type 是否成功，且对象是否可 cast 为 virtual_base_sequence。
7. basicTest::main_phase 是否持有 testcase objection 并执行 start(env.vsqr)。
8. p_sequencer 是否为 memblock_virtual_sequencer。
9. 目标 p_sequencer.<agent>_sqr 是否非 null。
10. child sequence 是否通过 uvm_do_on 启动到正确 sequencer。
11. child sequence 自身是否达到 terminal condition。
12. VSEQ_BODY completed、场景 completion 和最终 report summary 是否同时出现。
```
