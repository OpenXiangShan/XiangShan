...
`memblock_dispatch_fired_mask`、`mark_fired_items()`、driver ready bit 回填、fire report、redirect abort partial-fire 和 blocking full-mask 全部使用同源常量。
...
blocking full mask：
  full_mask = {MEMBLOCK_DUT_SCALAR_ISSUE_MASK_W{1'b1}}；
```
...
- blocking full-mask 使用参数化 all-ones。

driver `clear_ready_dispatch_issue_ports()`：

- 对每个 V2 展开 LDA/STA/STD port 逐一检查 valid && ready。
- 清对应 valid。
...
send_issue_cycle(cycle_idx, has_fire)：
  ...
  如果 driver 标记 redirect abort：
    effective_mask = tr.memblock_dispatch_fired_mask；
    如果 effective_mask 非 0：
      mark_fired_items(effective_mask)；
      has_fire = 1；
    返回；
  ...
  如果存在候选：
    如果 nonblocking：
      effective_mask = driver 回填 mask；
    否则：
      effective_mask = {MEMBLOCK_DUT_SCALAR_ISSUE_MASK_W{1'b1}}；
    如果 effective_mask 非 0：
      mark_fired_items(effective_mask)；
      has_fire = 1；
...
| fired-mask 宽度 | 字段/参数适配 | `[6:0]`、`7'h7f` | V2/V3 profile 可能分裂 | mask width 和 all-ones 由 compile localparam |
...
| send/mark fired | 功能逻辑收敛 | 可能按候选或 full mask 标记 | 只能标记真实 fire | driver 回填参数化 mask，partial-fire 保持 |
