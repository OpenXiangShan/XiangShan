...
| L2TLB response/permission、多 outstanding lifecycle | `AI_DOC/plan/test_framework/plan/do/mem_ut_v2_l2tlb_response_permission_adapt_execution_plan_20260708.md` 继续拥有 request token、queue、时序与 owner；`AI_DOC/plan/test_framework/plan/do/mem_ut_v2_l2tlb_response_random_payload_plan_20260729.md` 是已完成的 S1/S2 response data model 唯一权威，拥有独立 payload、fault、PPN、permission、PBMT、sector、snapshot 与 UID multicast 回填 |
...
| L2TLB response/permission、多 outstanding lifecycle 与随机 payload | 已完成并归档 | 保持 DTLB -> L2TLB request、L2TLB -> DTLB response；原 lifecycle plan 管理 per-fire token、pending/driving、due latency、ready、flush/reset/stop 和唯一 owner。新增已归档 payload plan 将共享 PTE 改为冻结的独立 S1/S2 payload，覆盖 G/U、四类 raw/effective fault、level、PPN、PBMT、S1 sector one-hot、generation 和 response-to-UID multicast；S2 不建立 V 字段。 | response payload 专项 VCS compile 为 `0 error(s), 0 warning(s)`；`memblock_dispatch_real_smoke_vseq` 在 idle 阈值为 1 时通过，`TEST CASE PASSED`、`UVM_ERROR=0`、`UVM_FATAL=0`。独立终审 `FINAL PASS`；基础 `tc_sanity` 的 DCache L2 flush 无关失败已记录。lifecycle review 位于 `review_doc/undo/mem_ut_v2_l2tlb_response_permission_adapt_implementation_review_20260722.md`，payload review 位于 `review_doc/undo/mem_ut_v2_l2tlb_response_random_payload_implementation_review_20260809.md`。 |
