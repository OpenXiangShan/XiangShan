...
```systemverilog
u_lsqenq_agent_agent.sqr.main_phase  -> memblock_lsqenq_dispatch_base_sequence
u_lintsissue_agent_agent.sqr.main_phase -> memblock_issue_dispatch_base_sequence
u_lsqcommit_agent_agent.sqr.main_phase -> memblock_lsqcommit_dispatch_base_sequence
```
...
真实 DUT flow 不直接调用该 soft helper。真实 LSQ admission 的调用链是：

```systemverilog
memblock_lsqenq_dispatch_base_sequence::send_lsqenq_cycle()
    -> start_item(tr) / finish_item(tr)
        -> lsqenq_agent_agent_driver::main_phase() launch当前item
    -> complete_v2_pending_sample()完成上一批
        -> complete_admission(uid)
            -> issue_sched.prepare_issue_route_for_uid(uid)
    -> confirm_lsq_candidates()只为当前已launch批建立reservation
        -> lsq_ctrl.commit_allocate(...)
    -> 后续driver边界
        -> complete_v2_pending_sample()完成当前批
            -> complete_admission(uid)
                -> issue_sched.prepare_issue_route_for_uid(uid)
```

因此 `confirm_lsq_candidates()` 不是 `complete_admission()` 的直接入口：它只保存当前批
reservation 和 pending-sample metadata。后续 driver 边界到来后，`complete_v2_pending_sample()`
比较 `pending_sample_flush_epoch` 与当前 `dispatch_flush_epoch`；只有两者相等且 global flush 未阻塞时
才调用 `complete_admission()`。两者不等时不调用、不开放 issue，由既有 redirect/cancel owner 回退
reservation。
...
```systemverilog
memblock_issue_dispatch_base_sequence::mark_fired_items()
