...
| L2TLB/PTW | 上一轮顶层 `io_l2_tlb_req_*` 映射语义错误，已改为接 V2 内部 `_inner_dtlbRepeater_io_ptw_req_0_valid`、`_inner_dtlbRepeater_io_ptw_req_0_bits_vpn`、`_inner_dtlbRepeater_io_ptw_req_0_bits_s2xlate` request，并由 agent response 驱动 `_inner_ptw_io_tlb_1_req_0_ready` 和 `_inner_ptw_io_tlb_1_resp_*`；其中 `s2_entry_perm_g/u` 已补齐 agent transaction/sequence/driver/interface 链路，由 `memblock_tlb_entry.pte_g/pte_u` 真实驱动。 | 顶层 `io_l2_tlb_req_*` 是 L2/L2Cache 侧 requestor，不作为 `L2TLB_agent` 接管点。当前接入点是生成后 Verilog internal wire，随 RTL 生成可能变化，后续更新 V2 RTL 时必须复查层级名。 |
