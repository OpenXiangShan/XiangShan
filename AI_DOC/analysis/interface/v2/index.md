...
|---|---|---|---|---|
| [LSQ enqueue agent](agents/lsqenq_agent.md) | `enqLsq`、`needAlloc`、无ready、standalone、`LsqWrapper`、`enqCancel`、`redirectCancelCount`、`lqIdx/sqIdx`、redirect恢复 | MemBlock `io_ooo_to_mem_enqLsq_*`、LsqWrapper、VirtualLoadQueue、StoreQueue | 六路`req.valid`、`needAlloc`、`lqIdx/sqIdx`、`numLsElem`、延迟后的内部redirect | LSQ入队与Redirect恢复flow |
...
| [StoreQueue 内部接口](agents/storequeue_agent.md) | `StoreQueue`、`enq_req`、`storeAddrIn`、`storeAddrInRe`、`storeDataIn`、`storeMaskIn`、`forwardMask`、`forwardData`、`sqIdxMask`、`dataInvalid`、`matchInvalid`、`addrInvalid`、`sbuffer`、`uncache`、`cmoOpReq`、`cmoOpResp.denied/corrupt`、CBO error、BEU、`mmioStout`、`sqCancelCnt`、`stAddrReadyVec`、`stDataReadyVec`、`stAddrReadySqPtr`、`stDataReadySqPtr`、`mmio`、`StoreMisalignBuffer`、`maControl`、`crossPageWithHit`、`crossPageCanDeq`、`withSamePtr`、`doDeq` | V2 `build/rtl/StoreQueue.sv`，SQ、SQAddrModule、SQDataModule、StoreMisalignBuffer、DCache CMOUnit、CoupledL2 CBO MSHR | `io_enq_req_*_valid`、`io_storeAddrIn_*_valid`、`io_storeAddrInRe_*_updateAddrValid`、`io_forward_*_valid`、`io_sbuffer_*_valid`、`io_uncache_req_valid`、`io_cmoOpResp_valid`、`io_brqRedirect_valid`、`io_maControl_toStoreQueue_crossPageWithHit`、`io_maControl_toStoreMisalignBuffer_doDeq` | LSQ 入队与 Redirect 恢复、StoreQueue feature flow、L2 内侧 TileLink/CBOAck flow、ROB 提交/出队、StoreMisalignBuffer split/retry flow |
