...
class XiangShanSimpleTUI:
    def __init__(self, pdb, console_max_height=10, content_asm_fix_width=55, console_prefix="(xiangshan)"):
        ...
        # Note: skip update_asm_abs_info() during init, will be called after loop starts
    ...
    def update_top_pane(self):
        ...
        try:
            ...
            self.content_asm_fix_width = max(10, min(self.content_asm_fix_width, 200))
            ...
            try:
                self.content_asm_fix_width = 55  # Reset to default
                self.root.body.contents[0] = (
                    urwid.Columns([
                        (self.content_asm_fix_width, self.file_box),
                        ("weight", 20, self.summary_box),
    ...
    def _get_output(self, txt="", clear=False):
        ...
        # Apply line wrapping based on console width
    ...
    def _get_console_width(self):
        """Get the current console width for line wrapping."""
        if not hasattr(self, 'loop') or self.loop is None:
            return None
        if not hasattr(self.loop, 'screen') or self.loop.screen is None:
            return None
        try:
    ...
    def _wrap_console_line(self, line, width):
        """Wrap a single line to fit within the specified width, preserving ANSI codes."""
        if width <= 0:
            return [line]
    ...
    def handle_input(self, key):
        ...
        if key == 'enter':
            ...
            try:
                self.content_asm_fix_width = max(10, self.content_asm_fix_width - 1)  # Minimum width
    ...
    def console_output_page_scroll(self, deta):
        ...
        if deta == "exit_page":
            ...
            try:
                ...
                # If scrolling fails, reset to safe state
                self.console_page_cache_index = 0
    ...
    def update_console_ouput(self, redirect_stdout=True):
        ...
        try:
            ...
            try:
                self.console_output.set_text(self._get_output(f"{YELLOW}Console update error: {e}{RESET}\n"))
            except:
                pass
    ...
    def exit(self, loop=None, arg=None):
        ...
        try:
            ...
            try:
                ...
                # If even console output fails, just print to stdout
    ...
    def check_exec_batch_cmds(self, loop, user_data=None):
        try:
            ...
            import traceback
            print(f"Error in check_exec_batch_cmds: {e}")
            traceback.print_exc()
...
def enter_simple_tui(pdb):
    ...
    try:
        ...
        signal.signal(signal.SIGINT, _sigint_handler)
        # Initialize UI content after loop is created
        def init_ui_content(loop, user_data):
            try:
                app.update_asm_abs_info()
            except Exception as e:
                import traceback
                print(f"[Error] Failed to initialize UI content: {e}")
                traceback.print_exc()
        loop.set_alarm_in(0.0, init_ui_content)
...
class ANSIText(urwid.Text):
    ...
    def get_line_translation(self, maxcol: int, ta=None):
        """Get line translation with error handling."""
        try:
            return super().get_line_translation(maxcol, ta)
        except Exception:
            # Fallback: if translation fails, update cache and try again
            if not self._cache_maxcol or self._cache_maxcol != maxcol or \
                ...
                try:
                    ...
                    # If cache update also fails, return empty translation
                    return [(0, None, None)]
            try:
