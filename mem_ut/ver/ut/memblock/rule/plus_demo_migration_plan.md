...
| 非通用硬 X/Z 诊断 | `MEMBLOCK_HARD_XZ_CHECK_EN` |
| scalar writeback 输出 trigger metadata 诊断 | `MEMBLOCK_CHECK_TRIGGER_EN` |
...
| L2TLB/PTW responder sequence | `MEMBLOCK_L2TLB_SEQ_EN`、`MEMBLOCK_L2TLB_MAX_OUTSTANDING`、`MEMBLOCK_L2TLB_RESP_REORDER_EN`、`MEMBLOCK_L2TLB_RESP_MID_LATENCY`、`MEMBLOCK_L2TLB_RESP_LONG_LATENCY`、`MEMBLOCK_L2TLB_RESP_1C_WT`、`MEMBLOCK_L2TLB_RESP_MID_WT`、`MEMBLOCK_L2TLB_RESP_LONG_WT`、`MEMBLOCK_L2TLB_IDLE_STOP_CYCLE`、`MEMBLOCK_L2TLB_PPN_REUSE_EN`、`MEMBLOCK_L2TLB_PPN_REUSE_HISTORY_SIZE`、`MEMBLOCK_L2TLB_PPN_REUSE_WT` |
...
L2TLB PPN reuse 的三个参数也由该公共入口管理：`EN=0` 是默认旁路，既不收集 completed-response history，也不消耗 reuse 随机数；`EN=1` 时 `HISTORY_SIZE` 记录最近返回数且范围为 `1..256`，`WT` 是新 4KB miss 使用 valid history PPN 的百分比。该历史表属于 `common_data_transaction` 运行期状态，不是 cfg、DUT filter 容量或 CSR 镜像；普通 SFENCE/HFENCE 删除 live entry 后仍保留 history，runtime reset 才清空。
