...
class plus;
    ...
    // 使能后每次LSQ admission从[1:编译期LSQ enqueue物理slot数]均匀随机本拍入队数量。
    `MEMBLOCK_PLUS_ARGS_DEFINE(MEMBLOCK_ENQ_PER_CYCLE_RAND_EN, bit, 1'b0)
    // LOAD/STA/STD issue运行期使用上限；物理pipe数量由memblock_compile_params.svh唯一配置。
    // 随机开关关闭时固定使用该上限，打开时每拍从[1:上限]采样。
    ...
    // 自动主表 normal 地址生成使用独立虚拟地址窗口；默认值仅用于保持既有 Bare smoke 行为。
    ...
    static function void reload_from_cmdline();
        ...
        load_hex64("MEMBLOCK_MAIN_VADDR_BASE", MEMBLOCK_MAIN_VADDR_BASE);
